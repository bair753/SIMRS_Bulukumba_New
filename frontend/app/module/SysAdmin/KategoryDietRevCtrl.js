define(['initialize'], function (initialize) {
	'use strict';
	initialize.controller('KategoryDietRevCtrl', ['$rootScope', '$scope', 'MedifirstService', '$window', '$timeout',
		function ($rootScope, $scope, medifirstService, $window, $timeout) {
			...
			$scope.Clear = function () {
				...
				delete $scope.item.namaEksternal
			...
			function loadData() {
				...
				medifirstService.get("sysadmin/master/get-daftar-kategorydiet?"
					+ kdKat
					+ kategory
					+ kel
			...
			$scope.save = function () {
				...
				medifirstService.post('sysadmin/master/save-kategorydiet',objSave).then(function (res) {
					loadData();
					$scope.Clear();
				})
			...
			function hapusData(e) {
				...
				medifirstService.post('sysadmin/master/delete-kategorydiet',itemDelete).then(function (res) {
					if (e.status === 201) {
						loadData();
						grid.removeRow(row);
					}
			...
			function editData(e) {
				...
				medifirstService.get("sysadmin/master/get-daftar-kategorydiet?id=" + dataItem.id).then(function (e) {
