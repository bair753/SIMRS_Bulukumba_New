define(['initialize'], function (initialize) {
	'use strict';
	initialize.controller('MasterKelompokTransaksiCtrl', ['$q', '$scope', 'MedifirstService', '$timeout',
		function ($q, $scope, medifirstService, $timeout) {
			...
			$scope.daftarMasterOpt = {
				...
				columns: [
					...
					{ field: "kelompoktransaksi", title: "Kelompok Transaksi" },
					// { field: "Status", title: "Status Enabled" },
					{ command: [{ name: "destroy", text: "Disable" }, { name: "edit", text: "Edit" }], title: "&nbsp;", width: 160 }
				],
				editable: "popup",
				save: function (e) {
					$scope.Save(e.model);
			...
			function init() {
				...
				$q.all([
					medifirstService.get("sysadmin/general/rensar/get-kelompok-transaksi", true),
				]).then(function (res) {
					if (res[0].statResponse) {
						$scope.dataSource = new kendo.data.DataSource({
										...
										kelompoktransaksi: {
											editable: true, validation: {
												validasikelompoktransaksi: function (input) {
													if (input.is("[name='kelompoktransaksi']") && input.val() === "") {
														return false;
													}
													return true;
												}
							...
							change: function (e) {
								...
								if (e.action === "remove") {
									var item = e.items[0];
									if (item.kelompoktransaksi !== "" && item.id !== "") {
			...
			$scope.Save = function (data) {
				var item = {
					...
					kelompoktransaksi: data.kelompoktransaksi,
				}
				if (data.action && data.action === "remove") item.statusenabled = false
				medifirstService.post('sysadmin/general/rensar/save-kelompok-transaksi',item).then(function (e) {
					// delete $scope.item;
					// $scope.item = {};
					init();
				});
			...
			$scope.$watch('item.jenisTransaksi', function (newVal, oldVal) {
				...
				timeoutPromise = $timeout(function () {
					if (newVal != oldVal) {
						applyFilter("kelompoktransaksi", newVal)
					}
				}, 500)
			})
