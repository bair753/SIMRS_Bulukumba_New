define(['initialize'], function (initialize) {
	'use strict';
	initialize.controller('JenisDietRevCtrl', ['$rootScope', '$scope', 'MedifirstService', '$window', '$timeout',
		function ($rootScope, $scope, medifirstService, $window, $timeout) {
			...
			function loadData() {
				...
				medifirstService.get("sysadmin/master/get-daftar-jenisdiet?"
					+ kdJenis
					+ jenis
					+ kel
			...
			$scope.save = function () {
				...
				medifirstService.post("sysadmin/master/save-jenisdiet", objSave).then(function (res) {
					loadData();
					$scope.Clear();
				})
			...
			function hapusData(e) {
				...
				medifirstService.post('sysadmin/master/delete-jenisdiet', itemDelete).then(function (e) {
					if (e.status === 201) {
						loadData();
						grid.removeRow(row);
					}
