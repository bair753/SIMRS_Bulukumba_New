define(['initialize'], function (initialize) {
	'use strict';
	initialize.controller('ViewBedCtrl', ['$scope', '$state', 'MedifirstService',
		function ($scope, $state, medifirstService) {
			...
			function FormLoad(){
				medifirstService.get("sysadmin/master/get-list-komponen?jenis=statusbed").then(function (dat) {
					$scope.listStatus = dat.data;
				});
				medifirstService.get("sysadmin/master/get-list-komponen?jenis=ruangan").then(function (dat) {
					$scope.listRuangan = dat.data;
				});
				medifirstService.get("sysadmin/master/get-list-komponen?jenis=kamar").then(function (dat) {
					$scope.listKamar = dat.data;
				});
				medifirstService.get("sysadmin/master/get-list-komponen?jenis=kelas").then(function (dat) {
					$scope.listKamar = dat.data;
					$scope.listKamar2 = dat.data;
				});
			}
			...
			function Carii() {
				...
				medifirstService.get("sysadmin/general/view-bed?" + nmR + nmK + nmB + Stt).then(function (data) {
					var no = 0;					
					var data = data.data;
			...
			$scope.save = function () {
				...
				medifirstService.post('sysadmin/master/save-data-bed',dataObjPost).then(function (e) {
				})
				Carii();
			}
			////////////////////////////////////////////////////////////
