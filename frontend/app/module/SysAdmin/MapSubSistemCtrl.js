...
define(['initialize'], function (initialize) {
	'use strict';
	initialize.controller('MapSubSistemCtrl', ['$scope', 'CacheHelper', 'DateHelper', 'MedifirstService', '$timeout',
		function ($scope, cacheHelper, dateHelper, medifirstService, $timeout) {
			$scope.item = {};
			...
			medifirstService.get("sysadmin/menu/data?jenis=subsistem", true).then(function (dat) {
			...
			//listKelompokUser
			medifirstService.get("sysadmin/menu/data?jenis=kelompokuser", true).then(function (dat) {
			...
			function initModulAplikasi() {
				...
				medifirstService.get("sysadmin/menu/data?jenis=modulaplikasi&id=" + $scope.dataSelected1.id, true).then(function (dat) {
					for (var i = 0; i < dat.data.length; i++) {
						dat.data[i].no = i + 1;
					}
			...
			function onSelect(e) {
				...
				medifirstService.get("sysadmin/menu/data?jenis=objekmodultokelompokuser&omaid=" + arrText[0] + "&id=" + idModul, true).then(function (dat) {
					...
					$scope.item.hideMenu = dat.data.data2.ishide;
				...
				medifirstService.get("sysadmin/menu/get-child-idhead?idOma=" + arrText[0]).then(function (res) {
			...
			function init() {
				medifirstService.get("sysadmin/menu/data?jenis=modulaplikasi&idhead=" + $scope.dataSelected2.id, true).then(function (dat) {
				...
				$scope.treeDataMenu = [];
				medifirstService.get("sysadmin/menu/data?jenis=objekMenuRecursive&id=" + $scope.dataSelected2.id, true).then(function (dat) {
			...
			$scope.simpan1 = function () {
				...
				var objSave = {
					"id": idModul,
					"reportdisplay": 'Menu',//$scope.item.nmModul,
					"modulaplikasi": $scope.item.nmModul,
					"iconimage": $scope.item.iconImage,
					"nourut": $scope.item.noUrut,
					"kdmodulaplikasihead": idModulHead
				};
				medifirstService.post("sysadmin/menu/simpan-modul-aplikasi", objSave).then(function (e) {
					initModulAplikasi();
				})
			}
			$scope.hapus1 = function () {
				...
				medifirstService.post("sysadmin/menu/hapus-modul-aplikasi", objSave).then(function (e) {
					initModulAplikasi();
				})
			}
			...
			$scope.simpan2 = function () {
				...
				var objSave = {
					...
					"ishide": $scope.item.hideMenu !=undefined? $scope.item.hideMenu: false
				};
				medifirstService.post("sysadmin/menu/simpan-objek-modul-aplikasi", objSave).then(function (e) {
					initModulAplikasi();
					$scope.batal2();
				})
			}
			$scope.hapus2 = function () {
				...
				medifirstService.post("sysadmin/menu/hapus-objek-modul-aplikasi", objSave).then(function (e) {
					initModulAplikasi();
				})
			}
			$scope.hapus2Map = function () {
				...
				medifirstService.post("sysadmin/menu/hapus-map-objek-modul-aplikasi", objSave).then(function (e) {
					initModulAplikasi();
				})
			}
			...
			$scope.simpan3 = function () {
				...
				medifirstService.post("sysadmin/menu/map-objek-modultokelompok-user", objSave).then(function (e) {
					initModulAplikasi();
					$scope.data4 = [];
				})
			}
			$scope.hapus3 = function () {
				...
				medifirstService.post("sysadmin/menu/hapus-objek-modultokelompok-user", objSave).then(function (e) {
					//initModulAplikasi(); 
				})
			}
			...
			$scope.cariMenuHead = function () {
				medifirstService.get("sysadmin/menu/data?jenis=objekmodultokelompokuser&omaid=" + $scope.item.idHeadMenu + "&id=" + idModul, true).then(function (dat) {
					//$scope.item.idHeadMenu =dat.data[0].omaid;
					$scope.item.nmModulHead = dat.data.data2.objekmodulaplikasi;
				});
			}
			$scope.cariMenuid = function () {
				medifirstService.get("sysadmin/menu/data?jenis=objekmodultokelompokuser&omaid=" + $scope.item.idMenu + "&id=" + idModul, true).then(function (dat) {
			...
			var timeoutPromise;
			$scope.$watch('item.cariSubSistem', function (newVal, oldVal) {
			...
			function applyFilter(filterField, filterValue) {
				var dataGrid = $("#idGridSub").data("kendoGrid");
			...
			$scope.$watch('item.cariModulAplikasi', function (newVal, oldVal) {
				$timeout.cancel(timeoutPromise);
				timeoutPromise = $timeout(function () {
					if (newVal && newVal !== oldVal) {
						applyFilter2("modulaplikasi", newVal)
					}
				}, 500)
			})
			function applyFilter2(filterField, filterValue) {
				var dataGrid = $("#idGridMod").data("kendoGrid");
				var currFilterObject = dataGrid.dataSource.filter();
				var currentFilters = currFilterObject ? currFilterObject.filters : [];
			...
			$scope.clearModAplikasi = function () {
				var dataGrid = $("#idGridMod").data("kendoGrid");
				dataGrid.dataSource.filter({});
				delete $scope.item.cariModulAplikasi
			}
