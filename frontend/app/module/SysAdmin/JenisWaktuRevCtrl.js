define(['initialize'], function (initialize) {
	'use strict';
	initialize.controller('JenisWaktuRevCtrl', ['$rootScope', '$scope', 'MedifirstService', '$window', '$timeout',
		function ($rootScope, $scope, medifirstService, $window, $timeout) {
			...
			function loadCombo() {
				...
				medifirstService.get("sysadmin/master/get-departemen"
				).then(function (e) {
					$scope.listDepartemen = e.data
			...
			function loadData() {
				...
				medifirstService.get("sysadmin/master/get-daftar-jeniswaktu?"
					+ kdJenis
					+ jenis
					+ kel
					+ id
			...
			$scope.save = function () {
				...
				medifirstService.post('sysadmin/master/save-jeniswaktu', objSave).then(function (res) {
					loadData();
					$scope.Clear();
				})
			...
			function hapusData(e) {
				...
				medifirstService.post('sysadmin/master/delete-jeniswaktu', itemDelete).then(function (e) {
					if (e.status === 201) {
						loadData();
						grid.removeRow(row);
					}
			...
			function editData(e) {
				...
				medifirstService.get("sysadmin/master/get-daftar-jeniswaktu?id=" + dataItem.id).then(function (e) {
