define(['initialize'], function(initialize) {
	'use strict';
	initialize.controller('DaftarLoggingUserCtrl', ['$scope', 'ModelItem', 'CacheHelper', 'MedifirstService',
		function($scope, ModelItem, cacheHelper, medifirstService) {
            ...
            medifirstService.get('sysadmin/get-list-combo').then(function(e) {
                $scope.listJenisPegawai = e.data.jenispegawai;
            });

          $scope.listJenisLog = [
                { id: 1, jenis: 'Pendaftaran Pasien' },
                { id: 2, jenis: 'Konsul Ruangan' },
                { id: 3, jenis: 'Input Resep Apotik' },
                { id: 4, jenis: 'Input Tindakan' },
                { id: 5, jenis: 'Verifikasi TataRekening' },
                { id: 6, jenis: 'Unverifikasi TataRekening' },
                { id: 7, jenis: 'Hapus Resep Apotik' },
                { id: 8, jenis: 'Batal Bayar Kasir' },
                { id: 9, jenis: 'Pasien Pindah' },
                { id: 10, jenis: 'Pasien Pulang' },
                { id: 11, jenis: 'Retur Resep Apotik' },
                { id: 12, jenis: 'Hapus Layanan' },
                { id: 13, jenis: 'Ubah Tgl Pelayanan' },
                { id: 14, jenis: 'Diskon Layanan' },
                { id: 15, jenis: 'Input/Ubah Petugas Layanan' },
                { id: 16, jenis: 'Hapus Konsul' },
                { id: 17, jenis: 'Ubah Tgl Detail Registrasi' },
                { id: 18, jenis: 'Edit Registrasi' },
                { id: 19, jenis: 'Stok Opname' },
                { id: 20, jenis: 'Batal Kirim' },
                { id: 21, jenis: 'Login User' },
                { id: 22, jenis: 'Batal Pulang Pasien' },
                { id: 23, jenis: 'Batal Rawat Inap' },
                { id: 24, jenis: 'CPPT' },
                { id: 25, jenis: 'Diagnosis' },
                { id: 26, jenis: 'Input SEP' },
                { id: 27, jenis: 'Konsultasi' },
                { id: 28, jenis: 'Order Laboratorium' },
                { id: 29, jenis: 'Pasien Pindah' },
                { id: 30, jenis: 'Pembayaran Tagihan' },
                { id: 31, jenis: 'Pengkajian Awal' },
                { id: 32, jenis: 'Pengkajian Keperawatan' },
                { id: 33, jenis: 'Pengkajian Medis' },
                { id: 34, jenis: 'Ubah Rekanan' },
                { id: 35, jenis: 'Pasien Mutasi' },
                { id: 36, jenis: 'EMR' },
                { id: 37, jenis: 'Hapus EMR' },
                { id: 38, jenis: 'Verifikasi Order Lab'},
                { id: 39, jenis: 'Verifikasi Order Rad'},
			...
			function GetData(){
                ...
                if ($scope.item.jenisPegawai != undefined) {
                    jenisPegawai ="&jenisPegawai=" + $scope.item.jenisPegawai.jenispegawai
                }  
                medifirstService.get("sysadmin/logging/Daftar-log-user?"
                    +"tglAwal="+tglAwal+
                    "&tglAkhir="+tglAkhir
                    +JenisLoging+UserId+Keterangan+LimitRow+jenisPegawai, true).then(function(dat){
                    $scope.isRouteLoading=false;
			...
			$scope.optionsDataKeluhan = {
                ...
                columns: [
                    ...
                    },
	                {
	                    ...
	                    "width": 85,
                        aggregates: ["count"],
                        groupHeaderTemplate: "(Jumlah: #= count#)"
	                }, 
	                {
	                    "field": "namalengkap",
	                    "title": "Nama User",
