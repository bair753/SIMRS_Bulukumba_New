define(['initialize'], function (initialize) {
	'use strict';
	initialize.controller('DaftarKirimMenuCtrl', ['$mdDialog', '$timeout', '$state', '$q', '$rootScope', '$scope', 'CacheHelper', 'DateHelper', 'MedifirstService',
		function ($mdDialog, $timeout, $state, $q, $rootScope, $scope, cacheHelper, dateHelper, medifirstService) {
			...
			function loadData() {
				...
				$q.all([
					medifirstService.get("gizi/get-daftar-kirim?" +
						"tglAwal=" + tglAwal +
						"&tglAkhir=" + tglAkhir +
			...
			$scope.updateOrder = function () {
				...
				medifirstService.post('gizi/update-orderpelayanan-gizi',objUpdate).then(function (res) {
					if (res.status === 201) {
						loadData();
			...
			$scope.kirimMenu = function () {
				if ($scope.dataPasienSelected != undefined) {
					var cache = {
						0: $scope.dataPasienSelected.norec_so,
			...
			$scope.ubahKirim = function () {
				...
				// medifirstService.get("gizi/get-pasien-bynoregistrasi?noReg="+$scope.dataPasienSelected.noregistrasi)         
				// .then(function (e) {
			...
			$scope.batalKirim = function () {
				...
				var confirm = $mdDialog.confirm()
					.title('Peringatan')
					.textContent('Peringatan, Apakah anda yakin mau membatalkan pengiriman ini ?')
					.ariaLabel('Lucky day')
					.cancel('Tidak')
					.ok('Ya')
			...
			function saveBatal() {
				var data = {
					norec: $scope.dataPasienSelected.norec_sk
				}
				medifirstService.posr('gizi/batal-kirim', data).then(function (e) {
