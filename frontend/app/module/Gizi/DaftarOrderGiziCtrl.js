define(['initialize'], function (initialize) {
	'use strict';
	initialize.controller('DaftarOrderGiziCtrl', ['$mdDialog', '$timeout', '$state', '$q', '$rootScope', '$scope', 'CacheHelper', 'DateHelper', 'MedifirstService',
		function ($mdDialog, $timeout, $state, $q, $rootScope, $scope, cacheHelper, dateHelper, medifirstService) {
				...
				medifirstService.get("gizi/get-daftar-order?" +
					...
					.then(function (data) {
						...
						for (var i = 0; i < result.length; i++) {
							...
							// 	result[i].status= "✘" ;
						    result[i].jenisdiet =[]
							if(result[i].arrjenisdiet){
								var arr =  result[i].arrjenisdiet.split(',') 
								if(arr.length > 0){
			...
			$scope.EditOrder = function () {
				...
				if(select.jenisdiet.length > 0){
					for (var i = 0; i < select.jenisdiet.length; i++) {
						const jen =	select.jenisdiet[i]
			...
			$scope.saveEditOrder = function(){
				...
				var objSave = {
					...
					'orderpelayanan' : {
						...
						"keterangan":ket,
						"objectjenisdietfk": jenisDiet,//$scope.pop.jenisDiet.id
					}
				}
