...
define(['initialize'], function (initialize) {
    ...
    'use strict';
    initialize.controller('DaftarRencanaPindahPasienCtrl', ['$state', '$rootScope', '$scope', 'ModelItem', 'DateHelper', 'CacheHelper', 'MedifirstService',
        function ($state, $rootScope, $scope, ModelItem, dateHelper, cacheHelper, medifirstService) {
            $scope.title = "Daftar Rencana Pindah Pasien";
            ...
            function LoadCache() {
                var chacePeriode = cacheHelper.get('DaftarRencanaPindahPasienCtrl');
            ...
            function init() {
                ...
                medifirstService.get("rawatinap/get-daftar-pasien-rencana-pindah?" +
                    "tglAwal=" + tglAwal +
                    "&tglAkhir=" + tglAkhir +
                    noCm + noReg, true).then(function (dat) {
                        ...
                        for (var i = 0; i < dat.data.daftar.length; i++) {
                            ...
                            if (dat.data.daftar[i].statusorder == 0) {
                                dat.data.daftar[i].status = "Pengajuan Pindah";
                            } else {
                                dat.data.daftar[i].status = "Pindah";
                            }
                        }
            ...
            $scope.pindah = function () {
                ...
                if ($scope.data.status === "Pengajuan Pindah") {
                    medifirstService.get("rawatinap/get-ruangan-last?norec_pd=" + $scope.data.norec_pd).then(function (e) {
                        $scope.item.ruanganLast = e.data.data[0].objectruanganlastfk
                        if ($scope.item.ruanganLast != undefined) {
                            $state.go('PindahPasien', {
                    ...
                    window.messageContainer.log("Pasien sudah Dipindahkan");
                }
