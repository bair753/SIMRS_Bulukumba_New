define(['initialize'], function (initialize) {
    'use strict';
    initialize.controller('PindahPasienCtrl', ['$q', '$rootScope', '$scope', '$mdDialog', 'ModelItem', '$state', 'CacheHelper', 'DateHelper', 'MedifirstService',
        function ($q, $rootScope, $scope, $mdDialog, ModelItem, $state, cacheHelper, dateHelper, medifirstService) {
            $scope.item = {};
            $scope.isNext = true;
            var responData = '';
            var norecOrder = '';
            ...
            function firstLoad() {
                ...
                medifirstService.get("rawatinap/get-pasien-bynorec?norec_pd=" + $state.params.norecPD + "&norec_apd=" + $state.params.norecAPD)
                    .then(function (e) {
                        $scope.isRouteLoading = false;
                        $scope.item.pasien = e.data[0];
                    });

                medifirstService.get("rawatinap/get-data-pasien-pindah?norec_pd=" + $state.params.norecPD + "&norec_so=" + $state.params.norecSO)
                    .then(function (dat) {
                        getdataCombo()
            ...
            $scope.$watch('item.kelas', function (e) {
                ...
                medifirstService.get("rawatinap/get-kamarbyruangankelas?" + kelasId + ruanganId)
                    .then(function (b) {
            ...
            $scope.$watch('item.kamar', function (e) {
                ...
                medifirstService.get("rawatinap/get-nobedbykamar?idKamar=" + kamarId)
                    .then(function (a) {
