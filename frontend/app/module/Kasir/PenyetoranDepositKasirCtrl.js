define(['initialize'], function (initialize) {
	'use strict';
	initialize.controller('PenyetoranDepositKasirCtrl', ['$state', '$q', '$rootScope', '$scope', 'MedifirstService',
		function ($state, $q, $rootScope, $scope, medifirstService) {
			...
			$q.all([
				medifirstService.get("kasir/detail-pasien-deposit/" + $scope.dataParams.noRegistrasi)
			])
				.then(function (data) {
					...
					if (data[0].statResponse) {
						...
						$scope.totalDeposit = 0
						for (let i = 0; i < data[0].data.detailDeposit.length; i++) {
							const element = data[0].data.detailDeposit[i];
							$scope.totalDeposit = $scope.totalDeposit + element.jumlahDeposit
						}
						$scope.dataDaftarPenyetoranDeposit = new kendo.data.DataSource({
							data: data[0].data.detailDeposit,
							schema: {
								model: {
									fields: {
			...
			$scope.KembaliDeposit = function () {
				...
				if($scope.item.jumlah  > $scope.totalDeposit  ){
					toastr.error('Jumlah kembali tidak boleh lebih dari total deposit')
					return
				}
				var listRawRequired = [
					"item.jumlah|ng-model|Jumlah",
