define(['initialize'], function (initialize) {
    'use strict';
    initialize.controller('KoneksiFingerprintCtrl', ['$q', '$rootScope', '$scope', '$state', '$timeout', 'ModelItem', 'MedifirstService',
        function ($q, $rootScope, $scope, $state, $timeout, modelItem, medifirstService) {
            ...
            $scope.mainGridOptions = {
                ...
                columns: [
                    ...
                    { "field": "namafield", "title": "Alat" },
                    { "field": "nilaifield", "title": "IP" },
                    { "field": "keteranganfungsi", "title": "Keterangan" },
                    { "command": [
                        // { name: "edit", text: "Edit" }, 
                        // { text: "Hapus", click: deleteRow },
                    { text: "Cek", click: cekKoneksi }
                ], width: 160 }
                ],
                selectable: "row",
                editable: "popup",
            ...
            $scope.refresh = function () {
                ...
                medifirstService.get("sdm/get-presensi-fingerprint", true).then(function (dat) {
                    ...
                    for (var i = 0; i < dat.data.data.length; i++) {
                        listData.push({
                            ...
                            "namafield": dat.data.data[i].namafield,
                            "nilaifield": dat.data.data[i].nilaifield,
                            "keteranganfungsi": dat.data.data[i].keteranganfungsi,
                        })
                    }
