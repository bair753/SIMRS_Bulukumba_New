define(['initialize'], function (initialize) {
    'use strict';
    initialize.controller('TenagaPengajarCtrl', ['$rootScope', '$scope', 'ModelItem', '$state', 'MedifirstService',
        function ($rootScope, $scope, ModelItem, $state, medifirstService) {
            $scope.item = {};
            // $scope.pegawai = {};
            $scope.item.id = '';
            medifirstService.get("sdm/pendidikan/get-cbo-tenagapengajar").then(function (dat) {
                $scope.listPegawai = dat.data.dataPegawai;
                $scope.listProgram = dat.data.dataProgramStudi;
            });
            var init = function () {
                medifirstService.get("sdm/pendidikan/get-daftar-tenagapengajar?nipOrNama=").then(function (dat) {
            ...
            $scope.getDetail = function () {
                medifirstService.get("sdm/pendidikan/get-detail-tenagapengajar?id=" + $scope.item.pegawai.id).then(function (dat) {
            ...
            $scope.simpan = function () {
                ...
                if (isValid.status) {
                    ...
                    medifirstService.post("sdm/pendidikan/save-tenagapengajar", tempData).then(function (e) {
                        $scope.item = {};
                        init();
                    });
                } else {
            ...
            $scope.hapus = function(){
				...
				medifirstService.post('sdm/pendidikan/hapus-tenagapengajar', { 'id':$scope.item.id }).then(function (e) {
					init();
				})
            }
