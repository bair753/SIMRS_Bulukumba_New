define(['initialize'], function (initialize) {
	'use strict';
	initialize.controller('KlaimBpjsPerBulanCtrl', ['$mdDialog', '$timeout', '$state', '$q', '$rootScope', '$scope', 'CacheHelper', 'DateHelper', 'MedifirstService',
		function ($mdDialog, $timeout, $state, $q, $rootScope, $scope, cacheHelper, dateHelper, medifirstService) {
			...
			$scope.totalPengajuan = 0;
			$scope.totalDisetujui = 0;
			$scope.totalTarifRs = 0;
			$scope.item.jmlRows = 50
			$scope.jmlRujukanMasuk = 0
			$scope.item.jenispelayanan = 1
			...
			// postKunjunganYankes()
			function loadCombo() {
				$scope.item.tahun = $scope.now.getFullYear();
				...
				columns:
					[
						...
						},
						{
							...
							"title": "Biaya Setujui",
							"width": "100px",
							"template": "<span class='style-right'>{{formatRupiah('#: biaya.bySetujui #', '')}}</span>",
							// "template": "<span class='style-left'>#: biaya.bySetujui #</span>"
			...
			function loadData() {
				...
				if ($scope.item.bulan == undefined) {
					toastr.error('Bulan harus di pilih')
					return
				}
				...
				var dataget = {
					...
					bulan: $scope.item.bulan.mm,
					status: $scope.item.statusKlaim.value
				}
				$scope.item.selisih = 0

				let hari = daysInMonth($scope.item.bulan.id, $scope.item.tahun)
				hari = hari + 1
				data2 = []
				$scope.isRouteLoading = true;

				medifirstService.get("bridging/bpjs/get-monitoring-klaim-status?jenispelayanan="
					+ $scope.item.jenispelayanan
					+ "&statusklaimfk=" + $scope.item.statusKlaim.value
					+ "&bulan=" + $scope.item.tahun + '-' + $scope.item.bulan.mm).then(function (res) {
						if (res.data.data != null) {
							toastr.info('Sudah di Posting')
						}
						for (var i = 1; i < hari; i++) {
							medifirstService.get("bridging/bpjs/get-monitoring-klaim?tglsep="
								+ dataget.tahun + "-" + dataget.bulan + "-" + i
								+ "&jenispelayanan="
								+ dataget.jenispelayanan
								+ "&status="
								+ dataget.status
							).then(function (data) {


								// datas=data.data.response.klaim;

								// data2.push(data.data.response.klaim[i])
								if (data.data.response != null) {
									for (var b = 0; b < data.data.response.klaim.length; b++) {
										// debugger;


										// data2.push(data.data.response.klaim[b])
										// jmlKlaim =0;
										// // jmlKlaim = $jmlKlaim + float($dataSaldoLama->debit);
										if (parseFloat(data.data.response.klaim[b].biaya.bySetujui) < parseFloat(data.data.response.klaim[b].biaya.byPengajuan))
											$scope.item.selisih = $scope.item.selisih + 1
										if ($scope.item.noFpk == undefined) {
											data.data.response.klaim[b] = data.data.response.klaim[b];
											data2.push(data.data.response.klaim[b])
										} else {
											var noFpk = $scope.item.noFpk;
											if (data.data.response.klaim[b].noFPK.match(noFpk)) {
												data2.push(data.data.response.klaim[b])
											}
											// dataB = data.data.response.klaim[b].noFpk.match(/^noFpk.*$/);
											// data2.push(dataB)
											// data2.push(data.data.response.klaim[b].noFpk.match(/noFpk.*/)
										}
								...
								var subTotalA = 0;
								var subTotalB = 0;
								for (var z = data2.length - 1; z >= 0; z--) {
									subTotalA = subTotalA + parseFloat(data2[z].biaya.bySetujui)
									subTotalB = subTotalB + parseFloat(data2[z].biaya.byTarifRS)
								}

								$scope.totalDisetujui = subTotalA;
								$scope.totalTarifRs = subTotalB;
								$scope.surplusDefisit = $scope.totalDisetujui - $scope.totalTarifRs;
			...
			$scope.posting = function () {
				...
				var json = {
					'jenispelayanan': $scope.item.jenispelayanan,
					'statusklaimfk': $scope.item.statusKlaim.value,
					'bulan': $scope.item.tahun + '-' + $scope.item.bulan.mm,
					'details': datas
				}
				medifirstService.post('bridging/bpjs/save-monitoring-klaim', json).then(function (e) {
					$scope.isRouteLoading = false;
				})
			}
			function searchTable() {
				...
				var s;
				var j;
				input = document.getElementById("noFpk");
