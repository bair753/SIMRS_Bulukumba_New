define(['initialize', 'Configuration'], function (initialize,configuration) {
	...
	initialize.controller('AsuransiLainCtrl', ['$mdDialog', '$timeout', '$state', '$q', '$rootScope', '$scope', 'ModelItem', 'CacheHelper', 'DateHelper', 'MedifirstService',
		function ($mdDialog, $timeout, $state, $q, $rootScope, $scope, ModelItem, cacheHelper, DateHelper, medifirstService) {
			...
			function loadCombo() {
				...
				medifirstService.get("bridging/inacbg/get-data-combo-ina", false).then(function (data) {
					...
					for (let i = 0; i < $scope.listKelompokPasien.length; i++) {
						const element = $scope.listKelompokPasien[0];
						// if(element.kelompokpasien.indexOf('Asuransi Lain')> -1 )
			...
			function loadDataObat() {
				...
				if ($scope.item.nosep != undefined) {
					var sep = "&nosep=" + $scope.item.nosep
				}
				var ins = ""
				if ($scope.item.instalasi != undefined) {
			...
			medifirstService.get("tatarekening/detail-tagihan/" + $scope.dataPasienSelected.noregistrasi + '?jenisdata=bill').then(function (dat) {
				...
				// medifirstService.get("tatarekening/get-data-login-cetakan").then(function (e) {
					...
					var local = JSON.parse(localStorage.getItem('profile'));
						window.open(baseTransaksi + 'report/cetak-rinc-billing?cetak-RincianBiaya=1&noregistrasi=' + $scope.dataPasienSelected.noregistrasi + '&strNoStruk=' + struk + '&strNoKwitansi=' + kwitansi + '&strIdPegawai=' + user.namaLengkap + '&kdprofile=' + local.id +'&view=' + stt);
				// })
				// }else{
				// 	medifirstService.get("tatarekening/get-data-login-cetakan").then(function (e) {
				//              	var client = new HttpClient(); 
	...
	$scope.tryUpload = function () {
		toastr.warning("Masih dalam tahap pengembangan")
			...
			$scope.columnDaftadAsesmenAwalKeperIGD = {
				...
				$scope.cpptrajal = function(){
					if ($scope.dataPasienSelected.noregistrasi == undefined) {
						toastr.error('Pilih Pasien Terlebih dahulu!!!')
						return;
					}
					var emrfk = [290015] // emrfk CPPT Rajal
					...
					$scope.popUpDaftarCPPTRajal.center().open();
					
				}

				$scope.columnDaftarCPPTRajal = {
					columns: [
							{
									"field": "tglemr",
									"title": "Tgl EMR",
			...
			$scope.cetakCPPTRajal = function () {
					if ($scope.dataSelectedCPPTRajal == undefined) {
						toastr.error('Data belum dipilih!!!')
						return;
					}
					...
					var local = JSON.parse(localStorage.getItem('profile'));
					var nama = medifirstService.getPegawaiLogin();
					window.open(baseTransaksi + 'report/cetak-cppt-rajal?nocm='
					+ $scope.dataSelectedCPPTRajal.nocm 
					+ '&norec_apd=' + $scope.dataSelectedCPPTRajal.norec_apd 
					+ '&emr=' + $scope.dataSelectedCPPTRajal.norec 
					+ '&emrfk=' + $scope.dataSelectedCPPTRajal.emrfk
					+ '&kdprofile=' + local.id
					+ '&nama=' + nama, '_blank');
			}

			$scope.partograf = function(){
				if ($scope.dataPasienSelected.noregistrasi == undefined) {
					toastr.error('Pilih Pasien Terlebih dahulu!!!')
					return;
				}
				var emrfk = [290071] // emrfk Partograf

				medifirstService.get("bridging/inacbg/get-rincian-operasi?noregistrasi=" + $scope.dataPasienSelected.noregistrasi + '&emrfk=' + emrfk
					// medifirstService.get("lab-radiologi/get-rincian-pelayanan?objectdepartemenfk=" + departemenfk + "&noregistrasi=" +   $scope.item.noregistrasi
					, true).then(function (dat) {
						$scope.dataDaftarPartograf = {
							data: dat.data.data,
							_data: dat.data.data,
							// pageSize: 10,
							selectable: true,
				...
				$scope.popUpDaftarPartograf.center().open();
				
			}

			$scope.columnDaftarPartograf = {
				columns: [
						{
							"field": "tglemr",
							"title": "Tgl EMR",
			...
			$scope.cetakPartograf = function () {
				if ($scope.dataSelectedPartograf == undefined) {
					toastr.error('Data belum dipilih!!!')
					return;
				}
				...
				var local = JSON.parse(localStorage.getItem('profile'));
				var nama = medifirstService.getPegawaiLogin();
				window.open(baseTransaksi + 'report/cetak-partograf?nocm='
				+ $scope.dataSelectedPartograf.nocm 
				+ '&norec_apd=' + $scope.dataSelectedPartograf.norec_apd 
				+ '&emr=' + $scope.dataSelectedPartograf.norec 
				+ '&emrfk=' + $scope.dataSelectedPartograf.emrfk
				+ '&index=' + $scope.dataSelectedPartograf.index
				+ '&kdprofile=' + local.id
				+ '&nama=' + nama, '_blank');
			}
			...
			$scope.cetakCatatanPemberianObat = function () {
				if ($scope.dataSelectedCatatanPemberianObat == undefined) {
					toastr.error('Data belum dipilih!!!')
						return;
					}
			...
			$scope.cetakObservasiTransfusi = function () {
				if ($scope.dataSelectedObservasiTransfusi == undefined) {
					toastr.error('Data belum dipilih!!!')
						return;
				}
			...
			$scope.cppt = function(){
				...
				$scope.popUpDaftarCPPT.center().open();
			}	

			$scope.columnDaftarCPPT = {
				columns: [
					{
						"field": "tglemr",
						"title": "Tgl EMR",
			...
			$scope.cetakCPPT = function () {
				if ($scope.dataSelectedCPPT == undefined) {
					toastr.error('Data belum dipilih!!!')
						return;
				}
				...
				var local = JSON.parse(localStorage.getItem('profile'));
				var nama = medifirstService.getPegawaiLogin();
				window.open(baseTransaksi + 'report/cetak-cppt?nocm='
				+ $scope.dataSelectedCPPT.nocm 
				+ '&norec_apd=' + $scope.dataSelectedCPPT.norec_apd 
				+ '&emr=' + $scope.dataSelectedCPPT.norec 
				+ '&emrfk=' + $scope.dataSelectedCPPT.emrfk
				+ '&index=' + $scope.dataSelectedCPPT.index
				+ '&kdprofile=' + local.id
				+ '&nama=' + nama, '_blank');
			}

			$scope.implementasiAsuhanIGD = function(){
				if ($scope.dataPasienSelected.noregistrasi == undefined) {
					toastr.error('Pilih Pasien Terlebih dahulu!!!')
						return;
				}
				var emrfk = [290045] // emrfk Implementasi Asuhan Keperawatan IGD

				medifirstService.get("bridging/inacbg/get-rincian-operasi?noregistrasi=" + $scope.dataPasienSelected.noregistrasi + '&emrfk=' + emrfk
					// medifirstService.get("lab-radiologi/get-rincian-pelayanan?objectdepartemenfk=" + departemenfk + "&noregistrasi=" +   $scope.item.noregistrasi
					, true).then(function (dat) {
						$scope.dataDaftarImplementasiAsuhanIGD = {
							data: dat.data.data,
							_data: dat.data.data,
							// pageSize: 10,
							selectable: true,
				...
				$scope.popUpDaftarImplementasiAsuhanIGD.center().open();	
			}	

			$scope.columnDaftarImplementasiAsuhanIGD = {
				columns: [
					{
						"field": "tglemr",
						"title": "Tgl EMR",
			...
			$scope.cetakImplementasiAsuhanIGD = function () {
				if ($scope.dataSelectedImplementasiAsuhanIGD == undefined) {
					toastr.error('Data belum dipilih!!!')
						return;
				}
				...
				var local = JSON.parse(localStorage.getItem('profile'));
				var nama = medifirstService.getPegawaiLogin();
				window.open(baseTransaksi + 'report/cetak-implementasi-asuhan-keperawatan-igd?nocm='
				+ $scope.dataSelectedImplementasiAsuhanIGD.nocm 
				+ '&norec_apd=' + $scope.dataSelectedImplementasiAsuhanIGD.norec_apd 
				+ '&emr=' + $scope.dataSelectedImplementasiAsuhanIGD.norec 
				+ '&emrfk=' + $scope.dataSelectedImplementasiAsuhanIGD.emrfk
				+ '&index=' + $scope.dataSelectedImplementasiAsuhanIGD.index
				+ '&kdprofile=' + local.id
				+ '&nama=' + nama, '_blank');
			}

			$scope.implementasiAsuhanRanap = function(){
				if ($scope.dataPasienSelected.noregistrasi == undefined) {
					toastr.error('Pilih Pasien Terlebih dahulu!!!')
						return;
				}
				var emrfk = [290021] // emrfk Implementasi Asuhan Keperawatan Ranap

				medifirstService.get("bridging/inacbg/get-rincian-operasi?noregistrasi=" + $scope.dataPasienSelected.noregistrasi + '&emrfk=' + emrfk
					// medifirstService.get("lab-radiologi/get-rincian-pelayanan?objectdepartemenfk=" + departemenfk + "&noregistrasi=" +   $scope.item.noregistrasi
					, true).then(function (dat) {
						$scope.dataDaftarImplementasiAsuhanRanap = {
							data: dat.data.data,
							_data: dat.data.data,
							// pageSize: 10,
							selectable: true,
				...
				$scope.popUpDaftarImplementasiAsuhanRanap.center().open();
			}

			$scope.columnDaftarImplementasiAsuhanRanap = {
				columns: [
					{
						"field": "tglemr",
						"title": "Tgl EMR",
			...
			$scope.cetakImplementasiAsuhanRanap = function () {
				if ($scope.dataSelectedImplementasiAsuhanRanap == undefined) {
					toastr.error('Data belum dipilih!!!')
						return;
				}
				var local = JSON.parse(localStorage.getItem('profile'));
				var nama = medifirstService.getPegawaiLogin();
				window.open(baseTransaksi + 'report/cetak-implementasi-asuhan-keperawatan-ranap?nocm='
				+ $scope.dataSelectedImplementasiAsuhanRanap.nocm 
				+ '&norec_apd=' + $scope.dataSelectedImplementasiAsuhanRanap.norec_apd 
				+ '&emr=' + $scope.dataSelectedImplementasiAsuhanRanap.norec 
				+ '&emrfk=' + $scope.dataSelectedImplementasiAsuhanRanap.emrfk
				+ '&index=' + $scope.dataSelectedImplementasiAsuhanRanap.index
				+ '&kdprofile=' + local.id
				+ '&nama=' + nama, '_blank');
			}
			...
			$scope.cetakRingkasanPulang = function () {
				if ($scope.dataSelectedRingkasanPulang == undefined) {
					toastr.error('Data belum dipilih!!!')
						return;
				}
			...
			$scope.cetakSuketKematian = function () {
				if ($scope.dataSelectedSuketKematian == undefined) {
					toastr.error('Data belum dipilih!!!')
						return;
				}
			...
			$scope.suketKelahiran = function(){
				if ($scope.dataPasienSelected.noregistrasi == undefined) {
					toastr.error('Pilih Pasien Terlebih dahulu!!!')
						return;
				}
				var emrfk = [290153,290154] // emrfk Surat Kelahiran Ranap Rajal

				medifirstService.get("bridging/inacbg/get-emr-pasien?noregistrasi=" + $scope.dataPasienSelected.noregistrasi + '&emrfk=' + emrfk
					// medifirstService.get("lab-radiologi/get-rincian-pelayanan?objectdepartemenfk=" + departemenfk + "&noregistrasi=" +   $scope.item.noregistrasi
					, true).then(function (dat) {
						$scope.dataDaftadSuketKelahiran = {
							data: dat.data.data,
							_data: dat.data.data,
							// pageSize: 10,
							selectable: true,
					...
					$scope.popUpDaftarSuketKelahiran.center().open();	
			}

			$scope.columnDaftadSuketKelahiran = {
				columns: [
					{
						"field": "tglemr",
						"title": "Tgl EMR",
			...
			$scope.cetakSuketKelahiran = function () {
				if ($scope.dataSelectedSuketKelahiran == undefined) {
					toastr.error('Data belum dipilih!!!')
						return;
				}
				...
				var local = JSON.parse(localStorage.getItem('profile'));
				var nama = medifirstService.getPegawaiLogin();
				window.open(baseTransaksi + 'report/cetak-suket-kelahiran?nocm='
				+ $scope.dataSelectedSuketKelahiran.nocm 
				+ '&norec_apd=' + $scope.dataSelectedSuketKelahiran.norec_apd 
				+ '&emr=' + $scope.dataSelectedSuketKelahiran.norec 
				+ '&emrfk=' + $scope.dataSelectedSuketKelahiran.emrfk
				+ '&kdprofile=' + local.id
				+ '&nama=' + nama, '_blank');
			}
			...
			$scope.ekg = function(){
				...
				$scope.popUpDaftarEKG.center().open();
			}

			$scope.columnDaftarEKG = {
				columns: [
					{
						"field": "tglemr",
						"title": "Tgl EMR",
			...
			$scope.cetakEKG = function () {
				if ($scope.dataSelectedEKG == undefined) {
					toastr.error('Data belum dipilih!!!')
						return;
				}
				var local = JSON.parse(localStorage.getItem('profile'));
				var nama = medifirstService.getPegawaiLogin();
				window.open(baseTransaksi + 'report/cetak-ekg?nocm='
				+ $scope.dataSelectedEKG.nocm 
				+ '&norec_apd=' + $scope.dataSelectedEKG.norec_apd 
				+ '&emr=' + $scope.dataSelectedEKG.norec 
				+ '&emrfk=' + $scope.dataSelectedEKG.emrfk
				+ '&index=' + $scope.dataSelectedEKG.index
				+ '&kdprofile=' + local.id
				+ '&nama=' + nama, '_blank');
			}
			...
			$scope.echocardiografi = function(){
				...
				var emrfk = [290157] // emrfk Cardiografi

				medifirstService.get("bridging/inacbg/get-rincian-operasi?noregistrasi=" + $scope.dataPasienSelected.noregistrasi + '&emrfk=' + emrfk
					// medifirstService.get("lab-radiologi/get-rincian-pelayanan?objectdepartemenfk=" + departemenfk + "&noregistrasi=" +   $scope.item.noregistrasi
					, true).then(function (dat) {
						$scope.dataDaftarEchocardiografi = {
							data: dat.data.data,
							_data: dat.data.data,
							// pageSize: 10,
							selectable: true,
				...
				$scope.popUpDaftarEchocardiografi.center().open();
			}

			$scope.columnDaftarEchocardiografi = {
				columns: [
					{
						"field": "tglemr",
						"title": "Tgl EMR",
			...
			$scope.cetakEchocardiografi = function () {
				if ($scope.dataSelectedEchocardiografi == undefined) {
					toastr.error('Data belum dipilih!!!')
						return;
				}
				var local = JSON.parse(localStorage.getItem('profile'));
			...
			$scope.profilRingkas = function(){
				if ($scope.dataPasienSelected.noregistrasi == undefined) {
					toastr.error('Pilih Pasien Terlebih dahulu!!!')
						return;
				}
				var emrfk = [290013] // emrfk Profil Ringkas Medis

				medifirstService.get("bridging/inacbg/get-emr-pasien?noregistrasi=" + $scope.dataPasienSelected.noregistrasi + '&emrfk=' + emrfk
					// medifirstService.get("lab-radiologi/get-rincian-pelayanan?objectdepartemenfk=" + departemenfk + "&noregistrasi=" +   $scope.item.noregistrasi
					, true).then(function (dat) {
						$scope.dataDaftarProfilRingkasMedis = {
							data: dat.data.data,
							_data: dat.data.data,
							// pageSize: 10,
							selectable: true,
				...
				$scope.popUpDaftarProfilRingkasMedis.center().open();
			}

			$scope.columnDaftarProfilRingkasMedis = {
				columns: [
					{
						"field": "tglemr",
						"title": "Tgl EMR",
			...
			$scope.cetakProfilRingkasMedis = function () {
				if ($scope.dataSelectedProfilRingkasMedis == undefined) {
					toastr.error('Data belum dipilih!!!')
						return;
				}
				var local = JSON.parse(localStorage.getItem('profile'));
				var nama = medifirstService.getPegawaiLogin();
				window.open(baseTransaksi + 'report/cetak-profil-ringkas?nocm='
				+ $scope.dataSelectedProfilRingkasMedis.nocm 
				+ '&norec_apd=' + $scope.dataSelectedProfilRingkasMedis.norec_apd 
				+ '&emr=' + $scope.dataSelectedProfilRingkasMedis.norec 
				+ '&emrfk=' + $scope.dataSelectedProfilRingkasMedis.emrfk
				+ '&kdprofile=' + local.id
				+ '&nama=' + nama, '_blank');
			}
			...
			$scope.bayiSepsis = function(){
				if ($scope.dataPasienSelected.noregistrasi == undefined) {
					toastr.error('Pilih Pasien Terlebih dahulu!!!')
					return;
				}
				var emrfk = [290085] // emrfk Bayi Sepsis

				medifirstService.get("bridging/inacbg/get-rincian-operasi?noregistrasi=" + $scope.dataPasienSelected.noregistrasi + '&emrfk=' + emrfk
					// medifirstService.get("lab-radiologi/get-rincian-pelayanan?objectdepartemenfk=" + departemenfk + "&noregistrasi=" +   $scope.item.noregistrasi
					, true).then(function (dat) {
						$scope.dataDaftarBayiSepsis = {
							data: dat.data.data,
							_data: dat.data.data,
							// pageSize: 10,
							selectable: true,
				...
				$scope.popUpDaftarBayiSepsis.center().open();
				
			}

			$scope.columnDaftarBayiSepsis = {
				columns: [
						{
							"field": "tglemr",
							"title": "Tgl EMR",
			...
			$scope.cetakBayiSepsis = function () {
				if ($scope.dataSelectedBayiSepsis == undefined) {
					toastr.error('Data belum dipilih!!!')
					return;
				}
				...
				var local = JSON.parse(localStorage.getItem('profile'));
				var nama = medifirstService.getPegawaiLogin();
				window.open(baseTransaksi + 'report/cetak-bayi-sepsis?nocm='
				+ $scope.dataSelectedBayiSepsis.nocm 
				+ '&norec_apd=' + $scope.dataSelectedBayiSepsis.norec_apd 
				+ '&emr=' + $scope.dataSelectedBayiSepsis.norec 
				+ '&emrfk=' + $scope.dataSelectedBayiSepsis.emrfk
				+ '&index=' + $scope.dataSelectedBayiSepsis.index
				+ '&kdprofile=' + local.id
				+ '&nama=' + nama, '_blank');
			}

			$scope.alatMonitoringCPAP = function(){
				if ($scope.dataPasienSelected.noregistrasi == undefined) {
					toastr.error('Pilih Pasien Terlebih dahulu!!!')
					return;
				}
				...
				$scope.popUpDaftarAlatMonitoringCPAP.center().open();
				
			}

			$scope.columnDaftarAlatMonitoringCPAP = {
				columns: [
						{
							"field": "tglemr",
							"title": "Tgl EMR",
			...
			$scope.cetakAlatMonitoringCPAP = function () {
				if ($scope.dataSelectedAlatMonitoringCPAP == undefined) {
					toastr.error('Data belum dipilih!!!')
					return;
				}
				...
				var local = JSON.parse(localStorage.getItem('profile'));
				var nama = medifirstService.getPegawaiLogin();
				window.open(baseTransaksi + 'report/cetak-alat-monitoring-cpap?nocm='
				+ $scope.dataSelectedAlatMonitoringCPAP.nocm 
			...
			$scope.flowsheet = function(){
				...
				$scope.popUpDaftarFlowsheet.center().open();
				
			}

			$scope.columnDaftarFlowsheet = {
				columns: [
						{
							"field": "tglemr",
							"title": "Tgl EMR",
			...
			$scope.cetakFlowsheet = function () {
				if ($scope.dataSelectedFlowsheet == undefined) {
					toastr.error('Data belum dipilih!!!')
					return;
				}
				...
				var local = JSON.parse(localStorage.getItem('profile'));
				var nama = medifirstService.getPegawaiLogin();
				window.open(baseTransaksi + 'report/cetak-flowsheet?nocm='
				+ $scope.dataSelectedFlowsheet.nocm 
				+ '&norec_apd=' + $scope.dataSelectedFlowsheet.norec_apd 
				+ '&emr=' + $scope.dataSelectedFlowsheet.norec 
				+ '&emrfk=' + $scope.dataSelectedFlowsheet.emrfk
				+ '&index=' + $scope.dataSelectedFlowsheet.index
				+ '&kdprofile=' + local.id
				+ '&nama=' + nama, '_blank');
			}

			$scope.pemantauanCPAP = function(){
				if ($scope.dataPasienSelected.noregistrasi == undefined) {
					toastr.error('Pilih Pasien Terlebih dahulu!!!')
					return;
				}
				var emrfk = [290132] // emrfk Pemantauan CPAP

				medifirstService.get("bridging/inacbg/get-rincian-operasi?noregistrasi=" + $scope.dataPasienSelected.noregistrasi + '&emrfk=' + emrfk
					// medifirstService.get("lab-radiologi/get-rincian-pelayanan?objectdepartemenfk=" + departemenfk + "&noregistrasi=" +   $scope.item.noregistrasi
					, true).then(function (dat) {
						$scope.dataDaftarPemantauanCPAP = {
							data: dat.data.data,
							_data: dat.data.data,
							// pageSize: 10,
							selectable: true,
				...
				$scope.popUpDaftarPemantauanCPAP.center().open();
				
			}

			$scope.columnDaftarPemantauanCPAP = {
				columns: [
						{
							"field": "tglemr",
							"title": "Tgl EMR",
			...
			$scope.cetakPemantauanCPAP = function () {
				if ($scope.dataSelectedPemantauanCPAP == undefined) {
					toastr.error('Data belum dipilih!!!')
					return;
				}
				...
				var local = JSON.parse(localStorage.getItem('profile'));
				var nama = medifirstService.getPegawaiLogin();
				window.open(baseTransaksi + 'report/cetak-pemantauan-cpap?nocm='
				+ $scope.dataSelectedPemantauanCPAP.nocm 
				+ '&norec_apd=' + $scope.dataSelectedPemantauanCPAP.norec_apd 
				+ '&emr=' + $scope.dataSelectedPemantauanCPAP.norec 
				+ '&emrfk=' + $scope.dataSelectedPemantauanCPAP.emrfk
				+ '&index=' + $scope.dataSelectedPemantauanCPAP.index
				+ '&kdprofile=' + local.id
				+ '&nama=' + nama, '_blank');
			}
			...
			$scope.buktiPelayananFisioterapi = function(){
				...
				$scope.popUpDaftarBuktiPelayananTindakan.center().open();
				
			}

			$scope.columnDaftarBuktiPelayananTindakan = {
				columns: [
						{
								"field": "tglemr",
								"title": "Tgl EMR",
		...
		$scope.cetakBuktiPelayananTindakan = function () {
			...
			$scope.lembarFormulirRajal = function(){
				...
				var emrfk = [290161] // emrfk Lembar Formulir Rajal

				medifirstService.get("bridging/inacbg/get-emr-pasien?noregistrasi=" + $scope.dataPasienSelected.noregistrasi + '&emrfk=' + emrfk
					// medifirstService.get("lab-radiologi/get-rincian-pelayanan?objectdepartemenfk=" + departemenfk + "&noregistrasi=" +   $scope.item.noregistrasi
					, true).then(function (dat) {
						$scope.dataDaftarlembarFormulirRajal = {
							data: dat.data.data,
							_data: dat.data.data,
							// pageSize: 10,
							selectable: true,
				...
				$scope.popUpDaftarlembarFormulirRajal.center().open();
				
			}

			$scope.columnDaftarlembarFormulirRajal = {
				columns: [
						{
								"field": "tglemr",
								"title": "Tgl EMR",
			...
			$scope.cetaklembarFormulirRajal = function () {
					if ($scope.dataSelectedlembarFormulirRajal == undefined) {
						toastr.error('Data belum dipilih!!!')
						return;
					}
					...
					var local = JSON.parse(localStorage.getItem('profile'));
					var nama = medifirstService.getPegawaiLogin();
					window.open(baseTransaksi + 'report/cetak-lembar-formulir-rajal?nocm='
					+ $scope.dataSelectedlembarFormulirRajal.nocm 
			...
			$scope.hiperbilirubin = function(){
				if ($scope.dataPasienSelected.noregistrasi == undefined) {
					toastr.error('Pilih Pasien Terlebih dahulu!!!')
					return;
				}
				var emrfk = [290121] // emrfk Hiperbilirubin

				medifirstService.get("bridging/inacbg/get-emr-pasien?noregistrasi=" + $scope.dataPasienSelected.noregistrasi + '&emrfk=' + emrfk
					// medifirstService.get("lab-radiologi/get-rincian-pelayanan?objectdepartemenfk=" + departemenfk + "&noregistrasi=" +   $scope.item.noregistrasi
					, true).then(function (dat) {
						$scope.dataDaftarHiperbilirubin = {
							data: dat.data.data,
							_data: dat.data.data,
							// pageSize: 10,
							selectable: true,
				...
				$scope.popUpDaftarHiperbilirubin.center().open();
				
			}

			$scope.columnDaftarHiperbilirubin = {
				columns: [
						{
								"field": "tglemr",
								"title": "Tgl EMR",
			...
			$scope.cetakHiperbilirubin = function () {
					if ($scope.dataSelectedHiperbilirubin == undefined) {
						toastr.error('Data belum dipilih!!!')
						return;
					}
					...
					var local = JSON.parse(localStorage.getItem('profile'));
					var nama = medifirstService.getPegawaiLogin();
					window.open(baseTransaksi + 'report/cetak-hiperbilirubin?nocm='
					+ $scope.dataSelectedHiperbilirubin.nocm 
					+ '&norec_apd=' + $scope.dataSelectedHiperbilirubin.norec_apd 
					+ '&emr=' + $scope.dataSelectedHiperbilirubin.norec 
					+ '&emrfk=' + $scope.dataSelectedHiperbilirubin.emrfk
					+ '&kdprofile=' + local.id
					+ '&nama=' + nama, '_blank');
			}

			$scope.formulirPermintaanDarah = function(){
				if ($scope.dataPasienSelected.noregistrasi == undefined) {
					toastr.error('Pilih Pasien Terlebih dahulu!!!')
					return;
				}
				var emrfk = [290095] // emrfk Formulir Permintaan Darah

				medifirstService.get("bridging/inacbg/get-all-page-inacbg?noregistrasi=" + $scope.dataPasienSelected.noregistrasi + '&emrfk=' + emrfk
					// medifirstService.get("lab-radiologi/get-rincian-pelayanan?objectdepartemenfk=" + departemenfk + "&noregistrasi=" +   $scope.item.noregistrasi
					, true).then(function (dat) {
						$scope.dataDaftarPermintaanDarah = {
							data: dat.data.data,
							_data: dat.data.data,
							// pageSize: 10,
							selectable: true,
				...
				$scope.popUpDaftarPermintaanDarah.center().open();
				
			}

			$scope.columnDaftarPermintaanDarah = {
				columns: [
						{
							"field": "tglemr",
							"title": "Tgl EMR",
			...
			$scope.cetakPermintaanDarah = function () {
				if ($scope.dataSelectedPermintaanDarah == undefined) {
					toastr.error('Data belum dipilih!!!')
					return;
				}
			...
			$scope.tindakanFisioterapi = function(){
				if ($scope.dataPasienSelected.noregistrasi == undefined) {
					toastr.error('Pilih Pasien Terlebih dahulu!!!')
					return;
				}
				var emrfk = [290162] // emrfk Tindakan Fisioterapi
				...
				$scope.popUpDaftartindakanFisioterapi.center().open();
				
			}

			$scope.columnDaftartindakanFisioterapi = {
				columns: [
						{
								"field": "tglemr",
								"title": "Tgl EMR",
			...
			$scope.cetaktindakanFisioterapi = function () {
					if ($scope.dataSelectedtindakanFisioterapi == undefined) {
						toastr.error('Data belum dipilih!!!')
						return;
					}
					...
					var local = JSON.parse(localStorage.getItem('profile'));
					var nama = medifirstService.getPegawaiLogin();
					window.open(baseTransaksi + 'report/cetak-tindakan-fisioterapi?nocm='
					+ $scope.dataSelectedtindakanFisioterapi.nocm 
					+ '&norec_apd=' + $scope.dataSelectedtindakanFisioterapi.norec_apd 
					+ '&emr=' + $scope.dataSelectedtindakanFisioterapi.norec 
			...
			$scope.asesmenAwalMedisRajal = function(){
				if ($scope.dataPasienSelected.noregistrasi == undefined) {
					toastr.error('Pilih Pasien Terlebih dahulu!!!')
					return;
				}
				var emrfk = [290012] // emrfk Asesmen Awal Medis Rajal

				medifirstService.get("bridging/inacbg/get-emr-pasien?noregistrasi=" + $scope.dataPasienSelected.noregistrasi + '&emrfk=' + emrfk
					// medifirstService.get("lab-radiologi/get-rincian-pelayanan?objectdepartemenfk=" + departemenfk + "&noregistrasi=" +   $scope.item.noregistrasi
					, true).then(function (dat) {
						$scope.dataDaftarAsesmenAwalMedisRajal = {
							data: dat.data.data,
							_data: dat.data.data,
							// pageSize: 10,
							selectable: true,
				...
				$scope.popUpDaftarAsesmenAwalMedisRajal.center().open();
				
			}

			$scope.columnDaftarAsesmenAwalMedisRajal = {
				columns: [
						{
								"field": "tglemr",
								"title": "Tgl EMR",
			...
			$scope.cetakAsesmenAwalMedisRajal = function () {
					if ($scope.dataSelectedAsesmenAwalMedisRajal == undefined) {
						toastr.error('Data belum dipilih!!!')
						return;
					}
					...
					var local = JSON.parse(localStorage.getItem('profile'));
					var nama = medifirstService.getPegawaiLogin();
					window.open(baseTransaksi + 'report/cetak-asesmen-awal-medis-rajal?nocm='
					+ $scope.dataSelectedAsesmenAwalMedisRajal.nocm 
					+ '&norec_apd=' + $scope.dataSelectedAsesmenAwalMedisRajal.norec_apd 
					+ '&emr=' + $scope.dataSelectedAsesmenAwalMedisRajal.norec 
					+ '&emrfk=' + $scope.dataSelectedAsesmenAwalMedisRajal.emrfk
					+ '&kdprofile=' + local.id
					+ '&nama=' + nama, '_blank');
			}

			$scope.lembarPenggunaanVentilator = function(){
				if ($scope.dataPasienSelected.noregistrasi == undefined) {
					toastr.error('Pilih Pasien Terlebih dahulu!!!')
					return;
				}
				var emrfk = [290158] // emrfk Lembar Penggunaan Ventilator

				medifirstService.get("bridging/inacbg/get-rincian-operasi?noregistrasi=" + $scope.dataPasienSelected.noregistrasi + '&emrfk=' + emrfk
					// medifirstService.get("lab-radiologi/get-rincian-pelayanan?objectdepartemenfk=" + departemenfk + "&noregistrasi=" +   $scope.item.noregistrasi
					, true).then(function (dat) {
						$scope.dataDaftarPenggunaanVentilator = {
							data: dat.data.data,
							_data: dat.data.data,
							// pageSize: 10,
							selectable: true,
				...
				$scope.popUpDaftarPenggunaanVentilator.center().open();
				
			}

			$scope.columnDaftarPenggunaanVentilator = {
				columns: [
						{
							"field": "tglemr",
							"title": "Tgl EMR",
			...
			$scope.cetakPenggunaanVentilator = function () {
				if ($scope.dataSelectedPenggunaanVentilator == undefined) {
					toastr.error('Data belum dipilih!!!')
					return;
				}
				...
				var local = JSON.parse(localStorage.getItem('profile'));
				var nama = medifirstService.getPegawaiLogin();
				window.open(baseTransaksi + 'report/cetak-lembar-penggunaan-ventilator?nocm='
				+ $scope.dataSelectedPenggunaanVentilator.nocm 
				+ '&norec_apd=' + $scope.dataSelectedPenggunaanVentilator.norec_apd 
				+ '&emr=' + $scope.dataSelectedPenggunaanVentilator.norec 
				+ '&emrfk=' + $scope.dataSelectedPenggunaanVentilator.emrfk
				+ '&index=' + $scope.dataSelectedPenggunaanVentilator.index
				+ '&kdprofile=' + local.id
				+ '&nama=' + nama, '_blank');
			}
			...
			$scope.cetakSuratKeteranganKontrol = function(){
				...
				var emrfk = [290175] // emrfk Surat Keterangan Kontrol

				medifirstService.get("bridging/inacbg/get-emr-pasien?noregistrasi=" + $scope.dataPasienSelected.noregistrasi + '&emrfk=' + emrfk
					// medifirstService.get("lab-radiologi/get-rincian-pelayanan?objectdepartemenfk=" + departemenfk + "&noregistrasi=" +   $scope.item.noregistrasi
					, true).then(function (dat) {
						$scope.dataDaftarSuratKeteranganKontrol = {
							data: dat.data.data,
							_data: dat.data.data,
							// pageSize: 10,
							selectable: true,
				...
				$scope.popUpDaftarSuratKeteranganKontrol.center().open();
				
			}

			$scope.columnDaftarSuratKeteranganKontrol = {
				columns: [
						{
								"field": "tglemr",
								"title": "Tgl EMR",
			...
			$scope.cetakSuratKeteranganKontrol = function () {
					if ($scope.dataSelectedSuratKeteranganKontrol == undefined) {
						toastr.error('Data belum dipilih!!!')
						return;
					}
					...
					var local = JSON.parse(localStorage.getItem('profile'));
					var nama = medifirstService.getPegawaiLogin();
					window.open(baseTransaksi + 'report/cetak-surat-keterangan-kontrol?nocm='
					+ $scope.dataSelectedSuratKeteranganKontrol.nocm 
					+ '&norec_apd=' + $scope.dataSelectedSuratKeteranganKontrol.norec_apd 
					+ '&emr=' + $scope.dataSelectedSuratKeteranganKontrol.norec 
					+ '&emrfk=' + $scope.dataSelectedSuratKeteranganKontrol.emrfk
					+ '&kdprofile=' + local.id
					+ '&nama=' + nama, '_blank');
			}
