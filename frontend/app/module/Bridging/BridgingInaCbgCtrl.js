								...
								dt1 = {
									...
									"data": {
										...
										"tarif_rs": {
											...
											"bmhp": dataSave[i].tarif_rs.bmhp,    //"400000",      
											"sewa_alat": dataSave[i].tarif_rs.sewa_alat,    //"210000"    
											"tidakan_lain": dataSave[i].tarif_rs.tidakan_lain,    //"210000"    
										},
										"tarif_poli_eks": dataSave[i].tarif_poli_eks,    //"100000",    
										"nama_dokter": dataSave[i].nama_dokter,    //"RUDY, DR",    
										"kode_tarif": dataSave[i].kode_tarif,    //"AP",    
	...
	$scope.tryUpload = function () {
		toastr.warning("Masih dalam tahap pengembangan")
			...
			$('body ').on('click','#cariDokumen',function(e){
                ...
                var noregistrasi = $(this).data("noreg");
                $scope.isRouteLoading = true
				medifirstService.get(`bridging/inacbg/get-parameter?id=${id}&noregistrasi=${noregistrasi}&user=${$scope.dataPasienSelected.namapasien}`).then(function (e) {
					$scope.isRouteLoading = false
