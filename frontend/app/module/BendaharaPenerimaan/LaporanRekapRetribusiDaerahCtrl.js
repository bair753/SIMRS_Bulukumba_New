define(['initialize'], function (initialize) {
    'use strict';
    initialize.controller('LaporanRekapRetribusiDaerahCtrl', ['CacheHelper', '$q', '$rootScope', '$scope', 'MedifirstService', 'DateHelper',
        function (cacheHelper, $q, $rootScope, $scope, medifirstService, DateHelper) {
            ...
            function LoadData() {
                ...
                cacheHelper.set('LaporanRekapRetribusiDaerahCtrl', chacePeriode);

                medifirstService.get("bendaharapenerimaan/get-data-rekapitulasi-pendapatan-daerah?"
                    + "tglAwal=" + tglAwal
                            ...
                            schema: {
                                model: {
                                    fields: {
                                        tunai: { type: "number" },
                                        bpjs: { type: "number" },
                                        kabbogor: { type: "number" },
                                        kotbogor: { type: "number" },
                                        kotdepok: { type: "number" },
                                    }
                                }
                            },
                            aggregate: [
                                ...
                                { field: 'kotbogor', aggregate: 'sum' },
                                { field: 'kabbogor', aggregate: 'sum' },
                                { field: 'kotdepok', aggregate: 'sum' },
                            ]                            
                        });
                    })
            }
            ...
            // $scope.group = [
            //     {
            //         field: "kelompokpenerimaan_produk",
            //         aggregates: [
            //             {
            //                 field: "kelompokpenerimaan_produk",
            //                 aggregate: "count"
                    ...
                    columns:
                        [
                            ...
                            },
                            {
                                ...
                                aggregates: ["sum"],
                                footerTemplate: "#: data.jassar.sum #",
                                footerTemplate: "<span class='style-right'>{{formatRupiah('#: data.tunai.sum #', 'Rp.')}}</span>",
                            ...
                            },
                            {
                                ...
                                // "template": "<span class='style-right'>{{formatRupiah('#: total #', 'Rp.')}}</span>",
                                aggregates: ["sum"],
                                footerTemplate: "#: data.jaspel.sum #",
                                footerTemplate: "<span class='style-right'>{{formatRupiah('#: data.bpjs.sum #', 'Rp.')}}</span>",
                                footerAttributes: { style: "text-align: right;" }
                            },
                            {
                                "field": "kabbogor",
                                "title": "Jamkesda Kabupaten Bogor",
                                "width": "80px",
                                //"template": "<span class='style-right'>{{formatRupiah('#: total #', 'Rp.')}}</span>",
                                aggregates: ["sum"],
                                footerTemplate: "#: data.jassar.sum #",
                                footerTemplate: "<span class='style-right'>{{formatRupiah('#: data.kabbogor.sum #', 'Rp.')}}</span>",
                                footerAttributes: { style: "text-align: right;" }
                            },
                            {
                                "field": "kotbogor",
                                "title": "Jamkesda Kota Bogor",
                                "width": "80px",
                                // "template": "<span class='style-right'>{{formatRupiah('#: total #', 'Rp.')}}</span>",
                                aggregates: ["sum"],
                                footerTemplate: "#: data.jaspel.sum #",
                                footerTemplate: "<span class='style-right'>{{formatRupiah('#: data.kotbogor.sum #', 'Rp.')}}</span>",
                                footerAttributes: { style: "text-align: right;" }
                            },
                            {
                                "field": "kotdepok",
                                "title": "Jamkesda Kota Depok",
                                "width": "80px",
                                // "template": "<span class='style-right'>{{formatRupiah('#: total #', 'Rp.')}}</span>",
                                aggregates: ["sum"],
                                footerTemplate: "#: data.jaspel.sum #",
                                footerTemplate: "<span class='style-right'>{{formatRupiah('#: data.kotdepok.sum #', 'Rp.')}}</span>",
                                footerAttributes: { style: "text-align: right;" }
                            },
                        ],
                    sortable: {
