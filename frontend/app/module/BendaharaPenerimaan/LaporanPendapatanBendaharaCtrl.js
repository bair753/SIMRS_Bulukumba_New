define(['initialize'], function (initialize) {
    'use strict';
    initialize.controller('LaporanPendapatanBendaharaCtrl', ['CacheHelper', '$q', '$rootScope', '$scope', 'MedifirstService', 'DateHelper',
        function (cacheHelper, $q, $rootScope, $scope, medifirstService, DateHelper) {
            ...
            function LoadData() {
                ...
                cacheHelper.set('LaporanPendapatanBendaharaCtrl', chacePeriode);

                medifirstService.get("bendaharapenerimaan/get-data-pendapatan-bendahara?"
                    + "tglAwal=" + tglAwal
                    + "&tglAkhir=" + tglAkhir).then(function (data) {
                        ...
                        $scope.dataPasienPulang = new kendo.data.DataSource({
                            ...
                            aggregate: [
                                { field: 'totalp', aggregate: 'sum' },
                                { field: 'totalt', aggregate: 'sum' },
                                { field: 'jaspelp', aggregate: 'sum' },
                                { field: 'jassarp', aggregate: 'sum' },
                                { field: 'jaspelt', aggregate: 'sum' },
                                { field: 'jassart', aggregate: 'sum' },                               
                            ]                            
                        });
                    })
            }
            ...
            $scope.columnPasienPulang =
                {
                    ...
                    excel: {
                        fileName: "LaporanPendapatan.xlsx",
                        allPages: true,
                    },
                    excelExport: function (e) {
                        ...
                        var myHeaders = [
                            {
                                value: "LAPORAN PENDAPATAN " + DateHelper.formatDate($scope.item.tglAwal, 'DD-MM-YY') + ' sampai ' +
                    ...
                    columns:
                        [
                            ...
                            },
                            {
                                ...
                                footerTemplate: "<span class='style-right'>{{formatRupiah('#: data.jumlah.sum #', 'Rp.')}}</span>",
                            ...
                            },
                            {
                                ...
                                // "template": "<span class='style-right'>{{formatRupiah('#: total #', 'Rp.')}}</span>",
                                aggregates: ["sum"],
                                footerTemplate: "#: data.totalp.sum #",
                                footerTemplate: "<span class='style-right'>{{formatRupiah('#: data.totalp.sum #', 'Rp.')}}</span>",
                                footerAttributes: { style: "text-align: right;" }
                            },
                            {
                                "field": "totalt",
                                ...
                                //"template": "<span class='style-right'>{{formatRupiah('#: total #', 'Rp.')}}</span>",
                                aggregates: ["sum"],
                                footerTemplate: "#: data.totalt.sum #",
                                footerTemplate: "<span class='style-right'>{{formatRupiah('#: data.totalt.sum #', 'Rp.')}}</span>",
                                footerAttributes: { style: "text-align: right;" }
                            },
                            {
                                "field": "jassarp",
                                ...
                                // "template": "<span class='style-right'>{{formatRupiah('#: total #', 'Rp.')}}</span>",
                                aggregates: ["sum"],
                                footerTemplate: "#: data.jassarp.sum #",
                                footerTemplate: "<span class='style-right'>{{formatRupiah('#: data.jassarp.sum #', 'Rp.')}}</span>",
                                footerAttributes: { style: "text-align: right;" }
                            },
                            {
                                "field": "jaspelp",
                                ...
                                // "template": "<span class='style-right'>{{formatRupiah('#: total #', 'Rp.')}}</span>",
                                aggregates: ["sum"],
                                footerTemplate: "#: data.jaspelp.sum #",
                                footerTemplate: "<span class='style-right'>{{formatRupiah('#: data.jaspelp.sum #', 'Rp.')}}</span>",
                                footerAttributes: { style: "text-align: right;" }
                            },
                            {
                                "field": "jassart",
                                ...
                                // "template": "<span class='style-right'>{{formatRupiah('#: total #', 'Rp.')}}</span>",
                                aggregates: ["sum"],
                                footerTemplate: "#: data.jassart.sum #",
                                footerTemplate: "<span class='style-right'>{{formatRupiah('#: data.jassart.sum #', 'Rp.')}}</span>",
                                footerAttributes: { style: "text-align: right;" }
                            },
                            {
                                "field": "jaspelt",
                                ...
                                // "template": "<span class='style-right'>{{formatRupiah('#: total #', 'Rp.')}}</span>",
                                aggregates: ["sum"],
                                footerTemplate: "#: data.jaspelt.sum #",
                                footerTemplate: "<span class='style-right'>{{formatRupiah('#: data.jaspelt.sum #', 'Rp.')}}</span>",
                                footerAttributes: { style: "text-align: right;" }
                            }
                        ],
                    sortable: {
