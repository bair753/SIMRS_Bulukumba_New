define(['initialize'], function (initialize) {
    'use strict';
    initialize.controller('LaporanPendapatanRetribusiDaerahCtrl', ['CacheHelper', '$q', '$rootScope', '$scope', 'MedifirstService', 'DateHelper',
        function (cacheHelper, $q, $rootScope, $scope, medifirstService, DateHelper) {
            ...
            function LoadData() {
                ...
                cacheHelper.set('LaporanPendapatanRetribusiDaerahCtrl', chacePeriode);

                medifirstService.get("bendaharapenerimaan/get-data-rekapitulasi-pendapatan-daerahtahun?"
                    + "tglAwal=" + tglAwal
                            ...
                            schema: {
                                model: {
                                    fields: {
                                        ...
                                        desember: { type: "number" },
                                        total: { type: "number" }
                                    }
                                }
                            },
                            aggregate: [
                                { field: 'januari', aggregate: 'sum' },
                                { field: 'febuari', aggregate: 'sum' },
                                { field: 'maret', aggregate: 'sum' },
                                { field: 'april', aggregate: 'sum' },
                                { field: 'mei', aggregate: 'sum' },
                                { field: 'juni', aggregate: 'sum' },
                                { field: 'juli', aggregate: 'sum' },
                                { field: 'agustus', aggregate: 'sum' },
                                { field: 'september', aggregate: 'sum' },
                                { field: 'oktober', aggregate: 'sum' },
                                { field: 'november', aggregate: 'sum' },
                                { field: 'desember', aggregate: 'sum' },
            ...
            $scope.columnPasienPulang =
                {
                    ...
                    excel: {
                        fileName: "LaporanRekapitulasiPendapatan.xlsx",
                        allPages: true,
                    },
                    excelExport: function (e) {
                        ...
                        var myHeaders = [
                            {
                                value: "LAPORAN REKAPITULASI PENDAPATAN " + DateHelper.formatDate($scope.item.tglAwal, 'DD-MM-YY') + ' sampai ' +
                    ...
                    columns:
                        [
                            ...
                            },
                            {
                                ...
                                "title": "URAIAN",
                                "width": "180px",
                                "template": "<span class='style-center'>#: lingkuppelayanan #</span>",
                                footerTemplate: "<span>Total </span>"
                            },
                            {
                                "field": "januari",
                                ...
                                aggregates: ["sum"],
                                footerTemplate: "#: data.januari.sum #",
                                footerTemplate: "<span class='style-right'>{{formatRupiah('#: data.januari.sum #', 'Rp.')}}</span>",
                            ...
                            },
                            {
                                ...
                                // "template": "<span class='style-right'>{{formatRupiah('#: total #', 'Rp.')}}</span>",
                                aggregates: ["sum"],
                                footerTemplate: "#: data.maret.sum #",
                                footerTemplate: "<span class='style-right'>{{formatRupiah('#: data.maret.sum #', 'Rp.')}}</span>",
                                footerAttributes: { style: "text-align: right;" }
                            },
                            {
                                "field": "april",
                                ...
                                //"template": "<span class='style-right'>{{formatRupiah('#: total #', 'Rp.')}}</span>",
                                aggregates: ["sum"],
                                footerTemplate: "#: data.april.sum #",
                                footerTemplate: "<span class='style-right'>{{formatRupiah('#: data.april.sum #', 'Rp.')}}</span>",
                                footerAttributes: { style: "text-align: right;" }
                            },
                            {
                                "field": "mei",
                                ...
                                // "template": "<span class='style-right'>{{formatRupiah('#: total #', 'Rp.')}}</span>",
                                aggregates: ["sum"],
                                footerTemplate: "#: data.mei.sum #",
                                footerTemplate: "<span class='style-right'>{{formatRupiah('#: data.mei.sum #', 'Rp.')}}</span>",
                                footerAttributes: { style: "text-align: right;" }
                            },                        
                            {
                                "field": "juni",
                                ...
                                //"template": "<span class='style-right'>{{formatRupiah('#: total #', 'Rp.')}}</span>",
                                aggregates: ["sum"],
                                footerTemplate: "#: data.juni.sum #",
                                footerTemplate: "<span class='style-right'>{{formatRupiah('#: data.juni.sum #', 'Rp.')}}</span>",
                                footerAttributes: { style: "text-align: right;" }
                            },
                            {
                                "field": "juli",
                                ...
                                // "template": "<span class='style-right'>{{formatRupiah('#: total #', 'Rp.')}}</span>",
                                aggregates: ["sum"],
                                footerTemplate: "#: data.juli.sum #",
                                footerTemplate: "<span class='style-right'>{{formatRupiah('#: data.juli.sum #', 'Rp.')}}</span>",
                                footerAttributes: { style: "text-align: right;" }
                            },
                            {
                                "field": "agustus",
                                ...
                                //"template": "<span class='style-right'>{{formatRupiah('#: total #', 'Rp.')}}</span>",
                                aggregates: ["sum"],
                                footerTemplate: "#: data.agustus.sum #",
                                footerTemplate: "<span class='style-right'>{{formatRupiah('#: data.agustus.sum #', 'Rp.')}}</span>",
                                footerAttributes: { style: "text-align: right;" }
                            },
                            {
                                "field": "september",
                                ...
                                // "template": "<span class='style-right'>{{formatRupiah('#: total #', 'Rp.')}}</span>",
                                aggregates: ["sum"],
                                footerTemplate: "#: data.september.sum #",
                                footerTemplate: "<span class='style-right'>{{formatRupiah('#: data.september.sum #', 'Rp.')}}</span>",
                                footerAttributes: { style: "text-align: right;" }
                            },
                            {
                                "field": "oktober",
                                ...
                                //"template": "<span class='style-right'>{{formatRupiah('#: total #', 'Rp.')}}</span>",
                                aggregates: ["sum"],
                                footerTemplate: "#: data.oktober.sum #",
                                footerTemplate: "<span class='style-right'>{{formatRupiah('#: data.oktober.sum #', 'Rp.')}}</span>",
                                footerAttributes: { style: "text-align: right;" }
                            },
                            {
                                "field": "november",
                                ...
                                // "template": "<span class='style-right'>{{formatRupiah('#: total #', 'Rp.')}}</span>",
                                aggregates: ["sum"],
                                footerTemplate: "#: data.november.sum #",
                                footerTemplate: "<span class='style-right'>{{formatRupiah('#: data.november.sum #', 'Rp.')}}</span>",
                                footerAttributes: { style: "text-align: right;" }
                            },
                            {
                                "field": "desember",
                                ...
                                // "template": "<span class='style-right'>{{formatRupiah('#: total #', 'Rp.')}}</span>",
                                aggregates: ["sum"],
                                footerTemplate: "#: data.desember.sum #",
                                footerTemplate: "<span class='style-right'>{{formatRupiah('#: data.desember.sum #', 'Rp.')}}</span>",
                                footerAttributes: { style: "text-align: right;" }
                            },
                            {
                                "field": "total",
