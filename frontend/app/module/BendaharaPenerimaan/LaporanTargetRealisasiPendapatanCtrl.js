define(['initialize'], function (initialize) {
    'use strict';
    initialize.controller('LaporanTargetRealisasiPendapatanCtrl', ['CacheHelper', '$q', '$rootScope', '$scope', 'MedifirstService', 'DateHelper',
        function (cacheHelper, $q, $rootScope, $scope, medifirstService, DateHelper) {
            ...
            function LoadData() {
                ...
                cacheHelper.set('LaporanTargetRealisasiPendapatanCtrl', chacePeriode);

                medifirstService.get("kasir/get-data-lap-target-realisasi-pendapatan?"
                    + "tglAwal=" + tglAwal
							...
							schema: {
								model: {
									fields: {
                                        ...
                                        totalrealisai: { type: "number" }
									}
								}
							},
							aggregate: [
								{ field: 'totaltarget', aggregate: 'sum' },
                                { field: 'totalrealisai', aggregate: 'sum' },
							]
						});                        
                    })
            }
            ...
            $scope.columnPasienPulang =
                {
                    ...
                    excel: {
                        fileName: "LaporanRincianPenerimaanMingguan.xlsx",
                        allPages: true,
                    },
                    ...
                    columns:
                        [
                            ...
                            },
                            {
                                ...
                                aggregates: ["sum"],
                                footerTemplate: "#: data.totaltarget.sum #",
                                footerTemplate: "<span class='style-right'>{{formatRupiah('#: data.totaltarget.sum #', 'Rp.')}}</span>",
                            ...
                            },
                            {
                                ...
                                "title": "Realisasi",
                                "width": "100px",
                                "template": "<span class='style-right'>{{formatRupiah('#: totalrealisai #', 'Rp.')}}</span>",
                                aggregates: ["sum"],
                                footerTemplate: "#: data.totalrealisai.sum #",
                                footerTemplate: "<span class='style-right'>{{formatRupiah('#: data.totalrealisai.sum #', 'Rp.')}}</span>",
                                footerAttributes: { style: "text-align: right;" }
                            },
                            {
                                "field": "persen",
                                ...
                                // aggregates: ["sum"],
                                // footerTemplate: "#: data.persen.sum #",
                                // footerTemplate: "<span class='style-right'>{{formatRupiah('#: data.persen.sum #', 'Rp.')}}</span>",
                                // footerAttributes: { style: "text-align: right;" }
                            },
                            {
                                hidden: true,
