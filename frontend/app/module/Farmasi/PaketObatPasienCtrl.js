define(['initialize'], function(initialize) {
    'use strict';
    initialize.controller('PaketObatPasienCtrl', ['SaveToWindow', '$rootScope', '$scope', 'ModelItem', '$state', 'CacheHelper','DateHelper',  '$mdDialog', 'CetakHelper', 'MedifirstService','$q', 
        function (saveToWindow,$rootScope, $scope, ModelItem, $state, cacheHelper, DateHelper, $mdDialog, cetakHelper, medifirstService, $q) {
            
    // initialize.controller('PaketObatPasienCtrl', ['$q', '$rootScope', '$scope', 'medifirstService','$state','CacheHelper',
    //     function($q, $rootScope, $scope,medifirstService,$state,cacheHelper) {
            ...
            //         }
            function LoadCache(){
                var chacePeriode = cacheHelper.get('PaketObatPasienCtrl');
                if(chacePeriode != undefined){
                   ...
                   statusLoad = chacePeriode[14]
                   noreckirim = chacePeriode[15]
           ...
           function init() {
            // debugger;
            medifirstService.get("farmasi/get-data-combo-transfer", true).then(function(dat){
                $scope.listPenulisResep = dat.data.penulisresep;
                $scope.listRuangan = dat.data.ruangan;
                $scope.listJenisKirim = [{id:1,jenis:'Amprahan'}]
                medifirstService.get("farmasi/get-detail-ruangan-apd?norec_apd="+norec_apd, true).then(function(dat){
                    $scope.item.ruanganTujuan = dat.data.data
                })
                ...
                if (statusLoad != '') {
                    if (statusLoad == 'EditKirim') {
                        medifirstService.get("farmasi/get-detail-kirim-barang?norec="+noreckirim, true).then(function(data_ih){
                    ...
                    if (statusLoad == 'KirimBarang') {
                        medifirstService.get("farmasi/get-detail-order-for-kirim-barang?norecOrder="+norecOrder, true).then(function(data_ih){
                            $scope.isRouteLoading=false;
        ...
        // ];
            ...
            $scope.simpan = function(){
                ...
                var strukkirim = {
                            ...
                            keteranganlainnyakirim: 'Kirim Barang',
                ...
                medifirstService.post('farmasi/save-kirim-barang',objSave).then(function(e) {
                    ...
                    // });
                    var client = new HttpClient();
                    client.get('http://127.0.0.1:1237/printvb/logistik?cetak-bukti-pengeluaran-paket=1&nores='+e.data.nokirim.norec+'&view='+stt+'&user='+pegawaiUser.namalengkap, function(response) {
                        //aadc=response;
                    });
                    window.history.back();
