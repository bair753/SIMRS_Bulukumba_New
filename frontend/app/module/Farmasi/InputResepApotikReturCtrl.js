define(['initialize'], function(initialize) {
    'use strict';
    initialize.controller('InputResepApotikReturCtrl', ['SaveToWindow', '$rootScope', '$scope', 'ModelItem', '$state', 'CacheHelper','DateHelper',  '$mdDialog', 'CetakHelper', 'MedifirstService','$q', 
        function (saveToWindow,$rootScope, $scope, ModelItem, $state, cacheHelper, DateHelper, $mdDialog, cetakHelper, medifirstService, $q) {
            
    // initialize.controller('InputResepApotikReturCtrl', ['$q', '$rootScope', '$scope', 'medifirstService','$state','CacheHelper',
                    ...
                    if (noOrder == 'EditResep') {
                        medifirstService.get("farmasi/get_detail-resep?norecResep="+norecResep, true).then(function(data_ih){
                            ...
                            if (resep[1].substr(2) != bulanNow) {
                                toastr.warning("Tanggal Resep Tidak Dapat Diretur (Hanya dapat diubah dibulan yang sama) ")
                                $scope.disTanggal = true;
                            }
            ...
            if ($scope.item.no != undefined){
                for (var i = data2.length - 1; i >= 0; i--) {
                    if (data2[i].no ==  $scope.item.no){
                        ...
                        data.jumlah = $scope.item.jumlahbulat-parseFloat($scope.item.jumlahRetur)
                        data.jumlahobat = $scope.item.jumlah-parseFloat($scope.item.jumlahRetur)
                        data.dosis = dosis
                        ...
                        data.keterangan = $scope.KeteranganPakai
...
// 30  0   t       jasa pelayanan Repacking obat injeksi
        ...
        $scope.klikGrid = function(dataSelected){
            ...
            $scope.KeteranganPakai = dataSelected.keterangan
            $scope.item.chkp = dataSelected.ispagi
            $scope.item.chks = dataSelected.issiang
            $scope.item.chksr = dataSelected.issore
            $scope.item.chkm = dataSelected.ismalam
            ...
            $scope.simpan = function(){
                ...
                medifirstService.post('farmasi/save-pelayananobat',objSave).then(function(e) {
                    $scope.item.resep = e.data.noresep.norec
                     //** save Logging user
                       medifirstService.get("sysadmin/logging/save-log-retur-resep?norec_resep="
                            + norecResep
                        ).then(function(data) {
                        }) 
                    //**end 
