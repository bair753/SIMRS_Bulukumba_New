define(['initialize'], function(initialize) {
    'use strict';
    initialize.controller('DaftarPaketObatPasienRuanganCtrl', ['SaveToWindow', '$rootScope', '$scope', 'ModelItem', '$state', 'CacheHelper','DateHelper',  '$mdDialog', 'CetakHelper', 'MedifirstService','$q', 
        function (saveToWindow,$rootScope, $scope, ModelItem, $state, cacheHelper, dateHelper, $mdDialog, cetakHelper, medifirstService, $q) {
            
    
    // initialize.controller('DaftarPaketObatPasienRuanganCtrl', ['$q', '$rootScope', '$scope', 'medifirstService','$state','CacheHelper','DateHelper',
    //     function($q, $rootScope, $scope,medifirstService,$state,cacheHelper,dateHelper) {
            ...
            loadCombo();
            function LoadCache(){
              var chacePeriode = cacheHelper.get('DaftarPaketObatPasienRuanganCtrl');
            ...
            function init() {
                ...
                cacheHelper.set('DaftarPaketObatPasienRuanganCtrl', chacePeriode);
            ...
            $scope.Cetak = function(){
                ...
                var client = new HttpClient();
                client.get('http://127.0.0.1:1237/printvb/logistik?cetak-bukti-pengeluaran-paket=1&nores='+$scope.dataSelected.norec+'&view='+stt+'&user='+pegawaiUser.userData.namauser, function(response) {
                    //aadc=response;
                });
            }
            ...
            $scope.HapusPenerimaan = function(){
                ...
                medifirstService.get("farmasi/delete-terima-barang-suplier?"+"norec_sp=" + $scope.dataSelected.norec, true).then(function(dat){
                    init()
                });
            }
