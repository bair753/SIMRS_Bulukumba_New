define(['initialize'], function (initialize) {
    'use strict';
    initialize.controller('ProduksiObatCtrl', ['$q', '$rootScope', '$scope', 'MedifirstService', '$state', 'CacheHelper',
        function ($q, $rootScope, $scope, medifirstService, $state, cacheHelper) {
            ...
            function init() {
                // debugger;
                medifirstService.get("farmasi/produksi/get-data-combo-produksi", true).then(function (dat) {
            ...
            function GETKONVERSIProd(jml) {
                ...
                $scope.item.qtyProduksi = $scope.item.produkProd.qtyhasil

                qtyProduksiAwal = $scope.item.produkProd.qtyhasil

                $scope.item.keterangan = $scope.item.produkProd.keteranganlainnya
                medifirstService.get("farmasi/produksi/get-detail-master-barang-produksi?idprodukhasil=" + $scope.item.produkProd.id +
            ...
            $scope.$watch('item.qtyProduksi', function (newValue, oldValue) {
                if (newValue != oldValue) {
                    // var subTotal=0
                    // for (var i = data2.length - 1; i >= 0; i--) {
                    //     var hasils = parseFloat(newValue )/ parseFloat(qtyProduksiAwal) 
                    //      data2[i].jumlah = hasils* parseFloat(data2[i].jumlah)
                    //     // data2[i].jumlah=   parseFloat(data2[i].jumlah)*parseFloat($scope.item.qtyProduksi)
                    //     data2[i].total=parseFloat(data2[i].jumlah)*parseFloat(data2[i].harganetto)
                    //     subTotal=subTotal+ parseFloat(data2[i].total)
                    // }
                    // $scope.item.totalSubTotal=parseFloat(subTotal).toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, "$1,")
            ...
            $scope.klikGrid = function (dataSelected) {
                medifirstService.get("farmasi/produksi/get-info-stok-detail?produkfk=" + dataSelected.produkfk + "&ruanganfk=" + $scope.item.ruangan.id, true)
                    .then(function (e) {
            ...
            $scope.simpan = function () {
                ...
                if (data2.length > 0) {
                    for (var i = data2.length - 1; i >= 0; i--) {
                        var hasil = parseFloat($scope.item.qtyProduksi) / parseFloat(qtyProduksiAwal)
                        data2[i].jumlah = hasil * data2[i].jumlah
                        data2[i].total = data2[i].hargasatuan * data2[i].jumlah
                        // data2[i].jmlstok
                    }
                }
                var struk = {
                    objectruanganfk: $scope.item.ruangan.id,
                    hargasatuan: hargaSatuanProduksi,
                ...
                medifirstService.post('farmasi/produksi/save-produksi-barang',objSave).then(function (e) {
