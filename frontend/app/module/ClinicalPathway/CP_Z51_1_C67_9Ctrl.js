define(['initialize'], function (initialize) {
    'use strict';
    initialize.controller('CP_Z51_1_C67_9Ctrl', ['$q', '$rootScope', '$scope', 'ModelItem', '$state', 'CacheHelper', 'DateHelper', 'MedifirstService',
        function ($q, $rootScope, $scope, ModelItem, $state, cacheHelper, dateHelper, medifirstService) {
            ...
            $scope.kodeCP =$state.params.kodeCP;
            $scope.jmlhari =$state.params.jmlhari;
            $scope.jmlhari3 =parseInt($state.params.jmlhari) + 3;
            // console.log($rootScope.editCP)
           ...
           $scope.range = function(min, max, step){
            ...
            $scope.ranged = function(q,i){
                let colChk = i
                var input = [];
                minx =(colChk*q)+1
                maxx=(colChk*q)+colChk
           ...
           LoadData($scope.cc.noregistrasi);
            function LoadData(noregistrasi) {
                ...
                if (noregistrasi != undefined) {
                    medifirstService.get("cp/get-cp-transaksi-detail?cp=" + $scope.kodeCP + "&noregistrasi=" + noregistrasi  , true).then(function (dat) {                
                        $scope.item.obj = []
                        $scope.item.obj2 = []
                ...
                medifirstService.get("cp/get-kegiatan-uraian?kodecp=" + $scope.kodeCP  , true).then(function (dat) {     
                    $scope.dataEpic = dat.data.data           
                    $scope.item.title = dat.data.mastercp[0].namacp
                    $scope.isRouteLoading = false;
                })
                medifirstService.get("cp/get-colorchk?kodecp=" + $scope.kodeCP  , true).then(function (dat) {     
                    dataLoad = dat.data.data           
                    for (var i = 0; i <= dataLoad.length - 1; i++) {
            ...
            $scope.Save = function () {
                ...
                medifirstService.post('cp/save-cp-dinamis', jsonSave).then(function (e) {
                    medifirstService.postLogging('CP', 'norec CP', e.data.data.norec,  
                    $scope.kodeCP + ' dengan noregistrasi - ' + $scope.cc.noregistrasi 
                    + $scope.cc.noregistrasi).then(function (res) {
                    })
            ...
            // $scope.kodeCP = $scope.kodeCP
            $scope.EditCaption = function(id,jenis){
                medifirstService.get("cp/get-caption?kodecp="+$scope.kodeCP+"&id="+id).then(function (data) {
                    let dt = data.data.data[0]
                    if($rootScope.isEditCP ==true){
                        if (jenis == 1) {
                            $rootScope.showPopUp2($scope.kodeCP,dt.id,dt.caption,jenis,dt.openlinkarr,dt.objectcpkegiatanfk,dt.sort,dt.kegiatan,dt.statusenabled)
                        }else{
                            $rootScope.showPopUp2($scope.kodeCP,dt.id,dt.keterangan,jenis,dt.openlinkarr,dt.objectcpkegiatanfk,dt.sort,dt.kegiatan,dt.statusenabled)
                        }
                        
                    }else{
                        openlinkID(dt.openlinkarr)
                    }
                })
            ...
            function openlinkID(id){
               ...
               if(obj != ''){
                ...
                // $scope.$watchCollection('item.obj', function(newValue, oldValue) {
                //   console.log(newValue);
                // });
                            // $scope.$watch('item.obj', function (newValue, oldValue) {
            //        if($rootScope.isEditCP ==true){
            //        }
