define(['initialize'], function (initialize) {
    'use strict';
    initialize.controller('CP_I21_4_E11_9Ctrl', ['$q', '$rootScope', '$scope', 'ModelItem', '$state', 'CacheHelper', 'DateHelper', 'MedifirstService',
        function ($q, $rootScope, $scope, ModelItem, $state, cacheHelper, dateHelper, medifirstService) {
           ...
           LoadData($scope.cc.noregistrasi);
            function LoadData(noregistrasi) {
                ...
                medifirstService.get("cp/get-cp-transaksi-detail?cp=CP_I21_4_E11_9&noregistrasi=" + noregistrasi  , true).then(function (dat) {                
                    $scope.item.obj = []
                    $scope.item.obj2 = []
                ...
                medifirstService.get("cp/get-colorchk?kodecp=CP_I21_4_E11_9"  , true).then(function (dat) {     
                    dataLoad = dat.data.data           
                    for (var i = 0; i <= dataLoad.length - 1; i++) {
            ...
            $scope.Save = function () {
                ...
                medifirstService.post('cp/save-cp-dinamis', jsonSave).then(function (e) {
                    medifirstService.postLogging('CP', 'norec CP', e.data.data.norec,  
                    'CP_I21_4_E11_9' + ' dengan noregistrasi - ' + $scope.cc.noregistrasi 
                    + $scope.cc.noregistrasi).then(function (res) {
                    })
