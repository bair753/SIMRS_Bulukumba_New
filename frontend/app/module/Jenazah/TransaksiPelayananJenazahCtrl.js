define(['initialize'], function (initialize) {
    'use strict';
    initialize.controller('TransaksiPelayananJenazahCtrl', ['$q', '$rootScope', '$scope', '$state', 'CacheHelper', 'MedifirstService',
        function ($q, $rootScope, $scope, $state, cacheHelper, medifirstService) {
            ...
            function init() {
                ...
                medifirstService.get("jenazah/get-data-for-combo", true).then(function (dat) {
                    $scope.listRuanganTujuan = dat.data.ruanganjenazah;
                });
                medifirstService.get("jenazah/get-order-pelayanan-jenazah?norec_so=" + norec_so
            ...
            $scope.Simpan = function () {
                ...
                medifirstService.post('jenazah/save-order-pelayanan-jenazah',objSave).then(function (e) {
                    medifirstService.postLogging('Order Pelayanan Jenazah', 'Norec strukorder_t', e.data.strukorder.norec,
                    'Order Pelayanan Jenazah No Order - ' + e.data.strukorder.noorder + ' dengan No Registrasi ' + $scope.item.noregistrasi).then(function (res) {
                        init()
                        clearTambah();
                        $scope.dataGridOrder = new kendo.data.DataSource({
                            data: []
            ...
            $scope.hapusTindakan = function () {
                ...
                medifirstService.post('jenazah/delete-order-pelayanan-jenazah',objDelete).then(function (e) {
                    medifirstService.postLogging('Order Pelayanan Jenazah', 'Norec strukorder_t', e.data.strukorder.norec,
                    'Hapus Pelayanan Jenazah No Order - ' + e.data.strukorder.noorder + ' dengan No Registrasi ' + $scope.item.noregistrasi).then(function (res) {
                        init()
                    })                    
                })
            }
