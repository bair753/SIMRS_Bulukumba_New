define(['initialize'], function (initialize) {
    'use strict';
    initialize.controller('LaporanTindakanBedahCtrl', ['MedifirstService', 'CacheHelper', '$scope', 'DateHelper', '$state', 'ModelItem',
        function (medifirstService, cacheHelper, $scope, DateHelper, $state, ModelItem) {
            ...
            $scope.item.listKelompokPasienMulti = [];
            ...
            medifirstService.get("kasir/get-data-combo-kasir", true).then(function (dat) {              
                ...
                $scope.selectOptionsKelompokPasien = {
            ...
            function loadFirst() {
                ...
                medifirstService.get("bedah/get-tindakan-bedah", false).then(function (data) {
                    $scope.listTindakan = data.data;
                }); 
            ...
            function loadData() {
                ...
                var tglAwal = moment($scope.item.periodeAwal).format('DD-MM-YYYY HH:mm');
                var tglAkhir = moment($scope.item.periodeAkhir).format('DD-MM-YYYY HH:mm');
                var tempKelPasienId = "";
                if ($scope.item.kelompokPasien != undefined) {
                    tempKelPasienId = "&kelompokPasien=" + $scope.item.kelompokPasien.id;
                }
                // var dk = ""
				// if ($scope.item.dokterPemeriksa != undefined) {
				// 	var dk = "&dokId=" + $scope.item.dokterPemeriksa.id
                // }
                var idDokter = ""
				...
				if ($scope.item.listKelompokPasienMulti.length != 0) {
					var a = ""
					var b = ""
					for (var i = $scope.item.listKelompokPasienMulti.length - 1; i >= 0; i--) {

						var c = $scope.item.listKelompokPasienMulti[i].id
						b = "," + c
						a = a + b
					}
				...
				if ($scope.item.tindakan != undefined) {
					var idTindakan = "&tindakan=" + $scope.item.tindakan.id
                }
                
                var idNocm = ""
				if ($scope.item.noCm != undefined) {
					idNocm = "&nocm=" + $scope.item.noCm
                }

                var idNamapasien = ""
				if ($scope.item.namapasien != undefined) {
					idNamapasien = "&namapasien=" + $scope.item.namapasien
                }

                medifirstService.get("bedah/lap-tindakan-bedah?tglAwal=" + tglAwal + "&tglAkhir=" + tglAkhir + idNocm + idNamapasien
                    + "&KpArr=" + listKelompokPasien
                    + "&dkArr="+ listDokterPemeriksa + idDokter + idTindakan ).then(function (data) {
                        $scope.isRouteLoading = false;
                        for (var i = 0; i < data.data.data.length; i++) {
                            ...
                            var total = 0;
                            var rptotal = 0;
                            total =  parseFloat(data.data.data[i].jumlah)*parseFloat(data.data.data[i].hargajual);
                            rptotal = "Rp. " + parseFloat(total).toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, "$1,");
                            harga = parseFloat(data.data.data[i].hargajual).toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, "$1,");
                            data.data.data[i].harga = "Rp. " + harga;
                            data.data.data[i].total = rptotal;
                        }
            ...
            $scope.columnGrid = {
                ...
                excel: {
                    fileName: "LaporanTindakanRadiologi.xlsx",
                    allPages: true,
                },
                excelExport: function (e) {
                    ...
                    var myHeaders = [{
                        value: "Laporan Tindakan Radiologi",
                        fontSize: 20,
                        textAlign: "center",
                        background: "#ffffff",
                        // color:"#ffffff"
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        "width": "100px",
                        aggregates: ["count"],
                        groupHeaderTemplate:" No Reg : #= value #"
                    },
                    {
                        "field": "nocm",
                        "title": "No RM",
                        "width": "100px",
                        aggregates: ["count"],
                        groupHeaderTemplate:" No RM : #= value #"
                    },
                    {
                        "field": "namapasien",
                        "title": "Nama Pasien",
                    ...
                    },
                    {
                        ...
                        "title": "Dokter Dpjp",
                        "width": "220px",
                        "template": "<span class='style-left'>#: dokterdpjp #</span>"
                    },
                    {
                        "field": "dokter",
                    ...
                    },
                    {
                        ...
                        "title": "Harga",
                        "width": "150px",
                        "template": "<span class='style-right'>#: harga #</span>"
                    },
                    {
                        "field": "total",
                        "title": "Total",
