define(['initialize'], function (initialize) {
    'use strict';
    initialize.controller('CapKakiBayiRevCtrl', ['$rootScope', '$scope', 'ModelItem', '$state', 'MedifirstService', '$sce','CacheHelper',
        function ($rootScope, $scope, ModelItem, $state, medifirstService, $sce,cacheHelper) {
            ...
            // $scope.noCM = $state.params.noC
            $scope.noRecPap = cacheHelper.get('noRecPap');
            $scope.noRegistrasi = $state.params.noRec;
            medifirstService.get('emr/get-cap-kaki-bayi?norecPap='+ $scope.noRecPap).then(function(e){
                if(e.data.image !=null){
                    $scope.imgData  = e.data.image.image
                }
            })
       //     debugger;
            ...
            // }
        //     medifirstService.get($state.params.noRec).then(function (e) {
        //    //     debugger;
        //         if (e.data.data) {
        //        //     debugger;
        ...
        //     })
            ...
            $scope.Save = function () {
                var tmpData = {
                    ...
                    "norecPap": $scope.noRecPap
                }
                // console.log(JSON.stringify(tmpData));
                medifirstService.post( "emr/image-upload",tmpData).then(function (e) {
                    // $scope.noRec = response.data.data.noRec;
                     $scope.imgData = $scope.newImage
                }).then(function () {
                    // $scope.isNext = true;
                    // $scope.isReport = true;
                });
