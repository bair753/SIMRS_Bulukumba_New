define(['initialize'], function (initialize) {
    'use strict';
    initialize.controller('SkorEWSDewasa5Ctrl', ['$q', '$rootScope', '$scope', 'ModelItem', '$state', 'CacheHelper', 'DateHelper', 'MedifirstService',
        function ($q, $rootScope, $scope, ModelItem, $state, cacheHelper, dateHelper, medifirstService) {
            ...
            medifirstService.get("emr/get-rekam-medis-dynamic?emrid=" + 252).then(function (e) {
                // debugger
            ...
            if (chacePeriode != undefined) {
                ...
                medifirstService.get("emr/get-emr-transaksi-detail?noemr=" + nomorEMR + "&emrfk=" + $scope.cc.emrfk, true).then(function (dat) {
                    $scope.item.obj = []
                    $scope.item.obj2 = []
                    $scope.item.obj[18988]={ value: $scope.cc.iddpjp, text: $scope.cc.dokterdpjp }
                    $scope.item.obj[18990]={text: $scope.cc.namaruangan, value: $scope.cc.objectruanganfk}
                    dataLoad = dat.data.data
                    for (var i = 0; i <= dataLoad.length - 1; i++) {
                        if (parseFloat($scope.cc.emrfk) == dataLoad[i].emrfk) {
                            if (dataLoad[i].type == "textbox") {
                                $scope.item.obj[dataLoad[i].emrdfk] = dataLoad[i].value
                            if (dataLoad[i].emrdfk=='18949') 
                            $scope.skor1 =parseFloat( dataLoad[i].value)
                            if (dataLoad[i].emrdfk=='18950') 
                                $scope.skor2 =parseFloat( dataLoad[i].value)
                            if (dataLoad[i].emrdfk=='18951') 
                                $scope.skor3 =parseFloat( dataLoad[i].value)
                            if (dataLoad[i].emrdfk=='18952') 
                                $scope.skor4 =parseFloat( dataLoad[i].value)
                            if (dataLoad[i].emrdfk=='18953') 
                                $scope.skor5 =parseFloat( dataLoad[i].value)
                            if (dataLoad[i].emrdfk=='18954') 
                                $scope.skor6 =parseFloat( dataLoad[i].value)
                            if (dataLoad[i].emrdfk=='18955') 
                                $scope.skor7 =parseFloat( dataLoad[i].value)
                            if (dataLoad[i].emrdfk=='18956') 
                                $scope.skor8 =parseFloat( dataLoad[i].value)
                            if (dataLoad[i].emrdfk=='18957') 
                                $scope.skor9 =parseFloat( dataLoad[i].value)
                            if (dataLoad[i].emrdfk=='18958') 
                                $scope.skor10 =parseFloat( dataLoad[i].value)
                            if (dataLoad[i].emrdfk=='18959') 
                                $scope.skor11 =parseFloat( dataLoad[i].value)
                            if (dataLoad[i].emrdfk=='18960') 
                                $scope.skor12 =parseFloat( dataLoad[i].value)
                            if (dataLoad[i].emrdfk=='18961') 
                                $scope.skor13 =parseFloat( dataLoad[i].value)


                            // $scope.item.obj[18949] = $scope.skor1
                            //             $scope.item.obj[18950] = $scope.skor2
                            //             $scope.item.obj[18951] = $scope.skor3
                            //             $scope.item.obj[18952] = $scope.skor4
                            //             $scope.item.obj[18953] = $scope.skor5
                            //             $scope.item.obj[18954] = $scope.skor6
                            //             $scope.item.obj[18955] = $scope.skor7
                            //             $scope.item.obj[18956] = $scope.skor8
                            //             $scope.item.obj[18957] = $scope.skor9
                            //             $scope.item.obj[18958] = $scope.skor10
                            //             $scope.item.obj[18959] = $scope.skor11
                            //             $scope.item.obj[18960] = $scope.skor12
                            //             $scope.item.obj[18961] = $scope.skor13
                            }
            ...
            //     $scope.item.obj[18989] = {id:datas.id, text:datas.namadiagnosa}
            // }) 
            ...
            $scope.Save = function () {
                ...
                medifirstService.post('emr/save-emr-dinamis', jsonSave).then(function (e) {
                    ...
                    'Skor Early Warning System Dewasa5 ' + ' dengan No EMR - ' +e.data.data.noemr +  ' pada No Registrasi ' 
