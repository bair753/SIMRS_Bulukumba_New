define(['initialize', 'Configuration'], function (initialize, config) {
    'use strict';
    initialize.controller('LembarKerjaBayiBaruLahirCtrl', ['$q', '$rootScope', '$scope', 'ModelItem', '$state', 'CacheHelper', 'DateHelper', 'MedifirstService',
        function ($q, $rootScope, $scope, ModelItem, $state, cacheHelper, dateHelper, medifirstService) {
            ...
            if (nomorEMR == '-') {
                ...
                medifirstService.get("emr/get-antrian-pasien-norec/" + noregistrasifk).then(function (e) {
                    ...
                    $scope.item.obj[430160] = new Date(moment(antrianPasien.tgllahir).format('YYYY-MM-DD'));
                ...
                medifirstService.get("emr/get-rekam-medis-dynamic?emrid=" + $scope.cc.emrfk).then(function (e) {
                    ...
                    medifirstService.get("emr/get-emr-transaksi-detail?noemr=" + nomorEMR + "&emrfk=" + $scope.cc.emrfk, true).then(function (dat) {
                        ...
                        for (var i = 0; i <= dataLoad.length - 1; i++) {
                            ...
                            $scope.item.obj[32116036] = 
                            Number($scope.item.obj[32116011] ?? 0) + 
                            Number($scope.item.obj[32116016] ?? 0) + 
                            Number($scope.item.obj[32116021] ?? 0) + 
                            Number($scope.item.obj[32116026] ?? 0) + 
                            Number($scope.item.obj[32116031] ?? 0);

                            $scope.item.obj[32116037] = 
                            Number($scope.item.obj[32116012] ?? 0) + 
                            Number($scope.item.obj[32116017] ?? 0) + 
                            Number($scope.item.obj[32116022] ?? 0) + 
                            Number($scope.item.obj[32116027] ?? 0) + 
                            Number($scope.item.obj[32116032] ?? 0);

                            $scope.item.obj[32116038] = 
                            Number($scope.item.obj[32116013] ?? 0) + 
                            Number($scope.item.obj[32116018] ?? 0) + 
                            Number($scope.item.obj[32116023] ?? 0) + 
                            Number($scope.item.obj[32116028] ?? 0) + 
                            Number($scope.item.obj[32116033] ?? 0);

                            $scope.item.obj[32116039] = 
                            Number($scope.item.obj[32116014] ?? 0) + 
                            Number($scope.item.obj[32116019] ?? 0) + 
                            Number($scope.item.obj[32116024] ?? 0) + 
                            Number($scope.item.obj[32116029] ?? 0) + 
                            Number($scope.item.obj[32116034] ?? 0);

                            $scope.item.obj[32116040] = 
                            Number($scope.item.obj[32116015] ?? 0) + 
                            Number($scope.item.obj[32116020] ?? 0) + 
                            Number($scope.item.obj[32116025] ?? 0) + 
                            Number($scope.item.obj[32116030] ?? 0) + 
                            Number($scope.item.obj[32116035] ?? 0);
            ...
            $scope.Save = function () {
                ...
                medifirstService.post('emr/save-emr-dinamis', jsonSave).then(function (e) {
                    ...
                    medifirstService.postLogging('EMR', 'norec emrpasien_t', e.data.data.norec,
                        'Lembar Kerja Bayi Baru Lahir' + ' dengan No EMR - ' + e.data.data.noemr + ' pada No Registrasi '
