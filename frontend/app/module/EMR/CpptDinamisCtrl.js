define(['initialize', 'Configuration'], function (initialize, config) {
    'use strict';
    initialize.controller('CpptDinamisCtrl', ['$mdDialog', '$q', '$rootScope', '$scope', '$state', 'CacheHelper', 'MedifirstService',
        function ($mdDialog, $q, $rootScope, $scope, $state, cacheHelper, medifirstService) {
            $scope.item = {};
            ...
            $scope.hapus = function(){
                if($scope.dataSelected == undefined){
                    ...
                    var confirm = $mdDialog.confirm()
						.title('Informasi')
						.textContent('Apakah Anda yakin ingin menghapus data ini ?')
						.ok('Ya')
						.cancel('Tidak')
					$mdDialog.show(confirm).then(function () {
						medifirstService.post('emr/hapus-emr-transaksi',{norec:$scope.dataSelected.norec ,emrfk:$scope.dataSelected.emrfk }).then(function(e){
                            init()
                            medifirstService.postLogging('Hapus Catatan Klinik', 'norec emrpasien_t', $scope.dataSelected.norec,
            ...
            $scope.cetak = function () {
                ...
                if ($scope.dataSelected == undefined)
                    return
                var norecEMR = $scope.dataSelected.norec
                 var nama = medifirstService.getPegawaiLogin().namaLengkap
                if (norecEMR == '') return
                var client = new HttpClient();
                var url = 'cetak-emr-cppt-rajal-new';
                if($scope.dataSelected.emrfk == 94)
                    url= 'cetak-emr-cppt-rajal';
                // client.get('http://127.0.0.1:1237/printvb/e-rekammedis?'+url+'&id=' + $scope.item.noMr + '&norec_apd='+  norec_apd +'&emr=' + norecEMR + '&view=true', function (response) {
                //     // do something with response
                // });
                window.open(config.baseApiBackend + 'report/cetak-catatan-klinik?nocm='
                + $scope.item.noMr + '&norec_apd='+  norec_apd +'&emr=' + norecEMR
                + '&emrfk='+  $scope.dataSelected.emrfk
                +'&kdprofile='+ profile
                +'&nama='+ nama,  '_blank');
                //  window.open(config.baseApiBackend  + 'cetak-catatan-klinik?norec_apd='
                //         + norec_apd + '&nocm=' +  + '&norec_emr=' + norecEMR + '&kdprofile='+ profile,);                        
            }
