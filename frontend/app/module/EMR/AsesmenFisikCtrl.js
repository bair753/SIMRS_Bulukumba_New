define(['initialize'], function (initialize) {
    'use strict';
    initialize.controller('AsesmenFisikCtrl', ['$q', '$rootScope', '$scope', 'ModelItem', '$state', 'CacheHelper', 'DateHelper', 'MedifirstService',
        function ($q, $rootScope, $scope, ModelItem, $state, cacheHelper, dateHelper, medifirstService) {
            ...
            medifirstService.get("emr/get-rekam-medis-dynamic?emrid=" + 179).then(function (e) {
            ...
            medifirstService.get("emr/get-emr-transaksi-detail?noemr=" + nomorEMR + "&emrfk=" + $scope.cc.emrfk, true).then(function (dat) {
                $scope.item.obj = []
                $scope.item.obj2 = []
                $scope.item.obj[10728]=$scope.cc.tglregistrasi
                $scope.item.obj[10730]={ value: $scope.cc.iddpjp, text: $scope.cc.dokterdpjp }
                $scope.item.obj[10729]={value: $scope.cc.objectruanganfk, text : $scope.cc.namaruangan}
                ...
                //         if ($scope.item.obj[10731] == undefined) {
                //             var str = datas.data.data[0].value
                //             var res = str.split("~");
                //             // $scope.item.objcbo[dataLoad[i].emrdfk]= {value:res[0],text:res[1]}
                //             $scope.item.obj[10731] = { value: res[0], text: res[1] }
                ...
                medifirstService.get("emr/get-nilai-statis-igd?noregistrasi=" + $scope.cc.noregistrasi + "&objectidawal=3037&objectidakhir=3063&idemr=124", true).then(function (datas) {
                    if (datas.data.data.length>0){
                        $scope.item.obj[11225]=datas.data.data[0].nilai
                    }
                })
            ...
            $scope.listRisikoJatuhSkor = [
                { id: 11298, nama: 'Rendah ( < 7 )' },
                { id: 11299, nama: 'Sedang ( 7 - 14 )'  },
                { id: 11300, nama: 'Tinggi ( > 14 )' },
            ]
