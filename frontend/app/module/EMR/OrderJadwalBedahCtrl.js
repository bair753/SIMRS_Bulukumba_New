define(['initialize'], function (initialize) {
    'use strict';
    initialize.controller('OrderJadwalBedahCtrl', ['$q', '$rootScope', '$scope', '$state', 'CacheHelper', 'MedifirstService', '$mdDialog',
        function ($q, $rootScope, $scope, $state, cacheHelper, medifirstService, $mdDialog) {
            ...
            function init() {
                ...
                //     });
                // });
                medifirstService.get("emr/get-emr-transaksi-detail-form?" + paramSearch + "&jenisEmr=bedah", true).then(function (dat) {
                    $scope.isRouteLoading = false
                ...
                medifirstService.get("emr/get-menu-rekam-medis-dynamic?namaemr=bedah" + mapemr).then(function (e) {
            ...
            $scope.asupKaForm = function (noemr, reportdisplay, emrfk, norec) {
                ...
                if (noemr2 != '-')
                    noemr2.trim()
                var url = "RekamMedis.OrderJadwalBedah.ProsedurKeselamatan"
            ...
            $scope.klikGridRiwayat = function (dataSelected) {
                $scope.dataSelected = dataSelected
            }

            $scope.klikGridRiwayatOrder = function (dataSelectedRiwayat) {
            ...
            $scope.Simpan = function () {
                ...
                var objSave = {
                    ...
                    keterangan: $scope.item.rencanaTindakan != undefined ? $scope.item.rencanaTindakan : null,
                    estimasiwaktu: $scope.item.Estimasi != undefined ? $scope.item.Estimasi : null,
                    iscito: $scope.item.iscito != undefined && $scope.item.iscito == true ? $scope.item.iscito : false,
                    isodc: $scope.item.isodc != undefined && $scope.item.isodc == true ? $scope.item.isodc : false,
                    iskebutuhanicu: $scope.item.iskebutuhanicu != undefined && $scope.item.iskebutuhanicu == true ? $scope.item.iskebutuhanicu : false,
                    details: data2,
                }
                ...
                medifirstService.post('emr/save-order-pelayanan', objSave).then(function (e) {
                    medifirstService.postLogging('Order Jadwal Bedah', 'Norec strukorder_t', e.data.strukorder.norec,
                        ...
                        $scope.item.Estimasi = null
                        $scope.item.rencanaTindakan = null
                        $scope.item.iscito = null
                        $scope.item.isodc = null
                        $scope.item.iskebutuhanicu = null
                    })
                    init();
                    $scope.BatalOrder();
            ...
            $scope.hapusOrder = function () {
                ...
                var data = {
                    norec_so: $scope.dataSelectedRiwayat.norec
                }
                medifirstService.post("bedah/delete-verif-bedah",data).then(function(e){
                        init()
                    })
            }
            // ,
