define(['initialize'], function (initialize) {
    'use strict';
    initialize.controller('AsesmenMedisDetailCtrl', ['$rootScope', '$scope', '$state', 'CacheHelper', 'MedifirstService',
        function ($rootScope, $scope, $state, cacheHelper, medifirstService) {
                            ...
                            if (parseFloat($scope.cc.emrfk) == dataLoad[i].emrfk) {
                                ...
                                if (dataLoad[i].type == "checkbox") {
                                    if (dataLoad[i].value == '1') {
                                        ...
                                        // if(dataLoad[i].satuan.indexOf(",") > 0)
                                            $scope.disablenulain(dataLoad[i].emrdfk, dataLoad[i].satuan);
                                    } else {
                                        chekedd = false
                                        $scope.item.obj[dataLoad[i].emrdfk] = chekedd
                                    }
            ...
            $scope.disablenulain = function (e, data)
            {
                if(data === "" || data === null)
                    return;
                ...
                listdata.forEach(element => {
                    var btnFalse = document.getElementById("item.obj" + element);
                    var btnThis = $scope.item.obj[e];
                    console.log(btnThis);
                    if(btnThis) 
