define(['initialize'], function (initialize) {
    'use strict';
    initialize.controller('DaftarIsianMectaCtrl', ['$q', '$rootScope', '$scope', 'ModelItem', '$state', 'CacheHelper', 'DateHelper', 'MedifirstService',
        function ($q, $rootScope, $scope, ModelItem, $state, cacheHelper, dateHelper, medifirstService) {
            ...
            $scope.cetakPdf = function () {
                ...
                client.get('http://127.0.0.1:1237/printvb/e-rekammedis?cetak-emr-daftarmecta&id='
                 + $scope.cc.nocm + '&emr=' + norecEMR + '&view=true' + '&idemr=' + 150  , function (response) {
            ...
            if (nomorEMR == '-') {
                ...
                medifirstService.get("emr/get-rekam-medis-dynamic?emrid=150").then(function (e) {
                    ...
                    for (var i = e.data.kolom1.length - 1; i >= 0; i--) {
                        ...
                        if (e.data.kolom1[i].cbotable != null) {
                            medifirstService.getPart2(e.data.kolom1[i].id, e.data.kolom1[i].cbotable, true, true, 20).then(function (data) {
                                ...
                                if(data.options.idididid ==4284){
                                    $scope.item.obj[4284] = $scope.cc.namapasien
                                }
                            })
                        }
