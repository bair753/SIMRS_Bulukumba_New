define(['initialize'], function (initialize) {
	'use strict';
	initialize.controller('KehamilanSekarangRevCtrl', ['$rootScope', '$scope', 'ModelItem', '$state', 'DateHelper', 'MedifirstService', 'CacheHelper',
		function ($rootScope, $scope, ModelItem, $state, dateHelper, medifirstService, cacheHelper) {
					...
					+ '&riwayatfk=' + $scope.noRecPap).then(function (e) {
						...
						if ($scope.dataKehilanSekarang.length > 0) {
							for (var i = 0; i < $scope.dataKehilanSekarang.length; i++) {
								...
								} else if ($scope.dataKehilanSekarang[i].objectfk == "KHS-000030") {
									$scope.norecKelainan7 = $scope.dataKehilanSekarang[i].norec
									$scope.listKelainan.forEach(function (e) {
										if (e.id == $scope.dataKehilanSekarang[i].nilai) {
											e.isChecked = true
											var dataid = {
			...
			$scope.Save = function () {
				...
				medifirstService.post('emr/save-data-rekam-medis', jsonSave).then(function (e) {
					medifirstService.postLogging('Pengkajian Keperawatan', 'Norec Antrian Pasien Diperiksa', $state.params.noRec, 'Kehamilan Sekarang').then(function (res) {
					})
				});
			}
			$scope.Back = function () {
