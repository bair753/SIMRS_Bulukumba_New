define(['initialize'], function (initialize) {
    'use strict';
    initialize.controller('AsesmenPerawatJiwaRemajaCtrl', ['$q', '$rootScope', '$scope', 'ModelItem', '$state', 'CacheHelper', 'DateHelper', 'MedifirstService',
        function ($q, $rootScope, $scope, ModelItem, $state, cacheHelper, dateHelper, medifirstService) {
            ...
            medifirstService.getPart('emr/get-datacombo-part-pegawai', true, true, 20).then(function (data) {
             ...
             medifirstService.get("emr/get-rekam-medis-dynamic?emrid=" + 201).then(function (e) {
            ...
            medifirstService.get("emr/get-emr-transaksi-detail?noemr=" + nomorEMR + "&emrfk=" + $scope.cc.emrfk, true).then(function (dat) {
                $scope.item.obj = []
                $scope.item.obj2 = []
                $scope.item.obj[14558]=$scope.now
                $scope.item.obj[14559]=$scope.now
                $scope.item.obj[14563]={ value: $scope.cc.iddpjp, text: $scope.cc.dokterdpjp }
                dataLoad = dat.data.data
                medifirstService.get("emr/get-vital-sign?noregistrasi=" + $scope.cc.noregistrasi + "&objectidawal=4241&objectidakhir=4246&idemr=147", true).then(function (datas) {
                    if (datas.data.data.length>0){
                        if ($scope.item.obj[14862] == undefined) {
                        $scope.item.obj[14862]=datas.data.data[0].value
                        }
                        if ($scope.item.obj[14863] == undefined) {
                            $scope.item.obj[14863]=datas.data.data[3].value
                        }
                        if ($scope.item.obj[14864] == undefined) {
                            $scope.item.obj[14864]=datas.data.data[4].value
                        }
                        if ($scope.item.obj[14865] == undefined) {
                            $scope.item.obj[14865]=datas.data.data[5].value
                        }
