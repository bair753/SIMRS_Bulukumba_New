define(['initialize'], function (initialize) {
    'use strict';
    initialize.controller('PermintaanRawatJalanCtrl', ['$q', '$rootScope', '$scope', 'ModelItem', '$state', 'CacheHelper', 'DateHelper', 'MedifirstService',
        function ($q, $rootScope, $scope, ModelItem, $state, cacheHelper, dateHelper, medifirstService) {
            ...
            if (nomorEMR == '-') {
                ...
                var status = "t";
                $scope.item.obj[427963] = $scope.now;
                medifirstService.get("emr/get-antrian-pasien-norec/" + noregistrasifk).then(function (e) {
                    var antrianPasien = e.data.result;
                    $scope.item.obj[427950] = $scope.cc.namapasien;
                    $scope.item.obj[427952] = new Date(moment(antrianPasien.tgllahir).format('YYYY-MM-DD'));
                    $scope.item.obj[427951] = antrianPasien.jeniskelamin;
                    $scope.item.obj[427953] = antrianPasien.alamatlengkap;
                    if (antrianPasien.dokterdpjp != null && antrianPasien.iddpjp != null) {
                        $scope.item.obj[427961] = {
            ...
            $scope.Save = function () {
                ...
                medifirstService.post('emr/save-emr-dinamis', jsonSave).then(function (e) {
                    ...
                    medifirstService.postLogging('EMR', 'norec emrpasien_t', e.data.data.norec,
                        'Permintaan Rawat Inap ' + ' dengan No EMR - ' + e.data.data.noemr + ' pada No Registrasi '
