define(['initialize'], function (initialize) {
	'use strict';
	initialize.controller('DaftarPasienByDiagnosaCtrl', ['$mdDialog', '$timeout', '$state', '$q', '$rootScope', '$scope', 'CacheHelper', 'DateHelper', 'ModelItem', 'CetakHelper', 'MedifirstService',
		function ($mdDialog, $timeout, $state, $q, $rootScope, $scope, cacheHelper, dateHelper, ModelItem, cetakHelper, medifirstService) {
			...
			function loadCombo() {
				var chacePeriode = cacheHelper.get('DaftarPasienByDianosa');
				...
				medifirstService.getPart("sysadmin/general/get-icd9-part", true, true, 10).then(function (data) {
					$scope.listDiagnosaTindakan = data;

				});

				medifirstService.getPart("sysadmin/general/get-icd10-part", true, true, 10).then(function (data) {
			...
			$scope.columnDaftarPasienPulang = [
				...
				// }
				{
					...
					"title": "Kde ICD 10",
					"width": "150px",
					"template": "<span class='style-left'>#: kddiagnosa #</span>"
				},
				{
					"field": "namadiagnosa",
					"title": "ICD 10",
					"width": "150px",
					"template": "<span class='style-left'>#: namadiagnosa #</span>"
				},
				{
					"field": "kddiagnosatindakan",
					"title": "Kde ICD 9",
					"width": "150px",
					"template": "<span class='style-left'>#: kddiagnosatindakan #</span>"
				},
				{
					"field": "namadiagnosatindakan",
					"title": "ICD 9",
					"width": "100px",
					"template": "<span class='style-left'>#: namadiagnosatindakan #</span>"
				}
			];
			...
			function loadData() {
				...
				var objectdiagnosafk = ""
				if ($scope.item.Diagnosa != undefined) {
					objectdiagnosafk = "&objectdiagnosafk=" + $scope.item.Diagnosa.id
				}
				var objectdiagnosatindakanfk = ""
				if ($scope.item.DiagnosaTindakan != undefined) {
					objectdiagnosatindakanfk = "&objectdiagnosatindakanfk=" + $scope.item.DiagnosaTindakan.id
				}
				// var dokId=""
				...
				$q.all([
					medifirstService.get("rawatjalan/get-daftarpasien-by-diagnosa?" +
						"tglAwal=" + tglAwal +
						"&tglAkhir=" + tglAkhir +
						reg + rm + nm + ins + rg + kp + dk + objectdiagnosafk + objectdiagnosatindakanfk
						// +dokId
					),
				]).then(function (data) {
					$scope.isRouteLoading = false;
					...
					// + "~" +$scope.item.dokter;
					cacheHelper.set('DaftarPasienByDianosa', chacePeriode);
				});
			...
			$scope.kartupasien = function () {
				...
				if ($scope.dataPasienSelected != undefined) {
					...
					medifirstService.get("operator/identifikasi-kartu-pasien?norec_pd="
			...
			$scope.pilihQty = function (data) {
				...
				if (isValid.status) {
					...
					if (qty !== undefined) {
						...
						medifirstService.get("operator/identifikasi-label?norec_pd="
			...
			$scope.SummaryList = function () {
				if ($scope.dataPasienSelected.nocm != undefined) {
					...
					medifirstService.get("operator/identifikasi-sum-list?norec_pd="
						+ $scope.dataPasienSelected.norec
					).then(function (data) {
						var datas = data.data;
					})
			...
			$scope.Tracer = function () {
				if ($scope.dataPasienSelected.noregistrasi != undefined) {
					...
					medifirstService.get("operator/identifikasi-tracer?norec_pd="
						+ $scope.dataPasienSelected.norec
					).then(function (data) {
						var datas = data.data;
					})
			...
			$scope.RMK = function () {
				...
				if ($scope.dataPasienSelected.noregistrasi != undefined) {
					medifirstService.get("operator/get-data-diagnosa-pasien?"
						+ norReg
					).then(function (data) {
			...
			$scope.cetakRMK = function () {
				...
				medifirstService.get("operator/get-data-diagnosa-pasien?"
					...
					manageTataRekening.postSaveDiagnosaRMK(detaildiagnosapasien).then(function (e) {
						...
						medifirstService.get("operator/identifikasi-rmk?norec_pd="
							+ $scope.dataPasienSelected.norec
						).then(function (data) {
							var datas = data.data;
						})
			...
			$scope.BatalPeriksa = function () {
				...
				medifirstService.get("operator/get-data-pasien-mau-batal?"
					+ norReg
				).then(function (data) {
					var datas = data.data
			...
			$scope.EditRegistrasi = function () {
				if ($scope.dataPasienSelected == undefined) {
					...
					var ruangan = "";
					medifirstService.get("registrasipasien/get-apd?noregistrasi="
			...
			$scope.popUpBL = function () {
				// medifirstService.get("operator/get-apd?noregistrasi="
				//           + $scope.dataPasienSelected.noregistrasi
				//    ).then(function (data) {
				...
				if ($scope.item != undefined) {
					...
					medifirstService.get("operator/identifikasi-buktiLayanan?norec_pd="
						+ $scope.dataPasienSelected.norec
					).then(function (data) {
						var datas = data.data;
					})
			...
			$scope.popUpInputTindakan = function () {
				...
				medifirstService.get("tatarekening/get-sudah-verif?noregistrasi=" + $scope.dataPasienSelected.noregistrasi).then(function (e) {
