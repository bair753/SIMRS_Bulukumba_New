        ...
        function (saveToWindow, $rootScope, $scope, ModelItem, $state, dateHelper, socket, $mdDialog, window, cetakHelper, medifirstService, cacheHelper, $q) {
            ...
            function loadCombo() {
                ...
                if (chacePeriode != undefined) {
                    ...
                    $scope.item.periodeAkhir = new Date(arrPeriode[1]);
                    $scope.item.ruangan = { id: arrPeriode[2], namaruangan: arrPeriode[3] }
                ...
                medifirstService.getPart("sysadmin/general/get-dokter-general", true, 10).then(function (e) {
                    $scope.listDokter = e
                });
            }
                ...
                $q.all([
                    ...
                    for (var i = 0; i < datas.length; i++) {
                        ...
                        if (datas[i].tgldipanggildokter!=null){
                            var tgldaftar = new Date(datas[i].tglregistrasi);
                            // if (datas[i].noreservasi != null){
                            //     var tgldaftar = new Date(datas[i].tanggalreservasi);
                            // } else {
                            //     var tgldaftar = new Date(datas[i].tglregistrasi);
                            // }
                            var tglpanggil = new Date(datas[i].tgldipanggildokter);
                            var diff = Math.abs(tglpanggil - tgldaftar);
                            var durasi = Math.round((diff/1000)/60) + ' menit';
                            datas[i].tgldipanggildokter = new moment(tglpanggil).format('DD-MM-YYYY HH:mm')
                        }
                        datas[i].durasi= durasi;
                        ...
                        if (datas[i].tgldipanggilsuster != null) {
                            datas[i].stts = 'Selesai Periksa Perawat'
                        }
                        ...
                        // if(datas[i].tgldipanggilsuster != null && datas[i].tgldipanggildokter != null)
                        //     datas[i].stts = 'Selesai'
                        if (datas[i].tglselesaiperiksa != null) {
                            datas[i].stts = 'Selesai Periksa Dokter'
                        }
            ...
            var onDataBound = function () {
                $('td').each(function () {
                    ...
                    if ($(this).text() == 'Selesai Periksa Perawat') { $(this).addClass('green') }
                    if ($(this).text() == 'Di Panggil Dokter') { $(this).addClass('blue') }
                    if ($(this).text() == 'Selesai Periksa Dokter') { $(this).addClass('birulaut') }
            ...
            $scope.panggil = function () {
                ...
                if ($scope.dataPasienSelected.stts == '-') {
                    toastr.info('Pasien belum Di Panggil Perawat')
                    return
                }
                let params = $scope.dataPasienSelected.noregistrasi    
                if($scope.dataPasienSelected.noreservasi != undefined && $scope.dataPasienSelected.noreservasi != '' && $scope.dataPasienSelected.noreservasi !='Kios-K'){
                ...
                //set CSS
                cell.css("background-color", "#1E90FF");
                cell.css("color", "#ffffff");

                $scope.dataPasienSelected.stts = 'Di Panggil Dokter'

                // update dokter
                if (medifirstService.getKelompokUser() == 'dokter') {
                    ...
                    var json = {
                        norec_apd: $scope.dataPasienSelected.norec_apd,
                        iddokter: medifirstService.getPegawaiLogin().id
                    }
                    medifirstService.postNonMessage('rawatjalan/save-update-dokter-antrian', json).then(function (e) {
                        $scope.dataPasienSelected.namadokter = medifirstService.getPegawaiLogin().namaLengkap
                    });
                } else {
                    $scope.winDialogss.center().open()
                }
            ...
            $scope.simpanDokter = function () {
                ...
                medifirstService.post('rawatjalan/save-update-dokter-antrian', tmpData).then(function (e) {
                    ...
                    $scope.dataPasienSelected.namadokter = $scope.item.pilihDokter.namalengkap
                    ...
                    // var client = new HttpClient();
                    // client.get('http://'+ipAddress+':2905/printvb/Pendaftaran?displaykeun=1&noantri=' + $scope.dataPasienSelected.noantrian 
                    // + '&namapasien=' + $scope.dataPasienSelected.nocm + ', ' + $scope.dataPasienSelected.namapasien + '&view=true', function (response) {
                    //     // do something with response
                    // });
            ...
            $scope.residenceShow = function () {
                if ($scope.dataPasienSelected == undefined) return
                $scope.winDialogResidence.center().open()
            }
            $scope.simpanResidence = function () {

                var tmpData = {
                    norec_apd: $scope.dataPasienSelected.norec_apd,
                    iddokter: $scope.item.residence.id
                }
                medifirstService.post('rawatjalan/save-update-residence', tmpData).then(function (e) {
                    $scope.winDialogResidence.close();
                    $scope.dataPasienSelected.residence = $scope.item.residence.namalengkap
                    $scope.item.residence = '';
            ...
            $scope.InsidenInternal = function () {
                ...
                $state.go('InsidenInternal', {
                    kpid: $scope.dataPasienSelected.norec_pd,
