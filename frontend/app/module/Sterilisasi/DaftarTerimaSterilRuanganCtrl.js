define(['initialize'], function (initialize) {
    'use strict';
    initialize.controller('DaftarTerimaSterilRuanganCtrl', ['$scope', '$state', 'CacheHelper', 'DateHelper', 'MedifirstService',
        function ($scope, $state, cacheHelper, dateHelper, medifirstService) {
            ...
            function LoadCache() {
                var chacePeriode = cacheHelper.get('DaftarTerimaSterilRuanganCtrl');
            ...
            function loadCombo() {
                ...
                $scope.item.namaruangantujuan = ruangan[0].namaruangan;
                $scope.item.namaruangantujuanid = ruangan[0].id;
            }
            ...
            function init() {
                ...
                // }
                var rg = ""
                if ($scope.item.namaruangantujuanid != undefined) {
                    var rg = "&ruangantujuanfk=" + $scope.item.namaruangantujuanid
                }
                var produkfk = ""
                ...
                medifirstService.get("sterilisasi/get-daftar-terima-steril?" +
                    "tglAwal=" + tglAwal +
                    "&tglAkhir=" + tglAkhir +
                ...
                cacheHelper.set('DaftarTerimaSterilRuanganCtrl', chacePeriode);
            ...
            $scope.pakaiAlat = function(){
                if ($scope.dataSelected == undefined) {
                    toastr.error("Pilih yg akan di ubah!!")
                    return;
                }

                if ($scope.dataSelected.statussteril == "Alat Terpakai") {
                    toastr.error("Status Alat Sudah Terpakai!!")
                    return;
                }

                var dataupdate = {
                    strukkirimfk: $scope.dataSelected.norec,
                    status: "Alat Terpakai"
                }

                medifirstService.post('sterilisasi/save-update-status-pemakaianalat', dataupdate).then(function (e) {
                    init();
                }, function (error) {
                    // $scope.saveShow = true;
                });
            ...
            $scope.kirim = function(){
                ...
                if ($scope.dataSelected.statussteril == undefined) {
                    toastr.error("Alat Yang Belum Terpakai, Belum Dapat Dikirim Kembali Ke CSSD!!")
                    return;
                }
                var chacePeriode = {
                    0: $scope.dataSelected.norec,
                    1: 'PENGIRIMAN BARANG STERIL TERPAKAI',
                    2: 'KirimBalik',
                    3: $scope.dataSelected.ruasalid,
                    4: $scope.dataSelected.namaruanganasal,
                    5: '',
                    6: ''
                }
                cacheHelper.set('PenerimaanBarangSterilCtrl', chacePeriode);
                $state.go('PenerimaanBarangSteril')
            }
