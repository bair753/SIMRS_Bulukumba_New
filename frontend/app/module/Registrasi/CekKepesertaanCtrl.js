define(['initialize'], function (initialize) {
    'use strict';
    initialize.controller('CekKepesertaanCtrl', ['$rootScope', '$scope', 'ModelItem', '$state', 'DateHelper', 'MedifirstService',
        function ($rootScope, $scope, ModelItem, $state, DateHelper, medifirstService) {
            ...
            $scope.cekNoKartu = function (noKartu) {
                $scope.isRouteLoading = true;
                medifirstService.get("bridging/bpjs/get-no-peserta?nokartu=" + noKartu + "&tglsep=" + new moment(new Date).format('YYYY-MM-DD')).then(function (e) {
            ...
            $scope.cekNik = function (nik) {
                $scope.isRouteLoading = true;
                medifirstService.get("bridging/bpjs/get-nik?nik=" + nik + "&tglsep=" + new moment(new Date).format('YYYY-MM-DD')).then(function (e) {
                    document.getElementById("json").innerHTML = JSON.stringify(e.data, undefined, 4);
                }).then(function () {
                    $scope.isRouteLoading = false;
                });
