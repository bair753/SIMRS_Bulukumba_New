define(['initialize'], function (initialize) {
	'use strict';
	initialize.controller('KlaimInaCbgCtrl', ['$state', '$q', '$scope', 'CacheHelper', 'MedifirstService',
		function ($state, $q, $scope, cacheHelper, medifirstService) {
			...
			$scope.columnDaftarPasienPulang = {
				...
				columns:
					[
						...
						{
							"field": "nokelasdijamin",
							"title": "Kelas Dijamin",
							"width": "9%",
							"template": '# if( nokelasdijamin==null) {# - # } else {# #= nokelasdijamin # #} #'
						}
						// ,
						// {
						// 	"field": "status",
				...
				$scope.item.detail =
					...
					$scope.listTarifRS =[{namatarif:'Prosedur Non Bedah' ,tarif:data.tarif_rs.prosedur_non_bedah},
						...
						{namatarif:'Alkes' ,tarif:data.tarif_rs.alkes},
						 ...
						 {namatarif:'Prosedur Non Bedah' ,tarif:data.tarif_rs.obat_kronis},
			...
			function loadData() {
				...
				$q.all([
					medifirstService.get("bridging/inacbg/get-daftar-pasien-inacbg?" +
						"tglAwal=" + tglAwal +
						"&tglAkhir=" + tglAkhir +
