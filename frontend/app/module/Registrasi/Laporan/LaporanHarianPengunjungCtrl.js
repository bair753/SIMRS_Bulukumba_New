define(['initialize'], function (initialize) {
    'use strict';
    initialize.controller('LaporanHarianPengunjungCtrl', ['MedifirstService', 'CacheHelper', '$scope', 'DateHelper', '$state', 'ModelItem',
        function (medifirstService, cacheHelper, $scope, DateHelper, $state, ModelItem) {
            ...
            function loadData() {
                ...
                medifirstService.get("registrasi/laporan/get-data-lap-pengunjung-pemeriksaan?" +
                    ...
                    .then(function (data) {
                        ...
                        // console.log(data);
                        const filterBpjs = data.data.data.filter(it => it.kelompokpasien == "BPJS");
                        const filterPBI = data.data.data.filter(it => it.kelompokpasien == "BPJS PBI");
                        const filterUmum = data.data.data.filter(it => it.kelompokpasien == "Umum/Pribadi");
                        const filterInhealth = data.data.data.filter(it => it.kelompokpasien == "MANDIRI INHEALTH");

                        const jumlahLakiLakiBpjs = filterBpjs.filter(it => it.jeniskelamin == "LAKI-LAKI").length
                        const jumlahPerempuanBpjs = filterBpjs.filter(it => it.jeniskelamin == "PEREMPUAN").length

                        const jumlahLakiLakiPBI = filterPBI.filter(it => it.jeniskelamin == "LAKI-LAKI").length
                        const jumlahPerempuanPBI = filterPBI.filter(it => it.jeniskelamin == "PEREMPUAN").length

                        const jumlahLakiLakiUmum = filterUmum.filter(it => it.jeniskelamin == "LAKI-LAKI").length
                        const jumlahPerempuanUmum = filterUmum.filter(it => it.jeniskelamin == "PEREMPUAN").length

                        const jumlahLakiLakiInhealth = filterInhealth.filter(it => it.jeniskelamin == "LAKI-LAKI").length
                        const jumlahPerempuanInhealth = filterInhealth.filter(it => it.jeniskelamin == "PEREMPUAN").length
                    });
            ...
            $scope.columnGrid = {
                ...
                excel: {
                    fileName: "LaporanHarianPengunjung.xlsx",
                    allPages: true,
                },
                excelExport: function (e) {
                    var sheet = e.workbook.sheets[0];
                    sheet.frozenRows = 2;
                    sheet.mergedCells = ["A1:Q1"];
                    sheet.name = "Orders";

                    var myHeaders = [{
                        value: "LAPORAN HARIAN PENGUNJUNG RSUD H.A.SULTHAN DG RADJA",
                        fontSize: 15,
                        textAlign: "center",
                        background: "#ffffff",
                        // color:"#ffffff"
                ...
                columns: [
                    {
                        ...
                        "title": "Tanggal",
                        "width": "100px",
                        "template": "<span class='style-left'>{{formatJam('#: tglregistrasi #')}} - {{formatTanggal('#: tglregistrasi #')}}</span>"
                    },
                    {
                        "field": "reportdisplay",
                        "title": "JK",
