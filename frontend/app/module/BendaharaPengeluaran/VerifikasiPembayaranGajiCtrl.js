define(['initialize'], function (initialize) {
  'use strict';
  initialize.controller('VerifikasiPembayaranGajiCtrl', ['CacheHelper', '$scope', 'MedifirstService', '$state', 'DateHelper',
    function (cacheHelper, $scope, medifirstService, $state, dateHelper) {
      ...
      function loadData() {
        ...
        if ($scope.item.status == undefined) {
          ...
          var urlString = "gaji-pegawai/daftar-gaji-pegawai-belum-verifikasi/?dateStart="
        } else {
          var idStatus = $scope.item.status.id
          if ($scope.item.status.namaStatus == "Verifikasi") {
            var vStatus2 = "Verifikasi"
            var urlString = "gaji-pegawai/daftar-gaji-pegawai-verifikasi/?dateStart="
          } else {
            var vStatus2 = "Belum Verifikasi"
            var urlString = "gaji-pegawai/daftar-gaji-pegawai-belum-verifikasi/?dateStart="
          }
        ...
        medifirstService.get(urlString + tglAwal1 +
      ...
      $scope.columnGrid = [
        {
          "field": "tglHistori",
          "title": "Tanggal",
          "template": "<span class='style-center'>{{'#: tglHistori #'}}</span>",
          "width": "70px"
        },
        {
          "field": "jenisPegawai",
          "title": "jenisPegawai",
          "template": "<span class='style-center'>{{'#: jenisPegawai #'}}</span>",
          "width": "80px"
        },
        {
          "field": "bulan",
      ...
      $scope.Verifikasi = function () {
        ...
        // }
        dataObjPost = {}
        // mnKeu.postData("gaji-pegawai/save-verifikasi-gaji-pegawai?noRec=" + $scope.dataSelected.noRecStrukVerifikasi, dataObjPost).then(function (e) {
        // })
        loadData();
      };
