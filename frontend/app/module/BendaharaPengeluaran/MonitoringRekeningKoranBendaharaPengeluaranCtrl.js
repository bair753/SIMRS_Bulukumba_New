define(['initialize'], function (initialize) {
	'use strict';
	initialize.controller('MonitoringRekeningKoranBendaharaPengeluaranCtrl', ['$scope', '$state', 'CacheHelper', 'DateHelper', 'MedifirstService',
		function ($scope, $state, cacheHelper, dateHelper, medifirstService) {
			...
			function loadData() {
				...
				medifirstService.get("bendaharapengeluaran/get-daftar-bku-bk?tglAwal=" + tglAwal1 + "&tglAkhir=" +
					tglAkhir1
					+ "&jenisTransaksi=" + jt
					+ "&noRekening=" + nr
					...
					$scope.isRouteLoading = false;
					var datas = data.data
					$scope.saldoLama = datas.saldolama
					$scope.saldoAkhir = datas.saldoAkhir					
					for (var i = 0; i < datas.data.length; i++) {
						datas.data[i].no = i + 1
						if (datas.data[i].penyetor != null)
						datas.data[i].kettransaksi = datas.data[i].kettransaksi + ' a.n ' + datas.data[i].penyetor
					}
					sourceDataGrid = datas.data;
			...
			function removeRow(e) {
				...
				if (confirm('Hapus data? ')) {
					...
					medifirstService.post('bendaharapengeluaran/hapus-bku',itemDelete).then(function (e) {
						if (e.status === 201) {
							loadData();
							grid.removeRow(row);
						}
			...
			function loadComboPopup() {
				...
				if ($scope.penerimaan == true) {
					ket = "Penerimaan BK"
					medifirstService.get("bendaharapengeluaran/get-data-combo-bk?keterangan="
						+ ket)
						.then(function (data) {
			...
			$scope.SavePenerimaan = function () {				
				...
				if ($scope.item.noBukti == undefined) {
					toastr.error("No Bukti belum diisi!");
					return;
				}
				...
				medifirstService.post('bendaharapengeluaran/simpan-pengeluaran-bku', dataObjPost).then(function (e) { })
				// Kembali();
				kosongkan();
				loadData()
				// loadCombo()
