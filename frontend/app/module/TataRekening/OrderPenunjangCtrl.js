define(['initialize'], function (initialize) {
    'use strict';
    initialize.controller('OrderPenunjangCtrl', ['$q', '$rootScope', '$scope', 'MedifirstService', '$state', 'CacheHelper',
        function ($q, $rootScope, $scope, medifirstService, $state, cacheHelper) {
            ...
            function loadPertama() {
                ...
                medifirstService.get("tatarekening/get-pasien-bynorec?norec_pd="
            ...
            function init() {
                medifirstService.get("tatarekening/get-data-combo-order", true).then(function (dat) {
                    $scope.listRuangan = dat.data.ruangantujuan;
                    $scope.listDokter = dat.data.dokter;
                })
            }
            $scope.getLayanan = function () {
                medifirstService.getPart("tatarekening/get-tindakan", true, 10, 10)
                .then(function (x) {
                  $scope.listLayanan = x;
                })                             
            }
