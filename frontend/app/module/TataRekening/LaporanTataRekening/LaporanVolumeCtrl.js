define(['initialize'], function(initialize) {
    'use strict';
    initialize.controller('LaporanVolumeCtrl', ['CacheHelper', '$scope', 'DateHelper', 'MedifirstService',
    function(cacheHelper, $scope, DateHelper, medifirstService) {
          ...
          function LoadData() {  
              ...
              if ($scope.item.departement != undefined) {
                  tempDepartemen = "&idDepartemen=" + $scope.item.departement.id;
              }
              var chacePeriode = {
                  0: tglAwal,
                  1: tglAkhir
              }
              cacheHelper.set('LaporanVolumeCtrl', chacePeriode);
  
              medifirstService.get("tatarekening/get-list-laporan-volume?"
                  + "tglAwal=" + tglAwal
                  + "&tglAkhir=" + tglAkhir
                  + tempDepartemen
                  + tempRuanganId
        ...
        $scope.panggil = function(cokot){
          ...
          $("#kGrid").kendoGrid({
            toolbar: ["excel"],
            excel: {
                fileName: "Laporan Pendapatan Volume.xlsx",
                filterable: true,
                allPages: true
            },
