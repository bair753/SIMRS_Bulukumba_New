define(['initialize'], function (initialize) {
	'use strict';
	initialize.controller('LaporanPenerimaanDepositCtrl', ['$q', '$scope', 'CacheHelper', 'DateHelper', 'MedifirstService',
		function ($q, $scope, cacheHelper, dateHelper, medifirstService) {
			$scope.now = new Date();
			$scope.item = {};
			...
			function loadData() {
				...
				$q.all([
					medifirstService.get("tatarekening/get-penerimaan-deposit?" +
						"tglAwal=" + tglAwal +
						"&tglAkhir=" + tglAkhir +
						reg + rm + nm + ins + rg + kp + dk),
				]).then(function (data) {
					$scope.isRouteLoading = false;
					var result = data[0].data.daftar
					var totalAll = 0
					for (var index = 0; index < result.length; index++) {
						...
						var hariRawat = dateHelper.CountAge(new Date(result[index].tglregistrasi), new Date(result[index].tglpulang));
						var bln = hariRawat.month,
						thn = hariRawat.year,
						day = hariRawat.day						
						result[index].hari = day  + ' hr '												
					}
