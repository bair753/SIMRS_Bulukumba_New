  ...
  initialize.controller('DaftarPasienPulangCtrl', ['CacheHelper', '$mdDialog', '$state', '$scope', 'MedifirstService',
    function (cacheHelper, $mdDialog, $state, $scope, medifirstService) {
      ...
      $scope.BtalPulang = function () {
        medifirstService.get("tatarekening/get-status-verif-piutang?noReg=" + $scope.dataPasienSelected.noRegistrasi, false).then(function (res) {
          ...
          medifirstService.post('kasir/save-batal-pulang', objsave).then(function (data) {
             medifirstService.postNonMessage('bridging/bpjs/aplicaresws/update-tt-by-ruangan' ,
              {'idruangan':$scope.dataPasienSelected.ruanganid,'idkelas':$scope.dataPasienSelected.kelasid}).then(function(xx){})
            LoadData();
          })
        })
      }
      ...
      $scope.data2 = function (dataItem) {
        ...
        return {
          ...
          columns: [
            ...
            {
              "field": "totaltagihanverifikasi",
              "title": "Total Tagihan Verifikasi",
              "width": "140px",
              "template": "<span class='style-right'>{{formatRupiah('#: totaltagihanverifikasi #', 'Rp.')}}</span>",
            },
            {
              "field": "totalprekanan",
              "title": "Total Klaim",
              "width": "110px",
              "template": "<span class='style-right'>{{formatRupiah('#: totalprekanan #', 'Rp.')}}</span>",
            },
            {
              "field": "totaldiscount",
              "title": "Total Diskon",
              "width": "110px",
              "template": "<span class='style-right'>{{formatRupiah('#: totaldiscount #', 'Rp.')}}</span>",
            },
            {
              "field": "totalharusdibayar",
              "title": "Total Harus Bayar",
      ...
      $scope.KonversiHarga = function() {
        ...
        if ($scope.dataPasienSelected.status != 'Belum Verifikasi') {
            toastr.error('Konversi harga hanya bisa dilakukan untuk yang belum verifikasi !')
            return
        }
