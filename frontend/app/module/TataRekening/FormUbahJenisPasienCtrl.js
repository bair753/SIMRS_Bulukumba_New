define(['initialize'], function (initialize) {
    'use strict';
    initialize.controller('FormUbahJenisPasienCtrl', ['$mdDialog', '$state', '$q', '$scope', 'DateHelper', 'ModelItem', 'MedifirstService',
        function ($mdDialog, $state, $q, $scope, dateHelper, ModelItem, medifirstService) {
            ...
            function loadCombo() {
                medifirstService.getPart("sysadmin/general/get-desa-kelurahan", true, true, 10).then(function (e) {
                    $scope.listDataKelurahanRelasi = e;
                    $scope.listDataKelurahan = e;
                });

                $q.all([
                    medifirstService.get("sysadmin/general/get-combo-registrasi-general"),
                    medifirstService.get("sysadmin/general/get-combo-address"),
                ]).then(function (result) {
                    if (result[0].statResponse) {
                        $scope.listJenisKelamin = result[0].data.jeniskelamin
                        $scope.listJenisKelamin = result[0].data.jeniskelamin
                        ...
                        $scope.listDataHubunganKeluarga = result[0].data.hubungankeluarga
                        $scope.listJenisAlamat = result[0].data.jenisalamat
                        $scope.listJenisPengantarPasien = result[0].data.pengantar
                    }
                    if (result[1].statResponse) {
                        $scope.listDataKecamatanRelasi = result[1].data.kecamatan
                        $scope.listDataKecamatan = result[1].data.kecamatan
                        $scope.listDataKotaKabupaten = result[1].data.kotakabupaten
                        $scope.listDataKotaKabupatenRelasi = result[1].data.kotakabupaten
                        $scope.listDataPropinsiRelasi = result[1].data.propinsi
            ...
            function FormLoad() {
                $q.all([
                    medifirstService.get("tatarekening/data-pasien-perjanjian/" + $scope.dataParams.noRegistrasi),
                    medifirstService.get("tatarekening/list-kelompok-pasienperjanjian")
                ])
                    .then(function (data) {
                        if (data[0].statResponse) {
                            var datas = data[0].data;
                            $scope.dataNoCm = datas.noCm;
                            editPasien();
                        }
            ...
            function editPasien() {
                ...
                medifirstService.get("sysadmin/general/get-data-bynocm?noCm=" + $scope.dataNoCm).then(function (e) {
                    // $scope.isRouteLoading = false
                    var result = e.data.data
                    $scope.item.noCm = result.nocm;
                    $scope.item.namaPasien = result.namapasien;
                    $scope.item.alamatLengkapP = result.alamatlengkap
                    $scope.item.tempatLahirP = result.tempatlahir;
                    $scope.item.tglLahir = new Date(result.tgllahir)
                    $scope.item.JenisKelamin = result.jeniskelamin
                    if (result.kodepos)
                        $scope.item.KodePos = result.kodepos;
                    medifirstService.get("sysadmin/general/get-desa-kelurahan?iddesakelurahan=" + result.objectdesakelurahanfk).then(function (re) {
                        if (re.data[0] != undefined) {
                            ...
                            $scope.item.desaKelurahanPasien = data.namadesakelurahan
                        }
                    });

                    if (result.objectkecamatanfk)
                        $scope.item.kecamatanPasien = result.namakecamatan
                    if (result.objectkotakabupatenfk)
                        $scope.item.kotaKabupatenPasien = result.namakotakabupaten
                    if (result.objectpropinsifk)
                        $scope.item.namaPropinsi = result.namapropinsi
            ...
            $scope.findKodePos = function () {
                $scope.isBusy = true;
                if (!$scope.item.KodePos) return;                
                medifirstService.get('sysadmin/general/get-alamat-bykodepos?kodePos=' + $scope.item.KodePos).then(function (res) {
                    if (res.data.data.length > 0) {
                        ...
                        $scope.listDataKelurahan.add(data)
                        $scope.item.desaKelurahanPasien = data.desa
                        $scope.item.kecamatanPasien = data.namakecamatan
                        $scope.item.kotaKabupatenPasien = data.namakotakabupaten
                        $scope.item.namaPropinsi = data.namapropinsi
                    }
                    $scope.isBusy = false;
                }, function (error) {
                    $scope.isBusy = false;
