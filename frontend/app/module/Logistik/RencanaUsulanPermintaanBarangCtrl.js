define(['initialize'], function (initialize) {
    'use strict';
    initialize.controller('RencanaUsulanPermintaanBarangCtrl', ['$scope', '$state', 'CacheHelper', 'MedifirstService',
        function ($scope, $state, cacheHelper, medifirstService) {
            ...
            $scope.getTanggalKebutahan = function () {
                $scope.item.tglKebutuhan = new Date($scope.item.tglDibutuhkan);
            }
            ...
            function init() {
                ...
                medifirstService.get("logistik/get-combo-logistik", true).then(function (dat) {
                    ...
                    BulanRomawi = dataCombo.bulanromawi

                    if (noOrder != '') {
                        if (noOrder == 'EditTerima') {
                            medifirstService.get("logistik/get-data-detail-rencana-usulan?norecOrder=" + norecResep, true).then(function (data_ih) {
                            ...
                            medifirstService.get("logistik/get-data-detail-rencana-usulan-ajukan?norecOrder=" + norecResep, true).then(function (data_ih) {
                                $scope.item.tglAwal = data_ih.data.detail.tglorder
                                $scope.item.keterangan = data_ih.data.detail.keterangan
                                $scope.item.penanggungjawab = {
                                    id: data_ih.data.detail.petugasid,
            ...
            function GETKONVERSI(jml) {
                ...
                if ($scope.dataSelected == undefined) {
                    if ($scope.item.ppnpersen == '' || $scope.item.ppnpersen == undefined) {
                        $scope.item.ppnpersen = 0
                    }
                    if ($scope.item.ppn == '' || $scope.item.ppn == undefined) {
                        $scope.item.ppn = 0
                    }
                    if ($scope.item.hargaDiskon == '' || $scope.item.hargaDiskon == undefined) {
                        $scope.item.hargaDiskon = 0
                    }
                    if ($scope.item.hargaDiskonPersen == '' || $scope.item.hargaDiskonPersen == undefined) {
                        $scope.item.hargaDiskonPersen = 0
                    }
                    if ($scope.item.jumlah == '' || $scope.item.jumlah == undefined) {
                        $scope.item.jumlah = 0
                    }
                    if ($scope.item.subTotal == '' || $scope.item.subTotal == undefined) {
                        $scope.item.subTotal = 0
                    }
                }

                if ($scope.item.hargaSatuan == 0 || $scope.item.hargaSatuan == undefined) {
                    medifirstService.get("logistik/get-data-harga?" +
                        "produkfk=" + $scope.item.produk.id +
                        "&ruanganfk=" + $scope.item.ruanganPengusul.id, true).then(function (dat) {
                            var dataProdukDetail = dat.data.detail;
                            if (jml != 0) {
                                $scope.item.hargaSatuan = $scope.item.hargaSatuan;
                            } else if (dataProdukDetail.length != 0) {
            ...
            $scope.getNilaiKonversi = function () {
                ...
                manageLogistikPhp.getDataTableTransaksi("logistik/get-data-harga?" +
                    "produkfk=" + $scope.item.produk.id +
                    "&ruanganfk=" + $scope.item.ruanganPengusul.id, true).then(function (dat) {
            ...
            $scope.$watch('item.noUsulan', function (newValue, oldValue) {
                if (newValue != oldValue) {
                    ...
                    medifirstService.get("logistik/get-nomor-usulan?NoSPK=" + $scope.item.noUsulan, true).then(function (data_ih) {
            ...
            $scope.$watch('item.jumlah', function (newValue, oldValue) {
                if (newValue != oldValue) {
                    ...
                    if ($scope.item.ppn != undefined) {
                        ppn = parseFloat($scope.item.ppn)
                    } else {
                        ppn = 0
                    }

                    if ($scope.item.ppnpersen != undefined) {
                        ppnpersen = parseFloat($scope.item.ppnpersen)
                    } else {
                        ppnpersen = 0
                    }

                    if ($scope.item.hargaDiskonPersen != undefined) {
                        persendiskon = parseFloat($scope.item.hargaDiskonPersen)
                    } else {
                        persendiskon = 0
                    }

                    if ($scope.item.hargaDiskon != undefined) {
            ...
            $scope.$watch('item.hargaDiskonPersen', function (newValue, oldValue) {
                if (newValue != oldValue) {
                    if ($scope.item.hargaDiskonPersen != 0) {
                        $scope.item.hargaDiskon = (parseFloat($scope.item.hargaDiskonPersen) * parseFloat($scope.item.hargaSatuan)) / 100
                    } else {
                        $scope.item.hargaDiskon = 0
                    }
            ...
            $scope.simpan = function () {
                ...
                medifirstService.post('logistik/save-data-rencana-usulan', objSave).then(function (e) {
