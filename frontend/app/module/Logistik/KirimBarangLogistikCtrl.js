define(['initialize'], function (initialize) {
    'use strict';
    initialize.controller('KirimBarangLogistikCtrl', ['$scope', 'CacheHelper', '$mdDialog', 'MedifirstService',
                ...
                if (statusLoad != '') {
                    if (statusLoad == 'EditKirim') {
                        medifirstService.get("logistik/get-detail-kirim-barang-ruangan?norec=" + noreckirim, true).then(function (data_ih) {
                            ...
                            var resep = data_ih.data.head.nokirim.split("-");
                            var bulanNow = moment($scope.now).format('MM');
                            if (resep[1].substr(2, 2) != bulanNow) {
                            ...
                            if (data_ih.data.head.jenispermintaanfk == 1) {
                                $scope.item.jenisKirim = { id: 1, jenis: 'Amprahan' };
                            } else if (data_ih.data.head.jenispermintaanfk == 2) {
                                $scope.item.jenisKirim = { id: 2, jenis: 'Transfer' };
                            }
                    ...
                    } else if (statusLoad == 'KirimBarang') {
                ...
                if (newValue != oldValue) {
                    ...
                    for (var i = 0; i < dataProdukDetail.length; i++) {
                        ...
                        if (parseFloat($scope.item.jumlah * parseFloat($scope.item.nilaiKonversi)) <= parseFloat(dataProdukDetail[i].qtyproduk)) {
                            ...
                            $scope.item.hargaNetto = parseFloat(dataProdukDetail[i].harganetto) * parseFloat($scope.item.nilaiKonversi)
            ...
            function CekStok() {
                if (data2.length > 0) {
                    for (let i = 0; i < data2.length; i++) {                        
                        ...
                        "&ruanganfk=" + $scope.item.ruangan.id, true).then(function (dat) {                            
                            var dataCek = dat.data.detail;
                            var cekstok = dat.data.jmlstok; // $scope.item.nilaiKonversi
                            if (parseFloat(element.jumlah) > parseFloat(cekstok)) {
                                window.messageContainer.error('Data Stok Barang ' + element.namaproduk + ' Kurang !')                                
                                $scope.saveShow = false;
                                return;
                            }
                        });
