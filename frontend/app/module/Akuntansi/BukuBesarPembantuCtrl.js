define(['initialize'], function (initialize) {
    'use strict';
    initialize.controller('BukuBesarPembantuCtrl', ['$scope', 'CacheHelper', 'MedifirstService',
        function ($scope, cacheHelper, medifirstService) {
            ...
            LoadCombo()
            //ON LOAD with Params
            var chacePeriode = cacheHelper.get('BukuBesarPembantuCtrl');
            ...
            function LoadCombo() {
                $scope.listJenisBB = [{ id: 1, jenis: 'Piutang' }, { id: 2, jenis: 'Hutang' }]
            ...
            function simpanCache() {
                ...
                cacheHelper.set('BukuBesarPembantuCtrl', chacePeriode);
                ///END
            };
            ...
            function LoadData() {
                ...
                if ($scope.item.nmRekanan != undefined) {
                    var jp = "rknid=" + $scope.item.nmRekanan.id;
                    if ($scope.item.nmRekanan.id == undefined) {
                        var jp = "";
                    };
                };
                var bb = '&jenis='
                if ($scope.item.jenisBB != undefined) {
                    bb = '&jenis=' + $scope.item.jenisBB.jenis;
                }
                // modelItemAkuntansi.getDataTableTransaksi("akuntansi/financial-report/general-ledger?"+ jp + "&tglAwal=" + tglAwal1 + "&tglAkhir=" + tglAkhir1 ).then(function(data){
                //  $scope.dataGrid=data;
                // });
                //debugger;
                medifirstService.get("akuntansi/get-data-buku-besar-pembantu?" + jp + "&tglAwal=" + tglAwal + "&tglAkhir=" + tglAkhir + bb).then(function (data) {
            ...
            $scope.DetailJurnal = function () {
                ...
                medifirstService.get("akuntansi/get-data-detail-buku-besar?nojurnal=" + $scope.dataSelected.nojurnal + '&accountid=' + $scope.item.kdAkun.id, true).then(function (dat) {
            ...
            $scope.ClosingJurnal = function () {
                ...
                medifirstService.get("akuntansi/get-data-daftar-saldo-awal?coaid=" + $scope.item.kdAkun.id, true).then(function (dat) {
                    for (var i = dat.data.length - 1; i >= 0; i--) {
                        dat.data[i].no = i + 1
                    }
                    $scope.dataPopUp = new kendo.data.DataSource({
            ...
            $scope.tambahSaldo = function () {
                ...
                medifirstService.post('akuntansi/save-data-saldo-awal', objSave).then(function (e) {
            ...
            $scope.hapusSaldo = function () {
                ...
                medifirstService.post('akuntansi/save-hapus-saldo-awal', objSave).then(function (e) {
                    medifirstService.get("akuntansi/get-data-daftar-saldo-awal?coaid=" + $scope.item.kdAkun.id, true).then(function (dat) {
                        for (var i = dat.data.length - 1; i >= 0; i--) {
                            dat.data[i].no = i + 1
                        }
