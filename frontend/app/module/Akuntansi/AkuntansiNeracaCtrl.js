define(['initialize', 'Configuration'], function (initialize, configuration) {
  ...
  var baseTransaksi = configuration.baseApiBackend;
    initialize.controller('AkuntansiNeracaCtrl', ['$scope', 'MedifirstService', '$state', 'CacheHelper', 'DateHelper',
        function ($scope, medifirstService, $state, cacheHelper, dateHelper) {
            ...
            function init() {
                ...
                // }
                var level = "&namalaporan='1','2','3'" //+$scope.item.level.id
                ...
                tittlSDblnini = 's/d ' + dateHelper.formatDate($scope.item.bulan, "MMMM")
            ...
            $scope.optionsDataGrid = {
                toolbar: [
                    "excel",
                    {
                        name: "  Cetak Neraca", text: "  Cetak Neraca",
                        template: '<button ng-click="CetakNeraca()" class="k-button k-button-icontext k-grid-upload" href="\\#"><span class="fa fa-file-pdf-o"></span>&nbsp;&nbsp;Cetak Neraca</button>'
                    },
                ],
                excel: {
                    fileName: "Neraca",
                ...
                columns: [
                    {
                        ...
                        "title": "No",
                        "width": "40px",
                        "template": "<span class='style-left'>#: nomap #</span>"
                    },
                    {
                        "field": "namamap",
                        "title": "Uraian",
            ...
            $scope.CetakNeraca = function(){
                var tahunss = $scope.item.tahun;                    
                var level = "&namalaporan='1','2','3'" //+$scope.item.level.id
                let tglmundur1bulan = moment($scope.item.bulan).add(-1, 'month').toDate()
                let thnmundur1tahun = moment($scope.item.bulan).add(-1, 'year').toDate()
                var bulan2 = dateHelper.formatDate(tglmundur1bulan, "MM")
                ...
                window.open(baseTransaksi + "report/cetak-neraca?tahun=" + tahunss + "&tglAwal=" + tglAwal1 + "&tglAkhir=" + tglAkhir1 + "&user=" + user.namaLengkap + "&tgltgl=" + tgltgl + level);
                // window.history.back();
            }
