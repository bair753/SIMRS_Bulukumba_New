...
define(['initialize'], function(initialize) {
	'use strict';
	initialize.controller('KeluhanPelangganCtrl', ['$rootScope', '$scope', 'ModelItem', '$mdDialog', '$state', 'DateHelper', 'CacheHelper', 'MedifirstService',
		function($rootScope, $scope, ModelItem, $mdDialog, $state, dateHelper, cacheHelper, medifirstService) {
	        ...
	        function LoadCache(){
               var chacePeriode = cacheHelper.get('KeluhanPelangganCtrl');
            ...
            function LoadCombo(){
				...
				medifirstService.get("humas/get-daftar-combo?", true).then(function (dat) {										
					...
					$scope.ListDataRuangan = dat.data.ruangan;
				});			  	
            }
		   ...
		   $scope.keluhan = function(){		   	
			...
			$scope.Simpan = function () {
				...
				medifirstService.post('humas/save-keluhan-pelanggan',objSave).then(function(e) {
					Kosong();
				});
			}
