define(['initialize'], function (initialize) {
    'use strict';
    initialize.controller('DaftarPenungguCtrl', ['$q', '$rootScope', '$scope', '$state', 'CacheHelper', 'DateHelper', 'MedifirstService',
        function ($q, $rootScope, $scope, $state, cacheHelper, dateHelper, medifirstService) {
            ...
            $scope.klikGrid = function (item) {
                if ($scope.item.norec == undefined || $scope.item.norec == ""){
                    $scope.item.norec = item.norec_pg
                }
            }
            ...
            $scope.AmbilKartu = function(){
                if ($scope.item.norec == undefined){
                    alert("Data Belum Dipilih!!");
                    return;
                }
                   
                $scope.winDialogs.center().open();
            }

            $scope.simpanPengambil = function (){
                var tglsekarang = new Date($scope.now)
                var tglAkhir = moment(tglsekarang).format('YYYY-MM-DD HH:mm:ss');     
                var data = {
                    "norec": $scope.item.norec,
                ...
                medifirstService.post('humas/save-pengambil-pasien', data).then(function (e) {
                    $scope.item.keterangan = ""
                    $scope.item.namapengambil = ""
                    $scope.winDialogs.center().close();
                    init()
                })
            ...
            $scope.Hapus = function() {
                ...
                medifirstService.post('humas/delete-penunggu-pasien', data).then(function (e) {
                    $scope.item.norec = "";
                    init()
                })
