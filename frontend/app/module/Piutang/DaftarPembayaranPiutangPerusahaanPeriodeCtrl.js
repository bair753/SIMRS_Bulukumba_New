define(['initialize'], function (initialize) {
	'use strict';
	initialize.controller('DaftarPembayaranPiutangPerusahaanPeriodeCtrl', ['$scope', 'MedifirstService',
		function ($scope, medifirstService) {
			...
			function cariData() {
				...
				medifirstService.get("piutang/daftar-pembayaran-piutang-perusahaan-periode?tglAwal=" + tglAwal + "&tglAkhir=" + tglAkhir 
					+ "&noPosting=" + noCollect + "&idPerusahaan=" + perusahaan).then(function (data) {
					$scope.isRouteLoading = false;					
					$scope.dataSource = {
					...
					for (var i = 0; i < data.data[0].rekap.length; i++) {
						data.data[0].rekap[i].no = i + 1
					}
					$scope.dataSourceRekap = {
						data: data.data[0].rekap,
						schema: {
							model: {
								fields: {
									no: { type: "number" },
