define(['initialize'], function (initialize) {
	'use strict';
	initialize.controller('PembayaranPiutangKasirCtrl', ['$mdDialog', '$state', '$scope', 'MedifirstService',
		function ($mdDialog, $state, $scope, medifirstService) {
			debugger;			
			var params = $state.params.dataPasien
			// if (params.dataPasien != undefined) {
			// 	$scope.dataParams = params.dataPasien;
			// }else{
			// 	$scope.dataParams = params.data;
			// }
			$scope.dataParams = params;
			if ($scope.dataParams != undefined) {				
				$scope.dataParams = JSON.parse($scope.dataParams);
				var strFilter = $scope.dataParams.noPosting;
			...
			$scope.$watch('item.jumlahBayar', function (newValue, oldValue) {
				if (newValue != oldValue) {
					...
					medifirstService.get("sysadmin/general/get-terbilang/" + newValue).then(function (dat) {
						var data = dat.data;
			...
			medifirstService.get("piutang/collected-piutang-layanan/" + noPostingC).then(function (dat) {
				var data = dat.data
				...
				medifirstService.get("piutang/detail-piutang-pasien-collect/" + $scope.item.noCollect, true).then(function (dat) {
			...
			$scope.Bayar = function () {
				if (parseFloat($scope.item.jumlahBayar) > $scope.sisaTagihan || parseFloat($scope.item.jumlahBayar) < $scope.sisaTagihan) {
					...
					$mdDialog.show(confirm).then(function () {
						$scope.changePage("PenerimaanPembayaranKasirPiutang");
					});
