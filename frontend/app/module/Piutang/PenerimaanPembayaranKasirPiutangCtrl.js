define(['initialize'], function (initialize) {
	'use strict';
	initialize.controller('PenerimaanPembayaranKasirPiutangCtrl', ['$mdDialog', '$state', '$q', '$scope', 'MedifirstService',
		function ($mdDialog, $state, $q, $scope, medifirstService) {			
			...
			function FormLoad() {
				medifirstService.get("piutang/collected-piutang-layanan/" + $scope.dataParams.noposting).then(function (dat) {
					$scope.dataParams.noRegistrasi
					var data = dat.data;
					...
					persen = parseFloat(($scope.dataParams.jumlahBayar * 100) / ttlKlaim);

					for (var i = 0; i < $scope.dataSource.length; i++) {
						ttlKlaimPasien = parseFloat($scope.dataSource[i].totalKlaim);
						ttlKlaimPerPasien = parseFloat($scope.dataSource[i].totalKlaim * persen) / 100;
						pembulatan = Math.round(ttlKlaimPerPasien, -1);
						var obj = {
							noRecSPP: $scope.dataSource[i].noRec,
							klaim: ttlKlaimPasien,
							bayarKlaim: pembulatan
						}
						$scope.dataPasien.push(obj)
					}
				})
			}
			...
			switch ($scope.dataParams.pageFrom) {
				...
				case "PembayaranPiutangKasir":
					...
					break;
				case "PembayaranPiutangKasirCollect":
					$scope.dataPrevPage = {
						noRecStrukPelayanan: $scope.dataParams.noRegistrasi,
