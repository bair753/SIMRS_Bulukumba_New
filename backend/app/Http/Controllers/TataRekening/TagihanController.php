...
//        ini_set('max_execution_time', 3000); //6 minutes
        ...
        $pelayanan = \DB::table('pasiendaftar_t as pd')
            ...
            ->select('pp.norec', 'pp.tglpelayanan', 'pp.rke', 'pr.id as prid', 'pr.namaproduk', 'pp.jumlah', 'kl.id as klid', 'kl.namakelas',
                ...
                'ru2.id as ruanganlastid','pp.istarifdetault','pp.hargadijamin'
            )
            ->where('pd.kdprofile', $kdProfile)
            ->where('pd.noregistrasi', $noRegister)
...
//        }
        ...
        //**      DATA TOTAL DIBAYAR BARU 28 APRIL 2021                   */
        $dataTotaldibayar = DB::select(DB::raw("
                select CAST(SUM(x.totaldibayar) AS FLOAT) AS total FROM 
                (select sbm.totaldibayar
                from pasiendaftar_t as pd
                ...
                GROUP BY sbm.totaldibayar) AS x
            "),
            array(
                'noregistrasi' => $noRegister ,
            )
        ...
        $diskontotal = 0;
        $diskontotal = $dataTotalDiskon[0]->total;
        if (count($pelayanan) > 0) {
            $alamat = [];
...
//            }
                ...
                $detail = array(
                    ...
                    'istarifdetault' => $value->istarifdetault,
                    'hargadijamin' => $value->hargadijamin,
                );
...
//                }
            ...
            $result = array(
                ...
                'istarifdetault' => $pelayanan[0]->istarifdetault,
            );
        }else{
            $pelayanan = \DB::table('pasiendaftar_t as pd')
                ->leftjoin('antrianpasiendiperiksa_t as apd', 'apd.noregistrasifk', '=', 'pd.norec')
                ...
                ->select('pp.norec', 'pp.tglpelayanan', 'pp.rke', 'pr.id as prid', 'pr.namaproduk', 'pp.jumlah', 'kl.id as klid', 'kl.namakelas',
                    ...
                    'sr.noresep', 'rk.namarekanan', 'rusr.namaruangan as ruanganfarmasi', 'pgsr.namalengkap as penulisresep', 'jp.jenisproduk', 'kpBpjs.kelompokprodukbpjs as kelompokprodukbpjs',
                    'pgpj.namalengkap as dokterpj', 'pp.jasa', 'kamar.namakamar', 'sp.totalharusdibayar', 'sp.totalprekanan', 'sppj.totalppenjamin',
                    'sp.totalbiayatambahan', 'pgsbm.namalengkap as namalengkapsbm','pd.kdprofile','pp.aturanpakai','pd.statuspasien','ru2.id as ruanganlastid','pp.istarifdetault','pp.hargadijamin'
                )
                ->where('pd.kdprofile', $kdProfile)
                ->where('pd.noregistrasi', $noRegister)
                ->orderBy('pp.tglpelayanan', 'pp.rke');
...
//                    ->first();
            ...
            $result = array(
                ...
                'istarifdetault' => $pelayanan->istarifdetault,
                'hargadijamin' => $pelayanan->hargadijamin,
            );
//            $result = array(
//                'pasienID' => '',
//                'noCm' => '',
...
//            ->where('ru.objectjenispegawaifk', 1)
        ...
        $dataBulan = \DB::table('bulan_m as bm')
            ->where('bm.statusenabled',true)
            ->orderBy('bm.id')
            ->get();
...
//        $pelayanan = $pasienDaftar->pelayanan_pasien()->select('pelayananpasien_t.*')->whereNull('strukfk')->get();
        ...
        if (count($dataLogin['data']['datachecklist']) == 0) {
            ...
            $noStruk = $this->generateCodeBySeqTable(new StrukPelayanan, 'nostruk_verif', 10, 'S',$kdProfile);
            if ($noStruk == ''){
            ...
            $strukPelayanan->tglstruk = $this->getDateTime();
            $strukPelayanan->objectruanganfk = $pasienDaftar->objectruanganlastfk;
            if (isset($request['data']['cekDiskonTotal']) && $request['data']['cekDiskonTotal'] == true){
                if (isset($request['data']['diskonTotal'])){
                    $strukPelayanan->totaldiscount = $request['data']['diskonTotal'];
...
//            $strukPelayanan->totalharusdibayar -= $totalKlaim;
            ...
            if(isset($dataLogin['data']['cekIurBayar']) && !empty($dataLogin['data']['iurbayar']) && $dataLogin['data']['cekIurBayar'] == true){
                $dataIur = $dataLogin['data']['iurbayar'];
                $SPPenjaminDet = new StrukPelayananPenjaminDetail();
                $SPPenjaminDet->norec = $SPPenjaminDet->generateNewId();
                $SPPenjaminDet->statusenabled = true;
                $SPPenjaminDet->kdprofile = $kdProfile;
                $SPPenjaminDet->nostrukfk = $strukPelayanan->norec;
                $SPPenjaminDet->totalppenjamin = $dataIur['klaim'];
                $SPPenjaminDet->totalharusdibayar = $dataIur['selisih'];
                $SPPenjaminDet->keteranganlainnya = 'IUR BAYAR';
                $SPPenjaminDet->kdrekananpenjamin = $pasienDaftar->objectrekananfk;
                $SPPenjaminDet->strukpelayananpenjaminfk = $SPPenjamin->norec;
                $SPPenjaminDet->kdkelompokpasien = $pasienDaftar->objectkelompokpasienlastfk;
                $SPPenjaminDet->kelasawalfk = $dataIur['idkelasawal'];
                $SPPenjaminDet->kelastujuanfk = $dataIur['idkelasakhir'];
                $SPPenjaminDet->totaltagihan = (float)$request['data']['billing'];
                $SPPenjaminDet->totaltagihankelasawal = $dataIur['tagihankelasawal'];
                $SPPenjaminDet->totaltagihankelastujuan = $dataIur['tagihankelasakhir'];
                $SPPenjaminDet->save();
          }
...
//						'urutan'=> $urutan,
    ...
    public function getDataDetailVerifikasi(Request $request){
        ...
        $data= \DB::table('pasiendaftar_t as pd')
            ...
            // ->leftjoin('logginguser_t as lg','lg.noreff','=','sp.norec')
             ->leftjoin('logginguser_t as lg',function($join){
                $join->on('lg.noreff','=','sp.norec');
                $join->where('lg.jenislog','=','Verifikasi TataRekening');
            })
...
//            ->whereNull('sp.statusenabled')
        ...
        foreach ($data as $item){
            ...
            $result[] = array(
                ...
                'totaltagihanverifikasi' => $item->totaltagihanverifikasi,
...
//        return $this->respond($pasien->noregistrasi);
    ...
    public function getDataKelasAntrian(Request $request){
        $kdProfile = (int) $this->getDataKdProfile($request);
        $noreg = $request['Noreg'];
        $data = DB::select(DB::raw("
            SELECT kls.id,kls.namakelas 
            ...
            INNER JOIN antrianpasiendiperiksa_t AS apd ON apd.noregistrasifk = pd.norec
            LEFT JOIN kelas_m AS kls ON kls.id = apd.objectkelasfk
            WHERE pd.kdprofile = $kdProfile AND pd.statusenabled = true 
            AND pd.noregistrasi = '$noreg'
            ORDER BY kls.id ASC
        "));
    ...
    public function konversiharga(Request $request) {
        ...
        try {
            ...
            foreach($datatagihan as $data) {
                ...
                if ($data['produkfk'] == $this->getProdukIdDeposit()) {
                    continue;
                }
                // cokot komponenna
                $datkonversi = $this->getHargakomponen(
                    $kdProfile, 
                    $data['objectruanganfk'],
                    $request['idKelas'], 
                    $data['produkfk'],
                    $data['jenispelayanan'],
                    $request['idPenjamin']);
                if(count($datkonversi['data2']) == 0) continue;
                $jasa = 0;
                $detailpelayanan = $data['detailpelayanan'];
                $norecdetailprev = '';
                foreach($detailpelayanan as $detail) {
                    if(count($datkonversi['data']) == 0) continue;
                    foreach($datkonversi['data'] as $komponen) { 
                ...
                $pelayananpasien[] = [
                    ...
                    "jasa" => $jasa,
                    "harganettokonversi" => $datkonversi['data2'][0]->hargasatuan,
                    "hargajualkonversi" => $datkonversi['data2'][0]->hargasatuan,
                    "hargasatuankonversi" => $datkonversi['data2'][0]->hargasatuan,
                ];
            }
            DB::table('pelayananpasien_temp_t')->insert($pelayananpasien);
            DB::table('pelayananpasiendetail_temp_t')->insert($pelayananpasiendetail);
            $datakonversi = $this->detailHasilKonversiHarga($kdProfile, $noRegister, $request['tglawal'], $request['tglakhir']);
    ...
    public function getHargakomponen($kdProfile, $idRuangan, $idKelas, $idProduk, $idJenisPelayanan, $idPenjamin) {
        ...
        ->where('mpr.objectruanganfk',$idRuangan)
        ->where('hnp.objectkelasfk',$idKelas)
        ->where('mpr.objectprodukfk',$idProduk)
        ->where('hnp.objectjenispelayananfk',$idJenisPelayanan)
        ...
        ->select('mpr.objectprodukfk','prd.id','prd.namaproduk','hnp.hargasatuan','hnp.objectkelasfk',
            'kls.namakelas','mpr.objectruanganfk','ru.namaruangan',
            'prd.namaproduk','hnp.hargadijamin'
        )
        ->where('mpr.objectruanganfk',$idRuangan)
        ->where('hnp.objectkelasfk',$idKelas)
        ->where('mpr.objectprodukfk',$idProduk)
    ...
    public function detailKonversiHarga(Request $request) {
        ...
        $pelayanan = $pelayanan->orderBy('pp.tglpelayanan', 'pp.rke')->get();
        $pelayanandetail = $pelayanandetail->orderBy('pp.tglpelayanan', 'pp.rke')->get();
    ...
    public function detailHasilKonversiHarga ($idProfile, $noRegister, $tglawal, $tglakhir){
        $pelayanan = \DB::table('pasiendaftar_t as pd')
        ->leftjoin('antrianpasiendiperiksa_t as apd', 'apd.noregistrasifk', '=', 'pd.norec')
        ...
        ->where('pd.kdprofile', $idProfile)
        ->where('pd.noregistrasi', $noRegister)
        ->where('pp.tglpelayanan', '>=', $tglawal)
        ->where('pp.tglpelayanan', '<=', $tglakhir)
        ->orderBy('pp.tglpelayanan', 'pp.rke')
        ->get();
    ...
    public function konversiobat(Request $request) {
        $kdProfile = (int) $this->getDataKdProfile($request);
        $noRegister = $request['noregistrasi'];
        DB::beginTransaction();
        try {
            ...
            foreach($datatagihan as $data) {
                ...
                $datkonversi = $this->getProdukDetail($kdProfile, $request['idKelPasien'], $data['produkfk'], $data['ruanganfk']);
                ...
                foreach($detailpelayanan as $detail) {
                    $pelayananpasiendetail[] = [
                        ...
                        'harganetto' => $detail['harganetto'],
                        'jasa' => $detail['jasa'],
                        "harganettokonversi" => $detail['komponenhargafk'] == '12' ? $hargaupnya - $harganettonya : $hargaupnya,
                        "hargajualkonversi" => $detail['komponenhargafk'] == '12' ? $hargaupnya - $harganettonya : $hargaupnya,
                        "hargasatuankonversi" => $detail['komponenhargafk'] == '12' ? $hargaupnya - $harganettonya : $hargaupnya,
                    ];
                }
                $pelayananpasien[] = [
                    'norec' => $data['norec'],
            ...
            $transStatus = true;
            $transMessage = "Konversi data obat Berhasil.";
        }  catch (\Exception $e) {
            $transStatus = false;
            $transMessage = "Konversi data obat Gagal.";
        }
    ...
    public function getProdukDetail($idProfile, $kpid, $produkfk, $ruanganfk) {
    ...
    public function simpankonversiharga(Request $request) {
        ...
        try {
            ...
            foreach($datatagihan as $data) {
                ...
                $jasad = implode(' ', $jasad);
                if (!empty($norecppd)) {
                    DB::update("UPDATE pelayananpasiendetail_t SET jasa = CASE norec {$jasad} END, hargajual = CASE norec {$hargajuald} END, hargasatuan = CASE norec {$hargasatuand} END, harganetto = CASE norec {$harganettod} END WHERE norec in ({$norecppd})");
                }
            }
            ...
            $jasa = implode(' ', $jasa);
            if (!empty($norecpp)) {
                DB::update("UPDATE pelayananpasien_t SET jasa = CASE norec {$jasa} END, hargajual = CASE norec {$hargajual} END, hargasatuan = CASE norec {$hargasatuan} END, harganetto = CASE norec {$harganetto} END WHERE norec in ({$norecpp})");
            }
