...
class BendaharaPengeluaranController extends ApiController {
    ...
    public function getDataCombo(Request $request){
        ...
        $listBankAccount = \DB::table('bankaccount_m')
            ->select('id','bankaccountnama as bankAccountNama','bankaccountnomor')
            ->where('kdprofile', $idProfile)
            ->where('statusenabled',true)
            ->get();
...
//
        ...
        $data = DB::select(DB::raw("select sbk.norec as norec_sbk,sbk.tglsbk,sbk.nosbk,sbk.totaldibayar,sbk.objectkelompoktransaksifk,
                ...
                sbk.keterangan, sbk.statusrekon;"));
    ...
    public function GetDaftarTagihanSuplier(Request $request) {
        ...
        $data = DB::select(DB::raw("select * from(select xx.norec,xx.tglstruk,xx.tgldokumen,xx.tgljatuhtempo,xx.rknid,xx.namarekanan,xx.nostruk,xx.nodokumen,xx.nopo,
                 ...
                 WHEN xx.totalbayar = xx.subtotal OR xx.sisautang = 0 THEN 'LUNAS' ELSE '' END) as status,xx.tglsbk,xx.partnercode,xx.nomorreferencebri,
                  xx.noreferral_fund, xx.date_fund,
                  xx.beneficiaryaccount,xx.sourceaccount,xx.bankrekeningnomor,xx.objectrekananfk
                 ...
                 x.nosbk,x.noverifikasi,x.tglsbk,x.partnercode,x.nomorreferencebri,
                 x.noreferral_fund, x.date_fund,
                 x.beneficiaryaccount,x.sourceaccount,x.bankrekeningnomor,x.objectrekananfk
                 ...
                 sbk.totalsisahutang,sbk.tglsbk,rkn.kodeexternal as partnercode,sp.nomorreferencebri,
                 ft.noreferral as noreferral_fund, ft.transactiondate as date_fund,
                 ft.beneficiaryaccount,ft.sourceaccount,rkn.bankrekeningnomor,sp.objectrekananfk
                 ...
                 left join fundtransfer_t as ft on ft.transaksifk = sp.norec
                 ...
                 GROUP BY sp.norec,sp.tglstruk,sp.tglfaktur,sp.nostruk,sp.nosppb,sp.nofaktur,sp.nosbklastfk,sbk.totaldibayar,
                          ...
                          ft.noreferral,ft.transactiondate,
                          ft.beneficiaryaccount,ft.sourceaccount,rkn.bankrekeningnomor,sp.objectrekananfk) as x) as xx) as su
                 $status;"
        ));
        ...
        foreach ($data as $item) {
            $results[] = array(
                ...
                'noreferral_fund' => $item->noreferral_fund,
                'date_fund' => $item->date_fund,
...
//                        'statusenabled' => false,
      ...
      public function saveReferenceBRI(Request $request){
        $kdProfile = $this->getDataKdProfile($request);
        DB::beginTransaction();
        try{
            StrukPelayanan::where('nostruk',$request['noPosting'])
            ->where('kdprofile',$kdProfile)
            ->update([
                'nomorreferencebri'=>$request['briNumber']
            ]);
        ...
        if ($transStatus == 'true') {
            ...
            $transMessage = "Simpan Nomor Referensi BRI Gagal";
            DB::rollBack();
            $result = array(
                'status' => 400,
                'as' => 'er@epic',
