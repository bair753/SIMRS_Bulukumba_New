...
class BridgingBPJSV2Controller extends ApiController
{
    ...
    public function deleteSEP(Request $request)
    {
        ...
        $methods = 'DELETE';
        $url = $this->getUrlBrigdingBPJS() . "SEP/2.0/delete";
        $response = $this->curlAPI($headers, $dataJsonSend, $url, $methods, $this->getPortBrigdingBPJS());
        return $this->respond($response);
    }
    ...
    public function getRuanganRI(Request $request)
    {
        ...
        $depInap = explode (',',$this->settingDataFixed('kdDepartemenRanapFix',$kdProfile));
        $kdDepInap = [];
        foreach ($depInap as $item){
            $kdDepInap []=  (int)$item;
        }
        $data = \DB::table('ruangan_m as ru')
            ->select('ru.id', 'ru.kdinternal', 'ru.namaruangan', 'ru.objectdepartemenfk')
            ->where('ru.statusenabled', true)
            ->wherein('ru.objectdepartemenfk', $kdDepInap)
    ...
    {
        $kdProfile = (int) $this->getDataKdProfile($request);
        $depInap = explode (',',$this->settingDataFixed('idDepRawatInap',$kdProfile));
        $depRajal = explode (',',$this->settingDataFixed('kdDepartemenRawatJalanFix',$kdProfile));
        $kdDepInap = [];
        $kdDepRajal = [];
        foreach ($depInap as $itemrp){
            $kdDepInap []=  (int)$itemrp;
        }
        foreach ($depRajal as $itemrj){
            $kdDepRajal []=  (int)$itemrj;
        }
        $kdDepInapJoin = implode(",", $kdDepInap);
        $kdDepRajalJoin = implode(",", $kdDepRajal);
        $data = \DB::table('pasiendaftar_t as pd')
            ...
            ->select(
                ...
                'ps.notelepon',
                'pas.nosep',
                DB::raw('case when ru.objectdepartemenfk in ('.$kdDepInapJoin.') then \'true\' 
               when ru.objectdepartemenfk in ('.$kdDepRajalJoin.') then \'false\' end as israwatinap')
            )
    ...
    public function getChecklistKlaim(Request $request)
    {
        ...
        $aingMacan = DB::select(
            DB::raw("select tgl,
                sum(case when objectdepartemenfk not in ($kdDepInapJoin) then  BPJS else 0 end) as bpjs_rajal,
                sum(case when objectdepartemenfk not in ($kdDepInapJoin) then  dokumen else 0 end) as berkas_rajal,
                sum(case when objectdepartemenfk in ($kdDepInapJoin) and objectkelasdijaminfk=3 then  dokumen else 0 end) as berkas_kls1,
                sum(case when objectdepartemenfk in ($kdDepInapJoin) and objectkelasdijaminfk=2 then  dokumen else 0 end) as berkas_kls2,
                sum(case when objectdepartemenfk in ($kdDepInapJoin) and objectkelasdijaminfk=1 then  dokumen else 0 end) as berkas_kls3,
                sum(case when objectdepartemenfk in ($kdDepInapJoin) and objectkelasdijaminfk=3 then  BPJS else 0 end) as bpjs_kls1,
                sum(case when objectdepartemenfk in ($kdDepInapJoin) and objectkelasdijaminfk=2 then  BPJS else 0 end) as bpjs_kls2,
                sum(case when objectdepartemenfk in ($kdDepInapJoin) and objectkelasdijaminfk=1 then  BPJS else 0 end) as bpjs_kls3
                 from
    ...
    {
        $tglMulai = Carbon::now()->subMonth(1)->format('Y-m-d');
    ...
    public function getLokalRujukan(Request $request)
    {
        ...
        $data = \DB::table('bpjsrujukan_t as br')
            ->leftJoin('pasien_m as ps', 'ps.nocm', '=', 'br.nocm')
    ...
    public function getReferensiKamar(Request $request)
    {
        ...
        $response = $this->curlAPI($headers, $dataJsonSend, $url, $methods, 'Aplicaresws');
        return $this->respond($response);
    }
    ...
    public function saveMappingDokterBpjsDokterRs(Request $request)
    {
        ...
        try {
            ...
            $logUser->jenislog = 'Mapping Dokter Bpjs To Dokter Rs ' . $request['namalengkap'];
            $logUser->noreff = $request['idpegawai'];
            $logUser->referensi = 'id pegawai_m';
            $logUser->objectloginuserfk = $dataLogin['userData']['id'];
            $logUser->tanggal = $tglAyeuna;
            $logUser->keterangan = 'Mapping Dokter Bpjs To Dokter Rs ' . $request['namalengkap'];
            $logUser->save();
    ...
    public function saveHapusMappingDokterBpjsDokterRs(Request $request)
    {
        ...
        try {
            ...
            $logUser->kdprofile = $kdProfile;
            $logUser->statusenabled = true;
            $logUser->jenislog = 'Hapus Mapping Dokter Bpjs To Dokter Rs ' . $request['namalengkap'];
            $logUser->noreff = $request['id'];
            $logUser->referensi = 'id pegawai_m';
            $logUser->objectloginuserfk = $dataLogin['userData']['id'];
            $logUser->tanggal = $tglAyeuna;
            $logUser->keterangan = 'Hapus Mapping Dokter Bpjs To Dokter Rs ' . $request['namalengkap'];
            $logUser->save();
    ...
    protected function curlAPI($headers, $dataJsonSend = null, $url, $method, $tipe = null)
    {
        ...
        if ($err) {
            $result = array(
                'metaData' => array(
                    'code' => 400,
                    'message' => 'Gagal menghubungkan ke BPJS (' . $url . ')'.$err
                ),
            );
        } else {
    ...
    function stringDecrypt3($key, $string)
    {
        ...
        try {
            ...
            if (empty($dataantrian)) {
                ...
                $pendaftaran = DB::table('pasiendaftar_t')
                    ->where('kdprofile', $kdProfile)
                    ->where('statusenabled', true)
                    ->where('statusschedule', $datareq['kodebooking'])
                    ->update([
                        'statusenabled' => false,
                    ]);
            }
    ...
    public function saveAntrian($request, $kdProfile)
    {
        ...
        try {
            ...
            $newptp->objectagamafk = $pasien ? $pasien->objectagamafk : null;
            if(!empty( $pasien)){
            ...
            $newptp->objectpendidikanfk = $pasien != null && $pasien->objectpendidikanfk !=null ?$pasien->objectpendidikanfk  : 0;
    ...
    public function updateAplicaresBedAfter(Request $request)
    {
        ...
        if (!empty($data)) {
            ...
            $response = $this->curlAPI($headers, json_encode($json), $url, $methods, 'Aplicaresws');
