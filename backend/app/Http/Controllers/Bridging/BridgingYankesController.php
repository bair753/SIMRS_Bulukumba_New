...
class BridgingYankesController extends ApiController
{
    ...
    public function bridgingIMUT(Request $request)
    {
        ...
        $hoscode = "3673004"; //Kode RS
        $indicator_id = "1"; //Kode Indikator, lihat tabel quality_indicator dan local_quality_indicator
        $date   = date('Y-m-d'); //Tanggal Laporan
        $variable_type = "N"; //N=Numerator; D=Denumerator
        $department_id = "3"; //Kode Departemen, lihat tabel master_institution_department
        $value = "110"; //Nilai yang mau dikirim

        $modules = "imut_lokal"; //imut_nasional; imut_lokal
        ...
        $url = "http://36.94.175.154:8080/sirsak/application/ws/ws_imut_simrs.php"; //WebService IMUT
        $response = $this->sendBridgingCurl($headers, $data, $url, "POST");
        return $this->respond($response);
    }
    public function getCombo(Request $request)
    {
        $kdProfile = $this->getDataKdProfile($request);
        $indikator =  DB::table('quality_indicator')->get();
    ...
    public function saveSismadak(Request $request)
    {
        DB::beginTransaction();
        ...
        try {
            ...
            $new->statusenabled = true;
            $new->date = $request['data'][0]['date'];
            $new->hoscode = $hoscode;
            $new->quality_indicator = $request['data'][0]['indicator_id'];
            $new->departemenfk = $request['data'][0]['department_id'];
            $new->modules = $request['data'][0]['modules'];
            $new->variable_type = $request['data'][0]['variable_type'];
            $new->value = $request['data'][0]['value'];
            $new->save();
            $transStatus = 'true';
        } catch (\Exception $e) {
            $transStatus = 'false';
    ...
    public function getSismadak(Request $request)
    {
        $kdProfile = $this->getDataKdProfile($request);

        $data =  DB::table('sismadak_t as ss')
            ->join('quality_indicator as ind', 'ind.indicator_id', '=', 'ss.quality_indicator')
            ->join('departemen_m as dep', 'dep.id', '=', 'ss.departemenfk')
            ->select(
                'ss.*',
                'ind.indicator_definition',
                'ind.indicator_element',
                'ind.indicator_type',
                'dep.namadepartemen'
            )
        ...
        if (isset($request['nama']) && $request['nama'] != '') {
            $data = $data->where('ind.indicator_element', 'ilike', '%' . $request['nama'] . '%');
        }
        $data = $data->get();
        $res = array(
            'data' => $data,
    ...
    public function deleteSismadak(Request $request)
    {
        DB::beginTransaction();
        try {
            Sismadak::where('norec',$request['norec'])->delete();
            $transStatus = 'true';
        } catch (\Exception $e) {
            $transStatus = 'false';
        }
