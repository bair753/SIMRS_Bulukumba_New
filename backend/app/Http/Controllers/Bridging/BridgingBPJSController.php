...
//          }
  ...
  public function generateSEPDummy (Request $request) {
    ...
    {
        $kdProfile = $this->getDataKdProfile($request);
        $headers =  $this->getHeaderBPJS();
        $dataJsonSend = $request['request'];
        $methods ='POST';
        $url = $this->getUrlBrigdingBPJS()."RencanaKontrol/insert";
        $response = $this->sendBridgingCurl($headers , $dataJsonSend, $url,$methods,$this->getPortBrigdingBPJS());
