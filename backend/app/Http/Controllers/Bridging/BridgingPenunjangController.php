...
//                "sysmex" => $newBRG,
    ...
    public function saveHapusOrderRad(Request $request) { // Syamsu batalkan Order di Alat
      $kdProfile = (int)$this->getDataKdProfile($request);
      DB::beginTransaction();
      ...
      try {
        foreach($request['data'] as $item ){
          ...
          if ($item['nourutrad'] != null) {
            $risoder = RisOrder::where('patient_id',$item['nocm'])
                      ->where('order_cnt',$item['nourutrad'])
                      ->where('order_no',$item['noorder'])
                      ->where('order_code',$item['produkfk'])
                      ->where('order_complete',0)
                      // ->where('study_remark','!=','-')    
                      ->update(['order_complete' => 2]);
          }
        }
...
//             CURLOPT_URL => $ip.'lab_result/api/examination_result/result/'.$request['noorder'],//$data->result_id,
...
//         if ($err) {
           ...
           $result= collect( DB::connection('sqlsrv')
               ->select("select 
                ...
                from HasilLIS
                where NOLAB_RS= '$request[noorder]'"));
           $res = array(
            'resBridging' =>  array(
                "status"=> count($result) > 0 ? 200 : 400,
                "message"=> count($result) > 0 ? "OK" : "Hasil Belum ada ",
                "jenis" => "non_bridging",
                "data" => $result
            ),
            'produk' => []
