...
use App\Transaksi\ClosingPersediaan;
use App\Transaksi\ClosingPersediaanFefo;
...
use App\Transaksi\RiwayatPerubahanHarga;
...
//            ->where('spd.qtyproduk','>',0)
    ...
    public function GetDataKartuStok(Request $request) {
        ...
        $data = \DB::table('kartustok_t as ks')
            ...
            ->where('ks.kdprofile',$idProfile)
            ->groupBy('ks.keterangan','ks.tglinput','ks.tglkejadian','ks.produkfk','pro.namaproduk','ks.ruanganfk','ru.namaruangan',
                      'ks.status','fg.flag','ks.jumlah','ks.saldoawal');
    ...
    public function getDetailPenerimaanBarang(Request $request){
        ...
        $dataStruk = \DB::table('strukpelayanan_t as sr')
            ...
            ->select('sr.tglstruk', 'sr.nostruk', 'rr.noorderintern as nousulan', 'sr.nokontrak', 'pg.id as pgid', 'pg.namalengkap', 'ru.id', 'ru.namaruangan', 'sr.nofaktur',
                ...
                'pg.id as pgid', 'pg.namalengkap', 'sr.objectpegawaipenanggungjawabfk', 'pg1.namalengkap as penanggungjawab', 'sr.namapengadaan', 'sr.noorderfk','sr.tgljatuhtempo','sr.objectruanganfk')
        ...
        $datakirim = collect(DB::select("
            ...
            and kp.kdprofile =$idProfile and kp.statusenabled=true 
        ...
        foreach ($data as $item) {
            ...
            for ($j=0; $j < count($data1res); $j++) { 
                if  ($item->produkfk == $data1res[$j]['objectprodukfk']  && $item->jumlah >= (float)$data2res[$j]['jml']){
        ...
        try {
		    ...
		    $qtyJumlahSisa = ((float)$item5->qtyproduk ) * (float)$item5->hasilkonversi;
                    ...
                    $dataSaldoAwal = collect(DB::select("
                        ...
                        where kdprofile = $idProfile and objectruanganfk=$item5->objectruanganfk and objectprodukfk=$item5->objectprodukfk
                    "))->first();
                    ...
                    //     if ($saldoAwal == $hit['jumlah'] || $saldoAwal >= $hit['jumlah']) {
                            ...
                            $newKS->jumlah = $qtyJumlahSisa;//$TambahStok; 
                            ...
                            $newKS->saldoawal = (float)$saldoKeun;
                            $newKS->status = 0;
                        ...
                        //     $newKS->produkfk = $item5->objectprodukfk;
                        //     $newKS->ruanganfk = $item5->objectruanganfk; //$item->ruanganfk;
                        //     $newKS->saldoawal = $saldoKeun; //(float)$saldoAwal - (float)$TambahStok;
                        //     $newKS->status = 0;
            ...
            foreach ($req['details'] as $item) {
                ...
                $dataSaldoAwal = collect(DB::select("
                    select sum(qtyproduk) as qty from stokprodukdetail_t
                    where kdprofile = $idProfile
                    and objectruanganfk = $item[ruanganfk]
                ...
                $sort = DB::table('stokprodukdetail_t')
                    ->where('kdprofile', $idProfile)
                    ->where('objectruanganfk', $req['struk']['ruanganfk'])
                    ->where('objectprodukfk', $item['produkfk'])
                ...
                $newKS->kdprofile = $idProfile;
                $newKS->statusenabled = true;
                $newKS->jumlah = $qtyJumlahSisa;//$qtyJumlah;
                ...
                $newKS->saldoawal = (float)$saldoAwalIns;
                $newKS->status = 1;
        ...
        try {
            ...
            if (count($dataStokSudahKirim) == 0) {
                foreach ($dataKembaliStok as $item5) {
                    ...
                    "))->first();
                    $saldoAwal = (float)$dataSaldoAwal->qty - $TambahStok;
...
//        select * from persenhargajualproduk_m where objectjenistransaksifk =5; .:25%
        ...
        $kdRangeDiBawah1Jt = (int) $this->settingDataFixed('kdRangeDiBawah1Jt', $idProfile);
        $kdRangeDiAtas1Jt = (int) $this->settingDataFixed('kdRangeDiAtas1Jt', $idProfile);
...
//        $strMetodeHargaNetto = $jenisTransaksi->metodeharganetto; //ketika penerimaan saja
        ...
        $persenHargaJualProduk = \DB::table('persenhargajualproduk_m as phjp')
            ...
            ->select('rg.rangemin','rg.rangemax','phjp.persenuphargasatuan','phjp.objectrangefk')
...
//                    }
                    if ((float)$hitem->rangemin < (float)$item->harganetto &&
                        (float)$hitem->rangemax >= (float)$item->harganetto &&
                        (int)(float)$hitem->objectrangefk == $kdRangeDiBawah1Jt){
                        $persenUpHargaSatuan = (float)$hitem->persenuphargasatuan;
                        break;
                    }
                    if  ((float)$item->harganetto > (float)$hitem->rangemax &&
                        (float)$item->harganetto > (float)$hitem->rangemin &&
                        (int)(float)$hitem->objectrangefk == $kdRangeDiAtas1Jt){
                        $persenUpHargaSatuan = (float)$hitem->persenuphargasatuan;
                        break;
                    }
                }
...
//                    }
        ...
        foreach ($result as $items){
            $jmlstok = $jmlstok+$items->qtyproduk;
        }
        ...
        try{
            ...
            foreach ($request['details'] as $item) {
                ...
                if ($request['strukkirim']['jenispermintaanfk'] == 2) {
                    ...
                    $saldoAwalPenerimaIn = (float)$dataSaldoAwalT->qty + $jumlah;
                    $saldoAwalPengirimIn = $dataSaldoAwaPengirim->qty - $jumlah;
...
//                                    ]);
                            ...
                            if ((float)$items->qty >0) {
                                // if ($jumlah > (float)$dataSaldoAwalT->qty){
                                //     $saldoAwalPenerimaIn = $jumlah;
                                // }else{
                                //     $saldoAwalPenerimaIn = (float)$dataSaldoAwalT->qty + $jumlah;
                                // }
...
//                                    );
                    ...
                    $newKS->saldoawal = (float)$saldoAwalPengirimIn;
                    $newKS->status = 0;
                    ...
                    $newKS2->saldoawal = (float)$saldoAwalPenerimaIn;
                    $newKS2->status = 1;
                ...
                if ($request['strukkirim']['jenispermintaanfk'] == 1) {
                    //PENGIRIM AMPRAHAN
                    $dataSaldoAwalK = DB::select(DB::raw("
                        select qtyproduk as qty,nostrukterimafk,norec,objectasalprodukfk as asalprodukfk,
                        hargadiscount,harganetto1 as harganetto,harganetto1 as hargasatuan
                        from stokprodukdetail_t 
                        where kdprofile = $idProfile and objectruanganfk=:ruanganfk and objectprodukfk=:produkfk and qtyproduk > 0 
...
//                return $this->respond($dataKs);
                ...
                //## KartuStok
                $newKS1 = new KartuStok();
                $norecKS1 = $newKS1->generateNewId();
                $newKS1->norec = $norecKS1;
                $newKS1->kdprofile = $idProfile;
                $newKS1->statusenabled = true;
                $newKS1->jumlah = (float)$item->qtyproduk;
                $newKS1->keterangan = 'Batal Kirim Barang ke Ruangan ' . $request['strukkirim']['ruangantujuan'] . ' No Kirim: ' . $dataSK->nokirim;
                $newKS1->produkfk = $item->objectprodukfk;
                $newKS1->ruanganfk = $request['strukkirim']['objectruanganasal'];
                $newKS1->saldoawal = (float)$saldoAwalPengirim;// + (float)$item->qtyproduk;
                $newKS1->status = 1;
                $newKS1->tglinput = date('Y-m-d H:i:s');
                $newKS1->tglkejadian = date('Y-m-d H:i:s');
                $newKS1->nostrukterimafk = $item->nostrukterimafk;
                $newKS1->save();
...
//                        ->where('qtyproduk','>',0)
                    ...
                    //## KartuStok
                    $newKS2 = new KartuStok();
                    $norecKS2 = $newKS2->generateNewId();
                    $newKS2->norec = $norecKS2;
                    $newKS2->kdprofile = $idProfile;
                    $newKS2->statusenabled = true;
                    $newKS2->jumlah = (float)$item->qtyproduk;
                    $newKS2->keterangan = 'Batal Terima Barang dari Ruangan ' . $request['strukkirim']['ruanganasal'] . ' No Kirim: ' . $dataSK->nokirim;
                    $newKS2->produkfk = $item->objectprodukfk;
                    $newKS2->ruanganfk = $request['strukkirim']['obejectruangantujuan'];
                    $newKS2->saldoawal = (float)$saldoAwalPenerima;// - (float)$item->qtyproduk;
                    $newKS2->status = 0;
                    $newKS2->tglinput = date('Y-m-d H:i:s');
                    $newKS2->tglkejadian = date('Y-m-d H:i:s');
                    $newKS2->nostrukterimafk = $item->nostrukterimafk;
                    $newKS2->save();
...
//                    );
                ...
                if ($request['strukkirim']['jenispermintaanfk'] == 2) {
                    //PENERIMA
                    $ruTerima = $request['strukkirim']['obejectruangantujuan'];
                    $dataSaldoAwalT = collect(DB::select("
                        select sum(qtyproduk) as qty from stokprodukdetail_t 
                        where kdprofile = $idProfile and objectruanganfk=$ruTerima and objectprodukfk=$item->objectprodukfk
                        "))->first();
                    $saldoAwalPenerima = (float)$dataSaldoAwalT->qty - (float)$item->qtyproduk;
...
//            'datatransaksibatal' => $dataPengurangUbahTransaksiTea,
    ...
    public function getDataComboKadaluarsa(Request $request) {
        ...
        $dataProduk =  \DB::select(DB::raw("select * from
                        ...
                        where spd.kdprofile = $idProfile and pr.statusenabled=true and spd.objectruanganfk=:objectruangan 
...
//                                ->first()
    ...
    public function getBarangKadaluarsa (Request $request){
        ...
        $data = \DB::select(DB::raw("select * from
                        ...
                        INNER JOIN satuanstandar_m as sa on sa.id = pr.objectsatuanstandarfk
                        INNER JOIN ruangan_m as ru on ru.id = spd.objectruanganfk
                        where spd.kdprofile = $idProfile and pr.statusenabled=true and spd.tglkadaluarsa >= '$tglkadaluarsa' 
                        $dataRuangan
                        $kdproduk
 	...
 	public function getStokMinimumGlobal(Request $request){
        ...
        $data = DB::select(DB::raw("
                select x.produkfk,pr.namaproduk,ss.satuanstandar,
                 ( x.total1*$leadtime) as stokminimun,y.stok,
                (x.total1*($jmlharipesan+$leadtime)) as qtypengadaan ,
                ...
                INNER JOIN satuanstandar_m as ss on ss.id=pr.objectsatuanstandarfk
                where x.total1*($leadtime*2) > y.stok;
            "));
        ...
        if(count($data)> 0){
            foreach($data  as $d){
                $d->stokminimun =  (float) $d->stokminimun;
                $d->qtypengadaan =  (float) $d->qtypengadaan;
                $d->stok =  (float) $d->stok;
                $d->rata30hari =  (float) $d->rata30hari;
            }
        }
        $result= array(
            'data' => $data,
...
//                "data" => $stokProdukDetail,
     ...
     public function getDaftarStockFlowDetailRev11(Request $request) {
        ini_set('max_execution_time', 1000); //6 minutes
        $tglawal = $request['tglawal'];
        $tglakhir = $request['tglakhir'];
        $ruanganfk=$request['ruanganfk'];
        ...
        if(isset( $request['KdSirs1'])&&  $request['KdSirs1']!=''){
            if(isset( $request['KdSirs2'])&&  $request['KdSirs2']!=''){
                ...
                $kdSirs1 = " and pr.kdproduk like '".$request['KdSirs1']."'";
//                $kdSirs2 = " and '%'";
            }
        }
        ...
        if(isset( $request['KdSirs2'])&&  $request['KdSirs2']!=''){
            if(isset( $request['KdSirs1'])&&  $request['KdSirs1']!=''){
                ...
                $kdSirs2 = " and pr.kdproduk like '".$request['KdSirs2']."%'";
//                $kdSirs1 = " and '%'";
            }
        }
        ...
        if($ruanganfk != null){
            $data = DB::select(DB::raw("
                select x.ruanganfk,x.produkfk, x.kodeexternal,x.namaproduk,x.satuanstandar,x.harga,
                sum(x.penerimaan) as terima,sum(x.pengeluaran) as keluar ,
                x.ruid,x.namaruangan,x.id,x.kdproduk,x.objectjenisprodukfk,x.detailjenisproduk
                from
                (select ks.ruanganfk,ks.produkfk, pr.kodeexternal, pr.namaproduk, ss.satuanstandar,$hrga as harga,
                case when ks.status ='t' then ks.jumlah else 0 end as penerimaan,
                case when ks.status ='f' then ks.jumlah else 0 end as pengeluaran,
                ru.id as ruid, ru.namaruangan,
                pr.id,pr.kdproduk,djp.objectjenisprodukfk,djp.detailjenisproduk
                 from kartustok_t as ks
                INNER JOIN produk_m as pr on pr.id=ks.produkfk
                LEFT JOIN detailjenisproduk_m as djp on pr.objectdetailjenisprodukfk=djp.id
                INNER JOIN satuanstandar_m as ss on ss.id=pr.objectsatuanstandarfk
                ...
                $kdSirs2
                $IdProduk
                and pr.statusenabled='t' and ks.jumlah >0 --and pr.id in (18876,1002116929)
                ) as x
                group by x.ruanganfk,x.produkfk,x.kodeexternal,x.namaproduk,x.satuanstandar,x.harga,
                x.ruid,x.namaruangan,x.id,x.kdproduk,x.objectjenisprodukfk,x.detailjenisproduk
                order by x.namaproduk;")
            ...
            $dataElseStok = DB::select(DB::raw("
                select spd.objectruanganfk as  ruanganfk,spd.objectprodukfk as  produkfk, 
                    ...
                    case when y.terima is null then 0 else y.terima end as terima,
                    case when y.keluar is null then 0 else y.keluar end as keluar,
                    pr.kdproduk,ru.namaruangan,djp.objectjenisprodukfk
                    from stokprodukdetail_t as spd
                    left JOIN 
                    (select ks.ruanganfk,ks.produkfk,
                    sum(case when ks.status ='t' then ks.jumlah else 0 end) as terima,
                    sum(case when ks.status ='f' then ks.jumlah else 0 end) as keluar
                    from kartustok_t as ks
                    INNER JOIN produk_m as pr on pr.id=ks.produkfk
                    where ks.tglinput BETWEEN '$tglakhir' and '$tglnow'
                    and ks.ruanganfk=$ruanganfk
                    ...
                    INNER JOIN satuanstandar_m as ss on ss.id=pr.objectsatuanstandarfk
                    INNER JOIN ruangan_m as ru on ru.id=spd.objectruanganfk
                    where spd.objectruanganfk=$ruanganfk and spd.qtyproduk>0 and pr.statusenabled= 't'
                    $jenisprodukfk
                    $detailjenisprodukfk
                    $kdSirs1
                    $kdSirs2
            ...
            $dataAwal = DB::select(DB::raw("select * from 
                (select ks.produkfk,
                case when ks.status ='f' then ks.saldoawal+ks.jumlah else ks.saldoawal-ks.jumlah end as saldoawal,ks.tglinput, row_number() over (partition by ks.produkfk order by ks.tglinput) as rownum 
                from kartustok_t as ks
                where ks.tglinput BETWEEN '$tglawal' and '$tglakhir'
                and ks.ruanganfk=$ruanganfk ) as x where rownum=1;
                    ")
...
//                        'asalproduk' => $item->asalproduk,
                    ...
                    'jenisProduk' => $item->objectjenisprodukfk,
//                        'tglstruk' =>$item->tglstruk,
//                        'tglkadaluarsa' =>$item->tglkadaluarsa,
                );
//                }
                ...
                if ($tosAya == false){
                    ...
                    if ($jmlAwal > 0){
                        $result[] = array(
                            ...
                            'qtyakhir' =>   $jmlAwal,
                            'ttlakhir' =>   $jmlAwal*$hrgAkhir2,
                            'objectjenisprodukfk' => $arem->objectjenisprodukfk,
                        );
                    }
                }
            }
...
//                'saldoakhir' => $dataSaldoAkhir,
    ...
    public function getComboLogistikMini(Request $request) {
        $idProfile = (int) $this->getDataKdProfile($request);
        $getPasswordStokOpname = $this->PasscodeAuthorization('StokOpname',$idProfile);
        $getPasswordAdjusment = $this->PasscodeAuthorization('AdjusmentStok', $idProfile);
        $getPasswordUbahHarga = $this->PasscodeAuthorization('PasswordUbahHargaPenerimaan', $idProfile);
        $dataRuangan = DB::select(DB::raw("
            SELECT ru.id,ru.namaruangan 
            FROM maploginusertoruangan_s AS mlur
            INNER JOIN loginuser_s lu on lu.id = mlur.objectloginuserfk
        ...
        $result = array(
            ...
            'passwordstokopname' => $getPasswordStokOpname,
            'jabatan' => $dataJabatan,
            'passwordubahharga' => $getPasswordUbahHarga,
            'by' => 'ea@epic',
        );
        return $this->respond($result);
    }
    public function cekTransaksiUse(Request $r)
    {
        $idProfile = (int) $this->getDataKdProfile($r);
        $datakirim = collect(DB::select("
            select * from kirimproduk_t where nostrukterimafk='$r[norec]'
            and kdprofile =$idProfile
            and statusenabled=true
            "))->first();
        $datapp = collect(DB::select("
            select * from pelayananpasien_t where strukterimafk='$r[norec]'
            and kdprofile =$idProfile
            and statusenabled=true
            "))->first();
        $lanjut = true;
       
       if ($datakirim!=null || $datapp!=null) {
            $lanjut = false;
       }
       $res['kirim'] = $datakirim;
       $res['pelayanan'] = $datapp;
       $res['isedit'] = $lanjut;
       return $this->respond($res);
    }

    public function getBarangStokMinimum(Request $request){
        $kdProfile = $this->getDataKdProfile($request);
        $idProfile = (int) $kdProfile;
        $tglawal = $request['tglawal'];
        $tglakhir = $request['tglakhir'];
        $produkId = "";
        if(isset( $request['produkId'])&&  $request['produkId']!=''){
            $produkId = " and dpr.id Ilike " . '%' . $request['produkId'].'%';
        }

//        $asalId = "";
//        if(isset( $request['asalProdukId'])&&  $request['asalProdukId']!=''){
//            $asalId = "and djp.id in (".$request['asalProdukId'].")";
//        }

        $data = DB::select(DB::raw("
            select spd.objectprodukfk, y.namaproduk,y.kdproduk,y.satuanstandar, sum(spd.qtyproduk) as qtyproduk,y.stok30day from stokprodukdetail_t as spd
            INNER JOIN
                (select prid,namaproduk,kdproduk,satuanstandar,avg(jml),round(avg(jml)),round((avg(jml))*30) as stok30day
                from (
                        select pr.id as prid,pr.namaproduk,sum(pp.jumlah) as jml ,date_trunc('day', pp.tglpelayanan),
                         pr.kdproduk,ss.satuanstandar
                        from pelayananpasien_t as pp
                        ...
                        $produkId
                        group by pr.id,pr.namaproduk,ss.satuanstandar,date_trunc('day', pp.tglpelayanan) ,pr.kdproduk
                    ) s
                    group by prid,namaproduk,kdproduk,satuanstandar
                ) as y
            on y.prid=spd.objectprodukfk
            where spd.qtyproduk < y.stok30day
            group by spd.objectprodukfk, y.namaproduk, y.stok30day,y.kdproduk,y.satuanstandar")
        );
        return $this->respond($data);
    }
    ...
    public function getLaporanPersediaan_v2 (Request $request) {
        ini_set('max_execution_time', 1000); //6 minutes
...
//        $tglresepBlnIni = 'O/' . date('ym', strtotime($tglAwal));
        ...
        if(isset($request['nmproduk']) && $request['nmproduk']!="" && $request['nmproduk'] != "undefined"){
            $filterProduk = " and pr.namaproduk ilike '%".$request['nmproduk']."%'";
            $filterProduks = " and pd.namaproduk ilike '%".$request['nmproduk']."%'";
        }
        $filterDjProduk = "";
        ...
        if(isset($request['idRuangan']) && $request['idRuangan']!="" && $request['idRuangan'] != "undefined"){
            ...
            $strsql = "
                ...
                --TRANSFER
                SELECT 0 AS noterima,pr.id,pr.namaproduk,ss.satuanstandar,0 AS saldoawal,
                0 as qtyterima,kp.qtyproduk AS qtykeluar,
                0 as saldoakhir,round(spd.harganetto2) AS harga,spd.objectasalprodukfk,ap.asalproduk
                --,'trf' as ket
                FROM strukkirim_t AS sk
                INNER JOIN kirimproduk_t AS kp ON kp.nokirimfk = sk.norec
                INNER JOIN stokprodukdetail_t as spd on
                --case when kp.nostrukterimafk = 'INIT-001                        '
                --then
                    spd.nostrukterimafk = kp.nostrukterimafk and kp.objectprodukfk=spd.objectprodukfk and 1=spd.sort and spd.objectruanganfk=sk.objectruanganfk
                --ELSE
                --spd.nostrukterimafk = kp.nostrukterimafk and kp.objectprodukfk=spd.objectprodukfk and sk.objectruanganfk=spd.objectruanganfk
                --end
                INNER JOIN produk_m as pr on pr.id = kp.objectprodukfk
                LEFT JOIN satuanstandar_m AS ss ON ss.id = pr.objectsatuanstandarfk
                INNER JOIN ruangan_m AS ru ON ru.id = sk.objectruanganfk
                LEFT JOIN asalproduk_m AS ap ON ap.id = spd.objectasalprodukfk
                WHERE sk.kdprofile =  $kdProfile and sk.jenispermintaanfk = 2
                and sk.statusenabled = TRUE and kp.qtyproduk > 0
                and sk.tglkirim BETWEEN '$tglAwal' and '$tglAkhir'
                and ru.statusenabled = TRUE and pr.statusenabled = TRUE 
                $filterProduk            
                $idRuangan  

                UNION ALL
                
                --Transfer Ruangan tujuan
                SELECT 0 AS noterima,pr.id,pr.namaproduk,ss.satuanstandar,0 AS saldoawal,
                kp.qtyproduk as qtyterima,0 AS qtykeluar,
                0 as saldoakhir,round(spd.harganetto2) AS harga,spd.objectasalprodukfk,ap.asalproduk
                --,'trf ke' as ket
                FROM strukkirim_t AS sk
                INNER JOIN kirimproduk_t AS kp ON kp.nokirimfk = sk.norec
                ...
                --case when kp.nostrukterimafk = 'INIT-001                        '
                -- then
                --spd.nostrukterimafk = kp.nostrukterimafk and kp.objectprodukfk=spd.objectprodukfk and kp.sort=spd.sort and spd.objectruanganfk=sk.objectruangantujuanfk
                -- ELSE
                 spd.nostrukterimafk = kp.nostrukterimafk and kp.objectprodukfk=spd.objectprodukfk  and 1=spd.sort and spd.objectruanganfk=sk.objectruangantujuanfk
                -- end
                INNER JOIN produk_m as pr on pr.id = kp.objectprodukfk
                LEFT JOIN satuanstandar_m AS ss ON ss.id = pr.objectsatuanstandarfk
                INNER JOIN ruangan_m AS ru ON ru.id = sk.objectruangantujuanfk
                LEFT JOIN asalproduk_m AS ap ON ap.id = spd.objectasalprodukfk
                WHERE sk.kdprofile = $kdProfile and sk.jenispermintaanfk = 2
                and sk.statusenabled = TRUE and kp.qtyproduk > 0
                and ru.statusenabled = TRUE and pr.statusenabled = TRUE 
        ...
        $data = DB::select(DB::raw("
            SELECT x.id,x.namaproduk,x.satuanstandar,sum(x.saldoawal) as saldoawal,SUM(x.qtyterima) as qtyterima,
                SUM(x.qtykeluar) as qtykeluar,x.harga,x.objectasalprodukfk,x.asalproduk
                --,x.sort_tr,x.sort
                FROM(
                --PENERIMAAN
                SELECT 0 AS noterima,pr.id,pr.namaproduk,ss.satuanstandar,0 AS saldoawal,
                spdd.qtyproduk as qtyterima,0 qtykeluar,
                0 as saldoakhir,round((spdd.harganetto)-spdd.hargadiscount) AS harga,spdd.objectasalprodukfk,ap.asalproduk
                --,'penerimaan' as ket
                FROM strukpelayanan_t as sp
                INNER JOIN strukpelayanandetail_t as spdd on spdd.nostrukfk = sp.norec
                LEFT JOIN produk_m as pr on pr.id = spdd.objectprodukfk
                LEFT JOIN satuanstandar_m AS ss ON ss.id = pr.objectsatuanstandarfk
                INNER JOIN ruangan_m AS ru ON ru.id = sp.objectruanganfk
                LEFT JOIN asalproduk_m AS ap ON ap.id = spdd.objectasalprodukfk
                WHERE sp.kdprofile = $kdProfile and sp.objectkelompoktransaksifk = 35 and sp.statusenabled = TRUE
                and ru.statusenabled = TRUE and pr.statusenabled = TRUE 
                ...
                --RESEP
                SELECT 0 AS noterima,
                pr.id,pr.namaproduk,ss.satuanstandar,0 AS saldoawal,0 AS qtyterima,pp.jumlah AS qtykeluar,0 as saldoakhir,
                round(spd.harganetto2) AS harga,spd.objectasalprodukfk,ap.asalproduk
                --,'resep' as ket
                FROM pelayananpasien_t AS pp
                INNER JOIN strukresep_t AS sr ON sr.norec = pp.strukresepfk
                INNER JOIN stokprodukdetail_t as spd on 
                --case when pp.strukterimafk = 'INIT-001                        '
                --then
                spd.nostrukterimafk = pp.strukterimafk and pp.produkfk=spd.objectprodukfk and 1=spd.sort and spd.objectruanganfk=sr.ruanganfk
                --ELSE
                --spd.nostrukterimafk = pp.strukterimafk and pp.produkfk=spd.objectprodukfk and spd.objectruanganfk=sr.ruanganfk
                --end
                ...
                --RESEP OBAT BEBAS
                SELECT 0 AS noterima,pr.id,pr.namaproduk,ss.satuanstandar,0 AS saldoawal,
                0 AS qtyterima,(spd.qtyproduk) AS qtykeluar,0 as saldoakhir,
                case when spd.strukterimafk is null then round((spd.harganetto/120)*100) 
                else round(spd2.harganetto2) end AS harga,spd.objectasalprodukfk,ap.asalproduk
                --,'ob' as ket
                FROM strukpelayanan_t AS sp
                INNER JOIN strukpelayanandetail_t AS spd ON spd.nostrukfk = sp.norec AND spd.objectprodukfk = spd.objectprodukfk
                left JOIN stokprodukdetail_t as spd2 on 
                spd.strukterimafk = spd2.nostrukterimafk 
                and spd2.objectprodukfk=spd.objectprodukfk and 1=spd2.sort and spd.objectruanganfk=spd2.objectruanganfk
                LEFT JOIN produk_m as pr on pr.id = spd.objectprodukfk
                LEFT JOIN satuanstandar_m AS ss ON ss.id = pr.objectsatuanstandarfk                
                INNER JOIN ruangan_m AS ru ON ru.id = sp.objectruanganfk
                LEFT JOIN asalproduk_m AS ap ON ap.id = spd.objectasalprodukfk
                WHERE sp.kdprofile = $kdProfile and sp.tglstruk BETWEEN '$tglAwal' and '$tglAkhir'
                and sp.statusenabled = true AND substring(sp.nostruk,1,2)='OB' and spd.objectprodukfk <> 10013803
                $filterProduk
                $filterDjProduk
                $idRuangan
                ...
                --RETUR RESEP
                SELECT 0 as noterima,pd.id,pd.namaproduk,ss.satuanstandar,0 AS saldoawal,
                (pr.jumlah) AS qtyterima,0 AS qtykeluar,0 as saldoakhir,round(spd.harganetto2) as harga,spd.objectasalprodukfk,ap.asalproduk
                --,'retur' as ket
                FROM strukretur_t AS sr
                INNER JOIN pelayananpasienretur_t AS pr ON sr.norec = pr.strukreturfk
                INNER JOIN stokprodukdetail_t as spd on-- spd.nostrukterimafk = pr.strukterimafk and pr.produkfk=spd.objectprodukfk
                --case when pr.strukterimafk = 'INIT-001                        '
                --then
                spd.nostrukterimafk = pr.strukterimafk and pr.produkfk=spd.objectprodukfk and 1=spd.sort and spd.objectruanganfk=sr.objectruanganfk
                --ELSE
                --spd.nostrukterimafk = pr.strukterimafk and pr.produkfk=spd.objectprodukfk and sr.objectruanganfk =spd.objectruanganfk
                --end
                LEFT JOIN produk_m as pd on pd.id = pr.produkfk
                LEFT JOIN satuanstandar_m AS ss ON ss.id = pd.objectsatuanstandarfk
                INNER JOIN ruangan_m AS ru ON ru.id = sr.objectruanganfk
                LEFT JOIN asalproduk_m AS ap ON ap.id = spd.objectasalprodukfk
                WHERE sr.kdprofile = $kdProfile AND sr.tglretur BETWEEN '$tglAwal' and '$tglAkhir'
                ...
                INNER JOIN stokprodukdetail_t as spd on 
                --case when spdo.nostrukterimafk = 'INIT-001                        '
                --then
                spd.nostrukterimafk = spdo.nostrukterimafk and spdo.objectprodukfk=spd.objectprodukfk and 1=spd.sort and spd.objectruanganfk=sc.objectruanganfk
                --ELSE
                --spd.nostrukterimafk = spdo.nostrukterimafk and spdo.objectprodukfk=spd.objectprodukfk and sc.objectruanganfk=spd.objectruanganfk
                --end
                ...
                and sc.tglclosing BETWEEN '$tglAwal' and '$tglAkhir' and spdo.qtyproduksystem <> spdo.qtyprodukreal
                $filterProduk
                $filterDjProduk
                $idRuangan
                ...
                --ADJUSMENT STOK
                select 0 AS noterima , pr.id,pr.namaproduk,ss.satuanstandar,0 AS saldoawal,
                case when spdo.qtyproduksystem-spdo.qtyprodukreal > 0 then (spdo.qtyproduksystem-spdo.qtyprodukreal) else 0 end as qtyterima,
                case when spdo.qtyproduksystem-spdo.qtyprodukreal < 0 then spdo.qtyproduksystem-spdo.qtyprodukreal * (-1) else 0 end as qtykeluar,
                0 as saldoakhir,round(spd.harganetto2) AS harga,spd.objectasalprodukfk,ap.asalproduk
                --,'adj' as ket
                from strukclosing_t as sc
                INNER JOIN stokprodukdetailadjustment_t as spdo on spdo.noclosingfk=sc.norec
                INNER JOIN stokprodukdetail_t as spd on --spd.nostrukterimafk = spdo.nostrukterimafk and spdo.objectprodukfk=spd.objectprodukfk
                --case when spdo.nostrukterimafk = 'INIT-001                        '
                --then
                spd.nostrukterimafk = spdo.nostrukterimafk and spdo.objectprodukfk=spd.objectprodukfk and 1=spd.sort and spd.objectruanganfk=sc.objectruanganfk
                --ELSE
                --spd.nostrukterimafk = spdo.nostrukterimafk and spdo.objectprodukfk=spd.objectprodukfk and spd.objectruanganfk =sc.objectruanganfk
                --end
                LEFT JOIN produk_m as pr on pr.id=spdo.objectprodukfk
                LEFT JOIN satuanstandar_m AS ss ON ss.id = pr.objectsatuanstandarfk
                INNER JOIN ruangan_m AS ru ON ru.id = sc.objectruanganfk
                ...
                --AMPRAHAN
                SELECT 0 AS noterima,
                pr.id,pr.namaproduk,ss.satuanstandar,0 AS saldoawal,
                0 qtyterima,kp.qtyproduk AS qtykeluar,
                0 as saldoakhir,round(spd.harganetto2) AS harga,spd.objectasalprodukfk,ap.asalproduk
                --,'amp' as ket
                FROM strukkirim_t AS sk
                INNER JOIN kirimproduk_t AS kp ON kp.nokirimfk = sk.norec
                ...
                --CLOSING
                select 0 AS noterima,pr.id,pr.namaproduk,ss.satuanstandar,
                       (cp.saldoakhir) AS saldoawal,
                       0 AS qtyterima,
                       0 AS qtykeluar,
                       0 AS saldoakhir,cp.harga,cp.asalprodukfk as objectasalprodukfk,ap.asalproduk
                from closingpersediaan_t AS cp
                inner join produk_m as pr on pr.id = cp.produkfk
                inner join ruangan_m as ru on ru.id = cp.ruanganfk
                LEFT JOIN satuanstandar_m AS ss ON ss.id = pr.objectsatuanstandarfk
                LEFT JOIN asalproduk_m AS ap ON ap.id = cp.asalprodukfk
                ...
                GROUP BY x.id,x.namaproduk,x.satuanstandar,x.harga,x.noterima,x.objectasalprodukfk,x.asalproduk
                --,x.sort_tr,x.sort
                --ORDER BY x.sort ASC
        "));
//        $dataSaldoAwal = DB::select(DB::raw("
//            SELECT 0 AS noterima,x.id,x.namaproduk,x.satuanstandar,
//            -- SUM(x.saldoakhir) + SUM(x.qtykeluar) - SUM(x.qtyterima) as saldoawal,
//            SUM(saldoawal) AS saldoawal,
//            SUM(x.qtyterima) as qtyterima,SUM(x.qtykeluar) as qtykeluar,
//            SUM(x.saldoakhir) as saldoakhir,x.harga
//            FROM(
//            --CLOSING
//            select '0' AS noterima,pr.id,pr.namaproduk,ss.satuanstandar,
...
//            $filterProduk
//            $idRuangan
//            GROUP BY pr.id,pr.namaproduk,ss.satuanstandar,cp.harga
//
//
//            ) as x
//            GROUP BY x.id,x.namaproduk,x.satuanstandar,x.harga
//        "));
//        $dataHargaAkhir = DB::select(DB::raw("select * from (
//                    select sp.tglstruk,spd.objectprodukfk,spd.harganetto1 as harganetto, row_number() over (partition by objectprodukfk order by tglstruk desc) as rownum
//                    from strukpelayanan_t as sp
//                    FULL OUTER JOIN stokprodukdetail_t as spd on spd.nostrukterimafk=sp.norec
//                    where sp.kdprofile = $kdProfile AND sp.statusenabled= true and spd.statusenabled= true
...
//            }
            ...
            $hrgAkhir = round($itemHiji->harga,2);
            $saldoAwalReal = round($itemHiji->saldoawal,2);
//            foreach ($dataSaldoAwal as $itemDua){
//                if ($itemDua->id == $itemHiji->id){
//                    if ((float)$itemDua->harga == (float)$itemHiji->harga){
//                        $saldoAwalReal = (float)$itemDua->saldoawal;
//                        break;
//                    }
//                }
//            }
//            $sal[0]=$saldoAkhir;
//            $sal[1]=$itemHiji->qtykeluar;
//            $sal[2]= $itemHiji->qtyterima;
            $saldoAkhirReal = $saldoAwalReal + round($itemHiji->qtyterima,2) - round($itemHiji->qtykeluar,2);
            $result[] = array(
//              'noterima' => $itemHiji->noterima,
                'id'=> $itemHiji->id,
                'namaproduk'=> $itemHiji->namaproduk,
                'satuanstandar'=> $itemHiji->satuanstandar,
                'objectasalprodukfk'=> $itemHiji->objectasalprodukfk,
                'asalproduk'=> $itemHiji->asalproduk,
                'saldoawal' =>  $saldoAwalReal,
                'hargasatuan' => (float)$hrgAkhir,
                'totalrpsaldoawal' => $saldoAwalReal * (float)$hrgAkhir,
                'qtyterima' => round($itemHiji->qtyterima,2),
                'hargaterima' => (float) $hrgAkhir,
                'totalhargaterima' => (float)round($itemHiji->qtyterima,2) * (float)$hrgAkhir,
                'qtykeluar' => round($itemHiji->qtykeluar,2),
                'hargakeluar' => (float)$hrgAkhir,
                'totalhargakeluar' => (float)round($itemHiji->qtykeluar,2) * (float)$hrgAkhir,
                'saldoakhir' => (float)round($saldoAkhirReal,2),//$saldoAkhir,
                'hargasaldoakhir' => (float)$hrgAkhir,
                'totalrpsaldoakhir' => $saldoAkhirReal * (float)$hrgAkhir,//$saldoAkhir * (float)$hrgAkhir,
            );
        }
        $res['data'] = $result;
//        $res['awal'] = $dataSaldoAwal;
        $res['tr'] = $data;
        return $this->respond($res);

//        -- CEK KIRIM PRODUK
//          select * from kirimproduk_t where norec in ( SELECT kp.norec
//          FROM strukkirim_t AS sk
//          INNER JOIN kirimproduk_t AS kp ON kp.nokirimfk = sk.norec
//          INNER JOIN strukpelayanan_t AS sp ON sp.norec = kp.nostrukterimafk
//          INNER JOIN produk_m as pr on pr.id = kp.objectprodukfk
//          LEFT JOIN satuanstandar_m AS ss ON ss.id = pr.objectsatuanstandarfk
//          INNER JOIN ruangan_m AS ru ON ru.id = sk.objectruangantujuanfk
//          WHERE sk.kdprofile = 21 and sk.jenispermintaanfk = 2
//          and sk.statusenabled = TRUE and kp.qtyproduk > 0
//          and ru.statusenabled = TRUE and pr.statusenabled = TRUE
//          and sk.tglkirim BETWEEN '2021-06-01 00:00' and '2021-06-30 23:59'
//          and pr.id =5012445
//          and ru.id = 94)

        //--CEK SALDO AWAL
        // select * from closingpersediaan_t where bulanclosing ='05.2021' and ruanganfk=94;
        // select * from stokprodukdetail_t where nostrukterimafk ilike '%INIT%' order by objectruanganfk,objectprodukfk;
        //select * from stokprodukdetailopname_t where nostrukterimafk ilike '%INIT%';
        //select * from stokprodukdetailadjustment_t where nostrukterimafk ilike '%INIT%';
        //select * from kirimproduk_t where nostrukterimafk ilike '%INIT%';
        //select * from pelayananpasienretur_t where strukterimafk ilike '%INIT%';
        //select * from pelayananpasien_t where strukterimafk ilike '%INIT%';
        //select * from strukpelayanandetail_t where strukterimafk ilike '%INIT%';
    }

    

    public function cekClosing(Request $r){
        $idProfile = (int) $this->getDataKdProfile($r);
        $ruangan = $r['ruanganfk'];
        $bulan = $r['bulan'];

        $data = collect(DB::select("
            select * from closingpersediaan_t where 
            bulanclosing = '$bulan'
            and ruanganfk = $ruangan
            and kdprofile = $idProfile           
            "));
    ...
    public function GetDataKartuStok_V20210821(Request $request) {
        $kdProfile = (int) $this->getDataKdProfile($request);
        $tglAwal = $request['tglAwal'];
        $tglAkhir = $request['tglAkhir'];
        $ruangan = $request['ruanganfk'];
        $produk = $request['produkfk'];
        $bulan = $request['bulan'];

        $dataClosing = collect(DB::select("
            SELECT pr.namaproduk,
                SUM(cp.saldoawal) AS saldoawal,
                SUM(cp.qtymasuk) AS qtyterima,
                SUM(cp.qtykeluar) AS qtykeluar,
                SUM(cp.saldoakhir) AS saldoakhir
            FROM closingpersediaan_t AS cp 
            INNER JOIN produk_m AS pr ON pr.id = cp.produkfk
            WHERE
            cp.kdprofile = $kdProfile
            AND cp.bulanclosing = '$bulan' 
            AND cp.produkfk = '$produk'
            AND cp.ruanganfk = '$ruangan'
            GROUP BY pr.namaproduk,cp.tglclosing 
            ORDER BY cp.tglclosing DESC 
        "))->first();

        $filterProduk = "";
        $filterProduks = "";
        if(isset($request['produkfk']) && $request['produkfk']!="" && $request['produkfk'] != "undefined"){
            $filterProduk = " and pr.id = ".$request['produkfk'];
            $filterProduks = " and pd.id = ".$request['produkfk'];
        }
        $idRuangan = '';
        if(isset($request['ruanganfk']) && $request['ruanganfk']!="" && $request['ruanganfk'] != "undefined"){
            $idRuangan = ' and ru.id = '. $request['ruanganfk'];
        }

        $data = DB::select(DB::raw("
            SELECT * FROM(            
            --PENERIMAAN
            SELECT sp.tglstruk as tanggal,sp.nostruk AS notransaksi,0 as noterima,pr.id,pr.namaproduk,ss.satuanstandar,
            spd.qtyproduk as qtyterima,0 qtykeluar,
            0 as saldoakhir,'Penerimaan Barang Supplier Nostruk : ' || sp.nostruk AS keterangan
            FROM strukpelayanan_t as sp
            ...
            $filterProduk            
            $idRuangan  
            GROUP BY sp.tglstruk,sp.nostruk,pr.id,pr.namaproduk,spd.qtyproduk,spd.hargasatuan,sp.norec,
                     sp.tglstruk,ss.satuanstandar
            
            UNION ALL
            
            --RESEP
            SELECT sr.tglresep as tanggal,sr.noresep AS notransaksi,0 AS noterima,
            pr.id,pr.namaproduk,ss.satuanstandar,0 AS qtyterima,pp.jumlah AS qtykeluar,0 as saldoakhir,
            'Pelayanan Resep Pasien, Noresep : ' || sr.noresep AS keterangan
            FROM pelayananpasien_t AS pp
            INNER JOIN strukresep_t as sr ON sr.norec = pp.strukresepfk
            INNER JOIN strukpelayanan_t as sp on sp.norec = pp.strukterimafk
            LEFT JOIN stokprodukdetail_t as spd on spd.nostrukterimafk = pp.strukterimafk AND pp.produkfk = spd.objectprodukfk
            LEFT JOIN produk_m as pr on pr.id = pp.produkfk
            LEFT JOIN satuanstandar_m AS ss ON ss.id = pr.objectsatuanstandarfk
            INNER JOIN ruangan_m AS ru ON ru.id = sr.ruanganfk
            WHERE pp.kdprofile = $kdProfile
            ...
            $filterProduk            
            $idRuangan  
            GROUP BY sr.tglresep,sr.noresep,pr.id,pr.namaproduk,
                     ss.satuanstandar,pp.jumlah
            ...
            SELECT sp.tglstruk AS tanggal,sp.nostruk AS notransaksi,0 AS noterima,pr.id,pr.namaproduk,ss.satuanstandar,
            0 AS qtyterima,spd.qtyproduk AS qtykeluar,0 as saldoakhir,
            'Pelayanan Resep Non Layanan, Noresep : ' || sp.nostruk AS keterangan
            FROM strukpelayanan_t AS sp
            INNER JOIN strukpelayanandetail_t AS spd ON spd.nostrukfk = sp.norec AND spd.objectprodukfk = spd.objectprodukfk
            LEFT JOIN strukpelayanan_t as sp1 on sp1.norec = spd.strukterimafk
            LEFT JOIN produk_m as pr on pr.id = spd.objectprodukfk
            LEFT JOIN satuanstandar_m AS ss ON ss.id = pr.objectsatuanstandarfk
            INNER JOIN ruangan_m AS ru ON ru.id = sp.objectruanganfk
            WHERE sp.kdprofile = $kdProfile and sp.tglstruk BETWEEN '$tglAwal' and '$tglAkhir'
            ...
            $filterProduk            
            $idRuangan  
            GROUP BY sp.tglstruk,sp.nostruk,pr.id,pr.namaproduk,ss.satuanstandar,spd.qtyproduk
            
            UNION ALL
            
            --RETUR RESEP
            SELECT sr.tglretur AS tanggal,sr.noretur AS notransaksi,0 AS noterima,pd.id,pd.namaproduk,ss.satuanstandar,
            pr.jumlah AS qtyterima,0 AS qtykeluar,0 as saldoakhir,
            'Pelayanan Retur Resep, Noretur : ' || sr.noretur AS keterangan
            FROM strukretur_t AS sr
            INNER JOIN pelayananpasienretur_t AS pr ON sr.norec = pr.strukreturfk
            INNER JOIN stokprodukdetail_t AS spd ON spd.nostrukterimafk = pr.strukterimafk and spd.objectprodukfk = pr.produkfk
            INNER JOIN strukpelayanan_t as sp1 on sp1.norec = pr.strukterimafk
            LEFT JOIN produk_m as pd on pd.id = pr.produkfk
            LEFT JOIN satuanstandar_m AS ss ON ss.id = pd.objectsatuanstandarfk
            INNER JOIN ruangan_m AS ru ON ru.id = sr.objectruanganfk
            WHERE sr.kdprofile = $kdProfile AND sr.tglretur BETWEEN '$tglAwal' and '$tglAkhir'
            ...
            $filterProduks            
            $idRuangan  
            GROUP BY sr.tglretur,sr.noretur,pd.id,pd.namaproduk,ss.satuanstandar,pr.jumlah
            
            UNION ALL
            
            --STOK OPNAME
            select sc.tglclosing AS tanggal,sc.noclosing AS notransaksi,0 as noterima , pr.id,pr.namaproduk,ss.satuanstandar,
            case when spdo.qtyproduksystem-spdo.qtyprodukreal < 0 then (spdo.qtyproduksystem-spdo.qtyprodukreal) * (-1) else 0 end as qtyterima,
            case when spdo.qtyproduksystem-spdo.qtyprodukreal > 0 then spdo.qtyproduksystem-spdo.qtyprodukreal else 0 end as qtykeluar,
            0 as saldoakhir,'Stok Opname, Noclosing : ' || sc.noclosing AS keterangan
            from strukclosing_t as sc
            INNER JOIN stokprodukdetailopname_t as spdo on spdo.noclosingfk=sc.norec
            LEFT JOIN produk_m as pr on pr.id=spdo.objectprodukfk
            LEFT JOIN satuanstandar_m AS ss ON ss.id = pr.objectsatuanstandarfk
            ...
            --ADJUSMENT STOK
            select sc.tglclosing AS tanggal,sc.noclosing,0 as noterima,pr.id,pr.namaproduk,ss.satuanstandar,
            case when spdo.qtyproduksystem-spdo.qtyprodukreal < 0 then (spdo.qtyproduksystem-spdo.qtyprodukreal) * (-1) else 0 end as qtyterima,
            case when spdo.qtyproduksystem-spdo.qtyprodukreal > 0 then spdo.qtyproduksystem-spdo.qtyprodukreal else 0 end as qtykeluar,
            0 as saldoakhir,'Adjusment Stok, Noclosing : ' || sc.noclosing AS keterangan
            from strukclosing_t as sc
            INNER JOIN stokprodukdetailadjustment_t as spdo on spdo.noclosingfk=sc.norec
            LEFT JOIN produk_m as pr on pr.id=spdo.objectprodukfk
            LEFT JOIN satuanstandar_m AS ss ON ss.id = pr.objectsatuanstandarfk
            ...
            --AMPRAHAN
            SELECT sk.tglkirim AS tanggal,sk.nokirim as notransaksi,
            0 AS noterima,pr.id,pr.namaproduk,ss.satuanstandar,
            0 qtyterima,sum(kp.qtyproduk) AS qtykeluar,
            0 as saldoakhir,'Kirim Amprahan Barang, Nokirim : ' || sk.nokirim AS keterangan
            FROM strukkirim_t AS sk
            INNER JOIN kirimproduk_t AS kp ON kp.nokirimfk = sk.norec
            INNER JOIN strukpelayanan_t AS sp ON sp.norec = kp.nostrukterimafk
            INNER JOIN produk_m as pr on pr.id = kp.objectprodukfk
            ...
            $filterProduk            
            $idRuangan  
            group by sk.tglkirim,sk.nokirim,CASE WHEN kp.nostrukterimafk = 'INIT-001 ' THEN kp.nostrukterimafk
            ELSE sp.nostruk END,pr.id,pr.namaproduk,ss.satuanstandar
            ...
            0 AS noterima,pr.id,pr.namaproduk,ss.satuanstandar,
            0 qtyterima,sum(kp.qtyproduk) AS qtykeluar,
            0 as saldoakhir,'Kirim Transfer Barang, Nokirim : ' || sk.nokirim AS keterangan
            FROM strukkirim_t AS sk
            INNER JOIN kirimproduk_t AS kp ON kp.nokirimfk = sk.norec
            INNER JOIN strukpelayanan_t AS sp ON sp.norec = kp.nostrukterimafk
            INNER JOIN produk_m as pr on pr.id = kp.objectprodukfk
            ...
            SELECT sk.tglkirim AS tanggal,sk.nokirim as notransaksi,
            0 AS noterima,pr.id,pr.namaproduk,ss.satuanstandar,
            sum(kp.qtyproduk) qtyterima,0 AS qtykeluar,
            0 as saldoakhir,'Terima Transfer Barang Ruangan, Nokirim : ' || sk.nokirim AS keterangan
            FROM strukkirim_t AS sk
            INNER JOIN kirimproduk_t AS kp ON kp.nokirimfk = sk.norec
            INNER JOIN strukpelayanan_t AS sp ON sp.norec = kp.nostrukterimafk
            INNER JOIN produk_m as pr on pr.id = kp.objectprodukfk
        ...
        $nilaiSaldoAwal = 0;
        if ($dataClosing != null){
            if ($dataClosing->saldoakhir != 0){
                $saldo = round($dataClosing->saldoakhir);
                $nilaiSaldoAwal = 0;
            }else{
                $saldo = 0;
                $nilaiSaldoAwal =  round($dataClosing->saldoakhir);
            }
        }
//        $result[] = array(
////            'tanggal' => $dataClosing[0]->tglawal,
////            'produk' =>  $dataClosing[0]->namaproduk,
////            'keterangan' => 'SALDO AWAL',// $dataClosing[0]->tanggal,
////            'saldoawal' => 3871,//$dataClosing[0]->saldoawal,
////            'qtyterima' => 0,//$dataClosing[0]->qtymasuk,
////            'qtykeluar' =>  0,//$dataClosing[0]->qtykeluar,
////            'saldoakhir' => 3871,//$dataClosing[0]->saldoawal,
////        );
        foreach ($data as $item){
            $nilaiSaldoAwal = $saldo;
            if ((float)$item->qtyterima == 0) {
                $saldo = $saldo + (float)round($item->qtyterima)- (float)round($item->qtykeluar);
            } else {
                $saldo = $saldo + (float)round($item->qtyterima);
            }
            $jumlahD = $jumlahD + (float)round($item->qtyterima);
            $jumlahK = $jumlahK +  (float)round($item->qtykeluar);
            $saldoAkhir = $saldo;
            $result[] = array(
                ...
                'qtyterima' => (float)round($item->qtyterima),
                'qtykeluar' => (float)round($item->qtykeluar),
                'saldoakhir' => $saldoAkhir
            );
        }

        $res['dataclosing'] = $dataClosing;
        $res['data'] = $data;
        return $this->respond($res);
    }

    public function getLaporanPersediaanPerPenerimaan_V2 (Request $request) {
        ini_set('max_execution_time', 1000); //6 minutes
        $kdProfile = (int) $this->getDataKdProfile($request);
        $tglAwal= $request['tglawal'];
        $tglAkhir = $request['tglakhir'];
...
//      $tglresepBlnIni = 'O/' . date('ym', strtotime($tglAwal));
        ...
        $data = DB::select(DB::raw("                           
            ...
            select 0 AS noterima,pr.id,pr.namaproduk,ss.satuanstandar,
                ...
                SUM(cp.qtymasuk) AS qtyterima,
                SUM(cp.qtykeluar) AS qtykeluar,
                SUM(cp.saldoakhir) AS saldoakhir,cp.harga,ap.asalproduk
            from closingpersediaan_t AS cp
            inner join produk_m as pr on pr.id = cp.produkfk
            inner join ruangan_m as ru on ru.id = cp.ruanganfk
            LEFT JOIN satuanstandar_m AS ss ON ss.id = pr.objectsatuanstandarfk
            ...
            $filterProduk
            $idRuangan
            GROUP BY pr.id,pr.namaproduk,ss.satuanstandar,cp.harga,ap.asalproduk                                         
        "));
        ...
        foreach ($data as $itemHiji){
            ...
            $saldoAkhir = 0;
            $saldoAwalReal = 0;
            $hrgAkhir = (float)round($itemHiji->harga);
//            $saldoAwalReal = round($itemHiji->saldoawal);
//            $saldoAkhirReal = $saldoAwalReal + round($itemHiji->qtyterima) - round($itemHiji->qtykeluar);
            $result[] = array(
                'id'=> $itemHiji->id,
                'namaproduk'=> $itemHiji->namaproduk,
                'satuanstandar'=> $itemHiji->satuanstandar,
                'asalproduk'=> $itemHiji->asalproduk,
                'saldoawal' =>  (float)round($itemHiji->saldoawal),
                'hargasatuan' => $hrgAkhir,
                'totalrpsaldoawal' => (float)round($itemHiji->saldoawal) * $hrgAkhir,
                'qtyterima' => (float)round($itemHiji->qtyterima),
                'hargaterima' => $hrgAkhir,
                'totalhargaterima' => (float)round($itemHiji->qtyterima) * $hrgAkhir,
                'qtykeluar' => (float)round($itemHiji->qtykeluar),
                'hargakeluar' => $hrgAkhir,
                'totalhargakeluar' => (float)round($itemHiji->qtykeluar) * $hrgAkhir,
                'saldoakhir' => (float)round($itemHiji->saldoakhir),
                'hargasaldoakhir' => $hrgAkhir,
                'totalrpsaldoakhir' => (float)round($itemHiji->saldoakhir) * $hrgAkhir,
            );
        }
        $res['data'] = $result;
        $res['tr'] = $data;
    ...
    public function cekTanggalClosing(Request $r){
        $idProfile = (int) $this->getDataKdProfile($r);
        $tglAyeuna = date('Y');

        $data = collect(DB::select("
            select DISTINCT tglawal from closingpersediaan_t            
    ...
    public function saveUbahHargaTerimaBarangSuplier(Request $request){
        $kdProfile = (int) $this->getDataKdProfile($request);
        $tglAyeuna = date('Y-m-d H:i:s');
        \DB::beginTransaction();
        // try {

             $data = new RiwayatPerubahanHarga();
             foreach ($request['details'] AS $item){
                 if ((float)$item['hargabaru'] != 0 || $item['hargabaru'] != null){
                     $strukPelayananDetail = StrukPelayananDetail::where('nostrukfk', $request['struk']['nostruk'])
                                             ->where('kdprofile', $kdProfile)
                                             ->where('objectprodukfk', $item['produkfk'])
                                             ->first();
                     ...
                     $data->nostrukterimafk = $request['struk']['nostruk'];
                     $data->objectstrukpelayanandetail = $strukPelayananDetail->norec;
                     $data->objectprodukfk = $item['produkfk'];
                     $data->qtyproduk = (float) $item['jumlah'];
                     $data->hargaawal = (float) $item['hargasatuan'];
                     $data->hargabaru = (float) $item['hargabaru'];
                     $data->objectpegawaifk = $request['struk']['iduser'];
                     $data->keterangan = "UBAH HARGA PENERIMAAN BARANG SUPPLIER";
                     $data->alasanperubahan = $request['struk']['alasan'];
                     $data->tanggal = $tglAyeuna;
                     $data->save();

                     $upStrukPelayananDetail = StrukPelayananDetail::where('nostrukfk', $request['struk']['nostruk'])
                         ->where('kdprofile', $kdProfile)
                         ->where('objectprodukfk', $item['produkfk'])
                         ->update([
                             'hargasatuan' => (float) $item['hargabaru']
                         ]);
                     $diskon = (((float)$item['persendiscount']) * (float)$item['hargabaru'])/100;
                     $hargaStlhDiskon = (float)$item['hargabaru'] - $diskon;
                     $ppn = ((float) $item['persenppn'] * $hargaStlhDiskon )/100;
                     $harganetto1 = ($hargaStlhDiskon + $ppn) / (float)$item['nilaikonversi'];
                     $harganetto2 = ((float)$item['hargabaru']) / (float)$item['nilaikonversi'];
                     $upStokProdDetail = StokProdukDetail::where('nostrukterimafk', $request['struk']['nostruk'])
                         ->where('objectstrukpelayanandetail', $strukPelayananDetail->norec)
                         ->where('kdprofile', $kdProfile)
                         ->where('objectprodukfk', $item['produkfk'])
                         ->update([
                             'harganetto1' => $harganetto1,
        ...
        if ($transStatus == 'true') {
            $transMessage = "Ubah Harga Penerimaan Berhasil";
            DB::commit();
            $result = array(
                "status" => 201,
                "message" => $transMessage,
            ...
            $transMessage = "Ubah Harga Penerimaan Gagal !";
            DB::rollBack();
            $result = array(
                "status" => 400,
                "message" => $transMessage,
    ...
    public function getDaftarPenerimaanSuplierHarga(Request $request){
        $kdProfile = $this->getDataKdProfile($request);
        $idProfile = (int) $kdProfile;
        $kdSirs1 = $request['KdSirs1'];
        $kdSirs2= $request['KdSirs2'];
        ...
        $data = \DB::table('strukpelayanan_t as sp')
            ->JOIN('riwayatperubahanharga_t as spd', 'spd.nostrukterimafk', '=', 'sp.norec')
            ->LEFTJOIN('rekanan_m as rkn', 'rkn.id', '=', 'sp.objectrekananfk')
            ->LEFTJOIN('pegawai_m as pg', 'pg.id', '=', 'sp.objectpegawaipenerimafk')
            ->LEFTJOIN('ruangan_m as ru', 'ru.id', '=', 'sp.objectruanganfk')
            ->LEFTJOIN('produk_m as pr', 'pr.id', '=', 'spd.objectprodukfk')
            ->LEFTJOIN('strukbuktipengeluaran_t as sbk', 'sbk.norec', '=', 'sp.nosbklastfk')
            ->select(\DB::raw("
                 sp.tglstruk,sp.nostruk,sp.nofaktur,sp.tglfaktur,pr.id AS kdproduk,
                 pr.kdproduk AS kdsirs,pr.namaproduk,rkn.namarekanan,
                 ru.namaruangan,sp.nosppb,
                 CAST (spd.qtyproduk AS FLOAT) AS qtyproduk,			 
                 CAST (spd.hargaawal AS FLOAT) AS hargasatuan,
                 CAST (spd.hargabaru AS FLOAT) AS hargabaru,
                 CAST (spd.qtyproduk * spd.hargabaru AS FLOAT) AS subtotal,
                 0 AS hargadiscount,0 AS hargappn,0 AS total,null AS tglkadaluarsa
            "))
            ->where('sp.kdprofile', $idProfile);
...
public function getLaporanPersediaan_v4_2 (Request $request) {
 ...
 //        $tglresepBlnIni = 'O/' . date('ym', strtotime($tglAwal));
    ...
    $data2 = DB::select(DB::raw("
            ...
            (select DISTINCT pr.id,pp.norec,pr.namaproduk,pp.jumlah,spd.harganetto,round(spd.hargasatuan) as hargasatuan,pp.strukterimafk ,ap.asalproduk,
            case when ap.id = 1 then 16 else ap.id end as apid,ss.satuanstandar
            ...
            --obat bebas
            select DISTINCT sp.norec ,pr.id,pr.namaproduk,0 as saldoawal,0 as qtyterima, (spd.qtyproduk) as qtykeluar,0 as saldoakhir, 0 as returjual,0 as returbeli,
            ...
            --amprah
            SELECT  DISTINCT sk.norec,  pr.id,pr.namaproduk,0 as saldoawal,0 as qtyterima,(kp.qtyproduk) as qtykeluar,0 as saldoakhir, 0 as returjual,0 as returbeli,
            round(spd.hargasatuan) as hargasatuan,ap.asalproduk,case when ap.id = 1 then 16 else ap.id end as apid,ss.satuanstandar
            FROM strukkirim_t AS sk
            INNER JOIN kirimproduk_t AS kp ON kp.nokirimfk = sk.norec
            ...
            union all
                 ...
                 SELECT DISTINCT 
                 sr.norec,pr.id,pr.namaproduk, 0 as saldoawal,0 as qtyterima,0 as qtykeluar, 
                 0 as saldoakhir,ppr.jumlah as returjual,0 as returbeli,round(spd.hargasatuan) as hargasatuan ,ap.asalproduk,
                 ap.id as apid,ss.satuanstandar
                 FROM strukretur_t AS sr
                 INNER JOIN pelayananpasienretur_t AS ppr ON sr.norec = ppr.strukreturfk
                 inner join strukpelayanandetail_t spd on spd.nostrukfk=ppr.strukterimafk and spd.objectprodukfk=ppr.produkfk
    ...
    "));
        ...
        foreach ($data as $itemHiji){
            $data1res[] = array(
                ...
                'id' => $itemHiji->id,
                'namaproduk' => $itemHiji->namaproduk,
                'qtykeluar' => $itemHiji->qtykeluar,
                'qtyterima' => $itemHiji->qtyterima,
                'saldoakhir' => $itemHiji->saldoakhir,
                 'returjual' => $itemHiji->returjual,
                 'returbeli' => $itemHiji->returbeli,
                'saldoawal' => $itemHiji->saldoawal,
                'satuanstandar' => $itemHiji->satuanstandar
            );
        }
        $data2res = [];
        foreach ($data2 as $itemDua){
            $data2res[] = array(
                ...
                'qtyterima' => $itemDua->qtyterima,
                 'returjual' => $itemDua->returjual,
                 'returbeli' => $itemDua->returbeli,
                 'ttlkeluar' => $itemDua->qtykeluar,
        ...
        for ($j = count($data1res)-1;$j>=0; $j--) { 
            if  ($data1res[$j]['saldoawal']  == 0 ){
                $saldoAwalReal = 0;
                $saldoAkhirReal = 0;
                $hrgAkhir = round((float)$data1res[$j]['hargasatuan'],3);
                $qtyterima = round($data1res[$j]['qtyterima'],3) ;
                $qtykeluar = 0;
                $saldoAwalReal = round($data1res[$j]['saldoawal'],3);
                ...
                $result[] = array(
                    ...
                    'returjual' => $data1res[$j]['returjual'],
                    'returbeli' => $data1res[$j]['returbeli'],
                    'hargasaldoakhir' => 0,
                    'totalrpsaldoakhir' => 0,
         ...
         for ($i=0; $i < count($data2res); $i++) { 
            for ($j = count($result)-1;$j>=0; $j--) { 
                if($data2res[$i]['id'] == $result[$j]['id'] && $data2res[$i]['qtyterima'] > 0){
                    $result[$j]['ttlterima'] = $result[$j]['ttlterima'] + $data2res[$i]['qtyterima'];
                    ...
                    $result[$j]['returbeli'] = $result[$j]['returbeli'] ;
                    $result[$j]['returbeli'] = 0;
                }
            }
        }
        for ($i=0; $i < count($data2res); $i++) { 
            for ($j = count($result)-1;$j>=0; $j--) { 
                if($data2res[$i]['id'] == $result[$j]['id'] && $data2res[$i]['ttlkeluar'] > 0){
                    if ($result[$j]['ttlterima']  > $data2res[$i]['ttlkeluar']){
                        $result[$j]['saldoakhir'] =  $result[$j]['ttlterima'] - $data2res[$i]['ttlkeluar'];
                        $result[$j]['qtykeluar'] = $data2res[$i]['ttlkeluar'];
                        $data2res[$i]['ttlkeluar'] = 0;
                    }else{
                        $result[$j]['saldoakhir'] = 0;
                        $data2res[$i]['ttlkeluar'] =   $data2res[$i]['ttlkeluar'] - $result[$j]['ttlterima'];
                        $result[$j]['qtykeluar'] = $result[$j]['ttlterima'];
                    }   
                }
            }
        }
        ...
        //              'satuanstandar' => '',//$data2res[$i]['satuanstandar'],
        //              'objectasalprodukfk' => '',//$data2res[$i]['apid'],
        //              'asalproduk' => '',//$data2res[$i]['asalproduk'],
        //              'saldoawal' =>  $saldoAwalReal,
        //              'hargasatuan' => $hrgAkhir,
        ...
        //  }
         for ($j = count($result)-1;$j>=0; $j--) { 
             ...
             $result[$j]['totalhargaterima'] =  $result[$j]['qtyterima']   * $result[$j]['hargasatuan'];
             $result[$j]['totalhargakeluar']  = $result[$j]['qtykeluar']  *  $result[$j]['hargasatuan'] ;
             $result[$j]['totalrpsaldoakhir'] = $result[$j]['saldoakhir'] *  $result[$j]['hargasatuan'] ;
             $result[$j]['saldoakhir'] = round($result[$j]['saldoakhir'],3);
         }
    ...
    public function getLaporanPersediaan_v4_1 (Request $request) {
       // ini_set('max_execution_time', 1000); //6 minutes
       $kdProfile = (int) $this->getDataKdProfile($request);
       $tglAwal= $request['tglawal'];
       $tglAkhir = $request['tglakhir'];
...
//        $tglresepBlnIni = 'O/' . date('ym', strtotime($tglAwal));
       ...
       x.hargasatuan,x.asalproduk,x.apid,x.satuanstandar  from ( 
           select '0' as norec,pr.id,pr.namaproduk,
                       ...
                       0 AS qtyterima,
                       0 AS qtykeluar,
                       0 AS saldoakhir,round(cp.harga) as hargasatuan,ap.asalproduk,case when cp.asalprodukfk = 1 then 16 else cp.asalprodukfk end as apid,ss.satuanstandar
                       from closingpersediaan_t AS cp
                       inner join produk_m as pr on pr.id = cp.produkfk
                       LEFT JOIN satuanstandar_m AS ss ON ss.id = pr.objectsatuanstandarfk
                       LEFT JOIN asalproduk_m AS ap ON ap.id = case when cp.asalprodukfk = 1 then 16 else cp.asalprodukfk end 
           ...
           union all
                       ...
                       -- penerimaan
                       select sp.norec,pr.id,pr.namaproduk, 0 as saldoawal,(spd.qtyproduk) as qtyterima,0 as qtykeluar, 
                       0 as saldoakhir,round(spd.hargasatuan) as hargasatuan ,ap.asalproduk,
                       case when ap.id = 1 then 16 else ap.id end as apid,ss.satuanstandar
                       from strukpelayanan_t sp
                       INNER JOIN strukpelayanandetail_t spd on spd.nostrukfk=sp.norec
                       INNER JOIN produk_m pr on pr.id=spd.objectprodukfk
   ...
   $data2 = DB::select(DB::raw("
           ...
           --resep
           select x.norec,x.id,x.namaproduk,0 as saldoawal,0 as qtyterima,sum(x.jumlah) as qtykeluar,0 as saldoakhir, x.hargasatuan,x.asalproduk,x.apid,x.satuanstandar from
           (select DISTINCT pr.id,pp.norec,pr.namaproduk,pp.jumlah,spd.harganetto,round(spd.hargasatuan) as hargasatuan,pp.strukterimafk ,ap.asalproduk,
           case when ap.id = 1 then 16 else ap.id end as apid,ss.satuanstandar
           from pelayananpasien_t pp
           INNER JOIN produk_m pr on pr.id=pp.produkfk
           ...
           --obat bebas
           select DISTINCT sp.norec ,pr.id,pr.namaproduk,0 as saldoawal,0 as qtyterima, (spd.qtyproduk) as qtykeluar,0 as saldoakhir, 
           round(spd.hargasatuan) as hargasatuan,ap.asalproduk,case when ap.id = 1 then 16 else ap.id end as apid,ss.satuanstandar
           from strukpelayanan_t sp
           INNER JOIN strukpelayanandetail_t spd on spd.nostrukfk=sp.norec
           INNER JOIN produk_m pr on pr.id=spd.objectprodukfk
           ...
           --amprah
           SELECT  DISTINCT sk.norec,  pr.id,pr.namaproduk,0 as saldoawal,0 as qtyterima,(kp.qtyproduk) as qtykeluar,0 as saldoakhir, 
           round(spd.hargasatuan) as hargasatuan,ap.asalproduk,case when ap.id = 1 then 16 else ap.id end as apid,ss.satuanstandar
           FROM strukkirim_t AS sk
           INNER JOIN kirimproduk_t AS kp ON kp.nokirimfk = sk.norec
           inner join strukpelayanandetail_t spd on spd.nostrukfk=kp.nostrukterimafk and spd.objectprodukfk=kp.objectprodukfk
           ...
           case when spdo.qtyproduksystem-spdo.qtyprodukreal < 0 then (spdo.qtyproduksystem-spdo.qtyprodukreal) * (-1) else 0 end as qtyterima,
           case when spdo.qtyproduksystem-spdo.qtyprodukreal > 0 then spdo.qtyproduksystem-spdo.qtyprodukreal else 0 end as qtykeluar,
           0 as saldoakhir,round(spdo.harganetto1) AS hargasatuan,ap.asalproduk,case when spdo.objectasalprodukfk = 1 then 16 else spdo.objectasalprodukfk end as apid,ss.satuanstandar
           from strukclosing_t as sc
           INNER JOIN stokprodukdetailopname_t as spdo on spdo.noclosingfk=sc.norec
           LEFT JOIN produk_m as pr on pr.id=spdo.objectprodukfk
           LEFT JOIN satuanstandar_m AS ss ON ss.id = pr.objectsatuanstandarfk
   ...
   "));
       ...
       $result=[];
        ...
        for ($i=0; $i < count($data2res); $i++) { 
            for ($j = count($result)-1;$j>=0; $j--) { 
                if($data2res[$i]['id'] == $result[$j]['id'] && $result[$j]['saldoawal'] > 0){
                    if ($result[$j]['saldoawal']+ $result[$j]['qtyterima'] +  $data2res[$i]['qtyterima']  > $data2res[$i]['qtykeluar']){
                        $result[$j]['qtyterima'] = $result[$j]['qtyterima'] + $data2res[$i]['qtyterima'];
                        $result[$j]['qtykeluar'] =  $result[$j]['qtykeluar'] + $data2res[$i]['qtykeluar'];
                        $result[$j]['saldoakhir'] = $result[$j]['saldoawal'] +  $result[$j]['qtyterima'] - $result[$j]['qtykeluar'];
                        $data2res[$i]['qtyterima'] = 0;
                        $data2res[$i]['qtykeluar'] = 0;
                    }else{
                        $result[$j]['qtyterima'] = $result[$j]['qtyterima'] + $data2res[$i]['qtyterima'];
                        $result[$j]['qtykeluar'] =  $result[$j]['qtykeluar'] + $result[$j]['saldoawal'] +  $data2res[$i]['qtyterima'];
                        $result[$j]['saldoakhir'] = $result[$j]['saldoawal']  + $result[$j]['qtyterima'] - $result[$j]['qtykeluar'];
                        $data2res[$i]['qtyterima'] = 0;
                        $data2res[$i]['qtykeluar'] = $data2res[$i]['qtykeluar'] - $result[$j]['qtykeluar'];
                    }
            ...
            for ($j = count($result)-1;$j>=0; $j--) { 
                if($data2res[$i]['id'] == $result[$j]['id'] && $result[$j]['saldoawal'] == 0){
                    if ($result[$j]['saldoawal'] + $result[$j]['qtyterima'] + $data2res[$i]['qtyterima']  > $data2res[$i]['qtykeluar']){
                        $result[$j]['qtyterima'] = $result[$j]['qtyterima'] + $data2res[$i]['qtyterima'];
                        $result[$j]['qtykeluar'] =  $result[$j]['qtykeluar'] + $data2res[$i]['qtykeluar'];
                        $result[$j]['saldoakhir'] = ($result[$j]['qtyterima'] ) - $result[$j]['qtykeluar'];
                        $data2res[$i]['qtyterima'] = 0;
                        $data2res[$i]['qtykeluar'] = 0;
                    }else{
                        $result[$j]['qtyterima'] = $result[$j]['qtyterima'] + $data2res[$i]['qtyterima'];
                        $result[$j]['qtykeluar'] =  $result[$j]['qtykeluar'] + $result[$j]['saldoawal'];
                        $result[$j]['saldoakhir'] = $result[$j]['saldoawal']  + $result[$j]['qtyterima'] - $result[$j]['qtykeluar'];
                        $data2res[$i]['qtyterima'] = 0;
                        $data2res[$i]['qtykeluar'] = $data2res[$i]['qtykeluar'] - ($result[$j]['saldoawal'] + $data2res[$i]['qtyterima']);
                    }
    ...
    public function getLaporanPersediaanDetail_v3 (Request $request) {
        $kdProfile = (int) $this->getDataKdProfile($request);
        $tglAwal= $request['tglawal'];
        $tglAkhir = $request['tglakhir'];
        $tglawalsaldo = $request['tglakhir'];
        ...
        $data = DB::select(DB::raw("
            select u.trans,u.norec,u.tgl,u.ket, u.saldoawal,
            u.qtyterima,u.qtykeluar, u.saldoakhir,u.hargasatuan ,u.asalproduk,u.apid,u.satuanstandar from
            (
                select 'Penerimaan' as trans,sp.norec,sp.tglstruk as tgl,sp.nofaktur || ' ' || sp.namarekanan as ket, 0 as saldoawal,(spd.qtyproduk) as qtyterima,0 as qtykeluar, 0 as saldoakhir,spd.hargasatuan ,ap.asalproduk,ap.id as apid,ss.satuanstandar
                from strukpelayanan_t sp
                INNER JOIN strukpelayanandetail_t spd on spd.nostrukfk=sp.norec
                INNER JOIN produk_m pr on pr.id=spd.objectprodukfk
...
select DISTINCT 'Resep' as trans,pp.norec, pp.tglpelayanan as tgl,sr.noresep || ' ' || ps.namapasien  as ket,
0 as saldoawal,0 as qtyterima,pp.jumlah as qtykeluar, 0 as saldoakhir,spd.hargasatuan ,ap.asalproduk,ap.id as apid,ss.satuanstandar
                from pelayananpasien_t pp
                INNER JOIN produk_m pr on pr.id=pp.produkfk
                inner join strukpelayanandetail_t spd on spd.nostrukfk=pp.strukterimafk and spd.objectprodukfk=pp.produkfk
                INNER JOIN asalproduk_m ap on ap.id=spd.objectasalprodukfk
...
union all
 ...
 select DISTINCT 'Obat Bebas' as trans,sp.norec ,sp.tglstruk as tgl,sp.nostruk || ' ' || sp.namapasien_klien as ket,0 as saldoawal,0 as qtyterima, (spd.qtyproduk) as qtykeluar,0 as saldoakhir, spd.hargasatuan ,ap.asalproduk,ap.id as apid,ss.satuanstandar
                from strukpelayanan_t sp
                INNER JOIN strukpelayanandetail_t spd on spd.nostrukfk=sp.norec
                INNER JOIN produk_m pr on pr.id=spd.objectprodukfk
                INNER JOIN asalproduk_m ap on ap.id=spd.objectasalprodukfk
...
union all
 ...
 SELECT DISTINCT 'Amprahan' as trans,sk.norec,sk.tglkirim as tgl,sk.nokirim || ' ' || ru.namaruangan as ket,0 as saldoawal,0 as qtyterima,(kp.qtyproduk) as qtykeluar,0 as saldoakhir, 
spd.hargasatuan,ap.asalproduk,ap.id as apid,ss.satuanstandar
                FROM strukkirim_t AS sk
                INNER JOIN kirimproduk_t AS kp ON kp.nokirimfk = sk.norec
                inner join strukpelayanandetail_t spd on spd.nostrukfk=kp.nostrukterimafk and spd.objectprodukfk=kp.objectprodukfk
                INNER JOIN produk_m as pr on pr.id = kp.objectprodukfk
...
INNER JOIN ruangan_m ru on ru.id=sk.objectruangantujuanfk
                ...
                -- union all
                -- 
                -- select DISTINCT 'SO' as trans,sc.norec, sc.tglclosing,sc.noclosing || ' Stock Opname' as ket,0 AS saldoawal,
                --                 case when spdo.qtyproduksystem-spdo.qtyprodukreal < 0 then (spdo.qtyproduksystem-spdo.qtyprodukreal) * (-1) else 0 end as qtyterima,
                --                 case when spdo.qtyproduksystem-spdo.qtyprodukreal > 0 then spdo.qtyproduksystem-spdo.qtyprodukreal else 0 end as qtykeluar,
                --                 0 as saldoakhir,round(spdo.harganetto1) AS hargasatuan,ap.asalproduk,spdo.objectasalprodukfk as apid,ss.satuanstandar--,pr.id
                --                 from strukclosing_t as sc
                --                 INNER JOIN stokprodukdetailopname_t as spdo on spdo.noclosingfk=sc.norec
                --                 LEFT JOIN produk_m as pr on pr.id=spdo.objectprodukfk
                --                 LEFT JOIN satuanstandar_m AS ss ON ss.id = pr.objectsatuanstandarfk
                --                 INNER JOIN ruangan_m AS ru ON ru.id = sc.objectruanganfk
                --                 LEFT JOIN asalproduk_m AS ap ON ap.id = spdo.objectasalprodukfk
                --                 where sc.kdprofile = 21
                --                 and sc.statusenabled = true
                --                 and sc.tglclosing BETWEEN '$tglAwal' and '$tglAkhir'
                --                 and pr.id = $idproduk
                ...
                -- union all
                -- 
                -- select DISTINCT 'ADJ' as trans,sc.norec ,sc.tglclosing as tgl, sc.noclosing || ' Adjusment' as ket,0 AS saldoawal,
                --                 case when spdo.qtyproduksystem-spdo.qtyprodukreal > 0 then (spdo.qtyproduksystem-spdo.qtyprodukreal) else 0 end as qtyterima,
                --                 case when spdo.qtyproduksystem-spdo.qtyprodukreal < 0 then spdo.qtyproduksystem-spdo.qtyprodukreal * (-1) else 0 end as qtykeluar,
                --                 0 as saldoakhir,round(spdo.harganetto1) AS hargasatuan,ap.asalproduk,spdo.objectasalprodukfk as apid,ss.satuanstandar
                --                 from strukclosing_t as sc
                --                 INNER JOIN stokprodukdetailadjustment_t as spdo on spdo.noclosingfk=sc.norec
                --                 LEFT JOIN produk_m as pr on pr.id=spdo.objectprodukfk
                --                 LEFT JOIN satuanstandar_m AS ss ON ss.id = pr.objectsatuanstandarfk
                ...
                --STOK OPNAME
                select DISTINCT 'Stok Opname' as trans,sc.norec,sc.tglclosing as tgl,sc.noclosing || ' Stock Opname' as ket,0 AS saldoawal,
                case when spdo.qtyproduksystem-spdo.qtyprodukreal < 0 then (spdo.qtyproduksystem-spdo.qtyprodukreal) * (-1) else 0 end as qtyterima,
                case when spdo.qtyproduksystem-spdo.qtyprodukreal > 0 then spdo.qtyproduksystem-spdo.qtyprodukreal else 0 end as qtykeluar,
                0 as saldoakhir,round(spdo.harganetto1) AS hargasatuan,ap.asalproduk,case when spdo.objectasalprodukfk = 1 then 16 else spdo.objectasalprodukfk end as apid,ss.satuanstandar
                from strukclosing_t as sc
                ...
                -- returjual
                SELECT DISTINCT 'Retur Jual' as trans,
                sr.norec,sr.tglretur as tgl, sr.noretur || ' Retur' as ket,
                0 as saldoawal,ppr.jumlah as qtyterima,0 as qtykeluar, 
                0 as saldoakhir,round(spd.hargasatuan) as hargasatuan ,ap.asalproduk,
                ap.id as apid,ss.satuanstandar
                FROM strukretur_t AS sr
                INNER JOIN pelayananpasienretur_t AS ppr ON sr.norec = ppr.strukreturfk
                ...
                -- returbeli
                 SELECT DISTINCT 'Retur Beli' as trans,
                 sr.norec,sr.tglretur as tgl, sr.noretur || ' Retur' as ket,
                 0 as saldoawal,0 as qtyterima,ppr.qtyproduk as qtykeluar, 
                 0 as saldoakhir,round(spd.hargasatuan) as hargasatuan ,ap.asalproduk,
                 ap.id as apid,ss.satuanstandar
                 FROM strukretur_t AS sr
                 INNER JOIN strukreturdetail_t AS ppr ON sr.norec = ppr.strukreturfk
    ...
    public function getDataComboStockFlow(Request $request) {
        ...
        $detailJenis =  \DB::table('detailjenisproduk_m as djp')
            ...
            ->where('djp.statusenabled',true)
            ->get();
        $kdProdukSirs =  \DB::table('produk_m as djp')
            ->select('djp.id','djp.kdproduk','djp.namaproduk')
            ->where('djp.statusenabled',true)
            ->whereNotNull('djp.kdproduk')
            ->where('djp.kdproduk', '<>', "")
            ->get();
        foreach ($jenisProduk as $item) {
            $detail = [];
        ...
        $passClosing = $this->settingDataFixed('PasswordClosing',$kdProfile);
        ...
        $result = array(
            ...
            'password' => $passClosing,
        );
        return $this->respond($result);
    }
    public function closingPersediaan_Agustus2021(Request $request){
        $idProfile = (int) $this->getDataKdProfile($request);
        $tglAwal = $request['tglawal'];
        $tglAkhir = $request['tglakhir'];
        $tglAyeuna = date('Y-m-d H:i:s');
        \DB::beginTransaction();
        try{
            $data = ClosingPersediaan::where('kdprofile', $idProfile)
                ->where('tglawal', $tglAwal)
                ->where('tglakhir', $tglAkhir)
                ->where('ruanganfk', $request['ruanganfk'])
                ->where('bulanclosing', $request['bulan'])
                ->delete();
            $dataInsert = [];
            foreach ($request['details'] as $item){
                $dataInsert[] = array(
                    ...
                    'tglawal' => $request['tglawal'],
                    'tglakhir' => $request['tglakhir'],
                    'bulanclosing' => $request['bulan'],
                    'produkfk' => $item['id'],
                    'saldoawal' => (float)$item['saldoawal'],
                    'qtymasuk' => (float)$item['qtyterima'],
                    'qtykeluar' => (float)$item['qtykeluar'],
                    'saldoakhir' => (float)$item['saldoakhir'],
                    'harga' => (float)$item['hargasatuan'],
                    'ruanganfk' => $request['ruanganfk']
                );
                if (count($dataInsert) > 100){
                    ClosingPersediaan::insert($dataInsert);
                    $dataInsert = [];
                }
            }
        ...
        if ($transStatus){
            $transMessage = "Closing Berhasil";
            \DB::commit();
            $result = array(
                'status' => 201,
            );
        } else {
            $transMessage = "Closing Gagal";
            DB::rollBack();
            $result = array(
                'status' => 400,
                'as' => 'ea@epic',
