    ...
    public function daftarCollectedPiutang(Request $request){
        ...
        $dataCollector = \DB::table('postinghutangpiutang_t as php')
            ...
            ->select('sp.norec', 'sp.tglposting', 'php.noposting','rkn.id as idrekanan','rkn.namarekanan',
                ...
                'php.statusenabled','p.namalengkap','php.nomorreferencebri',
        ...
        $dataCollector=$dataCollector->groupBy('sp.norec', 'php.noposting', 'sp.tglposting','rkn.id','rkn.namarekanan','php.statusenabled','p.namalengkap','php.nomorreferencebri',
          'rkn.kodeexternal');
...
//            ->distinct();
    ...
    public function getSettingBRI(Request $request){
            $kdProfile = (int) $this->getDataKdProfile($request);
            $data = array(
                'AnchorCode' => null,
                'PartnerCode' => null,
            ...
            $set = SettingDataFixed::where('kelompok', 'BANK BRI')
                ->where('kdprofile', $kdProfile)
                ->get();
            foreach ($set as $s){
                if($s->namafield == 'AnchorCode'){
                    $data['AnchorCode'] = $s->nilaifield ;
                }
                if($s->namafield == 'PartnerCode'){
                    $data['PartnerCode'] =  $s->nilaifield ;
                }
                if($s->namafield == 'AnchorName'){
                    $data['AnchorName'] =  $s->nilaifield ;
                }
                if($s->namafield == 'tanpaRelasiAPI'){
                    $data['tanpaRelasiAPI'] =  $s->nilaifield ;
                }
                if($s->namafield == 'sourceAccount'){
                    $data['sourceAccount'] =  $s->nilaifield ;
                }
            }

            $data['bankaccount'] = DB::table('bankaccount_m')
            ->select('id', 'bankaccountnama','bankaccountnomor','kdbankaccount','kdrekananfk')
            ->where('kdprofile',$kdProfile)
            ->get();
            foreach($data['bankaccount']  as $d){
                $d->nama = $d->bankaccountnama .' - '. $d->bankaccountnomor;
            }
            return $this->respond($data);
    }
    public function saveReferenceBRI(Request $request){
        ...
        DB::beginTransaction();
        try{
            PostingHutangPiutang::where('noposting',$request['noPosting'])
            ->where('kdprofile',$kdProfile)
            ->update([
    ...
    public function getNorecSPByCollect(Request $request){
            $kdProfile = (int) $this->getDataKdProfile($request);
            $data = DB::select(DB::raw("

                select sp.norec as norec_spp ,sp.norec as norec_sp,
                spp.totalppenjamin,
                php.noposting
                from postinghutangpiutang_t  as php
                inner join strukpelayananpenjamin_t as spp on spp.norec=php.nostrukfk
                inner join strukpelayanan_t as sp on sp.norec=spp.nostrukfk
                where php.noposting='$request[noPosting]'
                and php.kdprofile=$kdProfile;
                "));
            return $this->respond($data);
    }
 }   