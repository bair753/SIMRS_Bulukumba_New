        ...
        $dataPegawai = \DB::table('loginuser_s as lu')
	       ...
	       try{
            if ($request['strukplanning']['norecnoplanning'] == '') {
                $Noplanning = $this->generateCodeBySeqTable(new StrukPlanning, 'noplanningpelatihan', 10,'PP-'. date('ym'),$idProfile);                
        ...
        try{
            if ($request['data']['norec'] != '') {
                ...
                if ($request['data']['verifikasifk'] == ''){
                    $noVerifikasi = $this->generateCodeBySeqTable(new StrukVerifikasi, 'noverifpelatihan', 10, 'VP'.date('ym'),$idProfile);
                    $dataSV = new StrukVerifikasi();
                    $dataSV->norec = $dataSV->generateNewId();
                    $dataSV->noverifikasi = $noVerifikasi;
                    $dataSV->kdprofile =  (int) $idProfile;
