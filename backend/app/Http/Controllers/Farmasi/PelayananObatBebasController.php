...
//                'jmldosis' => (String)($item->jumlah/$item->nilaikonversi)/1 . '/' . (String)1,
    ...
    public function SaveInputTagihanObat(Request $request) {
        ...
        $req = $request->all();
        $norm = "";
        if (isset($request['strukresep']['nocm'])){
            $norm = $request['strukresep']['nocm'];
        }
...
//                            );
                        ...
                        $newKS->status = 1;
                        $newKS->tglinput = $tglUbah;
                        $newKS->tglkejadian = $tglUbah;
                    ...
                    select spd.qtyproduk ,spd.hasilkonversi  ,spd.objectruanganfk ,
                    spd.objectprodukfk,spd.harganetto
                    from strukpelayanandetail_t as spd
                    where spd.kdprofile = $kdProfile and spd.nostrukfk=:strukresepfk"),
                        array(
                    ...
                    foreach ($dataKembaliStok as $item5){
                        ...
                        where kdprofile = $kdProfile and objectruanganfk=$SPOld->objectruanganfk 
                        and objectprodukfk=$item5->objectprodukfk"))->first();
                        $saldoAwal=0;
                        $saldoAwal = (float)$dataSaldoAwal->qty+(float)$item5->qtyproduk;
                        $TambahStok = (float)$item5->qtyproduk*(float)$item5->hasilkonversi;
...
//                            );
            ...
            if (isset($request['strukresep']['keteranganlainnya'])){
                $SP->keteranganlainnya = $request['strukresep']['keteranganlainnya'];
            }
            if (isset($request['strukresep']['namapasien'])){
            ...
            if (isset($request['strukresep']['noTelepon'])){
                $SP->noteleponfaks =  $request['strukresep']['noTelepon'];//notlp
            }
            if (isset($request['strukresep']['alamat'])){
            ...
            $totalbayar = (float)$request['strukresep']['totalharusdibayar'];
            ...
            foreach ($request['details'] as $item) {
                ...
                $dataSaldoAwal = collect(DB::select("
                    ...
                    where kdprofile = $kdProfile and objectruanganfk='$ruang'
...
//                'spd.resepke as rke','jkm.jeniskemasan'
    ...
    public function DeleteResepOB(Request $request) {
        ...
        //##PENAMBAHAN KEMBALI STOKPRODUKDETAIL
        $dataKembaliStok = collect(DB::select("select spd.qtyproduk ,spd.hasilkonversi  ,spd.objectruanganfk ,
        ...
        try {
            ...
            foreach ($dataKembaliStok as $item5){
                ...
                $dataSaldoAwal = collect(DB::select("
                    select sum(qtyproduk) as qty from stokprodukdetail_t 
                    where kdprofile = $kdProfile and objectruanganfk=$item5->objectruanganfk and objectprodukfk=$item5->objectprodukfk                        
                "))
                    ->first();
            ...
            StrukPelayanan::where('norec', $strukfk)
                ->where('kdprofile', $kdProfile)
                ->update([
                    'statusenabled' => false
                ]);
                ...
                if ( $request['strukresep']['retur']!= '') {
                    ...
                    foreach ($r_PP as $r_PPLXXXX) {
                        if ((int)$r_PPLXXXX['jmlretur'] != 0) {
                            ...
                            $dataSaldoAwal = collect(DB::select("select sum(qtyproduk) as qty from stokprodukdetail_t 
                            where kdprofile = $kdProfile and objectruanganfk='$ruanganfk' and objectprodukfk=$r_PPLXXXX[produkfk]"))
                                ->first();
                            $saldoAwalin = (float)$dataSaldoAwal->qty + $TambahStok;
//                            $dati[1] = $saldoAwal;
//                            $dati[2] = $TambahStok;
//                            $dati[3] = (float)$r_PPLXXXX['jmlretur'];
//                            $dati[4] = (float)$dataSaldoAwal->qty;
//                            dd($dati);
