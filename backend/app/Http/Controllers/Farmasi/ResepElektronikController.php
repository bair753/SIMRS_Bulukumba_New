...
//                $dataPP->noresepfk = $request['norec'];
    ...
    public function getAlamat(Request $request){
        ...
        $data = \DB::table('pasiendaftar_t as pd')
            ...
            ->select(DB::raw("ps.nocm,alm.alamatlengkap,kp.kelompokpasien,to_char(ps.tgllahir, 'YYYY-MM-DD') AS tgllahir,ru.namaruangan as ruangrawat,
                                    ...
                                    sk.statuskeluar,sp.statuspulang,ru1.objectdepartemenfk,kp.id as kpid,
                                    pd.statusschedule as noreservasi, pd.norec as norec_pd"))
    ...
    public function updateStatusGrabExpress(Request $request) {
        $kdProfile = $this->getDataKdProfile($request);
        DB::beginTransaction();
        $dataReq = $request->all();
        try{
            StrukOrder::where('norec', $request['norec_order'])
                ->update([
                        'isordergrab' => $request['isordergrab'],
                        'statusordergrab' => $request['statusordergrab'],
                        'nokirimgrab' => $request['nokirimgrab']
                    ]
                );
