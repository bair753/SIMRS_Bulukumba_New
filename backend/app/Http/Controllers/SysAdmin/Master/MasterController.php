...
use App\Master\JenisArsip;
use App\Master\JenisDiet;
use App\Master\JenisSurat;
...
use App\Master\SifatSurat;
use App\Master\SlottingOnline;
use App\Master\StatusArsip;
use App\Master\StatusBed;
use App\Master\StatusDokumen;
use App\Master\SubJenisSurat;
use App\Master\TempatTidur;
use App\Master\TipeSurat;
    ...
    public function getDataComboMaster(Request $request){
        ...
        // Bank Darah //
            ...
            $dataKelas = \DB::table('kelas_m as kl')
                ...
                ->get();
            $KdDepartemenRawatJalan = explode (',',$this->settingDataFixed('KdDepartemenRawatJalan',$kdProfile));
            // (int)$this->settingDataFixed('KdDepartemenRawatJalan', $kdProfile);
            $kdDepartemenRawatPelayanan = [];
            foreach ($KdDepartemenRawatJalan as $itemPelayanan){
    ...
    public function getDataRekananMaster(Request $request) {
        ...
        $dataProduk  = \DB::table('rekanan_m as st')
            ->select('st.id','st.namarekanan','st.alamatlengkap','st.telepon','st.faksimile',
                'st.bankrekeningnama','st.bankrekeningnomor')
    ...
    public function getDataRekanan(Request $request) {
        ...
        $dataRekanan = \DB::table('rekanan_m as r')
            ...
            ->select('r.id', 'r.kdrekanan','r.kodeexternal' ,'r.namarekanan', 'jr.jenisrekanan',
                     'r.statusenabled','r.objectjenisrekananfk','r.bankrekeningnama','r.bankrekeningnomor')
...
//            $i = $i + 1;
    ...
    public function view_harganettoprodukbykelas(Request $request){
        ...
        $data= \DB::table('harganettoprodukbykelas_m as ht')
            ...
            ->leftjoin('rekanan_m as rk', 'rk.id', '=', 'ht.objectpenjaminfk')
            ->select('ht.id','ht.kdprofile','ht.statusenabled','ht.kodeexternal','ht.namaexternal',
                ...
                'rk.namarekanan as penjamin','ht.objectpenjaminfk','ht.hargadijamin')
        ...
        if(isset($request['penjaminid']) && $request['penjaminid']!="" && $request['id']!="undefined"){
            $data = $data->where('ht.objectpenjaminfk', $request['penjaminid']);
        }
...
//            ->join('kelas_m as kl', 'kl.id', '=', 'ht.objectkelasfk')
            ->select('ht.id','ht.kdprofile','ht.statusenabled','ht.kodeexternal','ht.namaexternal',
                ...
                'ht.id as id_hn_d','ht.objectpenjaminfk','ht.hargadijamin')
            ->where('ht.kdprofile', $kdProfile)
            ->where('ht.statusenabled', true);
        ...
        if(isset($request['objectpenjaminfk']) && $request['objectpenjaminfk']!="" && $request['objectpenjaminfk']!="undefined" && $request['objectpenjaminfk']!='null'){
            $data = $data->where('ht.objectpenjaminfk', $request['objectpenjaminfk'] );
        }
...
//            $JD->nourut = $request['objectkelompokprodukfk'];
    ...
    public function getAdmin(Request $request) {
        ...
        $adminMata = SettingDataFixed::where('id', 1597)->first();
        $adminMata = $adminMata->nilaifield;

        $adminKIA = SettingDataFixed::where('id', 1598)->first();
        $adminKIA = $adminKIA->nilaifield;

        $adminJiwa = SettingDataFixed::where('id', 1599)->first();
        $adminJiwa = $adminJiwa->nilaifield;

        $adminAnak = SettingDataFixed::where('id', 1600)->first();
        $adminAnak = $adminAnak->nilaifield;

        $adminParu = SettingDataFixed::where('id', 1601)->first();
        $adminParu = $adminParu->nilaifield;
        ...
        $adminGizi = SettingDataFixed::where('id', 1615)->first();
        $adminGizi = $adminGizi->nilaifield;
        ...
        if ($input['jenis'] == 'simpan'){
            ...
            if ($input['head']['id_hn_m']=='') {
                ...
                $del = HargaNettoProdukByKelasD::where('objectprodukfk',$input['head']['objectprodukfk'])
                    ...
                    ->where('objectpenjaminfk',$input['head']['objectpenjaminfk'])
                    ->delete();
            ...
            if(isset($input['head']['objectpenjaminfk']) && $input['head']['objectpenjaminfk']!="" && $input['head']['objectpenjaminfk']!="undefined"){
                $HHHH->objectpenjaminfk = $input['head']['objectpenjaminfk'];
            }
            if(isset($input['head']['reportdisplay']) && $input['head']['reportdisplay']!="" && $input['head']['reportdisplay']!="undefined"){
                $HHHH->reportdisplay = $input['head']['reportdisplay'];
            }
            if(isset($input['head']['hargadijamin']) && $input['head']['hargadijamin']!="" && $input['head']['hargadijamin']!="undefined"){
                $HHHH->hargadijamin = $input['head']['hargadijamin'];
            }
...
//                try {
                if(isset($input['head']['objectpenjaminfk']) && $input['head']['objectpenjaminfk']!="" && $input['head']['objectpenjaminfk']!="undefined"){
                    $DDDD->objectpenjaminfk = $input['head']['objectpenjaminfk'];
                }
                if(isset($input['reportdisplay']['reportdisplay']) && $input['head']['reportdisplay']!="" && $input['head']['reportdisplay']!="undefined"){
                    $DDDD->reportdisplay = $input['head']['reportdisplay'];
                }
                if(isset($r_Detail['hargadijamin']) &&$r_Detail['hargadijamin']!="" &&$r_Detail['hargadijamin']!="undefined"){
...
//            ->where('jd.statusenabled', true);
 ...
 public function getMasterKiosk(Request $request){
    ...
    public function getDataMasterRuangan(Request $request) {
        $kdProfile = (int) $this->getDataKdProfile($request);
        $data = \DB::table('ruangan_m as kd')
            ->leftjoin ('departemen_m as kp','kp.id','=','kd.objectdepartemenfk')
            ->select('kd.*','kp.namadepartemen')
            ->where('kd.kdprofile', $kdProfile)
            ->where('kd.statusenabled', true);
        ...
        if(isset($request['kdRuangan']) && $request['kdRuangan']!="" && $request['kdRuangan']!="undefined"){
            $data = $data->where('kd.kdruangan','=', $request['kdRuangan']);
        }
        if(isset($request['namaRuangan']) && $request['namaRuangan']!="" && $request['namaRuangan']!="undefined"){
            $data = $data->where('kd.namaruangan','ILIKE', '%'.$request['namaRuangan'].'%');
        }
        if(isset($request['idDept']) && $request['idDept']!="" && $request['idDept']!="undefined"){
            $data = $data->where('kd.objectdepartemenfk','=', $request['idDept']);
        }
    ...
    public function saveDataMasterRuangan(Request $request) {
        $kdProfile = (int) $this->getDataKdProfile($request);
        DB::beginTransaction();
        try{
            $idJenis = Ruangan::max('id');
            $idJenis= $idJenis +1;
            if ($request['id'] == "") {
                $JD = new Ruangan();
                $JD->norec = $JD->generateNewId();
                ...
                $JD = Ruangan::where('id',$request['id'])->where('kdprofile', $kdProfile)->first();
            }
            $JD->kodeexternal = $idJenis;
            $JD->namaexternal = $request['namaexternal'];
            $JD->reportdisplay = $request['namaruangan'];
            $JD->namaruangan = $request['namaruangan'];
            $JD->kdruangan = $idJenis;
            $JD->objectdepartemenfk = $request['objectdepartemenfk'];
            $JD->qruangan = $idJenis;
            $JD->save();
            $transStatus = 'true';
    ...
    public function deleteDataRuangan(Request $request) {
        $kdProfile = (int) $this->getDataKdProfile($request);
        DB::beginTransaction();
        try{
            $JD = Ruangan::where('id',$request['id'])
                ->where('kdprofile', $kdProfile)
                ->update([
                    'statusenabled' => false
                ]);
    ...
    public function getDataMasterDepartemen(Request $request) {
        $kdProfile = (int) $this->getDataKdProfile($request);
        $data = \DB::table('departemen_m as kd')
            ->select('kd.*')
            ->where('kd.kdprofile', $kdProfile)
            ->where('kd.statusenabled', true);
        ...
        if(isset($request['kdRuangan']) && $request['kdDepartemen']!="" && $request['kdDepartemen']!="undefined"){
            $data = $data->where('kd.id','=', $request['kdDepartemen']);
        }
        if(isset($request['namaDepartemen']) && $request['namaDepartemen']!="" && $request['namaDepartemen']!="undefined"){
            $data = $data->where('kd.namadepartemen','ILIKE', '%'.$request['namaDepartemen'].'%');
        }
        $data = $data->orderBy('kd.id');
        $data = $data->get();
        $result = array(
            'data'=> $data,
            'message' => 'inhuman',
    ...
    public function saveDataMasterDepartemen(Request $request) {
        $kdProfile = (int) $this->getDataKdProfile($request);
        DB::beginTransaction();
        try{
            $idJenis = Departemen::max('id');
            $idJenis= $idJenis +1;
            if ($request['id'] == "") {
                $JD = new Departemen();
                $JD->norec = $JD->generateNewId();
                ...
                $JD = Departemen::where('id',$request['id'])->where('kdprofile', $kdProfile)->first();
            }
            $JD->kodeexternal = $idJenis;
            $JD->namaexternal = $request['namaexternal'];
            $JD->reportdisplay = $request['namadepartemen'];
            $JD->namadepartemen = $request['namadepartemen'];
            $JD->kddepartemen = $idJenis;
            $JD->qdepartemen = $idJenis;
            $JD->save();
    ...
    public function deleteDataDepartemen(Request $request) {
        $kdProfile = (int) $this->getDataKdProfile($request);
        DB::beginTransaction();
        try{
            $JD = Departemen::where('id',$request['id'])
                ->where('kdprofile', $kdProfile)
                ->update([
                    'statusenabled' => false
                ]);
    ...
    public function getDataMasterTipeSurat(Request $request) {
        $kdProfile = (int) $this->getDataKdProfile($request);
        $data = \DB::table('tipepengirimsurat_m as kd')
            ->select('kd.*')
            ->where('kd.kdprofile', $kdProfile)
            ->where('kd.statusenabled', true);
        ...
        if(isset($request['kdTipeSurat']) && $request['kdTipeSurat']!="" && $request['kdTipeSurat']!="undefined"){
            $data = $data->where('kd.id','=', $request['kdTipeSurat']);
        }
        if(isset($request['namaTipeSurat']) && $request['namaTipeSurat']!="" && $request['namaTipeSurat']!="undefined"){
            $data = $data->where('kd.name','ILIKE', '%'.$request['namaTipeSurat'].'%');
        }
        $data = $data->orderBy('kd.id');
        $data = $data->get();
        $result = array(
    ...
    public function saveDataMasterTipeSurat(Request $request) {
        $kdProfile = (int) $this->getDataKdProfile($request);
        DB::beginTransaction();
        try{
            $idJenis = TipeSurat::max('id');
            $idJenis= $idJenis +1;
            if ($request['id'] == "") {
                $JD = new TipeSurat();
                $JD->norec = $JD->generateNewId();
                ...
                $JD = TipeSurat::where('id',$request['id'])->where('kdprofile', $kdProfile)->first();
            }
            $JD->kodeexternal = $idJenis;
            $JD->namaexternal = $request['namaexternal'];
            $JD->reportdisplay = $request['name'];
            $JD->name = $request['name'];
            $JD->save();
    ...
    public function deleteDataTipeSurat(Request $request) {
        $kdProfile = (int) $this->getDataKdProfile($request);
        DB::beginTransaction();
        try{
            $JD = TipeSurat::where('id',$request['id'])
                ->where('kdprofile', $kdProfile)
                ->update([
                    'statusenabled' => false
                ]);
    ...
    public function getDataMasterJenisSurat(Request $request) {
        $kdProfile = (int) $this->getDataKdProfile($request);
        $data = \DB::table('jenissurat_m as kd')
            ->select('kd.*')
            ->where('kd.kdprofile', $kdProfile)
            ->where('kd.statusenabled', true);
        ...
        if(isset($request['kdJenisSurat']) && $request['kdJenisSurat']!="" && $request['kdJenisSurat']!="undefined"){
            $data = $data->where('kd.id','=', $request['kdJenisSurat']);
        }
        if(isset($request['namaJenisSurat']) && $request['namaJenisSurat']!="" && $request['namaJenisSurat']!="undefined"){
            $data = $data->where('kd.name','ILIKE', '%'.$request['namaJenisSurat'].'%');
        }
        $data = $data->orderBy('kd.id');
        $data = $data->get();
        $result = array(
    ...
    public function saveDataMasterJenisSurat(Request $request) {
        $kdProfile = (int) $this->getDataKdProfile($request);
        DB::beginTransaction();
        try{
            $idJenis = JenisSurat::max('id');
            $idJenis= $idJenis +1;
            if ($request['id'] == "") {
                $JD = new JenisSurat();
                $JD->norec = $JD->generateNewId();
                ...
                $JD = JenisSurat::where('id',$request['id'])->where('kdprofile', $kdProfile)->first();
            }
            $JD->kodeexternal = $idJenis;
            $JD->namaexternal = $request['namaexternal'];
            $JD->reportdisplay = $request['name'];
    ...
    public function deleteDataJenisSurat(Request $request) {
        $kdProfile = (int) $this->getDataKdProfile($request);
        DB::beginTransaction();
        try{
            $JD = JenisSurat::where('id',$request['id'])
                ->where('kdprofile', $kdProfile)
                ->update([
                    'statusenabled' => false
                ]);
    ...
    public function getDataMasterJenisArsip(Request $request) {
        $kdProfile = (int) $this->getDataKdProfile($request);
        $data = \DB::table('jenisarsip_m as kd')
            ->select('kd.*')
            ->where('kd.kdprofile', $kdProfile)
            ->where('kd.statusenabled', true);
        ...
        if(isset($request['kdJenisArsip']) && $request['kdJenisArsip']!="" && $request['kdJenisArsip']!="undefined"){
            $data = $data->where('kd.id','=', $request['kdJenisArsip']);
        }
        if(isset($request['namaJenisArsip']) && $request['namaJenisArsip']!="" && $request['namaJenisArsip']!="undefined"){
            $data = $data->where('kd.name','ILIKE', '%'.$request['namaJenisArsip'].'%');
        }
        $data = $data->orderBy('kd.id');
        $data = $data->get();
        $result = array(
    ...
    public function saveDataMasterJenisArsip(Request $request) {
        $kdProfile = (int) $this->getDataKdProfile($request);
        DB::beginTransaction();
        try{
            $idJenis = JenisArsip::max('id');
            $idJenis= $idJenis +1;
            if ($request['id'] == "") {
                $JD = new JenisArsip();
                $JD->norec = $JD->generateNewId();
                ...
                $JD = JenisArsip::where('id',$request['id'])->where('kdprofile', $kdProfile)->first();
            }
            $JD->kodeexternal = $idJenis;
            $JD->namaexternal = $request['namaexternal'];
            $JD->reportdisplay = $request['name'];
    ...
    public function deleteDataJenisArsip(Request $request) {
        $kdProfile = (int) $this->getDataKdProfile($request);
        DB::beginTransaction();
        try{
            $JD = JenisArsip::where('id',$request['id'])
                ->where('kdprofile', $kdProfile)
                ->update([
                    'statusenabled' => false
                ]);
    ...
    public function getDataMasterStatusBerkas(Request $request) {
    $kdProfile = (int) $this->getDataKdProfile($request);
    $data = \DB::table('statusberkas_m as kd')
        ->select('kd.*')
        ->where('kd.kdprofile', $kdProfile)
        ->where('kd.statusenabled', true);
    ...
    if(isset($request['kdStatusBerkas']) && $request['kdStatusBerkas']!="" && $request['kdStatusBerkas']!="undefined"){
        $data = $data->where('kd.id','=', $request['kdStatusBerkas']);
    }
    if(isset($request['namaStatusBerkas']) && $request['namaStatusBerkas']!="" && $request['namaStatusBerkas']!="undefined"){
        $data = $data->where('kd.name','ILIKE', '%'.$request['namaStatusBerkas'].'%');
    }
    $data = $data->orderBy('kd.id');
    $data = $data->get();
    $result = array(
    ...
    public function saveDataMasterStatusBerkas(Request $request) {
        $kdProfile = (int) $this->getDataKdProfile($request);
        DB::beginTransaction();
        try{
            $idJenis = JenisArsip::max('id');
            ...
            if ($request['id'] == "") {
                ...
                $JD = StatusArsip::where('id',$request['id'])->where('kdprofile', $kdProfile)->first();
            }
            $JD->kodeexternal = $idJenis;
            $JD->namaexternal = $request['namaexternal'];
            $JD->reportdisplay = $request['name'];
    ...
    public function deleteDataStatusBerkas(Request $request) {
        $kdProfile = (int) $this->getDataKdProfile($request);
        DB::beginTransaction();
        try{
            $JD = StatusArsip::where('id',$request['id'])
                ->where('kdprofile', $kdProfile)
                ->update([
                    'statusenabled' => false
                ]);
    ...
    public function getDataMasterSubJenisSurat(Request $request) {
        $kdProfile = (int) $this->getDataKdProfile($request);
        $data = \DB::table('subjenissurat_m as kd')
            ->select('kd.*')
            ->where('kd.kdprofile', $kdProfile)
            ->where('kd.statusenabled', true);
        ...
        if(isset($request['kdSubJenisSurat']) && $request['kdSubJenisSurat']!="" && $request['kdSubJenisSurat']!="undefined"){
            $data = $data->where('kd.id','=', $request['kdSubJenisSurat']);
        }
        if(isset($request['namaSubJenisSurat']) && $request['namaSubJenisSurat']!="" && $request['namaSubJenisSurat']!="undefined"){
            $data = $data->where('kd.name','ILIKE', '%'.$request['namaSubJenisSurat'].'%');
        }
        $data = $data->orderBy('kd.id');
        $data = $data->get();
        $result = array(
    ...
    public function saveDataMasterSubJenisSurat(Request $request) {
        $kdProfile = (int) $this->getDataKdProfile($request);
        DB::beginTransaction();
        try{
            $idJenis = SubJenisSurat::max('id');
            $idJenis= $idJenis +1;
            if ($request['id'] == "") {
                $JD = new SubJenisSurat();
                $JD->norec = $JD->generateNewId();
                $JD->id = $idJenis;
                $JD->kdprofile = $kdProfile;
                $JD->statusenabled = true;
            }else{
                $JD = SubJenisSurat::where('id',$request['id'])->where('kdprofile', $kdProfile)->first();
            }
            $JD->kodeexternal = $idJenis;
            $JD->namaexternal = $request['namaexternal'];
            $JD->reportdisplay = $request['name'];
    ...
    public function deleteDataSubJenisSurat(Request $request) {
        $kdProfile = (int) $this->getDataKdProfile($request);
        DB::beginTransaction();
        try{
            $JD = SubJenisSurat::where('id',$request['id'])
                ->where('kdprofile', $kdProfile)
                ->update([
                    'statusenabled' => false
                ]);
    ...
    public function getDataMasterStatusDokumen(Request $request) {
        $kdProfile = (int) $this->getDataKdProfile($request);
        $data = \DB::table('statusediting_m as kd')
            ->select('kd.*')
            ->where('kd.kdprofile', $kdProfile)
            ->where('kd.statusenabled', true);
        ...
        if(isset($request['kdStatusDokumen']) && $request['kdStatusDokumen']!="" && $request['kdStatusDokumen']!="undefined"){
            $data = $data->where('kd.id','=', $request['kdStatusDokumen']);
        }
        if(isset($request['namaStatusDokumen']) && $request['namaStatusDokumen']!="" && $request['namaStatusDokumen']!="undefined"){
            $data = $data->where('kd.name','ILIKE', '%'.$request['namaStatusDokumen'].'%');
        }
        $data = $data->orderBy('kd.id');
        $data = $data->get();
        $result = array(
    ...
    public function saveDataMasterStatusDokumen(Request $request) {
        $kdProfile = (int) $this->getDataKdProfile($request);
        DB::beginTransaction();
        try{
            $idJenis = StatusDokumen::max('id');
            $idJenis= $idJenis +1;
            if ($request['id'] == "") {
                $JD = new StatusDokumen();
                $JD->norec = $JD->generateNewId();
                $JD->id = $idJenis;
                $JD->kdprofile = $kdProfile;
                $JD->statusenabled = true;
            }else{
                $JD = StatusDokumen::where('id',$request['id'])->where('kdprofile', $kdProfile)->first();
            }
            $JD->kodeexternal = $idJenis;
            $JD->namaexternal = $request['namaexternal'];
            $JD->reportdisplay = $request['name'];
            $JD->statusediting = $request['statusediting'];
            $JD->save();
    ...
    public function deleteDataStatusDokumen(Request $request) {
        $kdProfile = (int) $this->getDataKdProfile($request);
        DB::beginTransaction();
        try{
            $JD = StatusDokumen::where('id',$request['id'])
                ->where('kdprofile', $kdProfile)
                ->update([
                    'statusenabled' => false
                ]);
     ...
     public function getIcdSepuluh(Request $request) {
        $kdProfile = (int) $this->getDataKdProfile($request);
    ...
    public function saveIcdSepuluh(Request $request) {
        $kdProfile = (int) $this->getDataKdProfile($request);
        DB::beginTransaction();
        try{
            $idIcd = IcdSepuluh::max('id');
            $idIcd= $idIcd +1;
            if ($request['id'] == "") {

                $ICD = new IcdSepuluh();
                $ICD->norec = $ICD->generateNewId();
                $ICD->id = $idIcd;
                $ICD->kdprofile = $kdProfile;
                $ICD->statusenabled = true;
            }else{
                $ICD = IcdSepuluh::where('id',$request['id'])->where('kdprofile', $kdProfile)->first();

            }
            $ICD->kodeexternal = $request['kddiagnosa'];
            $ICD->namaexternal = $request['namadiagnosa'];
    ...
    public function deleteDiagnosa(Request $request) {
        $kdProfile = (int) $this->getDataKdProfile($request);
        DB::beginTransaction();
        try{
            $JD = IcdSepuluh::where('id',$request['id'])
                ->where('kdprofile', $kdProfile)
                ->update([
                    'statusenabled' => false
                ]);
    ...
    public function getIcdSembilan(Request $request) {
        $kdProfile = (int) $this->getDataKdProfile($request);
        ...
        if(isset($request['namadiagnosa']) && $request['namadiagnosa']!="" && $request['namadiagnosa']!="undefined"){
            $data = $data->where('dg.namadiagnosatindakan','ILIKE', '%'.$request['namadiagnosa'].'%');
        }
        $data = $data->get();
        $result = array(
            'data'=> $data,
    ...
    public function saveIcdSembilan(Request $request) {
        $kdProfile = (int) $this->getDataKdProfile($request);
        DB::beginTransaction();
        try{
            $idIcd = IcdSembilan::max('id');
            $idIcd= $idIcd +1;
            if ($request['id'] == "") {
                ...
                $ICD = IcdSembilan::where('id',$request['id'])->where('kdprofile', $kdProfile)->first();
    ...
    public function deleteDiagnosaSembilan(Request $request) {
        $kdProfile = (int) $this->getDataKdProfile($request);
        DB::beginTransaction();
        try{
            $JD = IcdSembilan::where('id',$request['id'])
                ->where('kdprofile', $kdProfile)
                ->update([
                    'statusenabled' => false
                ]);
    ...
    public function getBankAccount(Request $request){
        $kdProfile = $this->getDataKdProfile($request);
        $idProfile = (int) $kdProfile;
        $indikator = DB::table('bankaccount_m as bb')
            ->leftJoin('rekanan_m as rek','rek.id','=','bb.kdrekananfk')
            ->select('bb.*', 'rek.namarekanan')
            ->where('bb.statusenabled',1)
            ->where('bb.kdprofile', $idProfile)
            ->get();
        $result = array(
            'data' => $indikator,
            'message' => 'er@epic',
    ...
    public function saveBankAccount(Request $request){
        ...
        try{
            if ($request['id'] == ''){
                $TP = new BankAccount();
                $idMax = BankAccount::max('id');
                $newId = $idMax + 1;
                $TP->kdprofile = $idProfile;
                $TP->id =  $newId;
                $TP->norec = $TP->generateNewId();
                $TP->kodeexternal = $newId ;
                $TP->qbankaccount = $newId ;
                $TP->kdbankaccount = $newId ;
            }else{
                $TP = BankAccount::where('id', $request['id'])->first();
            }
            $TP->statusenabled = $request['statusenabled'];
            $TP->bankaccountnama = $request['bankaccountnama'];
            $TP->bankaccountnomor = $request['bankaccountnomor'];
            $TP->keteranganlainnya = $request['keteranganlainnya'];
            if(isset($request['rekanan']) && $request['rekanan']!='' ){
             
                 $TP->kdrekananfk = $request['rekanan'];
    ...
    public function getDataMasterSifatSurat(Request $request) {
        $kdProfile = (int) $this->getDataKdProfile($request);
        $data = \DB::table('sifatsurat_m as kd')
            ->select('kd.*')
            ->where('kd.kdprofile', $kdProfile)
            ->where('kd.statusenabled', true);
        ...
        if(isset($request['kdSifatSurat']) && $request['kdSifatSurat']!="" && $request['kdSifatSurat']!="undefined"){
            $data = $data->where('kd.id','=', $request['kdSifatSurat']);
        }
        if(isset($request['namaSifatSurat']) && $request['namaSifatSurat']!="" && $request['namaSifatSurat']!="undefined"){
            $data = $data->where('kd.name','ILIKE', '%'.$request['namaSifatSurat'].'%');
        }
        $data = $data->orderBy('kd.id');
        $data = $data->get();
        $result = array(
    ...
    public function saveDataMasterSifatSurat(Request $request) {
        $kdProfile = (int) $this->getDataKdProfile($request);
        DB::beginTransaction();
        try{
            $idJenis = SifatSurat::max('id');
            $idJenis= $idJenis +1;
            if ($request['id'] == "") {
                $JD = new SifatSurat();
                $JD->norec = $JD->generateNewId();
                ...
                $JD = SifatSurat::where('id',$request['id'])->where('kdprofile', $kdProfile)->first();
            }
            $JD->kodeexternal = $idJenis;
            $JD->namaexternal = $request['namaexternal'];
            $JD->reportdisplay = $request['name'];
    ...
    public function deleteDataSifatSurat(Request $request) {
        $kdProfile = (int) $this->getDataKdProfile($request);
        DB::beginTransaction();
        try{
            $JD = SifatSurat::where('id',$request['id'])
                ->where('kdprofile', $kdProfile)
                ->update([
                    'statusenabled' => false
                ]);
    ...
    public function getRekananALL(Request $request) {
        $kdProfile = (int) $this->getDataKdProfile($request);
