...
use App\Master\MapRuanganToEMR;
...
//            ->take(50);
    ...
    public function getComboMappingEMR (Request $request){
        $kdProfile = (int) $this->getDataKdProfile($request);

        $dataGroupEmr = DB::table('emr_t as emr')
            ->distinct()
            ->select('emr.namaemr')
    ...
    public function getMappingEMR (Request $request){
        $kdProfile = (int) $this->getDataKdProfile($request);
        $dataemr = DB::table('mapruangantoemr_t as maps')
            ->join('emr_t as em','em.id','=','maps.emrfk')
            ->leftJoin('departemen_m as dp','dp.id','=','maps.objectdepartemenfk')
            ->leftJoin('ruangan_m as rm','rm.id','=','maps.objectruanganfk')
            ->select('maps.*','dp.namadepartemen','rm.namaruangan', 'em.caption', 'em.namaemr')
            ->where('maps.statusenabled',true)
            ->where('maps.kdprofile',$kdProfile);

        if(isset($request['departemenId']) && $request['departemenId'] !='' ){
            $dataemr = $dataemr->where('maps.objectdepartemenfk',$request['departemenId']);
        }
        if(isset($request['ruanganId']) && $request['ruanganId'] !='' ){
            $dataemr = $dataemr->where('maps.objectruanganfk',$request['ruanganId']);
        }
        $dataemr = $dataemr->orderBy('em.namaemr');
        $dataemr = $dataemr->get();
        $result = array(
            'data' => $dataemr,
            'as' => 'inhuman'
    ...
    public function saveMapRuanganToEMR(Request $request){
        $kdProfile = (int) $this->getDataKdProfile($request);
        DB::beginTransaction();
        try {
            foreach ( $request['details'] as $item){
            ...
            $hapus = MapRuanganToEMR::where('statusenabled',true)
                ->where('objectdepartemenfk',$request['departemenId']);

            if(isset($request['ruanganId']) && $request['ruanganId'] !='' ){
                $hapus = $hapus->where('objectruanganfk',$request['ruanganId']);
            }
            $hapus = $hapus->whereIn('emrfk',$kode);
            $hapus->delete();
            foreach ( $request['details'] as $item){
                $map = new MapRuanganToEMR();
                $map->id = MapRuanganToEMR::max('id') + 1;
                $map->kdprofile = $kdProfile;//12;
                $map->statusenabled = true;
                $map->kodeexternal = "CBN";
                $map->norec = substr(\Webpatser\Uuid\Uuid::generate(), 0, 32);
                $map->emrfk = $item['id'];
                $map->objectdepartemenfk = $request['departemenId'];
    ...
    public function deleteMapRuanganToEMR(Request $request){
        $kdProfile = (int) $this->getDataKdProfile($request);
        DB::beginTransaction();
        try {
            foreach ($request['data'] as $item){
                MapRuanganToEMR::where('id',$item['id'])->where('kdprofile',$kdProfile)->delete();
            }
