...
use App\Web\PasswordAutorisasi;
...
//            ->whereRaw("(oma.ishide = false or oma.ishide is null)")
            ...
            ->select('oma.id', 'oma.kdobjekmodulaplikasihead', 'oma.objekmodulaplikasi','oma.alamaturlform','ma.modulaplikasi',
                'acdc.modulaplikasiid','oma.kodeexternal','oma.ishide')
...
//                    ->where('kdprofile', $kdProfile)
                ...
                $data2 [] = array(
                    ...
                    'modulAplikasiHead' => array(
                        'modulaplikasi' => !empty($mod) ? $mod->modulaplikasi :null,
                        'id' =>!empty($mod) ? $mod->id :null,
                    )
                );
            }else{
                $data2 [] = array(
    ...
    public function saveNewAuthPasscode (Request $request){
        $kdProfile = (int) $this->getDataKdProfile($request);
        DB::beginTransaction();
        try{
            if($request['id'] == ''){
                $user = PasswordAutorisasi::where('namaautorisasi',$request['namaautorisasi'])->where('kdprofile', $kdProfile)->get();
                if( count($user)>0 ){
                    $transMessage = 'User sudah ada' ;
                    $result = array(
                        "status" => 400,
                ...
                $new = new PasswordAutorisasi();
                $new->id = PasswordAutorisasi::max('id') + 1;
                $new->kdprofile = $kdProfile;
                $new->statusenabled = true;
                $new->norec = $new->generateNewId();
            }else{
                $message = "";
                $new = PasswordAutorisasi::where('id', $request['id'])->where('kdprofile', $kdProfile)->first();
                $cekUser = PasswordAutorisasi::where('namaautorisasi',$request['namaautorisasi'])
                    ->where('kdprofile', $kdProfile)
                    ->first();
                $cekPassword =  PasswordAutorisasi::where('id',$request['id'])
                    ->where('passcode','=',$this->encryptSHA1($request->input('passcode')))
                    ->where('kdprofile', $kdProfile)
                    ->first();
                $sama = false ;
                if(!empty($cekUser)){
                    if($cekUser->id != $request['id'] && $request['namaautorisasi'] == $cekUser->namaautorisasi ){
                        $sama = true;
                        $message = "Nama Autorisasi Sudah Ada";
                    }
                }
                ...
                if($sama ==  true){
                    ...
                    return $this->setStatusCode($result['status'])->respond($result, $message);
                }
            }
            $new->namaexternal = $request['namaautorisasi'];
            $new->reportdisplay = $request['namaautorisasi'];
            $new->keteranganfungsi = $request['keteranganfungsi'];
            $new->namaautorisasi = $request['namaautorisasi'];
            $new->passcode = $this->encryptSHA1($request['passcode']);
            $new->kelompok = $request['kelompok'];
            $new->save();
    ...
    public function deleteAuthPasscode (Request $request){
        $kdProfile = (int) $this->getDataKdProfile($request);
        try{
            if($request['id'] != ''){
                PasswordAutorisasi::where('id', $request['id'])
                    ->where('kdprofile', $kdProfile)
                    ->delete();
            }
            $transStatus = true;
    ...
    public function getDaftarAuthPasscode(Request $request) {
        $kdProfile = (int) $this->getDataKdProfile($request);
        $data  = \DB::table('passwordautorisasi_s as lo')
            ->select(DB::raw("
                lo.id,lo.keteranganfungsi,lo.namaautorisasi,lo.kelompok
            "))
            ->where('lo.statusenabled',true)
            ->where('lo.kdprofile', $kdProfile)
            ->orderBy('lo.id');

        if(isset($request['idAuth']) && $request['idAuth']!="" && $request['idAuth']!="undefined"){
            $data = $data->where('lo.id','=',$request['idAuth'] );
        }
        if(isset($request['keteranganFungsi']) && $request['keteranganFungsi']!="" && $request['keteranganFungsi']!="undefined"){
            $data = $data->where('lo.keteranganfungsi','ilike','%'.$request['keteranganFungsi'].'%' );
        }
        if(isset($request['namaAutorisasi']) && $request['namaAutorisasi']!="" && $request['namaAutorisasi']!="undefined"){
            $data = $data->where('lo.namaautorisasi','ilike','%'.$request['namaAutorisasi'].'%' );
        }
        if(isset($request['KelompokAutorisasi']) && $request['KelompokAutorisasi']!="" && $request['KelompokAutorisasi']!="undefined"){
            $data = $data->where('lo.kelompok','ilike','%'.$request['KelompokAutorisasi'].'%' );
        }
        if (isset($request['jmlRows']) && $request['jmlRows'] != "" && $request['jmlRows'] != "undefined" && $request['jmlRows'] != "0" && $request['jmlRows'] != 0) {
    ...
    public function updateAuthPasscode (Request $request){
        $kdProfile = (int) $this->getDataKdProfile($request);
        $passCode = $this->encryptSHA1($request['passcode']);
        try{
            if($request['id'] != ''){
                PasswordAutorisasi::where('id', $request['id'])
                    ->where('kdprofile', $kdProfile)
    ...
    public function getValidateAuthPasscode(Request $request) {
        $kdProfile = (int) $this->getDataKdProfile($request);
        $message = "";
    ...
    public function getMenuAll(Request $request){
        ...
        $data = \DB::table('objekmodulaplikasi_s as mo')
            ->join('mapobjekmodulaplikasitomodulaplikasi_s as acdc', 'acdc.objekmodulaplikasiid', '=', 'mo.id')
            ->select('mo.alamaturlform','mo.objekmodulaplikasi')
            ->where('mo.statusenabled', true)      
            ->whereNotNull('mo.kdobjekmodulaplikasihead')      
            ->where('mo.alamaturlform', '!=','')    
            ->where('mo.alamaturlform', '!=','#/')          
            ->whereNotNull('mo.alamaturlform')
            ->distinct()
            ->orderBy('mo.objekmodulaplikasi')
            ->get();
