...
use App\Transaksi\CPKegiatanUraian;
...
use App\Transaksi\CPPasien;
use App\Transaksi\CPPasienD;
use App\Transaksi\CPMapping;
...
use App\Transaksi\EMRTriase;
use App\Transaksi\EMRTriaseD;
...
//        $norecAPD = $request['norecAPD'];
        ...
        $data = \DB::table('antrianpasiendiperiksa_t as apd')
            ...
            ->select('apd.norec', 'pd.noregistrasi', 'pd.tglregistrasi', 'ps.nocm', 'ps.namapasien', 'ps.tgllahir', 'ru.objectdepartemenfk',
                ...
                'pdd.pendidikan', 'pk.pekerjaan',
                'rk.namarekanan', 'kls.namakelas', 'pd.nocmfk', 'pd.objectkelompokpasienlastfk', 'apd.objectruanganfk', 'apd.objectpegawaifk',
                'ps.objectjeniskelaminfk', 'apd.objectkelasfk', 'ps.objectgolongandarahfk', 'gd.golongandarah', 'ps.foto', 'pg1.namalengkap as dokterdpjp',
                'pg1.id as iddpjp', 'ps.noidentitas', 'ps.namaibu','ps.objectpekerjaanfk','pk.pekerjaan as peker', 'pk.id as idpeker','ps.nohp',
                'ps.penanggungjawab', 'ps.hubungankeluargapj','ps.alamatrmh','ps.teleponpenanggungjawab','pd.statuscovidfk','alm.namadesakelurahan',
                'alm.namakecamatan','alm.namakotakabupaten','asl.asalrujukan','ps.bahasa','ps.jeniskelaminpenanggungjawab as jkp','ps.umurpenanggungjawab as umurp',
                'ps.pekerjaanpenangggungjawab as gawe','ps.dokterpengirim','ps.alamatdokterpengirim')
    ...
    public function postResumeMedisInap(Request $request, $method){
        ...
        try {
            ...
            if ($method == 'save') {
                ...
                $RekamMedis->diagnosissekunder = $request['diagnosissekunder'];
                $RekamMedis->tindakanprosedur = $request['tindakanprosedur'];
                $RekamMedis->alergi = $request['alergi'];
                $RekamMedis->diet = $request['diet'];
                $RekamMedis->instruksianjuran = $request['instruksianjuran'];
                $RekamMedis->hasillab = $request['hasillab'];
...
//            ->whereIn('ru.objectdepartemenfk',$iddept);
    ...
    public function saveOrderKonsul(Request $request){
        ...
        try {
            ...
            $kelompokTransaksi = KelompokTransaksi::where('kelompoktransaksi', 'KONSULTASI DOKTER')->first();
...
//                $noemr = $this->generateCode(new EMRPasien, 'noemr', 14, 'MR' . $this->getDateTime()->format('ym') . '/');
            ...
            $EMR->nocm = $head['nocm'];
            if (isset($head['namapasien'])) {
                $EMR->namapasien = $head['namapasien'];
            }
            if (isset($head['jeniskelamin'])) {
            ...
            if (isset($head['nohp'])) {
                $EMR->notelepon = $head['nohp'];
            }
            if (isset($head['alamatlengkap'])) {
                $EMR->alamat = $head['alamatlengkap'];
            }
            if (isset($head['objectdesakelurahanfk'])) {
                $EMR->objectdesakelurahanfk = $head['objectdesakelurahanfk'];
            }
            if (isset($head['objectkecamatanfktr'])) {
                $EMR->objectkecamatanfk = $head['objectkecamatanfktr'];
            }
            if (isset($head['objectkotakabupatenfk'])) {
                $EMR->objectkotakabupatenfk = $head['objectkotakabupatenfk'];
            }
            if (isset($head['objectpropinsifk'])) {
                $EMR->objectpropinsifk = $head['objectpropinsifk'];
            }
             if (isset($head['kodepos'])) {
                 $EMR->kodepos = $head['kodepos'];
             }
            if (isset($head['jenisemr'])) {
                $EMR->jenisemr = $head['jenisemr'];
            }
            if (isset($head['kodeexternal'])) {
                $EMR->kodeexternal = $head['kodeexternal'];
            }
        if (isset($head['noidentitas'])) {
            $EMR->nik = $head['noidentitas'];
        }
        if (isset($head['tempatlahir'])) {
            $EMR->tempatlahir = $head['tempatlahir'];
        }
...
//                "dtdt" => $dtdt,
    ...
    public function getEMRTransaksiDetail(Request $request){
        ...
        $idProfile = (int) $kdProfile;
        //todo : detail
        // $data = \DB::table('cppasiend_t as emrdp')
        //     ->leftjoin('cppasien_t as cp', 'cp.noemr', '=', 'emrdp.emrpasienfk')
        ...
        $data = \DB::table('cppasiend_t as emrdp')
            ...
            // ->leftjoin('emrd_t as emrd', 'emrd.id', '=', 'emrdp.emrdfk')
            ->leftjoin('pegawai_m as pg', 'pg.id', '=', 'emrdp.pegawaifk')
            ->select('emrdp.*', 'emrdp.emrfk',  'pg.namalengkap')
            ->where('emrdp.statusenabled', true)
            ->where('emrdp.kdprofile',$idProfile )
            ->whereNotNull('emrdp.value');
        ...
        if (isset($request['noregistrasi']) && $request['noregistrasi'] != '') {
            $data = $data->where('cp.noregistrasifk', $request['noregistrasi'] );
        }
        if (isset($request['cp']) && $request['cp'] != '') {
            $data = $data->where('cp.kodecp', $request['cp'] );
        }
...
//                    }
    ...
    public function getDataRiwayatEMR(Request $request){
        ...
        if (isset($request['idlayanan']) && $request['idlayanan'] != '') {
            $paramidLayanan = " AND emrpd.value  = '".$request['idlayanan']."'" ;
        }
        ...
        $data2 = DB::select(DB::raw("SELECT
                    ...
                    ep.noregistrasi,
                    pm.tempatlahir,
                    pm.noidentitas as nik,ep.norec,
                    '' AS layanan,
                    null as  objectdesakelurahanfk
                FROM
                    emrpasien_t AS ep
                INNER JOIN pasien_m AS pm ON pm.nocm = ep.nocm
                INNER JOIN alamat_m AS alm ON alm.nocmfk = pm.id
                INNER JOIN jeniskelamin_m AS jk ON jk.id = pm.objectjeniskelaminfk and jk.statusenabled=true
                 WHERE ep.kdprofile = $idProfile
                 and   ep.statusenabled = true
                 and pm.statusenabled = true
                  and pm.kdprofile = $idProfile
                AND ep.namaruangan = 'Triage Gawat Darurat'
                -- and ep.kodeexternal ='triase'
                AND ep.nocm <> '-'
               ...
               $paramnamaPasien
                UNION ALL
                      ...
                      null as idpasien,
                        ...
                        ep.noregistrasi,
                        ep.tempatlahir,
                        ep.nik as nik,ep.norec,
                        '' AS layanan,
                        ep.objectdesakelurahanfk
                        FROM
                        emrpasien_t AS ep
                        JOIN jeniskelamin_m AS jk ON jk.jeniskelamin = ep.jeniskelamin and jk.statusenabled=true
                        --left Join propinsi_m as pro on pro.id =ep.objectpropinsifk
                        --left Join kotakabupaten_m as kot on kot.id=ep.objectkotakabupatenfk
                        --left Join kecamatan_m as kec on kec.id = ep.objectkecamatanfk
                        left Join desakelurahan_m as dk on dk.id=ep.objectdesakelurahanfk
        ...
        $dataTgl = DB::select(DB::raw("select emrp.noemr,emrpd.value
                   from emrpasien_t AS emrp
                   INNER JOIN pasien_m AS pm ON pm.nocm = emrp.nocm
                   LEFT JOIN emrpasiend_t AS emrpd ON emrpd.emrpasienfk = emrp.noemr 
                   where emrp.tglemr >= '$tglAwal' AND emrp.tglemr <= '$tglAkhir' 
                         and emrp.statusenabled = 't'
                         and emrpd.statusenabled = 't'                           
                         and emrpd.emrdfk in (18000111) and emrp.kdprofile=$idProfile
                         $paramidLayanan
                         "));

        $i=0;
        foreach ($data2 as $items){
            foreach ($dataTgl as $itemTgl){
                if ($data2[$i]->noemr == $itemTgl->noemr){
                    if ($itemTgl->value == 1){
                        $data2[$i]->layanan = 'Lanjut Rawat Tower 4/Lanjut Rawat Tower 5';
                    }
                    if ($itemTgl->value == 2){
                        $data2[$i]->layanan = 'Pindah Rawat Tower 6';
                    }
                    if ($itemTgl->value == 3){
                        $data2[$i]->layanan = 'Rujuk Kefaskes lain';
                    }
                }
            }
            $i = $i + 1;
...
//        return $this->respond($request['detaildiagnosatindakanpasien']);
...
//        try {
        ...
        if ($transStatus == 'true') {
            $transMessage = "Data Diagnosa Tersimpan";
            DB::commit();
            $result = array(
                'status' => 201,
                'message' => $transMessage,
            ...
            $transMessage = "Data Diagnosa Gagal Disimpan";
            DB::rollBack();
            $result = array(
                'status' => 400,
                'message' => $transMessage,
...
//                    ->on('apd.objectpegawaifk', '=', 'so.objectpegawaiorderfk');
    ...
    public function getInformasiStok(Request $request){
        ...
        $results = DB::select(DB::raw("select sk.norec,spd.objectprodukfk, sk.tglstruk,spd.objectasalprodukfk, 
                    ...
                    sum(spd.qtyproduk) as qtyproduk,spd.objectruanganfk as kdruangan
...
//                'ruanganid' => $request['ruanganfk'],
    ...
    public function SimpanOrderPelayananObat(Request $request){
        ...
        try {
            ...
            $newSO->totalpph = 0;
            $newSO->totalppn = 0;
            if (isset($r_SR['penginputresepfk'])) {
                $newSO->penginputresepfk = $r_SR['penginputresepfk'];
            }
            if (isset($r_SR['penginputresep'])) {
                $newSO->penginputresep = $r_SR['penginputresep'];
            }
            if (isset($r_SR['idrelawan'])) {
                $newSO->idrelawan = $r_SR['idrelawan'];
            }
            if (isset($r_SR['noruangan'])) {
                $newSO->nourutruangan = $r_SR['noruangan'];
            }
             if (isset($r_SR['iscito'])) {
                $newSO->cito = $r_SR['iscito'];
            }
...
//        $details=$request->all();
        ...
        $deptPenunjangCBO = explode(',', $this->settingDataFixed($request['departemenfk'], $idProfile));
        $kode2 = [];
        foreach ($deptPenunjangCBO as $it) {
            $kode2 [] = (int)$it;
        }
        $kode = [];
...
//            ->where('dp.kdprofile', $idProfile)
        ...
        $dataRuanganTujuan = \DB::table('ruangan_m as ru')
            ->select('ru.id', 'ru.namaruangan')
            ->where('ru.kdprofile', $idProfile)
            ->whereIn('ru.objectdepartemenfk',$kode2)
            ->where('ru.statusenabled', true)
            ->orderBy('ru.namaruangan')
            ->get();
...
//            ->LEFTJOIN('reshd as pp','pp.ono','=','so.noorder')
            ->select('so.norec', 'pd.norec as norecpd', 'pd.noregistrasi', 'so.tglorder', 'so.noorder',
                ...
                ,DB::raw("to_char(so.tglpelayananawal ,'YYYY-MM-DD') AS tglpelayananawal")
...
//        $data = $data->distinct();
        ...
        foreach ($data as $item) {
            $noorder = $item->noorder;
            if($idProfile ==18){
                $hasil = DB::select(DB::raw("select * from lisorder where ono='$noorder' and kdprofile=$idProfile"));
            }else{
                $hasil = DB::select(DB::raw("select * from order_lab where no_lab='$noorder' and kdprofile=$idProfile"));
            }
...
//            $dataSO->objectdiagnosafk = true;
                ...
                $dataSO->totalpph = 0;
                $dataSO->totalppn = 0;
                $dataSO->tglpelayananawal = $request['tglpemeriksaan'];
                $dataSO->save();
            ...
            if($idProfile == 18){
                if($isLab == true){
                    $peg = Pegawai::where('id',$dokter2)->first();
                    $name = '-';

                    if(!empty($peg)){
                        $name =  $peg->namalengkap;
                    }
                    $lisData['noorder'] = $noOrder;

                    $lis = $this->saveBridgingLis($lisData,$idProfile);
                ...
                if($isRad == true){
                    $lisData['noorder'] = $noOrder;
                    $lis = $this->saveBridgingZetaWisma($lisData,$idProfile);
    //                return $lis;
                    if($lis['status'] ==201){
                        $msg = 'Bridging Zetta ';
                    }else{
...
//                "nokirim" => $dataSO,//$noResep,
        ...
        return $this->setStatusCode($result['status'])->respond($result, $transMessage.' '.$msg);
    }
    protected $urlZeta = 'http://36.94.47.50:1080/';
    protected $userZeta = 'admin';
    protected $passwordZeta = 'P@csw0rd';
    public function authenticate(){
        $user =[
            'username' => $this->userZeta,
            'password' => $this->passwordZeta,
        ];
        $curl = curl_init();

        curl_setopt_array($curl, array(
            CURLOPT_PORT => '',
            CURLOPT_URL=> $this->urlZeta.'Users/authenticate',
    ...
    public function saveBridgingZetaWisma( $request,$idProfile)
    {
        $kdProfile =$idProfile;
//        $kdProfile = (int) $_SESSION['kdProfile'];
        $data = DB::select(DB::raw("
                 select prd.id as kdproduk,prd.namaproduk,prd.kodeexternal as kodeexternal_prd ,
                 op.norec,ps.namapasien,ps.nocm,ps.id as psid,op.qtyproduk,
                case when alm.alamatlengkap is null then '' else alm.alamatlengkap end as alamat,  
                ps.nohp,to_char(ps.tgllahir,'yyyy-MM-dd') as tgllahir,date_part('year',age(ps.tgllahir)) as age,jk.id as jkid,
                so.tglorder,so.noorder,pd.noregistrasi, pg.id as pgid,pg.namalengkap,
                kp.kelompokpasien,ru.namaruangan,ru.id as ruid ,
                ru2.id ruidtuju,ru2.namaruangan as ruangtujuan,ru.objectdepartemenfk,
                lis.order_key as idbridging,op.iscito,gd.golongandarah,
                so.keteranganlainnya ,dp.namadepartemen,pd.norec as norec_pd,so.norec as norec_so,
                pd.tglregistrasi
                FROM orderpelayanan_t as op
                 join produk_m as prd on prd.id=op.objectprodukfk
                 join strukorder_t as so on so.norec=op.strukorderfk
                 join pasien_m as ps on ps.id =so.nocmfk
                left join jeniskelamin_m as jk on jk.id=ps.objectjeniskelaminfk 
                left join alamat_m as alm on alm.nocmfk=ps.id
                 join pasiendaftar_t as pd on pd.norec=so.noregistrasifk
                ...
                left join ris_order as lis on lis.order_no=so.noorder AND lis.order_code=cast(op.objectprodukfk as text)
                left join ruangan_m as ru2 on ru2.id=so.objectruangantujuanfk
                left join departemen_m as dp on dp.id=ru.objectdepartemenfk
                where 
                 so.noorder= '$request[noorder]'
...
//        return $this->respond($data);
        ...
        $i = 0;
        foreach ($data as $d) {
            $tglOrder = date('Y-m-d H:i:s',strtotime($d->tglorder)) ;
            $ordeR_KEY = $this->generateCodeBySeqTable(new RisOrder(), 'order_key', 8, date('ym'),$kdProfile);
            if($ordeR_KEY ==''){
            ...
            if($d->kodeexternal_prd == null){
                $d->kodeexternal_prd = 'CR';
//                CT,MR,US,CR
            }
            $jenisRawat = '';
            foreach ($deptRanap as $itemRanap) {
                if ((int)$itemRanap == $d->objectdepartemenfk) {
                    $jenisRawat = 'I';
                    break;
                } else {
                    $jenisRawat = 'O';
...
//            dd(date('Ym.').$acc);
            ...
            /*
             * orderstaus "NW  :  NEW ORDER  ,CA   : Cancel
             */
            $acc =substr($ordeR_KEY,6,4);
//            dd(date('Ym.').$acc);
            $pg = Pegawai::where('id', $this->getCurrentUserID())->first();
            $json=[
                "ordeR_KEY" =>  (int) $ordeR_KEY,
                "creatE_DATE"=> date('Y-m-d H:i:s'),
                "flag" => "Y",//Y--CA
                "patienT_ID"=> $d->nocm,
                "patienT_NAME" => $d->namapasien,
                "patienT_UID"=> (string)$d->psid,
                "lasT_NAME"=> "",
                "firsT_NAME"=> "",
                "middlE_NAME"=> "",
                "aetitle"=>"",
                "studY_RESERVE_DATE"=> $tglOrder,
                "ordeR_MODALITY"=> $d->kodeexternal_prd,
                "accessioN_NUM"=> date('Ym.').$acc,//YYMM.XXXX
                "ordeR_STATUS"=> "NW",
                "ordeR_CODE"=>(string) $d->kdproduk,
                "ordeR_NAME"=> $d->namaproduk ,
                "ordeR_REASON"=> $d->keteranganlainnya,
                "studY_REMARK"=> "",
                "ordeR_BODYPART"=> "",
                "chargE_DOC_ID"=> (string) $this->getCurrentUserID(),//(string) $_SESSION['idPegawai'],
                "chargE_DOC_NAME"=> !empty($pg) ?$pg->namalengkap :'Administrator',
                "consulT_DOC_ID"=>  (string) $this->getCurrentUserID(),
                "consulT_DOC_NAME"=>  !empty($pg) ?$pg->namalengkap :'Administrator',
                "ordeR_DEPT"=> "RADIOLOGI",
                "ordeR_DATE"=> $tglOrder,
                "ordeR_NO"=> $d->noorder,
                "patienT_IO"=>$d->iscito !=null && $d->iscito == true ?"E" : $jenisRawat,
                "patienT_WARD"=> $d->namaruangan,
                "iO_DATE"=>  $jenisRawat,
                "patienT_BIRTH_DATE"=> date('Y-m-d',strtotime($d->tgllahir)) ,
                "patienT_SEX"=> $d->jkid==1? "M":"F",
                "ordeR_DIAG"=> "",
                "patienT_BLOOD"=> $d->golongandarah!=null? $d->golongandarah:"",
                "ordeR_CNT"=> 1,
                "grouP1"=> "",
                "grouP2"=> "",
                "grouP3"=> "",
                "ordeR_COMMENT"=> "-",
                "extensioN1"=> $d->noregistrasi,
                "extensioN2"=>  $d->kelompokpasien,
                "extensioN3"=> $d->namadepartemen,
                "extensioN4"=> "",
                "extensioN5"=> $d->alamat,
                "extensioN6"=> "",
                "extensioN7"=> "",
...
//            dd($json);
            ...
            curl_setopt_array($curl, array(
                CURLOPT_URL => 'http://36.94.47.50:1080/api/orders',
                CURLOPT_RETURNTRANSFER => true,
                CURLOPT_ENCODING => '',
                CURLOPT_MAXREDIRS => 10,
                CURLOPT_TIMEOUT => 0,
                ...
                CURLOPT_HTTPHEADER => array(
                    'Content-Type: application/json',
                    'Cookie: refreshToken='.$token->jwtToken,
            ...
            if(isset($result->ordeR_KEY)){
                $re =  $this->saveRisOrder($result,$kdProfile,null,$json,$d);
                if($i == count($data)){
                    $h = array(
                        "status" => $re['status'],
                        "message" => $re['message'],
                    ...
                    return $h;
////                    return $this->setStatusCode($re['status'])->respond($re, $re['message']);
                }
            }else{
                $msg = 'Error Brigding Zetta PACS, Harap hubungi IT Support Zeta';
                $h = array(
                    "status" => 400,
                    "message" => $msg,
                    "response"=> $json,
    ...
    function saveRisOrder($result, $kdProfile,$lisorderid,$r,$query){
        DB::beginTransaction();
        try{
            $newBRG = new RisOrder();
            $newBRG->order_key =  isset($result) && isset($result->ordeR_KEY) ? $result->ordeR_KEY :  $r['ordeR_KEY'];
            $newBRG->accession_num = isset($result) && isset($result->accessioN_NUM ) ? $result->accessioN_NUM: $r['accessioN_NUM'];
            $newBRG->aetitle = '';
            $newBRG->charge_doc_id = $r['chargE_DOC_ID'];
            $newBRG->charge_doc_name =  $r['chargE_DOC_NAME'];
            $newBRG->consult_doc_id = $r['consulT_DOC_ID'];
            $newBRG->consult_doc_name = $r['consulT_DOC_NAME'];
            $newBRG->create_date =  $r['creatE_DATE'];
            $newBRG->extension1 =  $r['extensioN1'];
            $newBRG->extension2 =  $r['extensioN2'];
            $newBRG->extension4 = '-';
            $newBRG->extension3 = '-';
            $newBRG->extension5 = $r['extensioN5'];
            $newBRG->extension6 = '-';
            $newBRG->extension7 = '-';
            $newBRG->extension8 = '-';
            $newBRG->extension9 = '-';
            ...
            $newBRG->group2 = '-';
            $newBRG->group3 = '-';
            $newBRG->io_date =$r['iO_DATE'];
            $newBRG->middle_name = '-';
            $newBRG->order_bodypart = '-';
            $newBRG->order_cnt = $r['ordeR_CNT'];
            $newBRG->order_code = $r['ordeR_CODE'];
            $newBRG->order_comment = $r['ordeR_COMMENT'];
            $newBRG->order_date= $r['ordeR_DATE'];
            $newBRG->order_dept =$r['ordeR_DEPT'];
            $newBRG->order_modality =$r['ordeR_MODALITY'];
            $newBRG->order_name =$r['ordeR_NAME'];
            $newBRG->order_no = $r['ordeR_NO'];
            $newBRG->order_reason = '-';
            $newBRG->order_status = $r['ordeR_STATUS'];
            $newBRG->patient_birth_date = $r['patienT_BIRTH_DATE'];
            $newBRG->patient_blood = $r['patienT_BLOOD'];
            $newBRG->patient_id =$r['patienT_UID'];
            $newBRG->patient_io = $r['patienT_IO'];
            $newBRG->patient_name = $r['patienT_NAME'];
            $newBRG->patient_sex =  $r['patienT_SEX'];
            $newBRG->patient_uid =  $r['patienT_UID'];
            $newBRG->patient_ward =  $r['patienT_WARD'];
            $newBRG->study_remark =  $r['studY_REMARK'];
            $newBRG->study_reserv_date =  $r['studY_RESERVE_DATE'];
            $newBRG->kelas = 6;
            $newBRG->kdprofile = $kdProfile;
            $newBRG->save();

            StrukOrder::where('noorder',$r['ordeR_NO'])
...
//            $pd = PasienDaftar::where('noregistrasi',$r['extensioN1'])->where('kdprofile', $kdProfile)->first();
            $apd = AntrianPasienDiperiksa::where('noregistrasifk',$query->norec_pd)
                ->where('kdprofile', $kdProfile)
                ->where('objectruanganfk',$query->ruidtuju)
                ->where('statusenabled',true)
                ->first();
//            dd($apd);
            if(empty($apd)){
                ...
                $dataAPD->noantrian = 1;
                $dataAPD->noregistrasifk =$query->norec_pd;
                $dataAPD->objectpegawaifk =(string) $this->getCurrentUserID();
                $dataAPD->objectruanganfk = $query->ruidtuju;
                $dataAPD->statusantrian = 0;
                $dataAPD->statuspasien = 1;
                $dataAPD->objectstrukorderfk = $query->norec_so;
                $dataAPD->tglregistrasi =$query->tglregistrasi;
                $dataAPD->tglmasuk = date('Y-m-d H:i:s');
                $dataAPD->tglkeluar = date('Y-m-d H:i:s');
                $dataAPD->save();
...
//            foreach ($query as $item){
            ...
            $PelPasienPetugas->objectpegawaifk =(string) $this->getCurrentUserID();
...
//            ->LEFTJOIN('ris_order as pp','pp.order_no','=','so.noorder')
            ->select('so.norec', 'so.tglorder', 'so.noorder', 'ru.namaruangan as ruanganasal', 'ru2.namaruangan as ruangantujuan', 'p.namalengkap', 'so.statusorder',
                'pd.noregistrasi', 'so.objectruangantujuanfk', 'so.statusorder','so.keteranganlainnya'
                ,DB::raw("to_char(so.tglpelayananawal,'YYYY-MM-DD') AS tglpelayananawal")
...
//        $data = $data->distinct();
        ...
        foreach ($data as $item) {
            ...
            if ($item->objectruangantujuanfk != $this->settingDataFixed('kdRuanganElektromedik',$idProfile)) {
                $risorder = RisOrder::where('order_no', $item->noorder)
                ->where('kdprofile', $idProfile)->get();
                if (count($risorder) > 0) {
                    $status = 'Selesai Diperiksa';
                } else {
                    $status = 'Belum Kirim Ke RIS';
                }
            } else {
    ...
    public function getOrderOK(Request $request){
        ...
        $data = \DB::table('strukorder_t as so')
            ...
            ->select('so.norec', 'pd.norec as norecpd', 'pd.noregistrasi', 'so.tglorder', 'so.noorder',
                ...
                DB::raw('case when pd.noregistrasi is null then \'PENDING\' else \'Verifikasi\' end as statusorder')
            )
            ->where('so.kdprofile', $idProfile);
        if (isset($request['noregistrasi']) && $request['noregistrasi'] != "" && $request['noregistrasi'] != "undefined") {
            $data = $data->where('pd.noregistrasi', '=', $request['noregistrasi']);
...
//                    "strukorder" => $dataSO,
    ...
    public function getDiagnosaPasien(Request $request){
        $kdProfile = $this->getDataKdProfile($request);
        $idProfile = (int) $kdProfile;
        // $data = \DB::table('detaildiagnosapasien_t as dgp')
        //     ->leftJoin('pasiendaftar_t as pd', 'pd.norec', '=', 'dgp.noregistrasifk')
        //     ->leftJoin('diagnosa_m as dg', 'dg.id', '=', 'dgp.objectdiagnosafk')
        //     ->select('dgp.tglinputdiagnosa', 'pd.noregistrasi','dg.kddiagnosa','dg.namadiagnosa')
        //     ->where('dgp.kdprofile', $idProfile)
        //     ->orderBy('dgp.tglinputdiagnosa', 'asc');
        ...
        $idDiagnosa= 0;
        if  (count($data) > 0 ){
            $idDiagnosa = $data[0]->kddiagnosa;
        }

        $datacp = \DB::table('cpmaster_m as cpm')
            ->select('cpm.*')
            ->where('cpm.kodeexternal', $idDiagnosa)
            ->get();
        $result = array(
            'data' => $data,
            'cp' => $datacp,
    ...
    public function getListCP(Request $request){
        ...
        $datacp = \DB::table('cpmaster_m as cpm')
            ->select('cpm.*')
            ->where('cpm.kodeexternal', $request['kddiagnosa'])
            ->get();
        $result = array(
            'data' => $request['kddiagnosa'],
            'cp' => $datacp,
            'message' => 'as@epic',
        );
        return $this->respond($result);
    }
    public function getListDiagnosaCP(Request $request){
        $kdProfile = $this->getDataKdProfile($request);
        $idProfile = (int) $kdProfile;

        $datacp = \DB::table('cpmaster_m as cpm')
            ->select('cpm.kodeexternal as kddiagnosa')
            ->distinct()
            ->get();
        $result = array(
            'cp' => $datacp,
...
//                "dtdt" => $dtdt,
    ...
    public function getDataKepatuhanHandHygiene(Request $request){
        ...
        $data = \DB::table('kepatuhanhandhygiene_t AS kh')
            ...
            ->select('kh.norec', 'kh.tanggal', 'kh.objectruanganfk', 'kh.objectindikasifk', 'ink.id', 'ink.indikasi', 'ru.namaruangan', 'kh.objecthygienefk', 'hh.tindakan', 'kh.objectpegawaifk', 'pg.namalengkap', 'kh.langkah', 'kh.objectjenispegawaifk', 'jp.jenispegawai', 'kh.kesempatan')
...
//            ->where('emr.kdprofile',$kdProfile)
    ...
    public function getPasienSelf (Request $r){
        $kdProfile = (int) $this->getKdProfilePasien($r);
        $data= DB::table('pasiendaftar_t AS pd')
...
//            ->whereNull('apd.tglkeluar')
            ...
            ->leftJOIN ('pekerjaan_m as pkj','pkj.id','=','p.objectpekerjaanfk')
            ->select(DB::raw("pd.tglregistrasi,p.nocm,
				...
				jk.jeniskelamin,pg.namalengkap AS namadokter,p.noidentitas,p.nohp,p.tgllahir,pkj.pekerjaan,
				pd.norec AS norec_pd,pd.tglpulang,pd.statuspasien,p.tgllahir,pd.objectruanganlastfk,p.tempatlahir,
		        ...
		        apd.norec as norec_apd,pd.tglpulang,
				CASE WHEN kmr.namakamar IS NOT NULL THEN ru.namaruangan || ' Kamar ' || kmr.namakamar || ' bed ' || tt.nomorbed
				ELSE '-' END AS kamarpasien,DATE_PART('day', (case when pd.tglpulang is null then now() else  pd.tglpulang  end ) - pd.tglregistrasi) || ' Hari' as lamarawat,
...
//            ->whereNull('pd.tglpulang')
    ...
    public function updateStatusKlinis(Request $request){
        $kdProfile = $this->getDataKdProfile($request);
        $idProfile = (int) $kdProfile;
        DB::beginTransaction();
        try {

            $dataSO = PasienDaftar::where('norec', $request['norec'])->where('kdprofile',$idProfile)->update([
                'ketklinis' =>  $request['sttaus']
            ]);
    ...
    public function getDataComboJenisPart(Request $request){
        $kdProfile = $this->getDataKdProfile($request);
        // $idProfile = (int) $kdProfile;
        $req = $request->all();
        $dataProduk = \DB::table('jenisdiagnosa_m')
    ...
    public function getDataComboCovidPart(Request $request){
        $kdProfile = $this->getDataKdProfile($request);
        // $idProfile = (int) $kdProfile;
        $req = $request->all();
        $dataProduk = \DB::table('statuscovid_m')
            ->select('id as value', 'status as text')
            ->where('statusenabled', true)
            // ->where('kdprofile',$idProfile)
            ->orderBy('status');
        if (isset($req['filter']['filters'][0]['value']) &&
            $req['filter']['filters'][0]['value'] != "" &&
            $req['filter']['filters'][0]['value'] != "undefined") {
            $dataProduk = $dataProduk->where('status', 'ilike', '%' . $req['filter']['filters'][0]['value'] . '%');
        };
        $dataProduk = $dataProduk->take(10);
        $dataProduk = $dataProduk->get();
    ...
    public function getDataComboGizi(Request $request){
        $kdProfile = $this->getDataKdProfile($request);
        $idProfile = (int) $kdProfile;
        $req = $request->all();
        $dataProduk = \DB::table('jenisdiet_m')
            ->select('id as value', 'jenisdiet as text')
            ->where('statusenabled', true)
            ->orderBy('jenisdiet');
        if (isset($req['filter']['filters'][0]['value']) &&
            $req['filter']['filters'][0]['value'] != "" &&
            $req['filter']['filters'][0]['value'] != "undefined") {
            $dataProduk = $dataProduk->where('jenisdiet', 'ilike', '%' . $req['filter']['filters'][0]['value'] . '%');
        };
        $dataProduk = $dataProduk->take(10);
        $dataProduk = $dataProduk->get();
    ...
    public function getDataComboGiziBentuk(Request $request){
        $kdProfile = $this->getDataKdProfile($request);
        $idProfile = (int) $kdProfile;
        $req = $request->all();
        $dataProduk = \DB::table('kategorydiet_m')
            ->select('id as value', 'kategorydiet as text')
            ->where('statusenabled', true)
            ->orderBy('kategorydiet');
        if (isset($req['filter']['filters'][0]['value']) &&
            $req['filter']['filters'][0]['value'] != "" &&
            $req['filter']['filters'][0]['value'] != "undefined") {
            $dataProduk = $dataProduk->where('kategorydiet', 'ilike', '%' . $req['filter']['filters'][0]['value'] . '%');
        };
        $dataProduk = $dataProduk->take(10);
        $dataProduk = $dataProduk->get();
    ...
    public function getDataComboGiziRute(Request $request){
        $kdProfile = $this->getDataKdProfile($request);
        $idProfile = (int) $kdProfile;
        $req = $request->all();
        $dataProduk = \DB::table('carapemberianobat_m')
            ->select('id as value', 'deskripsi as text')
            ->where('statusenabled', true)
            ->orderBy('deskripsi');
        if (isset($req['filter']['filters'][0]['value']) &&
            $req['filter']['filters'][0]['value'] != "" &&
            $req['filter']['filters'][0]['value'] != "undefined") {
            $dataProduk = $dataProduk->where('deskripsi', 'ilike', '%' . $req['filter']['filters'][0]['value'] . '%');
        };
        $dataProduk = $dataProduk->take(10);
        $dataProduk = $dataProduk->get();
    ...
    public function saveBridgingLis($request,$idProfile)
    {
        $kdProfile = $idProfile;
        $data = DB::select(DB::raw("
                 select prd.id as kdproduk,prd.namaproduk,prd.kodeexternal as test_cd ,op.norec,ps.namapasien,ps.nocm,
                case when alm.alamatlengkap is null then '' else alm.alamatlengkap end as alamat,  
                ps.nohp,to_char(ps.tgllahir,'yyyy-MM-dd') as tgllahir,date_part('year',age(ps.tgllahir)) as age,jk.id as jkid,
                so.tglorder,so.noorder,pd.noregistrasi, pg.id as pgid,pg.namalengkap,
                kp.kelompokpasien,ru.namaruangan,ru.id as ruid ,
                ru2.id ruidtuju,ru2.namaruangan as ruangtujuan,dp.namadepartemen,
                lis.id as lisorderid,pd.norec as norec_pd,so.norec as norec_so,op.qtyproduk
                FROM orderpelayanan_t as op
        ...
        $pcr = explode (',',$this->settingDataFixed('kodePCRproduk',  $kdProfile ));
...
//        3 | E    | Emergency
//        4 | B    | Obstetrics
//        5 | C    | Commercial Account
//        6 | P    | Preadmit
//        7 | R    | Recurring patient
//        8 | U    | Unknown
//        9 | N    | Not Applicable
        $cek = DB::table('lisorder')
            ->where('kdprofile',$kdProfile)
        ...
        $peg = Pegawai::where('id',$this->getCurrentUserID())->first();
        $json = [
            "patient_nm" => $data[0]->namapasien,
            "patient_class" => "I",
            "mrn" => $data[0]->nocm,
            "admission_dttm" => date('Y-m-d H:i:s'),
            "barcode" => $data[0]->noorder,
            "address" => $data[0]->alamat,
            "phone" => $data[0]->nohp,
            "birth_dt" => $data[0]->tgllahir,
            "age" => $data[0]->age,
            "gender_cd" => $data[0]->jkid,
            "batch_id" => $data[0]->noorder,
            "order_dttm" => $data[0]->tglorder,
            "att_physician_id" => 0,
            "req_physician_id" => $this->getCurrentUserID(),//(string) $_SESSION['idPegawai'],
            "req_physician_nm" =>  !empty($peg) && $peg->namalengkap!='-' ? $peg->namalengkap: 'Administrator',//,  $_SESSION['namaLengkap']=='-'?'Administrator':$_SESSION['namaLengkap'],
            "insurer_cd" => "lainnya", //probadi,asuransi,bPpjs,lainnya
            "insurer_nm" => $data[0]->kelompokpasien =='-'? 'lainnya' :$data[0]->kelompokpasien,
            "referring_physician_nm" => "",
            "referring_physician_phone" => "",
            "order_location_nm" => $data[0]->namaruangan. '-'.$data[0]->namadepartemen,
            "order_location_id" => $data[0]->ruid,
            "order_org_id" => $data[0]->ruidtuju,
            "order_org_nm" => $data[0]->ruangtujuan,
            "order_control_cd" => $con,//4.	order_control_cd -> ‘NW’ = New, ‘CA’ = Cancel, ‘RP’ = Replace
            "order_priority_cd" => "routine",//
            "test_items" => $detailsProd
        ];
//        dd($json);
//        return $json;
//        die;
...
//3 | stat    | Stat
//4 | asap    | ASAP
        $lisorderid = $data[0]->lisorderid;

        $post =  json_encode($json);

        $curl = curl_init();
        $url  = 'https://lisv2.healtri.com/rsdcwav2/api/integration/order/klinik';//$this->settingDataFixed('urlKlinik',$kdProfile);
        ...
        if(count($detailsProdPCR) > 0){
            // $url  = $this->settingDataFixed('urlPCR',$kdProfile);
            $url  = 'https://lisv2.healtri.com/rsdcwav2/api/integration/order/pcr';
            $json = [
                "patient_nm" => $data[0]->namapasien,
                "patient_class" => "I",
                "mrn" => $data[0]->nocm,
                ...
                "req_physician_id" => $this->getCurrentUserID(),//(string) $_SESSION['idPegawai'],
                "req_physician_nm" =>  !empty($peg) && $peg->namalengkap!='-' ? $peg->namalengkap: 'Administrator',//,  $_SESSION['namaLengkap']=='-'?'Administrator':$_SESSION['namaLengkap'],
                "insurer_cd" => "lainnya", //probadi,asuransi,bpjs,lainnya
                "insurer_nm" => $data[0]->kelompokpasien =='-'? 'lainnya' :$data[0]->kelompokpasien,
                "referring_physician_nm" => "",
                "referring_physician_phone" => "",
                "order_location_nm" => $data[0]->namaruangan. '-'.$data[0]->namadepartemen,
...
//            $msg = isset($result->message) ? $result->message: ' Error Brigding LIS, Harap hubungi IT Support LIS';
//            $h = array(
//                "status" => 400,
//                "message" => $msg,
//                "response"=> $json,
...
//        };
        if(isset($result) &&  $result->status == true){
            $re =  $this->saveLisOrder($result,$kdProfile,$lisorderid,$json,$data);
            $h = array(
                "status" => 201,
                "message" => $re['message'],
                "response"=>$re,
            ...
            $msg = isset($result->message) ? $result->message: ' Error Brigding LIS, Harap hubungi IT Support LIS2';
            $h = array(
                "status" => 400,
                "message" => $msg,
                "response"=>$json,
    ...
    function saveLisOrder($res, $kdProfile,$lisorderid,$v,$query){
//
//        DB::beginTransaction();
//        try{
            if (is_null($lisorderid)) {
                $newBRG = new LisOrder();
                ...
                $newBRG = LisOrder::where('id', $lisorderid)->first();
                DB::table('lisorderdetail')->where('lisorderfk',$lisorderid)->delete();
            }
            $newBRG->patient_nm = $v['patient_nm'];
            $newBRG->patient_class = $v['patient_class'];
            $newBRG->mrn = $v['mrn'];
            $newBRG->admission_dttm = $v['admission_dttm'];
            $newBRG->barcode = $v['barcode'];
            $newBRG->address = $v['address'];
            $newBRG->phone = $v['phone'];
            $newBRG->birth_dt = $v['birth_dt'];
            $newBRG->age = $v['age'];
            $newBRG->gender_cd = $v['gender_cd'];
            $newBRG->batch_id = $v['batch_id'];
            $newBRG->order_dttm = $v['order_dttm'];
            $newBRG->att_physician_id = $v['att_physician_id'];
            $newBRG->req_physician_id = $v['req_physician_id'];
            $newBRG->req_physician_nm = $v['req_physician_nm'];
            $newBRG->insurer_cd = $v['insurer_cd'];
            $newBRG->insurer_nm = $v['insurer_nm'];
            $newBRG->referring_physician_nm = $v['referring_physician_nm'];
            $newBRG->referring_physician_phone = $v['referring_physician_phone'];
            $newBRG->order_location_nm = $v['order_location_nm'];
            $newBRG->order_location_id = $v['order_location_id'];
            $newBRG->order_org_id = $v['order_org_id'];
            $newBRG->order_org_nm = $v['order_org_nm'];
            $newBRG->order_control_cd = $v['order_control_cd'];
            $newBRG->order_priority_cd = $v['order_priority_cd'];
            $newBRG->order_org_id = $v['order_org_id'];
            $newBRG->ono = $v['batch_id'];
            if($v['order_control_cd'] =='NW'){
                $newBRG->result_id = isset($res->data) && $res->data != null ? $res->data[0]->id : null;
            }
            $newBRG->save();

            $idB = $newBRG->id;
            foreach ($v['test_items'] as $it){
                $prod = DB::table('produk_m')->where('kodeexternal',$it['test_cd'])
                    ->where('kdprofile', $kdProfile)
                    ->where('statusenabled',true)
                    ->first();
                $dataz = array(
                    'id' => DB::table('lisorderdetail')->max('id') +1,
                    'kdprofile'  => $kdProfile,
                    'order_id'=> $it['order_id'],
                    'test_cd' => $it['test_cd'],
                    'status'=> $it['status'],
                    'lisorderfk'  => $idB,
                    'produkfk'=>!empty($prod) ?$prod->id :null,
                );
                $insrt = DB::table('lisorderdetail')->insert($dataz);
            }
            StrukOrder::where('noorder', $v['batch_id'])
                ->where('kdprofile', $kdProfile)
                ->update(
                    [
                        'statusorder' => 1
            ...
            $pd = PasienDaftar::where('norec',$query[0]->norec_pd)->where('kdprofile', $kdProfile)->first();
            $apd = AntrianPasienDiperiksa::where('noregistrasifk',$query[0]->norec_pd)
                ->where('kdprofile', $kdProfile)
                ->where('objectruanganfk',$query[0]->ruidtuju)
                ->where('statusenabled',true)
                ->first();
//            dd($apd);
            if(empty($apd)){
                ...
                $dataAPD->objectkelasfk = 6;// $request['objectkelasfk'];
                $dataAPD->noantrian = 1;
                $dataAPD->noregistrasifk =$query[0]->norec_pd;
                $dataAPD->objectpegawaifk = $this->getCurrentUserID();//['idPegawai'];
                $dataAPD->objectruanganfk = $query[0]->ruidtuju;
                $dataAPD->statusantrian = 0;
                $dataAPD->statuspasien = 1;
                $dataAPD->objectstrukorderfk = $query[0]->norec_so;
                $dataAPD->tglregistrasi = $pd->tglregistrasi;// date('Y-m-d H:i:s');
                $dataAPD->tglmasuk = date('Y-m-d H:i:s');
                $dataAPD->tglkeluar = date('Y-m-d H:i:s');
                $dataAPD->save();
            ...
            foreach ($query as $item){
                ...
                $PelPasienPetugas->statusenabled = true;
                $PelPasienPetugas->nomasukfk = $dataAPDnorec;
                $PelPasienPetugas->objectpegawaifk =$this->getCurrentUserID();// $_SESSION['idPegawai'];//$request['objectpegawaiorderfk'];
...
//        if ($transStatus == 'true') {
            $transMessage = "Simpan LIS Sukses";
//            DB::commit();
            $result = array(
                "status" => 201,
                "message" => $transMessage,
...
//        }
    ...
    public function SaveTransaksiCP(Request $request)
    {
        $kdProfile = (int) $this->getDataKdProfile($request);
        $dataReq = $request->all();
        ...
        try {
            ...
            if ($head['norec_emr'] == '-') {
                ...
                $noemr = $this->generateCodeBySeqTable(new CPPasien, 'noemr', 15, 'CP' . date('ym') . '/',$kdProfile);
                // return $noemr;
                if ($noemr == '') {
                    $transMessage = "Gagal mengumpukan data, Coba lagi.!";
                    DB::rollBack();
                ...
                $EMR = CPPasien::where('noemr', $head['norec_emr'])
                ...
                $EMRPASIENDETAIL = CPPasienD::where('emrpasienfk', $noemr)
                    ->select('emrdfk','value')
                    ->where('emrfk', $head['emrfk'])
                    ->where('kdprofile',$kdProfile)
...
//                $valueemr = $dtdt[$i];
            ...
            foreach ($data as $item) {
                ...
                if  ($sama ==  2){
                    $EMRPasienDUpdatekeun = CPPasienD::where('emrpasienfk', $norec_EMR)
                        ->where('emrfk', $head['emrfk'])
                        ->where('emrdfk', $emrdfk)
                        ->where('kdprofile',$kdProfile)
                        ->where('statusenabled', 1)
...
//                "dtdt" => $dtdt,
    ...
    public function getDataComboIntake(Request $request){
        $kdProfile = $this->getDataKdProfile($request);
        $idProfile = (int) $kdProfile;
        $req = $request->all();
        $dataProduk = \DB::table('problemdomain_m')
            ->select('id as value', 'problemdomain as text')
            ->where('statusenabled', true)
            ->where('objectkelompokproblemfk', 1)
            ->orderBy('problemdomain');
        if (isset($req['filter']['filters'][0]['value']) &&
            $req['filter']['filters'][0]['value'] != "" &&
            $req['filter']['filters'][0]['value'] != "undefined") {
            $dataProduk = $dataProduk->where('problemdomain', 'ilike', '%' . $req['filter']['filters'][0]['value'] . '%');
        };
        $dataProduk = $dataProduk->take(10);
        $dataProduk = $dataProduk->get();
    ...
    public function getDataComboClinic(Request $request){
        $kdProfile = $this->getDataKdProfile($request);
        $idProfile = (int) $kdProfile;
        $req = $request->all();
        $dataProduk = \DB::table('problemdomain_m')
            ->select('id as value', 'problemdomain as text')
            ->where('statusenabled', true)
            ->where('objectkelompokproblemfk', 2)
            ->orderBy('problemdomain');
        if (isset($req['filter']['filters'][0]['value']) &&
            $req['filter']['filters'][0]['value'] != "" &&
            $req['filter']['filters'][0]['value'] != "undefined") {
    ...
    public function getDataComboBeha(Request $request){
        $kdProfile = $this->getDataKdProfile($request);
        $idProfile = (int) $kdProfile;
        $req = $request->all();
        $dataProduk = \DB::table('problemdomain_m')
            ->select('id as value', 'problemdomain as text')
            ->where('statusenabled', true)
            ->where('objectkelompokproblemfk', 3)
            ->orderBy('problemdomain');
        if (isset($req['filter']['filters'][0]['value']) &&
            $req['filter']['filters'][0]['value'] != "" &&
            $req['filter']['filters'][0]['value'] != "undefined") {
     ...
     public function getProduk(Request $request){
        ...
        $req = $request->all();
        $kdjenisobat = $this->settingDataFixed('kdJenisProdukObat',$kdProfile);
        $nama ='';
        if (isset($req['filter']['filters'][0]['value']) &&
            $req['filter']['filters'][0]['value'] != "" &&
            $req['filter']['filters'][0]['value'] != "undefined") {
           $nama= " and namaproduk ilike '%". $req['filter']['filters'][0]['value'] . "%'";
        };
        $dataProduk = DB::select(DB::raw("SELECT 
                prd.ID,
                prd.namaproduk
            FROM
                produk_m as prd
            ...
            WHERE
                prd.statusenabled = TRUE
                and prd.kdprofile = $kdProfile 
                $nama
            AND (
                prd.ID IN (
                    SELECT objectprodukfk   FROM    mapruangantoproduk_m WHERE   statusenabled = TRUE
                    AND objectprodukfk IN (SELECT   objectprodukfk FROM harganettoprodukbykelas_m WHERE statusenabled = TRUE )
                )
                OR  jp.id in($kdjenisobat)
            ) 
     ...
     public function getFormTarget(Request $request){
        $kdProfile = $this->getDataKdProfile($request);
       
         $form = \DB::table('cpform_t')
            ->where('statusenabled', true)
            ->where('kdprofile', $kdProfile)
            ->orderBy('namaform')
            ->get();
         $kegiatan = \DB::table('cpkegiatan_t')
             ->where('statusenabled', true)
             ->where('kdprofile', $kdProfile)
             ->orderBy('sort')
             ->get();
        $res['form'] =$form;
         $res['kegiatan'] =$kegiatan;
     ...
     public function saveMapping(Request $request)
    {
        ...
        try {
            ...
            if ($request['id'] == '') {
                $EMR = new CPMapping();
                $EMR->id = CPMapping::max('id') +1;
                $EMR->norec =  $EMR->generateNewId();
                $EMR->kdprofile = $kdProfile;
                $EMR->statusenabled = 1;
            } else {
                $EMR = CPMapping::where('id', $request['id'])
                    ->first();
            }
            $EMR->idobjectcp = $request['idobjectcp'];
            $EMR->kodecp = $request['kodecp'];
            $EMR->urltarget = $request['urltarget'];
            $EMR->produkfk = $request['produkfk'];
            $EMR->jumlah = $request['jumlah'];
            $EMR->urltargetfk = $request['urltargetfk'];
            $EMR->checkisred = $request['checkisred'];
            $EMR->save();
            $transStatus = 'true';
        } catch (\Exception $e) {
            $transStatus = 'false';
    ...
    public function saveCaption(Request $request)
    {
        ...
        try {
            ...
            if ($request['id'] == '-') {
                $EMR = new CPKegiatanUraian();
                $EMR->id = CPKegiatanUraian::max('id') +1;
                $EMR->norec =  $EMR->generateNewId();
                $EMR->kdprofile = $kdProfile;
                $EMR->statusenabled = 1;
                $EMR->kodecp = $request['kodecp'];
            } else {
                $EMR = CPKegiatanUraian::where('id', $request['id'])
                    ->first();
            }
            if  ($request['jenis'] == 1){
                $EMR->caption = $request['caption'];
            }else{
                $EMR->keterangan = $request['caption'];
            }

            $EMR->openlinkarr = $request['openlinkid'];
            $EMR->objectcpkegiatanfk = $request['objectcpkegiatanfk'];
            $EMR->sort = $request['sort'];
            $EMR->statusenabled = $request['aktif'];
    ...
    {
        $kdProfile = (int) $this->getDataKdProfile($request);
        $form = \DB::table('cpmapping_t as mp')
            ->leftjoin('produk_m as prd','prd.id','=','mp.produkfk')
            ->leftjoin('cpform_t as fr','fr.id','=','mp.urltargetfk')
            ->select('mp.*','fr.namaform','prd.namaproduk','fr.urltarget as url','fr.jenis','fr.emrfk')
            ->where('mp.statusenabled', true)
            ->where('mp.kdprofile', $kdProfile)
            ->orderBy('fr.namaform');

          if(isset($request['kodecp'])  && $request['kodecp']!='' && $request['kodecp']!='undefined'){
                $form =$form->where('mp.kodecp','=',$request['kodecp']);
          }
          if(isset($request['idobjectcp'])  && $request['idobjectcp']!='' && $request['idobjectcp']!='undefined'){
                $idObj = explode (',',$request['idobjectcp']);
                $arr = [];
                foreach ($idObj as $u){
                    $arr []=  $u;
                }
                $form =$form->whereIn('mp.idobjectcp',$arr);
          }
          $form=$form->get();
          $res['data'] =$form;
    ...
    {
        $kdProfile = (int) $this->getDataKdProfile($request);
        $form = \DB::table('cpkegiatanuraian_t as mp')
            ->select('mp.*','kg.kegiatan')
            ->leftjoin('cpkegiatan_t as kg','kg.id','=','mp.objectcpkegiatanfk')
            ->where('mp.statusenabled', true)
            ->where('mp.kdprofile', $kdProfile)
            ->orderBy('mp.sort');
        ...
        if(isset($request['id'])  && $request['id']!='' && $request['idobjectcp']!='undefined'){

            $form =$form->where('mp.id',$request['id']);
        }
        $form=$form->get();
        $res['data'] =$form;
    ...
    public function getColorCheckBox(Request $request)
    {
        $kdProfile = (int) $this->getDataKdProfile($request);
        $form = \DB::table('cpmapping_t as mp')
            ->select('mp.idobjectcp','mp.checkisred')
            ->where('mp.statusenabled', true)
            ->where('mp.kdprofile', $kdProfile);

          if(isset($request['kodecp'])  && $request['kodecp']!='' && $request['kodecp']!='undefined'){
                $form =$form->where('kodecp','=',$request['kodecp']);
          }
          $form=$form->get();
          $res['data'] =$form;
    ...
    public function getKegiatanDanUraiannya(Request $request)
    {
        $kdProfile = (int) $this->getDataKdProfile($request);
        $kegiatan = \DB::table('cpkegiatan_t as mp')
            ->select('mp.*')
            ->where('mp.statusenabled', true)
            ->where('mp.kdprofile', $kdProfile)
            ->orderby('mp.sort','asc');
...
//           if(isset($request['kodecp'])  && $request['kodecp']!='' && $request['kodecp']!='undefined'){
//                 $kegiatan =$kegiatan->where('kodecp','=',$request['kodecp']);
//           }
        $kegiatan=$kegiatan->get();

        $uraian = \DB::table('cpkegiatanuraian_t as mp')
            ->select('mp.id as urid','mp.caption','mp.keterangan','mp.objectcpkegiatanfk','mp.openlinkarr')
            ->where('mp.statusenabled', true)
            ->where('mp.kdprofile', $kdProfile)
            ->orderby('mp.sort','asc');

        if(isset($request['kodecp'])  && $request['kodecp']!='' && $request['kodecp']!='undefined'){
              $uraian =$uraian->where('mp.kodecp','=',$request['kodecp']);
        }
        $uraian=$uraian->get();

        $mastercp = \DB::table('cpmaster_m as mp')
            ->select('mp.*')
            ->where('mp.statusenabled', true)
            ->where('mp.kdprofile', $kdProfile);

        if(isset($request['kodecp'])  && $request['kodecp']!='' && $request['kodecp']!='undefined'){
            $mastercp =$mastercp->where('mp.namaexternal','=',$request['kodecp']);
        }
        $mastercp=$mastercp->get();

        $data=[];
        $dtDetail =[];
        $jmlhari = $mastercp[0]->jmlhari;
        foreach($kegiatan as $item){
            $dtDetail =[];
            foreach($uraian as $itm){
                if  ($item->id == $itm->objectcpkegiatanfk){
                    $dtDetail[]=array(
                        'id' => $itm->urid,
                        'caption' => $itm->caption,
                        'keterangan' => $itm->keterangan,
                        'openlinkarr' => $itm->openlinkarr,
                    );
                }
            }
            $data [] = array(
                'id' => $item->id,
                'caption' => $item->kegiatan,
                'item' => $dtDetail,
                'jmlhari' => $jmlhari
            );
        }
