...
Begin VB.Form frmCetakLaporanOperasi 
   Caption         =   "Transmedic"
   ClientHeight    =   7005
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   5820
   Icon            =   "frmCetakLaporanOperasi.frx":0000
   LinkTopic       =   "Form1"
   ScaleHeight     =   7005
   ScaleWidth      =   5820
   ...
   End
End
Attribute VB_Name = "frmCetakLaporanOperasi"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
Option Explicit
Dim Report As New crEmrLaporanOperasi
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    Set frmCetakLaporanOperasi = Nothing
End Sub

Public Sub Cetak(nocm As String, emr As String, noreg As String, view As String)
...
Set Report = New crEmrLaporanOperasi

      
    strSQL = "select ps.nocm,ps.namapasien,EXTRACT(YEAR FROM AGE(pd.tglregistrasi, ps.tgllahir)) || ' Th ' || EXTRACT(MONTH FROM AGE(pd.tglregistrasi, ps.tgllahir)) || ' Bln ' || EXTRACT(DAY FROM AGE(pd.tglregistrasi, ps.tgllahir)) || ' Hr ' as umur,CASE WHEN ps.objectjeniskelaminfk=1 then 'L' else 'P' END as jk, " & _
            " al.alamatlengkap, al.kecamatan,al.kotakabupaten " & _
            " from pasiendaftar_t as pd  " & _
            " inner JOIN pasien_m as ps on ps.id = pd.nocmfk " & _
            " inner JOIN alamat_m as al on al.nocmfk=ps.id  " & _
            "where ps.statusenabled = true and pd.noregistrasi = '" & noreg & "' "
    ...
    strSQL2 = " SELECT emrp.nocm,emrdp.emrpasienfk ,emrdp.value ,emrdp.emrdfk," & _
                ...
                " AND emr.id in (210029) Order by emrdp.emrdfk ASC "
