...
Set Report = New crRincianBiayaPelayanan
    ...
    strSQL = " select tb.*,CASE WHEN tb.totalharusdibayar = tb.totaldibayar THEN 0 " & _
             " WHEN tb.totalharusdibayar <> tb.totaldibayar THEN CAST ( tb.totalharusdibayar AS FLOAT) + CAST ( tb.totaldibayar  AS FLOAT) " & _
             " ELSE CAST ( tb.totalharusdibayar  AS FLOAT) END AS bayarkeun, " & _
             " CASE WHEN tb.totalharusdibayar IS NULL THEN CAST (0 AS FLOAT) ELSE CAST ( tb.totalharusdibayar AS FLOAT ) END AS bayar, " & _
             ...
             " INNER JOIN pelayananpasien_t AS pp ON pp.norec = tb.norec_pp where tb.noregistrasi='" & strNoregistrasi & "' " & _
...
'            .usJenisProduk.SetUnboundFieldSource ("{ado.jenisproduk}")
            ...
            .ucTotal.SetUnboundFieldSource ("{ado.total}") ' ("{ado.totals}")
'            .ucTotalTotal.SetUnboundFieldSource ("{ado.bayarkeun}") '("{ado.sudahbayar}")
            .ucJmlahTlahDBayar.SetUnboundFieldSource ("{ado.sudahbayar}") '("{ado.totaldibayar}")
...
'            .ucDitanggungSendiri.SetUnboundFieldSource ("{ado.bayarkeun}")
