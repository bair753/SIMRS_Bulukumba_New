...
Begin VB.Form frmCRLaporanPendapatanRemun 
   Caption         =   "Transmedic"
   ClientHeight    =   7005
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   5820
   Icon            =   "frmLaporanPendapatanRemun.frx":0000
   LinkTopic       =   "Form1"
   ScaleHeight     =   7005
   ScaleWidth      =   5820
   ...
   End
End
Attribute VB_Name = "frmCRLaporanPendapatanRemun"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
Option Explicit
Dim Report As New crLaporanPendapatanRemun
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    Set frmCRLaporanPendapatanRemun = Nothing
End Sub
...
Set Report = New crLaporanPendapatanRemun
    strSQL = "select  apd.objectruanganfk,ru.namaruangan, apd.objectpegawaifk,pg.namalengkap,ps.nocm , " & _
             ...
             "case when pr.id =395 then ppd.hargajual* ppd.jumlah else 0 end as karcis, " & _
             "case when pr.id =10013116  then ppd.hargajual* ppd.jumlah else 0 end as embos, " & _
             "case when kp.id = 26 then ppd.hargajual* ppd.jumlah else 0 end as konsul, " & _
             "case when kp.id in (1,2,3,4,8,9,10,11,13,14) then ppd.hargajual* ppd.jumlah else 0 end as tindakan, " & _
             "(case when ppd.hargadiscount is null then 0 else ppd.hargadiscount end)* ppd.jumlah as diskon, " & _
             ...
             str2 & _
             str1 & _
             " and ((to_char( pd.tglregistrasi,'ID')::INTEGER < 6 and date_part('hour', pd.tglregistrasi) between 7 and 12) or (to_char( pd.tglregistrasi,'ID')::INTEGER > 5)) and ppd.komponenhargafk=35 " & _
