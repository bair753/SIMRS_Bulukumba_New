...
Begin VB.Form frmTransaksiPiutangPerusahaan 
   Caption         =   "Transmedic"
   ClientHeight    =   6705
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   6330
   Icon            =   "frmTransaksiPiutangPerusahaan.frx":0000
   LinkTopic       =   "Form1"
   ScaleHeight     =   6705
   ScaleWidth      =   6330
   ...
   End
End
Attribute VB_Name = "frmTransaksiPiutangPerusahaan"
...
Private Sub Form_Unload(Cancel As Integer)
    Set frmTransaksiPiutangPerusahaan = Nothing
End Sub

Public Sub cetakTgl(tglAwal As String, tglAkhir As String, KelompokPasien As String, idPerusahaan As String, namaPrinted As String, view As String)
...
Dim adocmd As New ADODB.Command
    ...
    End If
    If KelompokPasien <> "" Then
        str2 = " and  pd.objectkelompokpasienlastfk ='" & KelompokPasien & "' "
    End If
                
    Set Report = New CrTransasksiPiutangPerusahaan
...
'            "order by sbm.tglsbm"
    ...
    strSQL = "select sp.norec,pd.noregistrasi,sp.tglposting, php.noposting,kp.kelompokpasien,rkn.id as idrekanan,rkn.namarekanan, " & _
             "php.statusenabled,p.namalengkap,spp.totalppenjamin,spp.totalsudahdibayar,(spp.totalppenjamin-spp.totalsudahdibayar) as saldo, " & _
             "pd.tglregistrasi,pd.objectkelompokpasienlastfk " & _
             "FROM postinghutangpiutang_t as php " & _
             "left JOIN strukpelayananpenjamin_t as spp on spp.norec=php.nostrukfk " & _
             "left JOIN strukpelayanan_t as spy on spy.norec=spp.nostrukfk " & _
             "left JOIN pasiendaftar_t as pd on pd.norec=spy.noregistrasifk " & _
             ...
             str2 & _
             str1 & _
             "group by sp.norec,sp.tglposting, php.noposting,kp.kelompokpasien,rkn.id,rkn.namarekanan, " & _
             "php.statusenabled,p.namalengkap,spp.totalppenjamin,spp.totalsudahdibayar,pd.tglregistrasi, " & _
             "pd.objectkelompokpasienlastfk,pd.noregistrasi " & _
    ...
    For i = 0 To rs.RecordCount - 1
        tBayar = tBayar + CDbl(IIf(IsNull(rs!saldo), 0, rs!saldo))
        rs.MoveNext
    Next i
...
'            .usKeterangan.SetUnboundFieldSource ("{ado.keteranganlainnya}")
            ...
            .unSubtotal.SetUnboundFieldSource ("{ado.saldo}")
