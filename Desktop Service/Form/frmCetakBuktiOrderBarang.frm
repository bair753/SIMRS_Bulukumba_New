...
Public Sub Cetak(view As String, strNoKirim As String, pegawaiMengetahui As String, pegawaiMeminta As String, jabatanMeminta, jabatanMengetahui As String, pegawaiPenyerah As String, pegawaiPenerima As String, JabatanPenyerah, JabatanPenerima As String, test As String, strUser As String)
'On Error GoTo errLoad
Set frmCetakBuktiOrderBarang = Nothing
Dim strSQL As String
Dim pegawai1, pegawai2, pegawai3, nip1, nip2, nip3, pegawai4, pegawai5, nip4, nip5 As String
...
'                    "ru.namaruangan as ruangan, 'KA. ' + dp.namadepartemen as kepalaBagian, pg.namalengkap, jp.name + ' ' + ru.namaruangan + ' Tgl ' + CONVERT(varchar, so.tglorder) AS keteranganorder, " & _
...
'           POSTGRESQL
            strSQL = "select so.tglorder, so.noorder, jp.name,ru2.namaruangan as ruangantujuan,so.keteranganorder, " & _
                    "ru.namaruangan as ruangan, 'KA. ' || dp.namadepartemen as kepalaBagian, pg.namalengkap, jp.name || ' ' || ru.namaruangan || ' Tgl ' || to_char(so.tglorder, 'DD-MM-YYYY HH:MI:SS') AS keteranganorder, " & _
                    "pr.id as idproduk,pr.kdproduk as kdsirs,pr.namaproduk, ss.satuanstandar, op.qtyproduk, so.totalhargasatuan as hargasatuan, (so.totalhargasatuan * op.qtyproduk) as total " & _
                    "from strukorder_t as so " & _
                    "left join orderpelayanan_t as op on op.strukorderfk = so.norec " & _
                    "left join produk_m as pr on pr.id = op.objectprodukfk " & _
            ...
            If pegawaiPenyerah <> "" Then
                 ReadRs6 "select pg.namalengkap,pg.nippns,jb.namajabatan " & _
                         "from pegawai_m as pg " & _
                         "left join jabatan_m as jb on jb.id = pg.objectjabatanstrukturalfk " & _
                         "where pg.id = '" & pegawaiPenyerah & "'"
                
                If RS6.EOF = False Then
                    pegawai4 = RS6!namalengkap
                    nip4 = "NIP. " & RS6!nippns
                Else
                    pegawai4 = "-"
                    nip4 = "NIP. -"
            ...
            If pegawaiPenerima <> "" Then
                 ReadRs7 "select pg.namalengkap,pg.nippns,jb.namajabatan " & _
                         "from pegawai_m as pg " & _
                         "left join jabatan_m as jb on jb.id = pg.objectjabatanstrukturalfk " & _
                         "where pg.id = '" & pegawaiMeminta & "'"
...
'             .unDikirim.SetUnboundFieldSource ("{Ado.qtyproduk}")
             ...
             .txtPwgPenyerah.SetText pegawai4
             .txtPwgPenerima.SetText pegawai5
             .txtNipPenyerah.SetText nip4
             .txtNipPenerima.SetText nip4
             .txtJbtPenyerah.SetText JabatanPenyerah
             .txtJbtPenerima.SetText JabatanPenerima
