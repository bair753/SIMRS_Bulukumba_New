...
Begin VB.Form frmRekapffsIBSAnes 
   Caption         =   "Transmedic"
   ClientHeight    =   7005
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   5820
   Icon            =   "frmRekapffsIBSAnes.frx":0000
   LinkTopic       =   "Form1"
   ScaleHeight     =   7005
   ScaleWidth      =   5820
   ...
   End
End
Attribute VB_Name = "frmRekapffsIBSAnes"
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    Set frmRekapffsIBSAnes = Nothing
End Sub
...
Set Report = New crRekapffsIBS
    strSQL = "select *, " & SQLdateLibur & "  case when hari='Saturday ' then 'Sabtu' when hari='Sunday   ' then 'Minggu' when hari='Monday   ' then 'Senin' when hari='Tuesday  ' then 'Selasa' when hari='Wednesday' then 'Rabu' when hari='Thursday ' then 'Kamis' when hari='Friday   ' then 'Jumat' " & STREND & "  end as harihari from ( " & _
            ...
            "Where  ppp.objectjenispetugaspefk = 6  and ru.objectdepartemenfk=25  " & dokter & idRuangan & idKelompokPasien & "" & _
