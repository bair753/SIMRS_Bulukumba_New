...
Begin VB.Form frmCetakLabelFarmasiKecil 
   Caption         =   "Transmedic"
   ClientHeight    =   7170
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   5775
   Icon            =   "frmCetakLabelFarmasiKecil.frx":0000
   LinkTopic       =   "Form1"
   ScaleHeight     =   7170
   ScaleWidth      =   5775
   ...
   End
End
Attribute VB_Name = "frmCetakLabelFarmasiKecil"
...
Set Report = New Cr_cetakLabelKecil
    strSQL = "SELECT distinct ps.nocm, ps.namapasien,to_char(ps.tgllahir, 'DD-MM-YYYY') as tgllahir,sr.noresep, " & _
             "to_char(sr.tglresep, 'DD-MM-YYYY') as tglresep,pr.namaproduk || ' (' || CAST(pp.jumlah as varchar) || ')' AS namaproduk,pp.aturanpakai,pp.rke, " & _
            "CASE WHEN alm.alamatlengkap is null then '-' else alm.alamatlengkap end as alamat,ss.satuanstandar,pp.jumlah, " & _
            ...
            " '(' || EXTRACT(YEAR FROM AGE(to_date(to_char(pd.tglregistrasi,'YYYY-MM-DD'),'YYYY-MM-DD'),to_date(to_char(ps.tgllahir,'YYYY-MM-DD'),'YYYY-MM-DD'))) || 'Thn ' || EXTRACT(MONTH FROM AGE(to_date(to_char(pd.tglregistrasi,'YYYY-MM-DD'),'YYYY-MM-DD'),to_date(to_char(ps.tgllahir,'YYYY-MM-DD'),'YYYY-MM-DD'))) || ' Bln ' || EXTRACT(MONTH FROM AGE(to_date(to_char(pd.tglregistrasi,'YYYY-MM-DD'),'YYYY-MM-DD'),to_date(to_char(ps.tgllahir,'YYYY-MM-DD'),'YYYY-MM-DD'))) || ' Hr)' AS umur, " & _
            "CASE WHEN pp.tglkadaluarsa IS NULL THEN '' ELSE to_char(pp.tglkadaluarsa,'DD-MM-YYYY HH:mm') END AS tglkadaluarsa,CASE WHEN sn.satuanresep IS NOT NULL THEN sn.satuanresep ELSE ss.satuanstandar END AS satuanresep " & _
            "from pelayananpasien_t as pp inner join strukresep_t as sr on sr.norec= pp.strukresepfk " & _
            "INNER JOIN stokprodukdetail_t AS spd ON spd.nostrukterimafk = pp.strukterimafk and pp.produkfk = spd.objectprodukfk and sr.ruanganfk = spd.objectruanganfk " & _
            ...
            str1 & _
            ""
    strSQL = strSQL & " union all select distinct ps.nocm,ps.namapasien,to_char(ps.tgllahir, 'DD-MM-YYYY') as tgllahir,sr.noresep,to_char(sr.tglresep, 'DD-MM-YYYY') as tglresep, " & _
            " ' Racikan' || ' (' || CAST ((( CAST (pp.qtydetailresep AS INTEGER) / CAST (pp.dosis AS INTEGER)) * " & _
            " CASE WHEN pro.kekuatan IS NOT NULL THEN CAST (pro.kekuatan AS INTEGER) ELSE 1 END) AS VARCHAR) || ')' AS namaproduk,pp.aturanpakai,pp.rke, " & _
            " case when alm.alamatlengkap is null then '-' else alm.alamatlengkap end as alamat,CASE when jr.jenisracikan IS NULL THEN '' ELSE jr.jenisracikan END AS satuanstandar, " & _
            " ((CAST(pp.qtydetailresep as INTEGER)/CAST(pp.dosis as INTEGER))*CASE WHEN pro.kekuatan IS NOT NULL THEN CAST (pro.kekuatan AS INTEGER) ELSE 1 END) as jumlah, " & _
            " CASE WHEN pp.issiang = 't' THEN 'Siang' ELSE '-' END AS siang, CASE WHEN pp.ispagi = 't' THEN 'Pagi' ELSE '-' END AS pagi, " & _
            " CASE WHEN pp.ismalam = 't' THEN 'Malam' ELSE '-' END as malam, CASE WHEN pp.issore = 't' THEN 'Sore' ELSE '-' END as sore, " & _
            " CASE WHEN pp.keteranganpakai  = '' OR pp.keteranganpakai IS NULL THEN '' else pp.keteranganpakai END AS keteranganpakai, " & _
            " '(' || EXTRACT(YEAR FROM AGE(to_date(to_char(pd.tglregistrasi,'YYYY-MM-DD'),'YYYY-MM-DD'),to_date(to_char(ps.tgllahir,'YYYY-MM-DD'),'YYYY-MM-DD'))) || 'Thn ' || EXTRACT(MONTH FROM AGE(to_date(to_char(pd.tglregistrasi,'YYYY-MM-DD'),'YYYY-MM-DD'),to_date(to_char(ps.tgllahir,'YYYY-MM-DD'),'YYYY-MM-DD'))) || ' Bln ' || EXTRACT(MONTH FROM AGE(to_date(to_char(pd.tglregistrasi,'YYYY-MM-DD'),'YYYY-MM-DD'),to_date(to_char(ps.tgllahir,'YYYY-MM-DD'),'YYYY-MM-DD'))) || ' Hr)' AS umur, " & _
            " CASE WHEN pp.tglkadaluarsa IS NULL THEN '' ELSE to_char(pp.tglkadaluarsa,'DD-MM-YYYY HH:mm') END AS tglkadaluarsa,CASE WHEN sn.satuanresep IS NOT NULL THEN sn.satuanresep ELSE CASE when jr.jenisracikan IS NULL THEN '' ELSE jr.jenisracikan END END AS satuanresep " & _
...
errLoad:
End Sub
Public Sub CetakLabelKecilCeklis(norec As String, ProdukFk As String, view As String, User As String)
On Error GoTo errLoad
On Error Resume Next
    Dim str1, str2 As String
...
Set Report = New Cr_cetakLabelKecil
    strSQL = "SELECT distinct ps.nocm, ps.namapasien,to_char(ps.tgllahir, 'DD-MM-YYYY') as tgllahir,sr.noresep, " & _
            ...
            ""
        strSQL = strSQL & " union all select distinct ps.nocm,ps.namapasien,to_char(ps.tgllahir, 'DD-MM-YYYY') as tgllahir,sr.noresep,to_char(sr.tglresep, 'DD-MM-YYYY') as tglresep, " & _
            " ' Racikan' || ' (' || CAST ((( CAST (pp.qtydetailresep AS FLOAT) / CAST (pp.dosis AS FLOAT)) * " & _
            " CASE WHEN pro.kekuatan IS NOT NULL THEN CAST (pro.kekuatan AS FLOAT) ELSE 1 END) AS VARCHAR) || ')' AS namaproduk,pp.aturanpakai,pp.rke, " & _
            ...
            "((CAST(pp.qtydetailresep as FLOAT)/CAST(pp.dosis as FLOAT))* CASE WHEN pro.kekuatan IS NOT NULL THEN CAST(pro.kekuatan as FLOAT) ELSE 1 END) AS jumlah, " & _
            "CASE WHEN pp.issiang = 't' THEN 'Siang' ELSE '-' END AS siang, CASE WHEN pp.ispagi = 't' THEN 'Pagi' ELSE '-' END AS pagi, " & _
            "CASE WHEN pp.ismalam = 't' THEN 'Malam' ELSE '-' END as malam, CASE WHEN pp.issore = 't' THEN 'Sore' ELSE '-' END as sore, " & _
            "CASE WHEN pp.keteranganpakai  = '' OR pp.keteranganpakai IS NULL THEN '' else pp.keteranganpakai END AS keteranganpakai, " & _
            " '(' || EXTRACT(YEAR FROM AGE(to_date(to_char(pd.tglregistrasi,'YYYY-MM-DD'),'YYYY-MM-DD'),to_date(to_char(ps.tgllahir,'YYYY-MM-DD'),'YYYY-MM-DD'))) || 'Thn ' || EXTRACT(MONTH FROM AGE(to_date(to_char(pd.tglregistrasi,'YYYY-MM-DD'),'YYYY-MM-DD'),to_date(to_char(ps.tgllahir,'YYYY-MM-DD'),'YYYY-MM-DD'))) || ' Bln ' || EXTRACT(MONTH FROM AGE(to_date(to_char(pd.tglregistrasi,'YYYY-MM-DD'),'YYYY-MM-DD'),to_date(to_char(ps.tgllahir,'YYYY-MM-DD'),'YYYY-MM-DD'))) || ' Hr)' AS umur, " & _
            ...
            "LEFT JOIN satuanstandar_m as ss on ss.id = pp.satuanviewfk " & _
            "LEFT JOIN jenisracikan_m as jr on jr.id = pp.jenisobatfk " & _
            "LEFT JOIN antrianapotik_t as aa on aa.noresep = sr.noresep LEFT JOIN satuanresep_m AS sn ON sn.id = pp.satuanresepfk " & _
            "where pp.jeniskemasanfk =1 and pp.produkfk <> 10013803 and " & _
            str1 & _
            str2 & _
            ""
