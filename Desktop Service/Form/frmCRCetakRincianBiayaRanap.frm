...
Begin VB.Form frmCRCetakRincianBiayaRanap 
   Caption         =   "Transmedic"
   ClientHeight    =   7005
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   5820
   Icon            =   "frmCRCetakRincianBiayaRanap.frx":0000
   LinkTopic       =   "Form1"
   ScaleHeight     =   7005
   ScaleWidth      =   5820
   ...
   End
End
Attribute VB_Name = "frmCRCetakRincianBiayaRanap"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
Option Explicit
Dim Report As New crRincianBiayaRanap
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    Set frmCRCetakRincianBiayaRanap = Nothing
End Sub

Public Sub CetakRincianBiayaRanap(strNoregistrasi As String, strNoStruk As String, strNoKwitansi As String, strIdPegawai As String, view As String)
...
Set Report = New crRincianBiayaRanap
...
'             "and tglpelayanan is not null and namaproduk not in ('Biaya Administrasi','Biaya Materai') AND penulisresep is null order by tglpelayanan, namaproduk"
    
    strSQL = "SELECT *, ROW_NUMBER () OVER (ORDER BY t.norec_pp) AS nomor From (SELECT '-' AS norec_pp, x.tglstruk, " & _
             "x.nobilling, x.nokwitansi, x.noregistrasi, x.nocm, x.namapasienjk,x.unit,x.objectdepartemenfk, " & _
             "x.dokterpj, x.tglregistrasi,x.tglpulang,x.namarekanan, x.ruangantindakan, x.namakelaspd, " & _
             ...
             "x.namauser From ( SELECT *, USER AS namauser From temp_billing_t Where noregistrasi = '" & strNoregistrasi & "' " & _
             "   AND tglpelayanan IS NOT NULL AND namaproduk NOT IN ( 'Biaya Administrasi', 'Biaya Materai' ) " & _
             "   AND penulisresep IS NULL ) AS x Group By " & _
             "x.tglstruk,  x.nobilling, x.nokwitansi, x.noregistrasi, x.nocm, x.namapasienjk, x.unit, " & _
             " x.objectdepartemenfk,  x.dokterpj, x.tglregistrasi, x.tglpulang, x.namarekanan, x.ruangantindakan, " & _
             " x.namakelaspd, x.namakamar, x.tipepasien, x.totalharusdibayar, x.totalprekanan, x.totalppenjamin, " & _
             " x.totalbiayatambahan, x.namauser,x.namaproduk "
             
    strSQL = strSQL & " UNION ALL SELECT '-' AS norec_pp, x.tglstruk, x.nobilling, x.nokwitansi, x.noregistrasi, x.nocm, x.namapasienjk, " & _
             "x.unit, x.objectdepartemenfk, x.dokterpj, x.tglregistrasi, x.tglpulang, x.namarekanan, x.ruangan AS ruangantindakan, x.namakelaspd, " & _
             "'Pemakaian Obat & Alkes' AS namaproduk, '-' AS penulisresep, '-' AS jenisproduk, 0 AS jumlah, SUM (x.total) AS total, x.namakamar, " & _
             "X.tipepasien , X.totalharusdibayar, X.totalprekanan, X.totalppenjamin, X.totalbiayatambahan, X.namauser " & _
             "From ( SELECT *, USER AS namauser, SUBSTRING(ruangantindakan, 1, 10) As ruangan From temp_billing_t Where noregistrasi = '" & strNoregistrasi & "' " & _
             "AND tglpelayanan IS NOT NULL AND namaproduk NOT IN ( 'Biaya Administrasi', 'Biaya Materai' ) AND penulisresep IS NOT NULL ) AS x " & _
             "Group By x.tglstruk, x.nobilling, x.nokwitansi, x.noregistrasi, x.nocm, x.namapasienjk, x.unit, x.objectdepartemenfk, x.dokterpj, " & _
             "x.tglregistrasi, x.tglpulang, x.namarekanan, x.ruangan, x.namakamar, x.tipepasien, x.totalharusdibayar, x.totalprekanan, x.totalppenjamin, " & _
             "x.totalbiayatambahan, x.namauser, x.namakelaspd) AS t Order By ROW_NUMBER () OVER (ORDER BY t.norec_pp)"
