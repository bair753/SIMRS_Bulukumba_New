        ...
        try {
            if ($r->input('statusbeds') != 1) {
                DB::table('tempattidur_m')->where('id', $r['tt_id'])
                    ->where('kdprofile', session("kdProfile"))->update(
                        [
...
//                dd(session("role"));
                ...
                return redirect()->route("show_page", ["role" => session("role"), "pages" => $this->formAwal]);
            } else {
                toastr()->error('Incorrect username or password.', 'Error !');
...
//            dd($user);
        if (!empty($user)) {
            ...
            $profile =  DB::table('profile_m')
                ...
                ->first();
            session(["namaLengkap" => $pegawai->namalengkap]);
            session(["username" => $user->namauser]);
            session(["kdProfile"=> $profile->id]);
            session(["namaProfile"=> $profile->namaexternal]);
            session(["id"=> $user->id]);
            session(['pegawai'=>$pegawai]);
            session(["tokenLogin"=> $this->createToken2($user->namauser) . '']);//
            $sts = true;
        } else {
            $sts = false;
        }
    ...
    public function logoutKeun(Request $request)
    {
    ...
    public function saveOrderKonsul(Request $request)
    {
        ...
        try {
            ...
            $dataSO->objectpetugasfk =  session("id");
