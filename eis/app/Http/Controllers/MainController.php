...
class MainController extends ApiController
{
    use Valet;
    public $header_ai = [
        'x-api-key' => '3GcxHEOS8Qz8pNoVqkIqa5BeQPhpIqf8aGKqwWd3',
    ...
    public $base_api_ai = 'https://api.jasamedika.arogya.ai/';
...
//                    dd($res['pegawai']);
                ...
                case 'dashboard-ai':
                    ...
                    try {
                        $im_sum = Http::withHeaders($this->header_ai)
                        ->timeout(10)
                        ->get($this->base_api_ai. 'im/summary')
                        ->json();
                    } catch (\Exception $e) {
                        $im_sum =[];
                    }
                    ...
                    try {
                        $im_insights = Http::withHeaders($this->header_ai)
                        ->timeout(10)
                        ->get($this->base_api_ai. 'im/insights')
                        ->json();
                    ...
                    try {
                        $om_sum = Http::withHeaders($this->header_ai)
                        ->timeout(5)
                        ->get($this->base_api_ai. 'om/summary')
                        ->json();
                    } catch (\Exception $e) {
                        $om_sum =[];
                    }        
                    ...
                    try {
                        $om_insights = Http::withHeaders($this->header_ai)
                        ->timeout(5)
                        ->get($this->base_api_ai. 'om/insights')
                        ->json();
                    } catch (\Exception $e) {
                        $om_insights =[];
                    }
                    ...
                    try {
                        $supplier_risk= Http::withHeaders($this->header_ai)
                        ->timeout(5)
                        ->get($this->base_api_ai. 'om/supplierrisk')
                        ->json();
                    } catch (\Exception $e) {
                        $supplier_risk =[];
                    }
                    ...
                    $res['color'] =  $this->listWarna2();
                    $res['im_summary'] = $im_sum;
                    $res['im_insights'] = $im_insights;
                    $res['om_summary'] = $om_sum['message'] ? [] :  $om_sum;
                    $res['om_insights'] = $om_insights['message'] ? [] : $om_insights;
                    $res['supplier_risk'] = $supplier_risk['message'] ? [] :$supplier_risk;
...
//            'datalogin' => $dataLogin,
    ...
    public static function getInfoStok($tgl,$tgl2,$kd) {
        ...
        $data = DB::select(DB::raw("
            SELECT * FROM (select sum( cast (spd.qtyproduk as float))  as qtyproduk,prd.namaproduk,
...
//            'count' => count($data),
    ...
    public static function format_num($number,$cur)
    {
        return $cur.number_format((float)$number ,2,".",",");
    }
...
//                        'count' => $totalAll,
    ...
    public function listWarna2(){
        return [
            'bg-simple-c-blue', 'bg-simple-c-pink', 'bg-simple-c-green', 'bg-yellow-gradient', 'bg-red-gradient',
            'bg-light-blue-gradient', 'bg-maroon-gradient', 'bg-purple-gradient', 'bg-teal-gradient',
            'bg-aqua-gradient', 'bg-green-gradient', 'bg-blue-active', 'bg-yellow-gradient', 'bg-red-gradient',
            'bg-light-blue-gradient', 'bg-maroon-gradient', 'bg-purple-gradient', 'bg-teal-gradient'];
    }
    ...
    public function getKunjungan($tglAwal,$tglAkhir,$kdProfile)
    {
        ...
        $data = DB::select(DB::raw("SELECT dp.id ,dp.namadepartemen,count(pd.norec) as jumlah, dp.qdepartemen
                ...
                order by dp.qdepartemen asc
        "));
           $data2 = DB::select(DB::raw("SELECT dp.id ,'Instalasi Hemodialisa' as namadepartemen,count(pd.norec) as jumlah, dp.qdepartemen
           FROM departemen_m dp
           join ruangan_m as ru on ru.objectdepartemenfk=dp.id
           LEFT JOIN (SELECT antrianpasiendiperiksa_t.norec,objectruanganfk FROM antrianpasiendiperiksa_t
           where tglregistrasi BETWEEN '$tglAwal' and '$tglAkhir' 
...
//            dd($profile);
    ...
    public function getArogya(Request $r){
        $new = str_replace(' ', '%20', $this->base_api_ai.$r['url']);
        $curl = curl_init();
        curl_setopt_array($curl, array(
            ...
            CURLOPT_HTTPHEADER => $this->header_ai_2
        ));
        ...
        curl_close($curl);
        // $data = Http::withHeaders($this->header_ai)
        // ->get($this->base_api_ai.$r['url'])
        // ->body();
    ...
    public static function getArogyaF($r){
        $data= Http::withHeaders([
            'x-api-key' => '3GcxHEOS8Qz8pNoVqkIqa5BeQPhpIqf8aGKqwWd3',
            'Content-Type' => 'application/json',
            'Accept' => 'application/json'
        ])
        ->get('https://api.jasamedika.arogya.ai/'.$r)
        ->json();

        return $data;
    }
    public function  getDetailImArog(Request $r){
        $idProfile = session("kdProfile");
        $title =  $r['kode'];
        $optimal = Http::withHeaders($this->header_ai)
        ->get($this->base_api_ai. 'im/status?status=optimal')
        ->json();

        // $datax = Http::withHeaders($this->header_ai)
        // ->get($this->base_api_ai. 'im/status')
        // ->json();

        $overstok = Http::withHeaders($this->header_ai)
        ->get($this->base_api_ai. 'im/status?status=overstock')
        ->json();
        $understock = Http::withHeaders($this->header_ai)
        ->get($this->base_api_ai. 'im/status?status=understock')
        ->json();
        // $optimal = [];
        // $overstok = [];
        // $understock = [];
        // foreach($datax as $d){
        //     if($title == 'optimal' && $d['status'] == 'optimal'){
        //         $optimal[] = $d;
        //     }
        //     if($title == 'overstock' && $d['status'] == 'overstock'){
        //         $overstok[] = $d;
        //     }
        //     if($title == 'understock' && $d['status'] == 'understock'){
        //         $understock[] = $d;
        //     }
        // }

         return view('module.shared.detail-inventory-manager',compact('overstok','understock','optimal','title'));

    }
    public function  getDetailOmArog(Request $r){
        $idProfile = session("kdProfile");
        $title =  $r['kode'];
        $datax = Http::withHeaders($this->header_ai)
        ->get($this->base_api_ai. 'im/insights')
        ...
        $understock = [];
        foreach($datax as $d){
            if($title == 'Optimalstocked' && $d['status'] == 'Optimal'){
                $optimal[] = $d;
            }
            if($title == 'Overstocked' && $d['status'] == 'Overstock'){
                $overstok[] = $d;
            }
            if($title == 'Understocked' && $d['status'] == 'Understock'){
                $understock[] = $d;
            }
        }
